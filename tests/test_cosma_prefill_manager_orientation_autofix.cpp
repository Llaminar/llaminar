...
{
    int init=0; MPI_Initialized(&init); if(!init){ int argc=0; char** argv=nullptr; MPI_Init(&argc,&argv);}    
    ...
    for(int i=0;i<k*n;++i) B[i] = std::cos(i*0.02f);
    ...
    double num=0.0, den=0.0; for(int i=0;i<m*n;++i){ double d=(double)C1[i]- (double)C2[i]; num+=d*d; den += (double)C1[i]*C1[i]; }
