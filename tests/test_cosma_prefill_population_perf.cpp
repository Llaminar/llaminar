...
namespace {
struct MPIFinalizerPerf { ~MPIFinalizerPerf(){ int init=0,fin=0; MPI_Initialized(&init); if(init){ MPI_Finalized(&fin); if(!fin) MPI_Finalize(); } } }; static MPIFinalizerPerf _mpi_finalizer_perf;

void fill_rand(std::vector<float> &v, int seed){ std::mt19937 gen(seed); std::uniform_real_distribution<float> dist(-1.f,1.f); for(auto &x:v) x=dist(gen);}    

struct Shape { int m,k,n; };    

struct Metrics {
    double us_popA=0, us_popB=0, us_matmul=0, us_total=0; double rel_l2=0; double gflops=0; size_t bytesA=0, bytesB=0; std::string variant; Shape s; };
...
void run_variant(std::vector<Metrics> &out, const Shape &shape, bool legacy_forward, int world, int rank) {
    ...
    if(rank==0){ fill_rand(A, 100 + m + k + n); fill_rand(B, 200 + m + k + n); }
    ...
    if(legacy_forward) setenv("LLAMINAR_COSMA_POP_FORWARD_LEGACY","1",1); else unsetenv("LLAMINAR_COSMA_POP_FORWARD_LEGACY");
    ...
    // rel L2
    double num=0,den=0; for(int i=0;i<m*n;++i){ double d=(double)C[i]-C_ref[i]; num+=d*d; den+=(double)C_ref[i]*C_ref[i]; }
    ...
    Metrics mrec; mrec.us_popA=us_popA; mrec.us_popB=us_popB; mrec.us_matmul=us_matmul; mrec.us_total=us_total; mrec.rel_l2=rel_l2; mrec.gflops=gflops; mrec.bytesA=(size_t)m*k*sizeof(float); mrec.bytesB=(size_t)k*n*sizeof(float); mrec.variant = legacy_forward?"legacy_forward":"dest_local"; mrec.s=shape; out.push_back(mrec);
}
...
TEST(CosmaPrefillManagerPopulationPerfTest, PopulationBenchmark) {
    ...
    int initialized=0; MPI_Initialized(&initialized); if(!initialized){ int argc=0; char** argv=nullptr; MPI_Init(&argc,&argv);}    
    int world=1, rank=0; MPI_Comm_size(MPI_COMM_WORLD,&world); MPI_Comm_rank(MPI_COMM_WORLD,&rank);
    if(world<2) { GTEST_SKIP() << "Requires >=2 ranks"; }
    ...
    if(rank==0){
        ...
        for(auto &mrec: metrics){ EXPECT_LT(mrec.rel_l2, 2e-2) << "rel_l2 too large for variant "<<mrec.variant; }
    }
}
