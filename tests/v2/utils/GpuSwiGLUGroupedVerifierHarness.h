/**
 * @file GpuSwiGLUGroupedVerifierHarness.h
...
namespace llaminar2::test::gpu_swiglu_verifier
{
    /** @brief Enable route telemetry and restore the caller environment on exit. */
    ...
    /** @brief Create a native zero-filled output tensor. */
    template <ActivationPrecision Precision>
    ...
    /** @brief Return a required telemetry tag or a visible missing marker. */
    ...
    /** @brief Require native byte equality and identify the first mismatch. */
    ...
    /**
     * @brief Verify one native specialization rejects an unbound GPU stream.
     *
     * Tensor allocation uses a valid stream; the production kernel is left
     * unbound and must fail without publishing grouped telemetry.
     */
        ...
        const char *format_label)
    {
        ...
        constexpr int cols = 544;
        const auto gate_values = makeValues(
            static_cast<size_t>(rows) * cols, 0x51510001u);
        const auto up_values = makeValues(
            static_cast<size_t>(rows) * cols, 0x51510002u);
        auto gate = makeNativeTensor<Precision>(
            {static_cast<size_t>(rows), static_cast<size_t>(cols)},
            gate_values.data());
        auto up = makeNativeTensor<Precision>(
            {static_cast<size_t>(rows), static_cast<size_t>(cols)},
            up_values.data());
        auto output = makeZeroNativeTensor<Precision>(
            {static_cast<size_t>(rows), static_cast<size_t>(cols)});

        ASSERT_TRUE(gate->ensureOnDevice(device, allocation_stream));
        ASSERT_TRUE(up->ensureOnDevice(device, allocation_stream));
        ...
        EXPECT_FALSE(unbound_kernel.apply_tensor(
            gate.get(), up.get(), output.get(), rows, cols,
            false, nullptr, device.toKernelDeviceIndex()))
            << format_label << " SwiGLU accepted a default stream";
     ...
     * Serial witnesses consume exact copies of the grouped native operands.
     */
    template <ActivationPrecision Precision, typename Kernel, typename Runtime>
        ...
        for (int cols : column_counts)
        {
            ...
            for (int verifier_rows : {2, 3, 4})
            {
                ...
                ASSERT_TRUE(kernel.apply_tensor(
                    ...
                    verifier_rows, cols, false, nullptr,
                    device.toKernelDeviceIndex()));
    ...
    /** @brief Execute every native GPU format and explicit-stream regression. */
