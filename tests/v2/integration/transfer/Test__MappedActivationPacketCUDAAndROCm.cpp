...
namespace
{
    ...
    public:
        ...
        ~PacketRoundTrip()
        {
            ...
            source_dense_output_tensor_.reset();
            ...
            target_local_output_tensor_.reset();
        ...
        bool initialize()
        {
            ...
            source_routing_weights_tensor_ = std::make_shared<FP32Tensor>(
                std::vector<std::size_t>{
                    static_cast<std::size_t>(kRows),
                    static_cast<std::size_t>(kTopK)});
            source_dense_output_tensor_ = std::make_shared<FP32Tensor>(
                std::vector<std::size_t>{
                    static_cast<std::size_t>(kRows),
                    static_cast<std::size_t>(kDModel)});
            ...
            target_routing_weights_tensor_ = std::make_shared<FP32Tensor>(
                std::vector<std::size_t>{
                    static_cast<std::size_t>(kRows),
                    static_cast<std::size_t>(kTopK)});
            target_local_output_tensor_ = std::make_shared<FP32Tensor>(
                std::vector<std::size_t>{
                    static_cast<std::size_t>(kRows),
                    static_cast<std::size_t>(kDModel)});
        ...
        bool captureDeviceOwnedGraphs()
        {
            ...
            if (!source_execution_context_ || !target_execution_context_ ||
                ...
                !source_arena_->registerExternalBuffer(
                    ...
                    source_dense_output_tensor_.get()) ||
                !source_arena_->registerExternalBuffer(
                ...
                !target_arena_->registerExternalBuffer(
                    BufferId::MOE_COMBINED_OUTPUT,
                    target_local_output_tensor_.get()))
            {
                error_ =
            ...
            clear_params.output = source_dense_output_tensor_.get();
            clear_params.num_elements =
                static_cast<std::size_t>(kRows * kDModel);
            clear_params.input_buffer_id = BufferId::RESIDUAL;
            clear_params.output_buffer_id = BufferId::MOE_COMBINED_OUTPUT;
            const std::string clear_name = "source_clear_dense_output";
                 ...
                 ++stage_index)
            {
                ...
                if constexpr (Rows > 1)
                {
                    ...
                    return_consume_params.dense_output =
                        source_dense_output_tensor_.get();
                    return_consume_params.dense_output_buffer_id =
                        BufferId::MOE_COMBINED_OUTPUT;
                    source_graph_->addNode(
                        return_consume_name,
                        ComputeStageFactory::
                ...
                device_work_params.input = target_hidden_tensor_.get();
                device_work_params.output = target_local_output_tensor_.get();
                device_work_params.num_elements =
                    static_cast<std::size_t>(kRows * kDModel);
                device_work_params.input_buffer_id = BufferId::NORMALIZED;
                ...
                return_pack_params.local_output =
                    target_local_output_tensor_.get();
                 ...
                 stage_index < kLayers.size(); ++stage_index)
            {
                ...
                if (stage_index == 0u)
                {
                    ...
                    clear_params.output_buffer_id =
                        BufferId::MOE_COMBINED_OUTPUT;
                    dispatch_dependency = "clear_dense_output";
                    dispatch_graph->addNode(
                        dispatch_dependency,
                        ComputeStageFactory::createResidualAdd(clear_params),
                        source_device_);
                    const std::string semantic_clear_name =
                        "source_clear_dense_output";
        ...
        bool awaitAndValidate()
        {
            ...
            std::array<float, kRows * kDModel> dense{};
            ...
            if (!source_backend_->deviceToHost(
                    dense.data(), source_dense_output_, sizeof(dense), 0,
                    source_stream_) ||
                ...
                !target_backend_->deviceToHost(
                    &target_return_first, target_local_output_,
            ...
            for (std::int32_t row = 0; row < kRows; ++row)
            {
                ...
                {
                    float expected = 0.0f;
                    if (rowVisitsTarget(row))
                    {
                        // Reproduce the exact ordered fold performed once by
                        // every retained stage instead of changing arithmetic
                        // order through a multiplication oracle.
                        for (std::size_t stage = 0;
                             stage < kLayers.size(); ++stage)
                        {
                            expected += sourceValue(row, column);
                        }
                    }
                    const float observed = dense[
                        static_cast<std::size_t>(row * kDModel + column)];
                    if (observed != expected)
                    {
                        const auto &control =
                            source_transport_->activationEpochControl(
                                kTargetParticipant, 0u);
                        error_ =
                            "continuation dense output did not preserve exact compact-row bytes"
                            " generation=" +
                            std::to_string(
                                identity_ ? identity_->epoch_generation : 0u) +
                            ...
                            "/" +
                            std::to_string(
                                control.buffers[0].return_signal.value) +
    ...
    private:
        ...
        {
            if (!source_placement_banks_ ||
                bank >= kDeviceMoEOverlayEpochBankCount)
            {
                return {};
            }
            auto *const bank_base =
                static_cast<std::byte *>(source_placement_banks_) +
        ...
        bool initializeSourcePayload()
        {
            ...
            constexpr std::array<float, 8> weight_pattern{
            ...
            for (std::size_t slot = 0u; slot < kEntries; ++slot)
            {
                ...
                weights[slot] = weight_pattern[slot % weight_pattern.size()];
            ...
            clear_tensor(*source_dense_output_tensor_);
            ...
            clear_tensor(*target_local_output_tensor_);
            ...
            const bool tensors_ready =
                ...
                source_routing_weights_tensor_->ensureOnDevice(
                    source_device_, source_stream_) &&
                source_dense_output_tensor_->ensureOnDevice(
                    source_device_, source_stream_) &&
                ...
                target_routing_weights_tensor_->ensureOnDevice(
                    target_device_, target_stream_) &&
                target_local_output_tensor_->ensureOnDevice(
                    target_device_, target_stream_);
            if (!tensors_ready)
            {
            ...
            source_dense_output_ =
                source_dense_output_tensor_->gpu_data_ptr();
            ...
            target_local_output_ =
                target_local_output_tensor_->gpu_data_ptr();
        ...
        bool arm(std::uint64_t generation)
        {
            ...
            const auto ticket = makeMoEOverlayInferenceExecutionTicket(
                ...
                /*placement_epoch=*/kPlacementEpoch,
                MoEOverlayInferenceGraphRole::MainPrefill,
                /*request_count=*/1,
        ...
        std::shared_ptr<FP32Tensor> source_dense_output_tensor_;
        ...
        std::shared_ptr<FP32Tensor> target_local_output_tensor_;
    ...
    void runRoleAssignment(DeviceId source, DeviceId target)
    {
        ...
        double bulk_d2h_bytes = 0.0;
        double bulk_h2d_bytes = 0.0;
                 ...
                 {"moe_overlay_activation_epoch", "forward_graph"}))
        {
                     ...
                     record.name == "moe_overlay_async_lane_joins")
            {
                ...
                ASSERT_EQ(record.tags.at("lanes"), "1");
                ASSERT_EQ(record.tags.at("rows"), std::to_string(Rows));
                ASSERT_EQ(record.tags.at("canonical_fold"), "true");
            }
        ...
        EXPECT_EQ(
            ...
            single_row_direct ? 2.0 * kLayers.size() : 0.0);
        EXPECT_EQ(bulk_d2h_bytes, expected_shared_dispatch_bytes);
        EXPECT_EQ(bulk_h2d_bytes, 0.0);
        EXPECT_EQ(
            shared_dispatch_payload_publications,
    ...
    /** @brief Exercise three device-owned epochs without a host-side wait. */
        ...
        DeviceId producer_device)
    {
        ...
        std::vector<std::int32_t> route_participants(route_slots);
        std::vector<float> root_routes(
            static_cast<std::size_t>(route_slots) * d_model, 0.0f);
        std::vector<float> producer_routes(root_routes.size(), 0.0f);
        std::vector<float> expected(
            static_cast<std::size_t>(rows) * d_model, 0.0f);
        for (std::uint32_t slot = 0u; slot < route_slots; ++slot)
        {
            const bool producer_owns = (slot % 3u) != 0u;
            route_participants[slot] = producer_owns
                                           ? producer_participant
                                           : root_participant;
            auto &owner_routes = producer_owns
                                     ? producer_routes
                                     : root_routes;
