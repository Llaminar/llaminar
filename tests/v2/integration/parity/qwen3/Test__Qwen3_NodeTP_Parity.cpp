/**
 * @file Test__Qwen3_NodeTP_Parity.cpp
 * @brief Qwen3 NodeTP parity tests across MPI ranks on one node.
 *
 ...
 * NodeTP uses MPI collectives for cross-rank communication — the correct
 ...
 *
 * Test configurations:
 *   - NodeTP_2xMPI_CPU: 2 MPI ranks, CPU, Qwen3-0.6B Q8_0
 *
 ...
 */
TEST_P(Qwen3NodeTPParityTest, NodeTPContextInitialization)
{
    ...
    LOG_INFO("[NodeTP Qwen3] Rank " << mpi_ctx_->rank() << " verified TPContext");
}

/**
 * @brief Test NodeTP allreduce operation
 */
TEST_P(Qwen3NodeTPParityTest, NodeTPAllreduce)
{
    ASSERT_TRUE(setupPipeline()) << "Pipeline setup failed";
    ...
    LOG_INFO("[NodeTP Qwen3] Rank " << mpi_ctx_->rank() << " allreduce result: "
 ...
 */
TEST_P(Qwen3NodeTPParityTest, NodeTPBroadcast)
{
    ASSERT_TRUE(setupPipeline()) << "Pipeline setup failed";
    ASSERT_NE(global_tp_ctx_, nullptr) << "NodeTP context required";
    ...
    LOG_INFO("[NodeTP Qwen3] Rank " << mpi_ctx_->rank() << " received: " << received_value);
}

/**
 * @brief Test NodeTP barrier synchronization
 */
TEST_P(Qwen3NodeTPParityTest, NodeTPBarrier)
{
    ASSERT_TRUE(setupPipeline()) << "Pipeline setup failed";
    ASSERT_NE(global_tp_ctx_, nullptr) << "NodeTP context required";

    LOG_INFO("[NodeTP Qwen3] Rank " << mpi_ctx_->rank() << " entering barrier");
    global_tp_ctx_->barrier();
    LOG_INFO("[NodeTP Qwen3] Rank " << mpi_ctx_->rank() << " exited barrier");
}

/**
 * @brief Full prefill/decode production parity with cross-rank TP sharding.
 */
TEST_P(Qwen3NodeTPParityTest, ProductionParity)
{
