...
namespace llaminar2::test::parity::qwen36
{
     ...
     * shared by CPU, CUDA, and ROCm production cells. Typed registration binds
     * each pack to its model; the cheap filename/size descriptor catches a
     * misselected fixture, while prompt/token digests and numerical checkpoints
     * prove the inference inputs without a whole-model content scan.
     */
    ...
    /**
     * @brief Retire the MoE MTP campaign's final prepared-model authority.
     *
     * The short-lived parity executables use `_exit` to avoid unsafe global
     * destructor ordering after MPI finalization.  Their one bounded campaign
     * cache must therefore cross the reusable-model retirement boundary
     * explicitly before backend shutdown instead of relying on process cleanup
     * to make GPU capacity visible to the following campaign.
     *
     * @param error Receives the exact lifecycle or backend diagnostic.
     * @return True when no cached contract existed or retirement completed.
     */
    inline bool releaseMoEMTPModelContextCampaignCache(
        std::string *error)
    {
        if (error)
            error->clear();
        ...
        catch (const std::exception &exception)
        {
            ...
            {
                *error =
                    "MoE MTP campaign final model retirement failed: " +
     ...
     * kernel-workspace defect because both inputs remain entirely device
         * resident. This diagnostic calls the existing mirrored digest API
     * only after snapshot diagnostics have already made the test an explicit
     * host-observation boundary. It reports scalar live positions, the bounded
     * verifier geometry, and shifted-MTP K/V tail hashes so an attention-state
     * divergence is not mistaken for a RoPE-position defect.
     *
        ...
        IInferenceRunner &runner)
    {
        ...
        {
            const auto digests =
                participants[participant]->captureMirroredMTPDigests();
            for (const auto &digest : digests)
            {
                const bool shifted_kv_digest =
                    digest.name.rfind("mtp_live_kv_", 0) == 0;
        ...
        std::vector<int32_t> serial_token_path = {})
    {
        ...
        if (verify_published_state_continuation)
        {
            ...
            sidecar_preservation_options.gdn_state_policy =
                MTPGDNStateComparisonPolicy::ExactBytes;
            ...
            if (runner->supportsGreedyAllPositionBatchOutcomeOnDevice())
            {
                ...
                ASSERT_TRUE(runner->beginDeviceResidentGeneration(
                    DeviceGenerationAdmissionRequest{
                        ...
                        .depth_policy =
                            resolveMTPDeviceGenerationDepthPolicy(config.mtp),
        ...
        if (verify_published_state_continuation)
        {
            ...
            if (committed_verifier_rows == 1)
            {
                ...
                pre_publication_row0_compare_options.gdn_state_policy =
                    MTPGDNStateComparisonPolicy::ExactBytes;
            ...
            publication_compare_options.gdn_state_policy =
            ...
            strict_publication_compare_options.gdn_state_policy =
                MTPGDNStateComparisonPolicy::ExactBytes;
            ...
            main_only_publication_compare_options.gdn_state_policy =
                MTPGDNStateComparisonPolicy::ExactBytes;
