/**
 * @file Test__NCCLCoordinator.cpp
 * @brief Integration tests for NCCLCoordinator with NVIDIA GPUs
 ...
 *
 * NOTE: These tests require NVIDIA GPU(s) with NCCL support.
 * Tests will be skipped if no CUDA devices are available.
 *
 * @see NCCLCoordinator for the implementation
...
class Test__NCCLCoordinator : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        if (cuda_device_count_ > 0)
        {
            ...
            for (int i = 0; i < cuda_device_count_; ++i)
            {
                ...
                std::cout << "  CUDA GPU " << i << ": " << prop.name << std::endl;
            }
        }
    }
...
TEST_F(Test__NCCLCoordinator, InitializeSingleGPU)
{
...
TEST_F(Test__NCCLCoordinator, InitializeMultiGPU)
{
    ...
    for (int i = 0; i < std::min(cuda_device_count_, 2); ++i)
    {
        devices.push_back(i);
    }
...
{
    if (cuda_device_count_ < 3)
    {
        GTEST_SKIP() << "Need at least 3 CUDA devices for non-contiguous test";
    }
...
TEST_F(Test__NCCLCoordinator, AllreduceSingleGPU)
{
    if (cuda_device_count_ < 1)
    {
        GTEST_SKIP() << "No CUDA devices available";
...
TEST_F(Test__NCCLCoordinator, AllreduceMultiGPU)
{
    if (cuda_device_count_ < 2)
    {
...
TEST_F(Test__NCCLCoordinator, AllgatherMultiGPU)
{
    if (cuda_device_count_ < 2)
    {
        GTEST_SKIP() << "Need at least 2 CUDA devices for allgather";
    }
    ...
    // Zero recv buffers
    cudaSetDevice(0);
    cudaMemset(d_recv_0, 0, RECV_COUNT * sizeof(float));
    cudaSetDevice(1);
    cudaMemset(d_recv_1, 0, RECV_COUNT * sizeof(float));
...
TEST_F(Test__NCCLCoordinator, BroadcastMultiGPU)
{
    if (cuda_device_count_ < 2)
    {
        GTEST_SKIP() << "Need at least 2 CUDA devices for broadcast";
    }
    ...
    cudaSetDevice(1);
    cudaMemset(d_buffer_1, 0, COUNT * sizeof(float));
...
TEST_F(Test__NCCLCoordinator, ThreadSafety)
{
    if (cuda_device_count_ < 2)
    {
        GTEST_SKIP() << "Need at least 2 CUDA devices for thread safety test";
    }
...
TEST_F(Test__NCCLCoordinator, ShutdownIdempotent)
{
    if (cuda_device_count_ < 1)
    {
        GTEST_SKIP() << "No CUDA devices available";
...
TEST_F(Test__NCCLCoordinator, ShutdownWithoutInitialize)
{
...
TEST_F(Test__NCCLCoordinator, GetCompletionEvent)
{
    if (cuda_device_count_ < 2)
    {
