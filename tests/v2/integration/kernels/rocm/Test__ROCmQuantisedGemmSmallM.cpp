...
TEST(Test__ROCmQuantisedGemmSmallM, GDNProjectionStageMixedCodebookM4UsesSingleFullBatchedRoute)
{
    if (!hasROCmDevice())
        GTEST_SKIP() << "No ROCm device available";

    ScopedEnv enable_stats("LLAMINAR_PERF_STATS_JSON", "1");
    ScopedEnv enable_graphs("LLAMINAR_GPU_GRAPHS", "1");
    PerfStatsCollector::reset();

    constexpr int M = 4;
    constexpr int K = 5120;
    constexpr int N_QKV = 10240;
    constexpr int N_Z = 10240;
    constexpr int N_A = 1024;
    constexpr int N_B = 1024;
    const DeviceId device = DeviceId::rocm(0);

    auto input = TestTensorFactory::createFP32Random({static_cast<size_t>(M), static_cast<size_t>(K)}, -0.5f, 0.5f, 9917);
    auto w_qkv = TestTensorFactory::createQ4_KRandom({static_cast<size_t>(N_QKV), static_cast<size_t>(K)}, 9918);
    auto w_z = TestTensorFactory::createQ4_KRandom({static_cast<size_t>(N_Z), static_cast<size_t>(K)}, 9919);
    auto w_a = TestTensorFactory::createQ5_KRandom({static_cast<size_t>(N_A), static_cast<size_t>(K)}, 9920);
    auto w_b = TestTensorFactory::createQ5_KRandom({static_cast<size_t>(N_B), static_cast<size_t>(K)}, 9921);

    ROCmPackedWeights packed_qkv;
    ROCmPackedWeights packed_z;
    ROCmPackedWeights packed_a;
    ROCmPackedWeights packed_b;
    ASSERT_TRUE(packWeightsToROCm(w_qkv.get(), packed_qkv));
    ASSERT_TRUE(packWeightsToROCm(w_z.get(), packed_z));
    ASSERT_TRUE(packWeightsToROCm(w_a.get(), packed_a));
    ASSERT_TRUE(packWeightsToROCm(w_b.get(), packed_b));
    expectPackedPath(packed_qkv, PackedPath::NativeVNNI);
    expectPackedPath(packed_z, PackedPath::NativeVNNI);
    expectPackedPath(packed_a, PackedPath::NativeVNNI);
    expectPackedPath(packed_b, PackedPath::NativeVNNI);

    ROCmQuantisedGemmKernel gemm_qkv(&packed_qkv, 0);
    ROCmQuantisedGemmKernel gemm_z(&packed_z, 0);
    ROCmQuantisedGemmKernel gemm_a(&packed_a, 0);
    ROCmQuantisedGemmKernel gemm_b(&packed_b, 0);

    auto separate_qkv = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_QKV)});
    auto separate_z = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_Z)});
    auto separate_a = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_A)});
    auto separate_b = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_B)});
    auto fused_qkv = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_QKV)});
    auto fused_z = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_Z)});
    auto fused_a = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_A)});
    auto fused_b = TestTensorFactory::createFP32({static_cast<size_t>(M), static_cast<size_t>(N_B)});

    ASSERT_TRUE(input->ensureOnDevice(device));
    for (auto *tensor : {separate_qkv.get(), separate_z.get(), separate_a.get(), separate_b.get(),
                         fused_qkv.get(), fused_z.get(), fused_a.get(), fused_b.get()})
    {
    ...
    params.output_qkv = fused_qkv.get();
    params.n_qkv = N_QKV;
    params.w_z = w_z.get();
    params.output_z = fused_z.get();
    params.n_z = N_Z;
    params.w_a = w_a.get();
    params.output_a = fused_a.get();
    params.n_a = N_A;
    params.w_b = w_b.get();
    params.output_b = fused_b.get();
    params.n_b = N_B;
    params.gemm_qkv = &gemm_qkv;
    params.gemm_z = &gemm_z;
    params.gemm_a = &gemm_a;
    params.gemm_b = &gemm_b;

    GDNProjectionStage stage(params);
    ROCmDeviceContext ctx(device, 0);

    auto reqs = stage.getWorkspaceRequirements(M, 0, K);
    DeviceWorkspaceManager workspace(device, reqs.total_bytes_with_alignment() + 64 * 1024 * 1024);
    ASSERT_TRUE(workspace.allocate(reqs));
    stage.bindWorkspace(&workspace);

    ASSERT_TRUE(gemm_qkv.multiply_tensor(input.get(), separate_qkv.get(), M, N_QKV, K));
    ASSERT_TRUE(gemm_z.multiply_tensor(input.get(), separate_z.get(), M, N_Z, K));
    ASSERT_TRUE(gemm_a.multiply_tensor(input.get(), separate_a.get(), M, N_A, K));
    ASSERT_TRUE(gemm_b.multiply_tensor(input.get(), separate_b.get(), M, N_B, K));
#ifdef HAVE_ROCM
    ASSERT_EQ(hipDeviceSynchronize(), hipSuccess);
#endif
    for (auto *tensor : {separate_qkv.get(), separate_z.get(), separate_a.get(), separate_b.get()})
    ...
    {
        for (auto *tensor : {fused_qkv.get(), fused_z.get(), fused_a.get(), fused_b.get()})
        {
            ASSERT_EQ(hipMemsetAsync(tensor->gpu_data_ptr(),
                                     0,
                                     tensor->size_bytes(),
                                     stream),
                      hipSuccess);
        }
        ASSERT_EQ(hipGraphLaunch(exec, stream), hipSuccess) << "replay=" << replay;
    ...
    EXPECT_GT(cosineSim(fused_qkv->data(), separate_qkv->data(), static_cast<size_t>(M) * N_QKV), 0.9999f);
    EXPECT_GT(cosineSim(fused_z->data(), separate_z->data(), static_cast<size_t>(M) * N_Z), 0.9999f);
    EXPECT_GT(cosineSim(fused_a->data(), separate_a->data(), static_cast<size_t>(M) * N_A), 0.9999f);
    EXPECT_GT(cosineSim(fused_b->data(), separate_b->data(), static_cast<size_t>(M) * N_B), 0.9999f);

    const auto records = PerfStatsCollector::snapshot({"kernel"});
    double full_mixed_batched_calls = 0.0;
    double same_codebook_pair_calls = 0.0;
    for (const auto &record : records)
    {
        if (record.domain != "kernel" ||
            record.name != "rocm_native_vnni_small_m_batched_calls" ||
        ...
        if (record.tags.count("projections") != 0 &&
            record.tags.at("projections") == "4" &&
            record.tags.count("codebook") != 0 &&
            record.tags.at("codebook") == "mixed")
        {
            full_mixed_batched_calls += record.value;
        }
        if (record.tags.count("projections") != 0 &&
            record.tags.at("projections") == "2")
        {
            same_codebook_pair_calls += record.value;
        }
    }

    EXPECT_GE(full_mixed_batched_calls, 1.0)
        << "GDNProjectionStage should submit one full mixed-codebook native small-M group";
    EXPECT_EQ(same_codebook_pair_calls, 0.0)
        << "GDNProjectionStage should not greedily split mixed GDN projections into same-codebook pairs";
