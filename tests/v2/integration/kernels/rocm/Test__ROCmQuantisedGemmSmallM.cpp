...
TEST(Test__ROCmQuantisedGemmSmallM, DispatchQ80M2MatchesReference)
{
    ...
    LOG_INFO("[SmallM] Q8_0 M=2 cosine=" << cos);
    EXPECT_GT(cos, 0.985f);
...
TEST(Test__ROCmQuantisedGemmSmallM, FusedQ80QKVM2MatchesSeparate)
{
    ...
    projections.emplace_back(&q_kernel, fused_q.get(), Nq, bias_q.get(), "q_q8_small_m");
    projections.emplace_back(&k_kernel, fused_k.get(), Nk, bias_k.get(), "k_q8_small_m");
    projections.emplace_back(&v_kernel, fused_v.get(), Nv, bias_v.get(), "v_q8_small_m");
    ...
    LOG_INFO("[SmallM] fused QKV M=2 cosine q=" << q_cos << " k=" << k_cos << " v=" << v_cos);
