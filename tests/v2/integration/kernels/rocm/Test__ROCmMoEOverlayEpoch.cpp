...
namespace llaminar2::test
{
    ...
    protected:
        void SetUp() override
        {
            ...
            ASSERT_EQ(hipMalloc(reinterpret_cast<void **>(&device_movement_waves_),
                                  20u * sizeof(*device_movement_waves_)), hipSuccess);
            ASSERT_EQ(hipMalloc(reinterpret_cast<void **>(&device_movement_edges_),
                                  40u * sizeof(*device_movement_edges_)), hipSuccess);
        ...
        void TearDown() override
        {
            ...
            if (device_movement_waves_)
                (void)hipFree(device_movement_waves_);
            if (device_movement_edges_)
                (void)hipFree(device_movement_edges_);
           ...
           DurableMaintenanceUsesReservationAfterInferenceTicketRelease)
    {
        ...
        ASSERT_EQ(hipMemcpyAsync(
                      ...
                      sizeof(command_header), hipMemcpyDeviceToHost,
                      maintenance_stream_),
                  hipSuccess);
        ASSERT_EQ(hipStreamSynchronize(maintenance_stream_), hipSuccess);
        ...
        ASSERT_EQ(hipMemcpyAsync(&controller, device_controller_, sizeof(controller),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipMemcpyAsync(&command_header, device_command_header_, sizeof(command_header),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipMemcpyAsync(&reservation_status, &device_statuses_[2], sizeof(reservation_status),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipStreamSynchronize(maintenance_stream_), hipSuccess);
        ASSERT_TRUE(reservation_status.succeeded());
           ...
           CapturedAllLayerFinalizeAbortsNoWorkThenPublishesTwentyEpochs)
    {
                                 ...
                                 bool prepare_changed_layer)
        {
            ...
            ASSERT_EQ(hipMemcpyAsync(device_controller_, &controller, sizeof(controller),
                                       hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(device_command_header_, &header, sizeof(header),
                                       hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
        ...
        ASSERT_EQ(hipMemcpyAsync(device_apply_status_, &apply_status, sizeof(apply_status),
                                   hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipGraphLaunch(graph_exec_, maintenance_stream_), hipSuccess);
        DeviceMoERebalanceGraphControllerState rejected_controller{};
        DeviceMoERebalanceCommandBufferHeader retained_header{};
        ASSERT_EQ(hipMemcpyAsync(&rejected_controller, device_controller_, sizeof(rejected_controller),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipMemcpyAsync(&retained_header, device_command_header_, sizeof(retained_header),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipMemcpyAsync(&control, device_control_, sizeof(control),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipMemcpyAsync(&publication_status, &device_statuses_[2], sizeof(publication_status),
                                   hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipStreamSynchronize(maintenance_stream_), hipSuccess);
...
/**
     ...
     */
    TEST_F(ROCmMoEOverlayEpochTest, NativeMovementJournalCapturedPublicationAndReset)
    {
        NativeMoEMovementArchive archive({.workspace_generation = 1, .layers = 2, .experts = 2,
            .wave_capacity = 20, .edge_capacity = 40,
            .participants = {{DeviceId::rocm(0), 0, 0}, {DeviceId::rocm(1), 0, 0}}});
        DeviceMoERebalanceGraphControllerState controller{};
        controller.participant_count = 2u;
        ...
        ASSERT_EQ(hipMemcpyAsync(device_controller_, &controller, sizeof(controller),
                                 hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipStreamSynchronize(maintenance_stream_), hipSuccess);
        ASSERT_EQ(hipStreamBeginCapture(maintenance_stream_, hipStreamCaptureModeGlobal), hipSuccess);
        ASSERT_TRUE(kernel_->reserveMoEOverlayEpochCandidate(
        ...
        ASSERT_TRUE(kernel_->finalizeMoEOverlayRebalancePublication(
            ...
            device_plan_entry_, 2u, 4096u, journalView()));
        ASSERT_EQ(hipStreamEndCapture(maintenance_stream_, &graph_), hipSuccess);
        ASSERT_EQ(hipGraphInstantiate(&graph_exec_, graph_, nullptr, nullptr, 0), hipSuccess);
        ...
        for (std::uint32_t replay = 0u; replay < 27u; ++replay)
        {
            ...
            ASSERT_EQ(hipMemcpyAsync(device_runtime_family_, family.data(), sizeof(family),
                                     hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(device_controller_, &controller, sizeof(controller),
                                     hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(device_command_header_, &header, sizeof(header),
                                     hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(device_apply_status_, &applied, sizeof(applied),
                                     hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(device_plan_entry_, plans.data(), sizeof(plans),
                                     hipMemcpyHostToDevice, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipGraphLaunch(graph_exec_, maintenance_stream_), hipSuccess);
            DeviceMoEOverlayEpochStatus publication{};
            ASSERT_EQ(hipMemcpyAsync(&controller, device_controller_, sizeof(controller),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(&header, device_command_header_, sizeof(header),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(&control, device_control_, sizeof(control),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(&publication, &device_statuses_[2], sizeof(publication),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(waves.data(), device_movement_waves_, sizeof(waves),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipMemcpyAsync(edges.data(), device_movement_edges_, sizeof(edges),
                                     hipMemcpyDeviceToHost, maintenance_stream_), hipSuccess);
            ASSERT_EQ(hipStreamSynchronize(maintenance_stream_), hipSuccess);
