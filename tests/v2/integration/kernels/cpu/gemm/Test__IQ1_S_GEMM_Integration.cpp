/**
 * @file Test__IQ1_S_GEMM_Integration.cpp
 * @brief Integration tests for IQ1_S GEMM
...
class IQ1_S_GEMM_Integration : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
TEST_F(IQ1_S_GEMM_Integration, BasicMultiplication)
{
    // M=1, N=32, K=256
    // A: 1x256 (FP32) = 1.0
    // B: 32x256 (IQ1_S) = 1.0 (constructed)
    // C: 1x32 (FP32)
    ...
    // Create B (weight) - IQ1_S
    // IQ1_S block size is 256. So 1 block per row for K=256.
    size_t num_blocks = n * (k / 256); // 32 * 1 = 32 blocks
    std::vector<uint8_t> raw_data(num_blocks * sizeof(IQ1_SBlock));
    IQ1_SBlock *blocks = reinterpret_cast<IQ1_SBlock *>(raw_data.data());

    for (size_t i = 0; i < num_blocks; ++i)
    {
        std::memset(&blocks[i], 0, sizeof(IQ1_SBlock));
    ...
    auto B = std::make_shared<IQ1_STensor>(std::vector<size_t>{static_cast<size_t>(n), static_cast<size_t>(k)}, raw_data);
...
TEST_F(IQ1_S_GEMM_Integration, QuantizedVsFP32Parity)
{
    // Test dimensions: M=4, N=64, K=256
    // A: FP32 activation (random values)
    // B: IQ1_S weight (random data with valid scales)
    ...
    // === Create IQ1_S weight tensor with random but valid data ===
    ...
    // IQ1_S is a 1-bit format. Relative L2 error should be under 1%.
    ...
    std::cout << "[IQ1_S vs FP32] Relative L2 error: " << rel_l2 << std::endl;
    ...
    std::cout << "[IQ1_S vs FP32] First 5 values:\n";
