/**
 * @file Test__CUDAAttentionPaddingParity.cpp
 * @brief CUDA attention parity tests for hostile padded prefill bucket tails.
 *
 * These tests exercise the CUDA FP32 attention kernel directly, without model
 * loading or graph construction. They validate the Phase 6 bucket-padding
 * requirement that real causal prefill rows must be unchanged when future pad
 * rows contain values that would dominate attention if the mask exposed them.
 */
...
namespace
{
     ...
     *
     * The values are intentionally small but not uniform. That keeps exact and
     * padded causal runs numerically tight while still giving the softmax real
     ...
     *
     * The pad K/V magnitudes are high enough that non-causal attention should
     * visibly change the real-row prefix if those rows participate.
     */
...
#ifdef HAVE_CUDA
    /// @brief Result bundle for a direct CUDA attention invocation.
    struct CUDAAttentionRunResult
    {
        bool ok = false;           ///< True when allocation, kernel launch, and copy-back succeeded.
        std::string error;         ///< Diagnostic text for the first failed CUDA operation.
        std::vector<float> output; ///< Host copy of the output tensor.
    };

    /**
     * @brief Run CUDA FP32 attention on raw device buffers and copy output back.
     *
     * This deliberately bypasses tensors, model loading, and graph execution so
     * the test isolates the backend causal mask behavior for padded prefill.
     */
    CUDAAttentionRunResult runCUDAAttention(
        ...
        const MPIContext &mpi_ctx)
    {
        ...
        if (err != cudaSuccess)
            return fail(std::string("cudaSetDevice failed: ") + cudaGetErrorString(err));
        ...
        err = cudaMalloc(reinterpret_cast<void **>(&device_query), query_bytes);
        if (err != cudaSuccess)
            return fail(std::string("cudaMalloc(Q) failed: ") + cudaGetErrorString(err));
        err = cudaMalloc(reinterpret_cast<void **>(&device_key), key_value_bytes);
        if (err != cudaSuccess)
            return fail(std::string("cudaMalloc(K) failed: ") + cudaGetErrorString(err));
        err = cudaMalloc(reinterpret_cast<void **>(&device_value), key_value_bytes);
        if (err != cudaSuccess)
            return fail(std::string("cudaMalloc(V) failed: ") + cudaGetErrorString(err));
        err = cudaMalloc(reinterpret_cast<void **>(&d_output), output_bytes);
        if (err != cudaSuccess)
            return fail(std::string("cudaMalloc(output) failed: ") + cudaGetErrorString(err));

        err = cudaMemcpy(device_query, inputs.query.data(), query_bytes, cudaMemcpyHostToDevice);
        if (err != cudaSuccess)
            return fail(std::string("cudaMemcpy(Q) failed: ") + cudaGetErrorString(err));
        err = cudaMemcpy(device_key, inputs.key.data(), key_value_bytes, cudaMemcpyHostToDevice);
        if (err != cudaSuccess)
            return fail(std::string("cudaMemcpy(K) failed: ") + cudaGetErrorString(err));
        err = cudaMemcpy(device_value, inputs.value.data(), key_value_bytes, cudaMemcpyHostToDevice);
        if (err != cudaSuccess)
            return fail(std::string("cudaMemcpy(V) failed: ") + cudaGetErrorString(err));
        err = cudaMemset(d_output, 0, output_bytes);
        if (err != cudaSuccess)
            return fail(std::string("cudaMemset(output) failed: ") + cudaGetErrorString(err));

        llaminar2::cuda::CUDAFlashAttentionKernelT<ActivationPrecision::FP32> kernel(cuda_ordinal);
        const bool kernel_ok = kernel.compute(
            device_query, device_key, device_value, d_output,
            seq_len, n_heads, n_kv_heads, head_dim,
            causal, -1,
            nullptr, nullptr, nullptr, nullptr,
            false, &mpi_ctx, cuda_ordinal);
        err = cudaDeviceSynchronize();
        if (!kernel_ok)
            return fail("CUDA attention kernel returned false");
        if (err != cudaSuccess)
            return fail(std::string("cudaDeviceSynchronize failed: ") + cudaGetErrorString(err));

        err = cudaMemcpy(result.output.data(), d_output, output_bytes, cudaMemcpyDeviceToHost);
        if (err != cudaSuccess)
            return fail(std::string("cudaMemcpy(output) failed: ") + cudaGetErrorString(err));
...
class Test__CUDAAttentionPaddingParity : public CUDATestBase
{
protected:
...
TEST_F(Test__CUDAAttentionPaddingParity, CausalMaskPreventsHostilePaddingFromChangingRealRows)
{
    SKIP_IF_NO_CUDA();
    ...
    const auto exact = runCUDAAttention(real, real_seq_len, n_heads, n_kv_heads, head_dim, true, cuda_ordinal_, mpi_ctx_);
    ASSERT_TRUE(exact.ok) << exact.error;
    const auto bucket = runCUDAAttention(padded, bucket_seq_len, n_heads, n_kv_heads, head_dim, true, cuda_ordinal_, mpi_ctx_);
...
TEST_F(Test__CUDAAttentionPaddingParity, NonCausalAttentionExposesHostilePaddingRows)
{
    SKIP_IF_NO_CUDA();
    ...
    const auto exact = runCUDAAttention(real, real_seq_len, n_heads, n_kv_heads, head_dim, false, cuda_ordinal_, mpi_ctx_);
    ASSERT_TRUE(exact.ok) << exact.error;
    const auto bucket = runCUDAAttention(padded, bucket_seq_len, n_heads, n_kv_heads, head_dim, false, cuda_ordinal_, mpi_ctx_);
    ASSERT_TRUE(bucket.ok) << bucket.error;
...
TEST_F(Test__CUDAAttentionPaddingParity, LMHeadPaddedBucketUsesLastRealRowOnGPU)
{
    SKIP_IF_NO_CUDA();
    ...
    const DeviceId device = DeviceId::cuda(cuda_ordinal_);
    ...
    EXPECT_LT(maxLogitDifference(*logits, last_real_reference), 8e-2f)
        << "CUDA LM-head logits should come from the last real row.";
...
TEST_F(Test__CUDAAttentionPaddingParity, DecodeContinuationUsesRealKVLengthOnGPU)
{
    SKIP_IF_NO_CUDA();
    ...
    EXPECT_LT(maxTensorVectorDiff(*output, expected_real_prefix), 2e-2f)
        << "CUDA decode attention should use the real KV prefix length.";
