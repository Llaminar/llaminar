...
namespace
{
    ...
    /**
     * @brief Rehydrate one FP32 value from an exact production snapshot word.
     *
     * Decimal text is not an acceptable fixture representation for a byte-level
     * grouped-decode regression because parsing can change the least-significant
     * bits.  Copying the IEEE-754 word preserves the activation bytes presented
     * to both CUDA execution schedules.
     */
    float f32FromBitsForCUDAMoEKernelTest(uint32_t bits)
    {
        float value = 0.0f;
    ...
    /**
     * @brief Build valid participant-local state for command-projection tests.
     *
     * Projection now authenticates physical transfer-slot leases against the
     * live runtime and directory. Even tests whose commands target another
     * participant provide this state so the production API is exercised
     * without a null-directory escape hatch.
     */
    llaminar2::DeviceMoELayerRuntime makeProjectionRuntime(
    ...
    makeEmptyProjectionTransferSlots(
        ...
        uint32_t slot_count)
    {
        std::vector<llaminar2::DeviceMoEExpertDirectoryEntry> slots(slot_count);
    ...
    /**
     * @brief Stamp the immutable transfer ticket used by focused publisher tests.
     */
    void stampRebalanceCopyTransaction(
        llaminar2::DeviceMoERebalanceApplyStatus &status,
        uint32_t wave_index,
        uint32_t epoch,
        uint32_t command_count)
    {
        status.transaction_wave_index = wave_index;
        status.transaction_epoch = epoch;
        status.transaction_command_count = command_count;
    }
     ...
     *
     * The MTP verifier bugs chased by these tests often involve tensor
     * residency and publication state.  The helper therefore requires the
     * production tensor event to drive the D2H observation; a missing event is
     * a fatal assertion rather than something a raw cudaMemcpy can conceal.
     */
        ...
        cudaStream_t stream)
    {
        ...
        EXPECT_TRUE(tensor->ensureOnHost(stream))
            << "CUDA producer did not publish a completion event";
        std::copy_n(tensor->data(), tensor->numel(), host.data());
        return host;
    }
     ...
     *
     * The fixture-wide setup disables router Q8 for legacy tests that compare
     * directly with an FP32 CPU router.  Grouped verifier integration tests need
     * the opposite contract: routing quantizes each hidden row once on device,
     * then routed and shared experts consume those exact bytes.  This guard
     * changes only those two production capabilities and restores the parsed
     * process configuration after the focused test.
     */
    class ScopedCudaMoERouterQ8Config
    {
    public:
        ScopedCudaMoERouterQ8Config(bool router_q8, bool reuse_router_q8_hidden)
            : old_router_q8_(llaminar2::mutableDebugEnv().gemm.cuda_moe_router_q8),
              old_reuse_router_q8_hidden_(
                  llaminar2::mutableDebugEnv().gemm.cuda_moe_reuse_router_q8_hidden)
        {
            auto &gemm = llaminar2::mutableDebugEnv().gemm;
        ...
        ~ScopedCudaMoERouterQ8Config()
        {
            auto &gemm = llaminar2::mutableDebugEnv().gemm;
            gemm.cuda_moe_router_q8 = old_router_q8_;
            gemm.cuda_moe_reuse_router_q8_hidden = old_reuse_router_q8_hidden_;
        }

        ScopedCudaMoERouterQ8Config(const ScopedCudaMoERouterQ8Config &) = delete;
        ScopedCudaMoERouterQ8Config &operator=(const ScopedCudaMoERouterQ8Config &) = delete;

    private:
        bool old_router_q8_ = true;
        bool old_reuse_router_q8_hidden_ = true;
    };
...
#ifdef HAVE_CUDA
        /**
         * @brief Build the explicit launch contract for rebalance and LLEP calls.
         *
         * Passing this immutable context proves that the multi-stream operations
         * do not depend on even their stage-owned kernel's inherited stream.
         */
        [[nodiscard]] llaminar2::MoEKernelLaunchContext launchContext() const noexcept
        {
            return {
                .stream = stream_,
       ...
       RuntimeVerifierResidentAssignmentIsCapturedAndMTotal)
{
    ...
    for (const int m : verifier_rows)
    {
        ...
        llaminar2::DeviceMoELayerRuntime observed{};
        ASSERT_EQ(
            cudaMemcpy(
                &observed,
                runtime_table.deviceLayerState(0),
                ...
                cudaMemcpyDeviceToHost),
            cudaSuccess);
        EXPECT_EQ(observed.reserved_u64[3], 0ULL)
            << "Resident-only verifier assignment published a weight transfer";

        std::vector<int32_t> participants(
            static_cast<size_t>(m * top_k));
        ASSERT_EQ(
            cudaMemcpy(
                participants.data(),
...
TEST_F(Test__CUDAMoEKernel, RuntimePrefillLeastLoadedCurrentBatchPlannerMaterializesSpans)
{
    ...
    EXPECT_EQ(materialized_plan[0].destination_slot,
    ...
    ASSERT_EQ(cudaMemcpy(d_apply_status,
                         &apply_status,
                         sizeof(apply_status),
                         cudaMemcpyHostToDevice),
              cudaSuccess);
...
/**
 * @brief Prove LLEP materialization publishes logical commands only.
 *
 ...
 * directory bound.
 */
TEST_F(Test__CUDAMoEKernel, RuntimePrefillLeastLoadedMaterializerLeavesPhysicalSlotUnassigned)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    const llaminar2::least_loaded_ep::LeastLoadedExpertAssignmentSpan protected_span{
        .expert = 1,
        .owner_participant = 1,
    ...
    ASSERT_EQ(cudaMemcpyAsync(runtime.reserved_ptrs[1],
                              &protected_span,
    ...
    llaminar2::DeviceMoERebalanceCommandBufferHeader *d_header = nullptr;
    llaminar2::DeviceMoERebalanceStatus *d_status = nullptr;
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan), sizeof(*d_plan)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan_count), sizeof(*d_plan_count)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_header), sizeof(*d_header)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_status), sizeof(*d_status)),
              cudaSuccess);
    ...
    ASSERT_EQ(cudaMemcpy(&plan, d_plan, sizeof(plan), cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpy(&plan_count,
                         d_plan_count,
                         sizeof(plan_count),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
    ...
    EXPECT_EQ(status.status_code,
              static_cast<uint32_t>(
                  llaminar2::DeviceMoERebalanceStatusCode::Ok));
...
TEST_F(Test__CUDAMoEKernel, RuntimePrefillLeastLoadedTransferCommandsPreserveTransferOrder)
{
    ...
    EXPECT_EQ(materialized_plan[1].destination_slot,
              llaminar2::kDeviceMoEInvalidSlot);
    ...
    EXPECT_EQ(materialized_plan[2].destination_slot,
              llaminar2::kDeviceMoEInvalidSlot);
...
/**
 * @brief Prove the CUDA planner cannot overcommit transfer-backed payload slots.
 *
 * The route geometry reproduces a request boundary where participant 1 retains
 * expert 1 from an earlier transfer wave while hotter expert 0 needs a new
 * arrival. A one-entry transfer budget alone permits both payloads because the
 * resident replica needs no new copy. The physical working-set limit must make
 * the device planner select one non-owner expert while preserving every row.
 */
TEST_F(Test__CUDAMoEKernel, RuntimePrefillLeastLoadedWorkingSetCountsResidentReplica)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    std::array<llaminar2::least_loaded_ep::LeastLoadedExpertAssignmentSpan, max_spans> spans{};
    std::array<llaminar2::least_loaded_ep::LeastLoadedExpertWeightTransfer, 1> transfers{};
    ASSERT_EQ(cudaMemcpy(spans.data(),
                         runtime_state.reserved_ptrs[1],
                         sizeof(spans),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpy(transfers.data(),
                         runtime_state.reserved_ptrs[2],
                         sizeof(transfers),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);

    llaminar2::least_loaded_ep::LeastLoadedExpertAssignmentStatus published_status{};
    published_status.span_count =
        static_cast<uint32_t>(device_runtime_state.reserved_u64[2]);
    EXPECT_EQ(
        llaminar2::least_loaded_ep::countNonOwnerExpertAssignments(
            spans.data(),
            published_status,
            1u),
        1u);
...
/**
 * @brief Prove a one-slot participant can project an ownership swap.
 *
 * Participant 1 owns expert 0 in its only transfer slot. The root wave sends
 * expert 0 away and brings expert 2 into participant 1. Projection must lease
 * the outgoing slot with its exact prior identity and generation; allocating a
 * second slot would hide the bounded-capacity lifecycle exercised in serving.
 */
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceDomainProjectionLeasesSameWaveOutgoingSlot)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    llaminar2::DeviceMoERebalancePlanEntry outgoing_plan;
    outgoing_plan.op =
        static_cast<uint32_t>(llaminar2::DeviceMoERebalancePlanOp::OwnershipTransfer);
    ...
    root_wave.payload_bucket_index = 0;
    ...
    claimed_slot.flags |=
        ...
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceDirectoryFlags::LocalCompute) |
        static_cast<uint32_t>(
    ...
    CudaAllocation d_projection_runtime(sizeof(projection_runtime));
    CudaAllocation d_projection_slots(
    ...
    ASSERT_TRUE(cuda_kernel_->projectDeviceRebalanceDomainCommands(
        ...
        d_local_wave_states,
        static_cast<llaminar2::DeviceMoELayerRuntime *>(
            d_projection_runtime.get()),
        static_cast<llaminar2::DeviceMoEExpertDirectoryEntry *>(
            d_projection_slots.get()),
        payload_slot_capacity));
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    ...
    EXPECT_EQ(local_plan_entries[0].destination_slot, 0u)
        << "the outgoing ownership transfer releases the sole slot after payload pack";
    EXPECT_EQ(local_plan_entries[0].destination_previous_layer, 0u);
    EXPECT_EQ(local_plan_entries[0].destination_previous_expert, 0u);
    EXPECT_EQ(local_plan_entries[0].destination_generation, 41u);
    ...
    EXPECT_EQ(local_plan_entries[1].payload_slot, outgoing_plan.payload_slot)
        << "the outgoing payload remains selected before destination unpack reuses its slot";
    ...
    EXPECT_EQ(status.payload_bucket_index, 0u);
    EXPECT_EQ(status.payload_bucket_overflow, 0u);

    /*
     * A same-expert replica copy is not a release. Repeat projection with the
     * outgoing command changed from OwnershipTransfer to ExpertPayloadArrival;
     * the sole live slot must remain protected and capacity failure must become
     * explicit in status rather than silently overwriting active bytes.
     */
    gathered_plan_entries[1].op =
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalancePlanOp::ExpertPayloadArrival);
    status = llaminar2::DeviceMoERebalanceStatus{};
    ASSERT_EQ(cudaMemcpyAsync(d_gathered_plan,
                              gathered_plan_entries.data(),
                              gathered_plan_entries.size() *
                                  sizeof(gathered_plan_entries[0]),
                              cudaMemcpyHostToDevice,
                              stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemsetAsync(d_local_plan,
                              0,
                              local_plan_entries.size() *
                                  sizeof(local_plan_entries[0]),
                              stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemsetAsync(d_local_headers,
                              0,
                              local_headers.size() *
                                  sizeof(local_headers[0]),
                              stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemsetAsync(d_local_wave_states,
                              0,
                              local_wave_states.size() *
                                  sizeof(local_wave_states[0]),
                              stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(
                  d_status,
    ...
    ASSERT_EQ(cudaMemcpy(
                  ...
                  cudaMemcpyDeviceToHost),
              cudaSuccess);
    EXPECT_EQ(local_plan_entries[0].destination_slot,
              llaminar2::kDeviceMoEInvalidSlot);
    EXPECT_EQ(status.capacity_limited_candidates, 1u);
    EXPECT_EQ(status.invalid_runtime_layers, 1u);
    EXPECT_EQ(status.plan_overflow, 1u);
    EXPECT_EQ(status.payload_bucket_overflow, 1u);
...
/**
 * @brief Prove ping-pong command waves lease destination storage independently.
 *
 * Projection materializes both command buffers, but the copy transaction
 * selects exactly one wave per graph replay. Both alternatives may therefore
 * name the same one-slot destination: after the active wave applies, the next
 * replay reprojects the other wave against the newly published directory.
 *
 * The initial slot models a completed transaction whose placement descriptor
 * has already retired. Its transaction epoch deliberately equals the next
 * projected command header. Header equality is not physical ownership, so the
 * stale logical publication must not pin otherwise-unclaimed storage.
 *
 * The second slot is a resident, remote-owned cached replica with no local
 * compute assignment. Two arrivals in each wave consume the stale slot first,
 * then prove that bounded maintenance can transactionally replace the cold
 * replica while preserving its prior identity for apply-side retirement.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceDomainProjectionLeasesPingPongWavesIndependently)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    for (uint32_t wave = 0; wave < command_buffer_count; ++wave)
    {
        for (uint32_t arrival = 0u; arrival < 2u; ++arrival)
        {
            auto &plan =
                gathered_plan_entries[
                    static_cast<size_t>(wave) * plan_capacity + arrival];
            plan.op = static_cast<uint32_t>(
                llaminar2::DeviceMoERebalancePlanOp::ExpertPayloadArrival);
            plan.layer = 0;
            plan.expert = 4u + wave * 2u + arrival;
            plan.source_participant = 0;
            plan.destination_participant = 1;
            plan.source_resident_mask = 0b001u;
            plan.destination_slot = arrival;
            plan.payload_slot = arrival;
        }

        auto &header = gathered_headers[wave];
        header.epoch = 7u + wave;
        header.phase = static_cast<uint32_t>(
            llaminar2::DeviceMoERebalancePipelinePhase::PlanAssignments);
        header.command_count = 2;
        header.command_capacity = plan_capacity;
        header.participant_id = 0;
    ...
    auto projection_slots =
        makeEmptyProjectionTransferSlots(config, payload_slot_capacity);
    projection_slots[0].layer = 0u;
    projection_slots[0].expert = 0u;
    projection_slots[0].descriptor.logical_expert_id = 0;
    projection_slots[0].epoch = 7u;
    projection_slots[0].generation = 41u;
    projection_slots[0].flags |=
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceDirectoryFlags::Resident) |
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceDirectoryFlags::CopyComplete);
    auto &cached_replica = projection_slots[1];
    cached_replica.layer = 0u;
    cached_replica.expert = 1u;
    cached_replica.descriptor.logical_expert_id = 1;
    cached_replica.descriptor.owner_participant = 0;
    cached_replica.descriptor.flags |=
        llaminar2::toMoEExpertFlags(
            llaminar2::DeviceMoEExpertFlags::Resident |
            llaminar2::DeviceMoEExpertFlags::Replicated);
    cached_replica.resident_mask =
        (1u << 0u) | (1u << config.participant_id);
    cached_replica.epoch = 6u;
    cached_replica.generation = 43u;
    cached_replica.flags |=
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceDirectoryFlags::Resident) |
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceDirectoryFlags::CopyComplete);
    auto &active =
        projection_runtime.banks[projection_runtime.active_bank];
    active.experts[1] = cached_replica.descriptor;
    active.local_compute_mask[1] = 0u;
    active.replica_role[1] =
        static_cast<uint8_t>(llaminar2::DeviceMoEReplicaRole::Replica);
    active.resident_participant_mask[1] =
        cached_replica.resident_mask;
    CudaAllocation d_projection_runtime(sizeof(projection_runtime));
    CudaAllocation d_projection_slots(
        projection_slots.size() * sizeof(projection_slots[0]));
    ...
    EXPECT_EQ(local_headers[0].command_count, 2u);
    EXPECT_EQ(local_headers[1].command_count, 2u);
    EXPECT_EQ(local_plan_entries[0].destination_slot, 0u);
    EXPECT_EQ(local_plan_entries[1].destination_slot, 1u);
    EXPECT_EQ(local_plan_entries[plan_capacity].destination_slot, 0u)
        << "The inactive ping-pong wave must not consume a lease from the "
           "active wave; each alternative is reprojected before execution.";
    EXPECT_EQ(local_plan_entries[plan_capacity + 1u].destination_slot, 1u);
    EXPECT_EQ(local_plan_entries[0].destination_previous_layer, 0u);
    EXPECT_EQ(local_plan_entries[0].destination_previous_expert, 0u);
    EXPECT_EQ(local_plan_entries[0].destination_generation, 41u);
    EXPECT_EQ(local_plan_entries[1].destination_previous_layer, 0u);
    EXPECT_EQ(local_plan_entries[1].destination_previous_expert, 1u);
    EXPECT_EQ(local_plan_entries[1].destination_generation, 43u);
    EXPECT_EQ(status.status_code, static_cast<uint32_t>(llaminar2::DeviceMoERebalanceStatusCode::Ok));
    EXPECT_EQ(status.planned_arrivals, 4u);
...
TEST_F(Test__CUDAMoEKernel, PrefillLLEPDomainProjectionMergesDestinationLocalRequestsForSourcePacking)
{
    ...
    llaminar2::DeviceMoELayerRuntime *d_runtime = nullptr;
    std::vector<llaminar2::DeviceMoEExpertDirectoryEntry> transfer_slots(
        payload_slot_capacity);
    ...
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_local_header), sizeof(local_header)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_status), sizeof(status)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_runtime), sizeof(runtime)), cudaSuccess);
    ...
    EXPECT_EQ(local_plan_entries[1].destination_slot,
              llaminar2::kDeviceMoEInvalidSlot)
...
/**
 * @brief Prove compact payload width never truncates physical slot allocation.
 *
 * The production transfer directory retains active replicas across all MoE
 * layers and appends staging slots for the next complete wave. Compact NCCL
 * payload width is intentionally much smaller. Earlier code reused that compact
 * width as the physical search bound, so layer 14 repeatedly proposed slots 0
 * and 1 even when those allocations were still published by layer 13.
 *
 * This regression seeds two live layer-0 claims in slots 0 and 1, then projects
 * two layer-1 arrivals with a two-lane collective into a four-slot directory.
 * Destination projection must lease the empty physical slots 2 and 3 without
 * weakening the live-claim checks or reporting capacity overflow.
 */
TEST_F(Test__CUDAMoEKernel, PrefillLLEPProjectionUsesFullPhysicalDirectoryBeyondCompactPayloadWidth)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    llaminar2::DeviceMoERuntimeTable::Config runtime_config;
    runtime_config.device_id = llaminar2::DeviceId::cuda(0);
    ...
    for (uint32_t layer = 0; layer < num_layers; ++layer)
    {
        ...
        for (uint32_t expert = 0; expert < num_experts; ++expert)
        {
            ...
            descriptor.gate = fakeNativeVNNIDesc(
                0x41000000u + static_cast<uintptr_t>(layer) * 0x100000u +
    ...
    std::array<llaminar2::DeviceMoEExpertDirectoryEntry, transfer_slot_count>
        transfer_slots{};
    for (uint32_t slot_index = 0; slot_index < transfer_slot_count; ++slot_index)
    {
        ...
        slot.descriptor.gate =
            fakeNativeVNNIDesc(0x42000000u + slot_index * 0x10000u + 0x10u);
        slot.descriptor.up =
            fakeNativeVNNIDesc(0x42000000u + slot_index * 0x10000u + 0x20u);
        slot.descriptor.down =
            fakeNativeVNNIDesc(0x42000000u + slot_index * 0x10000u + 0x30u);
        slot.descriptor.flags = llaminar2::toMoEExpertFlags(
            llaminar2::DeviceMoEExpertFlags::Valid |
            llaminar2::DeviceMoEExpertFlags::TransferSlot);
        slot.flags =
            static_cast<uint32_t>(
    ...
    std::array<llaminar2::DeviceMoERebalancePlanEntry,
    ...
    llaminar2::DeviceMoERebalanceStatus *d_status = nullptr;
    llaminar2::DeviceMoEExpertDirectoryEntry *d_transfer_slots = nullptr;
    ASSERT_EQ(cudaMalloc(&d_gathered_plan, sizeof(gathered_plan)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_gathered_headers, sizeof(gathered_headers)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_local_plan, plan_capacity * sizeof(*d_local_plan)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_local_plan_count, sizeof(*d_local_plan_count)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_local_header, sizeof(*d_local_header)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_status, sizeof(*d_status)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(&d_transfer_slots, sizeof(transfer_slots)), cudaSuccess);

    llaminar2::DeviceMoERebalanceStatus initial_status{};
    ASSERT_EQ(cudaMemcpyAsync(d_gathered_plan,
                              gathered_plan.data(),
    ...
    std::array<llaminar2::DeviceMoERebalancePlanEntry, plan_capacity> local_plan{};
    uint32_t local_plan_count = 0;
    llaminar2::DeviceMoERebalanceStatus status{};
    ASSERT_EQ(cudaMemcpy(local_plan.data(),
                         ...
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpy(&local_plan_count,
                         d_local_plan_count,
...
TEST_F(Test__CUDAMoEKernel, RuntimePrefillGatherScatter_ZeroCountExpertNoOps)
{
    ...
    ASSERT_TRUE(batch->ensureOnHost(stream_));
    ...
    ASSERT_TRUE(cuda_kernel_->scatterPrefillExpertResultsFromRuntime(
        ...
        /*expert_id=*/1, seq_len, d_model));
    ASSERT_TRUE(output->ensureOnHost(stream_));
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceControllerPlansMissingReplicaArrivals)
{
    ...
    EXPECT_FALSE(llaminar2::hasMoEExpertFlag(bank.experts[0].flags,
                                             llaminar2::DeviceMoEExpertFlags::Replicated));
    EXPECT_FALSE(llaminar2::hasMoEExpertFlag(
    ...
    EXPECT_EQ(bank.experts[0].gate.payload, nullptr);
    EXPECT_EQ(bank.experts[0].up.payload, nullptr);
    EXPECT_EQ(bank.experts[0].down.payload, nullptr);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceControllerPlansCapacityReleasingDynamicOwnershipTransfers)
{
    ...
    std::vector<uint64_t> gathered(static_cast<size_t>(config.participant_count) *
                                       config.num_layers * config.num_experts,
                                   0);
    gathered[0] = llaminar2::moe_rebalance_policy::packCollectedState(
        70, 1, /*physically_resident=*/true, /*transfer_backed=*/false);
    gathered[1] = llaminar2::moe_rebalance_policy::packCollectedState(
        30, 0, /*physically_resident=*/true, /*transfer_backed=*/true);
    gathered[config.num_experts] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            0, 1, /*physically_resident=*/false, /*transfer_backed=*/false);
    gathered[config.num_experts + 2] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            1, 0, /*physically_resident=*/true, /*transfer_backed=*/false);
    gathered[config.num_experts + 3] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            9, 0, /*physically_resident=*/true, /*transfer_backed=*/true);
    ...
    const auto &bank = runtime.banks[runtime.active_bank];
    EXPECT_EQ(bank.experts[1].owner_participant, 0)
...
/**
 * @brief Prove that LLEP never publishes ownership past durable capacity.
 *
 * Participant 1 already consumes its only persistent transfer slot. LLEP's
 * ideal row assignment would move expert 0 from participant 0, but that expert
 * is backed by static source storage and therefore needs a new destination
 * slot. The CUDA controller must retain the current valid ownership instead of
 * publishing a command that can only fit in transaction-only staging storage.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceControllerRejectsLLEPOwnershipBeyondPersistentCapacity)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    config.routed_assignment_policy =
        llaminar2::kDeviceMoERebalanceAssignmentLeastLoadedResident;
    config.flags = 0;
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::PlanMissingArrivals);
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::CollectLoadStats);
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::DeferRuntimeApply);
    ...
    gathered[0] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            80u, 0u, /*physically_resident=*/true,
            /*transfer_backed=*/false);
    gathered[1] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            20u, 0u, /*physically_resident=*/true,
            /*transfer_backed=*/false);
    gathered[config.num_experts] =
        llaminar2::moe_rebalance_policy::packCollectedState(
            0u, 1u, /*physically_resident=*/false,
            /*transfer_backed=*/false);
    gathered[config.num_experts + 2u] =
        llaminar2::moe_rebalance_policy::packCollectedState(
    ...
    uint32_t *d_plan_count = nullptr;
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_gathered),
                  gathered.size() * sizeof(uint64_t)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
              ...
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_plan),
                  sizeof(llaminar2::DeviceMoERebalancePlanEntry)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_plan_count),
                  sizeof(uint32_t)),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(
                  d_gathered,
    ...
    EXPECT_EQ(status.planned_arrivals, 0u);
    EXPECT_EQ(status.candidate_arrivals_considered, 1u);
    EXPECT_GE(status.capacity_limited_candidates, 1u);
 ...
 */
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceControllerUsesExactLLEPGroupedLoadsForReplicaEconomy)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    config.flags = 0;
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::HotReplicaCache);
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::PlanMissingArrivals);
    config.flags |= static_cast<uint32_t>(
        llaminar2::DeviceMoERebalanceFlags::CollectLoadStats);
    ...
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan),
                         sizeof(llaminar2::DeviceMoERebalancePlanEntry)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan_count), sizeof(uint32_t)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_command_header),
                         sizeof(llaminar2::DeviceMoERebalanceCommandBufferHeader)),
              cudaSuccess);
    ...
    ASSERT_EQ(cudaMemcpy(&plan, d_plan, sizeof(plan), cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpy(&command_header,
                         d_command_header,
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalancePublishNoopClearsStaleActiveWave)
{
    ...
    stampRebalanceCopyTransaction(copy_status, 1u, 0u, 0u);
    for (auto &peer_status : gathered_copy_status)
        stampRebalanceCopyTransaction(peer_status, 1u, 0u, 0u);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalancePublishUsesProjectedWavePayloadBucket)
{
    ...
    stampRebalanceCopyTransaction(copy_status, 0u, 9u, 1u);
    for (auto &peer_status : gathered_copy_status)
        stampRebalanceCopyTransaction(peer_status, 0u, 9u, 1u);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalancePublishRequiresGatheredDestinationCopyStatus)
{
    ...
    stampRebalanceCopyTransaction(copy_status, 0u, 11u, 1u);
    for (auto &peer_status : gathered_copy_status)
        stampRebalanceCopyTransaction(peer_status, 0u, 11u, 1u);
    ...
    ASSERT_TRUE(cuda_kernel_->publishDeviceRebalanceTransferComplete(
        ...
        command_buffer_count));
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    ASSERT_EQ(cudaMemcpy(&result, d_controller_state, sizeof(result),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
    EXPECT_EQ(result.last_error_code,
              static_cast<uint32_t>(llaminar2::DeviceMoERebalanceStatusCode::MissingTransferCompletion));
    ...
    EXPECT_EQ(result.last_error_copied_arrivals, 0u);
    EXPECT_EQ(
        result.last_error_copy_failure_flags,
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceCopyFailure::ArrivalShortfall));
    EXPECT_EQ(result.waves[0].state,
              static_cast<uint32_t>(llaminar2::DeviceMoERebalanceWaveLifecycle::Error));
    EXPECT_EQ(result.waves[0].epoch, 11u);
    EXPECT_EQ(result.maintenance_launches, 1u);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalancePublishRejectsSourceSideCopyErrors)
{
    ...
    plan_entries[0].layer = 0;
    plan_entries[0].expert = 1;
    plan_entries[0].source_participant = 1;
    plan_entries[0].destination_participant = 0;
    plan_entries[0].source_resident_mask = 0b10u;
    plan_entries[0].destination_slot = 0;
    plan_entries[0].payload_slot = 0;

    /*
     * Regression for long-context Dynamic+MTP+prefix cache: the source
     * participant 1 failed to pack a descriptor, so destination participant 0
     * also reports a missing arrival.  Publication must report the producer's
     * structural error before the downstream arrival-count symptom.  The old
     * single-pass validation stopped at participant 0 and hid the root cause.
     */
    llaminar2::DeviceMoERebalanceApplyStatus copy_status;
    gathered_copy_status[0].copied_arrivals = 0;
    gathered_copy_status[1].missing_source_descriptors = 1;
    /*
     * Copy-status diagnostics are an opaque participant-local evidence record
     * at publication time. Preserve every field from the first failing peer so
     * a secondary destination-lease symptom is not lost behind the primary
     * source-descriptor failure.
     */
    gathered_copy_status[1].first_missing_destination_slot = 17;
    gathered_copy_status[1].first_missing_destination_layer = 3;
    gathered_copy_status[1].first_missing_destination_expert = 6;
    gathered_copy_status[1].first_missing_destination_source = 0;
    gathered_copy_status[1].local_transfer_slot_count = 1;
    stampRebalanceCopyTransaction(copy_status, 0u, 12u, 1u);
    for (auto &peer_status : gathered_copy_status)
        stampRebalanceCopyTransaction(peer_status, 0u, 12u, 1u);
    ...
    EXPECT_EQ(wave.error_code,
              static_cast<uint32_t>(llaminar2::DeviceMoERebalanceStatusCode::MissingTransferCompletion));
    EXPECT_EQ(wave.error_participant, 1u);
    EXPECT_EQ(result.last_error_expected_arrivals, 0u);
    EXPECT_EQ(result.last_error_copied_arrivals, 0u);
    EXPECT_EQ(result.last_error_copy_status_code, 0u);
    EXPECT_EQ(
        result.last_error_copy_failure_flags,
        static_cast<uint32_t>(
            llaminar2::DeviceMoERebalanceCopyFailure::
                MissingSourceDescriptor));
    EXPECT_EQ(result.last_error_copy_plan_entries_seen, 0u);
    EXPECT_EQ(result.last_error_copy_skipped_wrong_destination, 0u);
    EXPECT_EQ(result.last_error_missing_destination_slot, 17u);
    EXPECT_EQ(result.last_error_missing_destination_layer, 3u);
    EXPECT_EQ(result.last_error_missing_destination_expert, 6u);
    EXPECT_EQ(result.last_error_missing_destination_source, 0u);
    EXPECT_EQ(result.last_error_local_transfer_slot_count, 1u);
...
TEST_F(Test__CUDAMoEKernel, ApplyReadyDeviceRebalanceWaveClearsAppliedCommandHeader)
{
    ...
    ASSERT_TRUE(cuda_kernel_->applyReadyDeviceRebalanceWave(
        ...
        command_buffer_count));
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    ASSERT_EQ(cudaMemcpy(&apply_status,
                         d_apply_status,
                         sizeof(apply_status),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
...
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceTransferTicketSurvivesActiveWaveAdvanceAndPreservesPackErrors)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    std::array<llaminar2::DeviceMoERebalancePlanEntry, command_buffer_count>
        plans{};
    plans[0] = plan;
    ...
    for (uint32_t wave = 0; wave < command_buffer_count; ++wave)
    {
        command_headers[wave].epoch = 11u + wave;
        command_headers[wave].command_capacity = plan_capacity;
        command_headers[wave].participant_id = config.participant_id;
        command_headers[wave].participant_count = config.participant_count;
    }
    command_headers[0].command_count = 1;
    ...
    llaminar2::DeviceMoERebalanceApplyStatus *d_copy_status = nullptr;
    llaminar2::DeviceMoERebalanceGraphControllerState *d_controller_state = nullptr;
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan), sizeof(plans)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_command_header),
                         sizeof(command_headers)),
              cudaSuccess);
    ...
    ASSERT_TRUE(cuda_kernel_->packDeviceRebalanceCompactPayloads(
        ...
        command_buffer_count));
    /*
     * Reproduce the production overlap: planning publishes the next command
     * wave while the preceding payload transaction is still in flight. Unpack
     * must consume the immutable ticket established by pack, not sample this
     * newly advanced controller cursor.
     */
    controller_state.active_wave = 1;
    ASSERT_EQ(cudaMemcpyAsync(d_controller_state,
                              &controller_state,
    ...
    EXPECT_EQ(copy_status.transaction_wave_index, 0u);
    EXPECT_EQ(copy_status.transaction_epoch, 11u);
    EXPECT_EQ(copy_status.transaction_command_count, 1u);
...
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceUnpackPublishesMissingDestinationLeaseProvenance)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    constexpr uint32_t command_buffer_count = 2;
    constexpr uint32_t local_payload_slot_count = 1;
    constexpr uint32_t local_transfer_slot_count = 1;
    constexpr uint64_t payload_slot_bytes = 512;
    constexpr uint32_t missing_destination_slot = 17;

    std::array<llaminar2::DeviceMoERebalancePlanEntry,
               command_buffer_count * plan_capacity>
        plans{};
    plans[0].op = static_cast<uint32_t>(
        llaminar2::DeviceMoERebalancePlanOp::OwnershipTransfer);
    plans[0].layer = 3;
    plans[0].expert = 6;
    plans[0].source_participant = 1;
    plans[0].destination_participant = config.participant_id;
    plans[0].source_resident_mask = 0b10u;
    plans[0].destination_slot = missing_destination_slot;
    plans[0].payload_slot = 0;
    ...
    command_headers[0].epoch = 19;
    command_headers[0].command_count = 1;

    llaminar2::DeviceMoERebalanceApplyStatus copy_status;
    stampRebalanceCopyTransaction(copy_status, 0u, 19u, 1u);
    llaminar2::DeviceMoERebalanceGraphControllerState controller_state;
    controller_state.participant_id = config.participant_id;
    controller_state.participant_count = config.participant_count;
    controller_state.active_wave = 1;
    ...
    const size_t gathered_payload_bytes =
        ...
        local_payload_slot_count *
        payload_slot_bytes;
    llaminar2::DeviceMoERebalancePlanEntry *d_plans = nullptr;
    llaminar2::DeviceMoERebalanceCommandBufferHeader *d_command_headers =
        nullptr;
    uint8_t *d_gathered_payload = nullptr;
    llaminar2::DeviceMoEExpertDirectoryEntry *d_transfer_slots = nullptr;
    llaminar2::DeviceMoERebalanceApplyStatus *d_copy_status = nullptr;
    llaminar2::DeviceMoERebalanceGraphControllerState *d_controller_state =
        nullptr;
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plans), sizeof(plans)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_command_headers),
                         sizeof(command_headers)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_gathered_payload),
    ...
    ASSERT_EQ(cudaMemcpyAsync(d_plans,
                              plans.data(),
                              sizeof(plans),
                              cudaMemcpyHostToDevice,
                              stream_),
    ...
    ASSERT_EQ(cudaMemsetAsync(
                  d_gathered_payload, 0, gathered_payload_bytes, stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemsetAsync(
                  d_transfer_slots,
                  0,
    ...
    ASSERT_EQ(cudaMemcpy(&copy_status,
                         d_copy_status,
                         sizeof(copy_status),
                         cudaMemcpyDeviceToHost),
              cudaSuccess);
    EXPECT_EQ(copy_status.missing_destination_slots, 1u);
    EXPECT_EQ(copy_status.first_missing_destination_slot,
              missing_destination_slot);
    EXPECT_EQ(copy_status.first_missing_destination_layer, 3u);
    EXPECT_EQ(copy_status.first_missing_destination_expert, 6u);
    EXPECT_EQ(copy_status.first_missing_destination_source, 1u);
    EXPECT_EQ(copy_status.local_transfer_slot_count,
              local_transfer_slot_count);
    EXPECT_EQ(copy_status.plan_entries_seen, 1u);
    EXPECT_EQ(copy_status.skipped_wrong_destination, 0u);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceCopyAndApplyArrivalUsesTransferSlot)
{
    ...
    plan_entries[0] = plan;
    llaminar2::DeviceMoERebalancePlanEntry metadata_plan;
    metadata_plan.op =
        static_cast<uint32_t>(llaminar2::DeviceMoERebalancePlanOp::ResidentExpertAssignment);
    ...
    metadata_plan.destination_slot = llaminar2::kDeviceMoEInvalidSlot;
    metadata_plan.payload_slot = llaminar2::kDeviceMoEInvalidSlot;
    ...
    EXPECT_EQ(bank.experts[0].gate.payload, projections[0].dst_payload);
    EXPECT_EQ(bank.experts[0].gate.scales, projections[0].dst_scales);
    EXPECT_EQ(bank.experts[0].up.payload, projections[1].dst_payload);
    EXPECT_EQ(bank.experts[0].up.scales, projections[1].dst_scales);
    EXPECT_EQ(bank.experts[0].down.payload, projections[2].dst_payload);
    EXPECT_EQ(bank.experts[0].down.scales, projections[2].dst_scales)
        << "arrival apply must publish destination-local slot addresses, never "
           "the packed source descriptor's device pointers";
    EXPECT_TRUE(llaminar2::hasMoEExpertFlag(bank.experts[0].flags,
                                            llaminar2::DeviceMoEExpertFlags::Replicated));
    ...
    EXPECT_EQ(rebuilt_bank.experts[0].gate.payload, projections[0].dst_payload);
    EXPECT_EQ(rebuilt_bank.experts[0].up.payload, projections[1].dst_payload);
    EXPECT_EQ(rebuilt_bank.experts[0].down.payload, projections[2].dst_payload)
        << "an unrelated runtime-bank rebuild must preserve destination-local "
           "transfer-slot pointer provenance";
    ...
    ASSERT_EQ(cudaMemsetAsync(d_gathered_payload,
                              ...
                              stream_),
              cudaSuccess);
    auto clean_copy_status = makeCleanRebalanceApplyStatus();
    stampRebalanceCopyTransaction(
        clean_copy_status,
        /*wave_index=*/0,
        destination_command_header.epoch,
        destination_command_header.command_count);
    ASSERT_EQ(cudaMemcpyAsync(d_copy_status,
                              &clean_copy_status,
    ...
    /*
     * Reuse the same production pack/unpack/apply buffers for an ownership
     * transfer of expert 0.  The payload header still names participant 0 as
     * the old owner.  Unpack must nevertheless publish participant 1 in the
     * destination transfer directory because apply publishes that same owner
     * into the runtime bank.  Keeping the old source owner in the directory
     * makes the slot look transfer-backed to capacity accounting but causes the
     * next full-capacity lease to reject its runtime/directory identity.
     */
    plan.op = static_cast<uint32_t>(
        llaminar2::DeviceMoERebalancePlanOp::OwnershipTransfer);
    plan.destination_previous_layer = transfer_slots[0].layer;
    plan.destination_previous_expert = transfer_slots[0].expert;
    plan.destination_generation = transfer_slots[0].generation;
    source_command_header.epoch = 6;
    source_command_header.command_count = 1;
    source_command_header.participant_id = source_config.participant_id;
    destination_command_header = source_command_header;
    destination_command_header.participant_id = config.participant_id;
    ASSERT_EQ(cudaMemcpyAsync(
                  d_plan, &plan, sizeof(plan), cudaMemcpyHostToDevice, stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(d_command_header,
                              &source_command_header,
                              sizeof(source_command_header),
    ...
    ASSERT_EQ(cudaMemcpy(
                  ...
                  cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(copy_status.copied_arrivals, 1u);
    EXPECT_EQ(transfer_slots[0].descriptor.owner_participant,
              config.participant_id)
        << "ownership-transfer unpack must publish the new owner in the "
           "physical directory, not retain the payload source owner";
    ...
    ASSERT_EQ(cudaMemcpy(
                  ...
                  cudaMemcpyDeviceToHost),
              cudaSuccess);
    const auto &ownership_bank = runtime.banks[runtime.active_bank];
    EXPECT_EQ(ownership_bank.experts[0].owner_participant,
              config.participant_id);
    EXPECT_EQ(ownership_bank.experts[0].owner_participant,
              transfer_slots[0].descriptor.owner_participant)
        << "runtime and transfer-directory publications must agree exactly";
    EXPECT_EQ(ownership_bank.replica_role[0],
              static_cast<uint8_t>(
                  llaminar2::DeviceMoEReplicaRole::Primary));
    EXPECT_EQ(ownership_bank.resident_participant_mask[0], 0b010u);
...
/**
 * @brief Prove stale transfer commands cannot mutate a published CUDA slot.
 *
 * The destination directory is authoritative. A command projected against an
 * older generation must fail its compare-and-replace check before source
 * inspection or any destructive directory update.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceUnpackRejectsStaleTransferSlotGenerationWithoutMutation)
{
    ...
    llaminar2::DeviceMoEExpertDirectoryEntry slot;
    ...
    status.transaction_wave_index = 0u;
    status.transaction_epoch = header.epoch;
    status.transaction_command_count = header.command_count;

    constexpr uint64_t payload_slot_bytes = 512;
    CudaAllocation d_plan(sizeof(plan));
    CudaAllocation d_header(sizeof(header));
    CudaAllocation d_gathered_payload(
        config.participant_count * payload_slot_bytes);
    CudaAllocation d_slot(sizeof(slot));
    CudaAllocation d_status(sizeof(status));
    ASSERT_EQ(cudaMemcpyAsync(
                  d_plan.get(), &plan, sizeof(plan), cudaMemcpyHostToDevice, stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(
                  d_header.get(), &header, sizeof(header), cudaMemcpyHostToDevice, stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(
                  d_slot.get(), &slot, sizeof(slot), cudaMemcpyHostToDevice, stream_),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(
                  d_status.get(), &status, sizeof(status), cudaMemcpyHostToDevice, stream_),
              cudaSuccess);

    ASSERT_TRUE(cuda_kernel_->unpackDeviceRebalanceCollectivePayloads(
        launchContext(),
        static_cast<llaminar2::DeviceMoERebalancePlanEntry *>(d_plan.get()),
        nullptr,
        /*plan_capacity=*/1,
        ...
        payload_slot_bytes,
        static_cast<llaminar2::DeviceMoEExpertDirectoryEntry *>(d_slot.get()),
        /*local_transfer_slot_count=*/1,
        config,
        static_cast<llaminar2::DeviceMoERebalanceApplyStatus *>(d_status.get())));
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    ASSERT_EQ(cudaMemcpy(
                  &status, d_status.get(), sizeof(status), cudaMemcpyDeviceToHost),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpy(
                  &slot, d_slot.get(), sizeof(slot), cudaMemcpyDeviceToHost),
              cudaSuccess);
...
/**
 * @brief Prove that two transfer waves cannot publish two experts for one slot.
 *
 * A transfer-directory slot owns one mutable gate/up/down payload allocation.
 * Reusing that allocation for a later expert must atomically retire the earlier
 * cached replica from the runtime bank before the new expert becomes locally
 * computable. Otherwise both logical experts retain valid descriptors pointing
 * at the bytes of the second arrival, producing route-dependent corruption.
 *
 * This test drives the real CUDA apply kernel twice. The first wave publishes
 * expert 0 into stable slot 7; the second wave rewrites that same slot with
 * expert 1. The final bank must contain exactly one live transfer-backed
 * occupant for slot 7, while preserving expert 0's remote residency metadata.
 */
TEST_F(Test__CUDAMoEKernel, DeviceRebalanceTransferSlotReuseRetiresPreviousReplica)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    uint32_t *d_plan_count = nullptr;
    llaminar2::DeviceMoERebalanceCommandBufferHeader *d_command_header = nullptr;
    llaminar2::DeviceMoEExpertDirectoryEntry *d_transfer_slot = nullptr;
    llaminar2::DeviceMoERebalanceApplyStatus *d_apply_status = nullptr;
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan), sizeof(*d_plan)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_plan_count), sizeof(*d_plan_count)), cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_command_header),
                         sizeof(*d_command_header)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_transfer_slot),
                         sizeof(*d_transfer_slot)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(reinterpret_cast<void **>(&d_apply_status),
                         sizeof(*d_apply_status)),
              cudaSuccess);
    ASSERT_EQ(cudaMemcpyAsync(d_plan_count,
                              &legacy_plan_count,
    ...
    auto publish_wave = [&](uint32_t expert, uint32_t epoch)
    {
        ...
        ASSERT_EQ(cudaMemcpyAsync(d_command_header,
                                  ...
                                  stream_),
                  cudaSuccess);
        ASSERT_EQ(cudaMemcpyAsync(d_transfer_slot,
                                  &slot,
                                  sizeof(slot),
                                  cudaMemcpyHostToDevice,
                                  stream_),
        ...
        EXPECT_EQ(status.status_code,
                  static_cast<uint32_t>(
                      llaminar2::DeviceMoERebalanceApplyStatusCode::Ok));
    ...
    EXPECT_EQ(bank.resident_participant_mask[0], 0b001u)
        << "retiring a local cache entry must retain the authoritative remote residency";
    ...
    EXPECT_TRUE(llaminar2::hasMoEExpertFlag(
    ...
    EXPECT_EQ(live_slot_occupants, 1u)
        << "one mutable transfer allocation must have one live logical owner";

    cudaFree(d_plan);
    cudaFree(d_plan_count);
    cudaFree(d_command_header);
    cudaFree(d_transfer_slot);
    cudaFree(d_apply_status);
#endif
}

/**
 * @brief Prove graph-native apply retires a reused slot across layer banks.
 *
 * Decode applies one target layer per captured graph boundary. A transfer slot
 * is model-wide storage, so an arrival for layer 1 can replace a cold replica
 * previously published by layer 0. The apply transition must rebuild both
 * placement banks atomically even though only layer 1 advances the wave cursor.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceReadyWaveRetiresCrossLayerTransferSlotOccupant)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    constexpr uint32_t num_layers = 2;
    constexpr uint32_t num_experts = 4;
    constexpr uint32_t target_layer = 1;
    constexpr uint32_t old_expert = 0;
    constexpr uint32_t new_expert = 1;
    constexpr uint32_t transfer_slot_count = 1;
    constexpr uint32_t command_buffer_count = 2;
    constexpr uint32_t plan_capacity = 1;

    DeviceMoERuntimeTable::Config table_config;
    table_config.device_id = DeviceId::cuda(0);
    ...
    for (uint32_t layer = 0; layer < num_layers; ++layer)
    {
        ...
        if (layer == 0u)
        {
            auto &descriptor = update.experts[old_expert];
            ...
            update.local_compute_mask[old_expert] = 1u;
            update.replica_role[old_expert] =
                static_cast<uint8_t>(DeviceMoEReplicaRole::Replica);
            update.resident_participant_mask[old_expert] = 0b011u;
    ...
    std::array<DeviceMoERebalancePlanEntry,
               plan_capacity * command_buffer_count>
        plans{};
    auto &plan = plans[0];
    plan.op = static_cast<uint32_t>(
        DeviceMoERebalancePlanOp::ExpertPayloadArrival);
    plan.layer = target_layer;
    plan.expert = new_expert;
    plan.source_participant = 0;
    plan.destination_participant = config.participant_id;
    plan.source_resident_mask = 0b001u;
    plan.destination_slot = 0;
    plan.payload_slot = 0;
    plan.destination_previous_layer = 0;
    plan.destination_previous_expert = old_expert;
    plan.destination_generation = 0;

    std::array<DeviceMoERebalanceCommandBufferHeader,
               command_buffer_count>
        headers{};
    ...
    headers[0].epoch = 2;
    headers[0].command_count = 1;

    DeviceMoERebalanceGraphControllerState controller;
    controller.participant_id = config.participant_id;
    controller.participant_count = config.participant_count;
    controller.next_epoch = 3;
    controller.active_wave = 0;
    controller.wave_count = command_buffer_count;
    auto &wave = controller.waves[0];
    wave.epoch = headers[0].epoch;
    wave.state = static_cast<uint32_t>(
        DeviceMoERebalanceWaveLifecycle::ReadyToApply);
    wave.planned_start_layer = target_layer;
    wave.planned_layer_count = 1;
    wave.command_count = 1;
    wave.copied_arrivals = 1;
    wave.requested_payload_slots = 1;
    ...
    slot.descriptor.gate = fakeNativeVNNIDesc(0x41000010u);
    slot.descriptor.up = fakeNativeVNNIDesc(0x41000020u);
    slot.descriptor.down = fakeNativeVNNIDesc(0x41000030u);
    slot.descriptor.logical_expert_id =
        static_cast<int32_t>(new_expert);
    slot.descriptor.owner_participant = 0;
    slot.descriptor.local_slot = 0;
    slot.descriptor.flags = toMoEExpertFlags(
        DeviceMoEExpertFlags::Valid |
        DeviceMoEExpertFlags::Resident |
        DeviceMoEExpertFlags::LocalCompute |
        DeviceMoEExpertFlags::TransferSlot);
    slot.layer = target_layer;
    slot.expert = new_expert;
    slot.participant = config.participant_id;
    slot.resident_mask =
        1u << config.participant_id;
    slot.epoch = headers[0].epoch;
    slot.slot_index = 0;
    slot.generation = 1;
    slot.flags =
        static_cast<uint32_t>(
    ...
    DeviceMoERebalancePlanEntry *d_plans = nullptr;
    DeviceMoERebalanceCommandBufferHeader *d_headers = nullptr;
    DeviceMoERebalanceGraphControllerState *d_controller = nullptr;
    DeviceMoEExpertDirectoryEntry *d_slot = nullptr;
    DeviceMoERebalanceApplyStatus *d_status = nullptr;
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_plans),
                  sizeof(plans)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_headers),
                  sizeof(headers)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_controller),
                  sizeof(controller)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_slot),
                  sizeof(slot)),
              cudaSuccess);
    ASSERT_EQ(cudaMalloc(
                  reinterpret_cast<void **>(&d_status),
    ...
    std::array<DeviceMoELayerRuntime, num_layers> runtime{};
    ASSERT_EQ(cudaMemcpy(
                  &status,
                  d_status,
                  sizeof(status),
    ...
    const auto &old_bank =
        runtime[0].banks[runtime[0].active_bank];
    EXPECT_EQ(old_bank.local_compute_mask[old_expert], 0u);
    EXPECT_EQ(
        old_bank.resident_participant_mask[old_expert] &
            (1u << config.participant_id),
        0u);
    EXPECT_EQ(old_bank.experts[old_expert].local_slot, -1);
    EXPECT_FALSE(hasMoEExpertFlag(
        old_bank.experts[old_expert].flags,
        DeviceMoEExpertFlags::TransferSlot));
    EXPECT_EQ(old_bank.experts[old_expert].gate.payload, nullptr);

    const auto &new_bank =
        runtime[target_layer]
            .banks[runtime[target_layer].active_bank];
    EXPECT_EQ(new_bank.local_compute_mask[new_expert], 1u);
    EXPECT_EQ(new_bank.experts[new_expert].local_slot, 0);
    EXPECT_TRUE(hasMoEExpertFlag(
        new_bank.experts[new_expert].flags,
        DeviceMoEExpertFlags::TransferSlot));
    ...
    const auto claims =
        deviceMoETransferSlotClaimSummary(
            runtime.data(),
            config);
    EXPECT_EQ(claims.active_claims, 1u);
    EXPECT_EQ(claims.unique_claims, 1u);
    EXPECT_EQ(claims.duplicate_claims, 0u);
    EXPECT_EQ(claims.invalid_claims, 0u);
...
/**
 * @brief Stress the complete graph-native transfer-slot publication lifecycle.
 *
 * This workload deliberately keeps every durable slot occupied while rotating
 * the whole directory through eight seeded traffic regimes across the 20-layer
 * Qwen 3.6 topology. Waves carry one to four arrivals, deliberately pause with
 * one incomplete copy, retry after publication, and occasionally replay a
 * consumed target. Device allocations are graph-lifetime fixtures: no
 * allocation, deallocation, or pointer rebinding occurs inside the wave loop.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceTransferStateMachineFullCapacityStress)
{
    ...
    using namespace llaminar2;
    namespace model =
        llaminar2::test::moe_transfer_state_machine;
    constexpr uint32_t plan_capacity =
        model::kMaxCommandsPerWave;
    constexpr uint32_t command_buffer_count = 2u;

    auto occupants = model::initialOccupants();

    DeviceMoERuntimeTable::Config table_config;
    table_config.device_id = DeviceId::cuda(0);
    table_config.num_layers = model::kLayerCount;
    table_config.num_experts = model::kExpertCount;
    table_config.top_k = model::kTopK;
    table_config.mirror_to_device = true;
    DeviceMoERuntimeTable runtime_table(table_config);

    for (uint32_t layer = 0;
         layer < model::kLayerCount;
         ++layer)
    {
        const auto update =
            model::makeInitialLayerUpdate(layer, occupants);
        ASSERT_TRUE(runtime_table.prepareInactiveBank(
            static_cast<int>(layer),
            update));
        ASSERT_TRUE(runtime_table.flipActiveBank(
    ...
    DeviceMoERebalanceConfig config;
    config.num_layers = model::kLayerCount;
    config.num_experts = model::kExpertCount;
    config.top_k = model::kTopK;
    config.participant_id = model::kParticipantId;
    config.participant_count = model::kParticipantCount;
    config.window_size_tokens = 1u;
    config.flags |= static_cast<uint32_t>(
        DeviceMoERebalanceFlags::CollectLoadStats);
    config.active_transfer_slot_capacity =
        model::kTransferSlotCount;
    config.transfer_slot_directory_capacity =
        model::kTransferSlotCount;
    ...
    std::array<DeviceMoELayerRuntime,
               model::kLayerCount>
        runtime{};
    auto runtime_before =
        std::make_unique<
            std::array<DeviceMoELayerRuntime,
                       model::kLayerCount>>();
    DeviceMoERebalanceApplyStatus status{};
    uint32_t same_layer_replacements = 0u;
    uint32_t cross_layer_replacements = 0u;
    uint32_t delayed_publications = 0u;
    uint32_t idempotent_replays = 0u;
    ...
    ASSERT_EQ(cudaMemcpyAsync(
                  ...
                  stream_),
              cudaSuccess);
    ASSERT_EQ(cudaStreamSynchronize(stream_),
              cudaSuccess);
    ...
    for (uint32_t wave_number = 0;
         wave_number < model::kStressWaveCount;
         ++wave_number)
    {
        SCOPED_TRACE(
            ::testing::Message()
        ...
        const auto transaction =
            model::makeAdversarialWave(
                occupants,
                wave_number,
                command_epoch);
        same_layer_replacements +=
            transaction.same_layer_replacements;
        cross_layer_replacements +=
            transaction.cross_layer_replacements;
        const uint32_t wave_index =
            controller.active_wave %
            command_buffer_count;

        auto plan_begin =
            plans.begin() +
            wave_index * plan_capacity;
        std::fill_n(
            plan_begin,
            plan_capacity,
            DeviceMoERebalancePlanEntry{});
        std::copy_n(
            transaction.plans.begin(),
            transaction.command_count,
            plan_begin);

        auto &header = headers[wave_index];
        header.epoch = command_epoch;
        header.phase = static_cast<uint32_t>(
            DeviceMoERebalancePipelinePhase::PlanAssignments);
        header.command_count =
            transaction.command_count;

        auto &wave = controller.waves[wave_index];
        wave = {};
        wave.epoch = command_epoch;
        wave.state = static_cast<uint32_t>(
            DeviceMoERebalanceWaveLifecycle::ReadyToApply);
        wave.planned_start_layer =
            transaction.target_layer;
        wave.planned_layer_count = 1u;
        wave.command_count =
            transaction.command_count;
        wave.copied_arrivals =
            transaction.command_count;
        wave.requested_payload_slots =
            transaction.command_count;
        wave.payload_bucket_slots =
            transaction.command_count;
        controller.next_epoch = command_epoch + 1u;

        for (uint32_t command = 0;
             command < transaction.command_count;
             ++command)
        {
            transfer_slots[
                transaction.slot_indices[command]] =
                transaction.completed_slots[command];
        }
        ...
        const bool delay_last_publication =
            (wave_number % 17u) == 0u;
        for (uint32_t command = 0;
             command < transaction.command_count;
             ++command)
        {
            auto publication =
                transaction.completed_slots[command];
            if (delay_last_publication &&
                command + 1u ==
                    transaction.command_count)
            {
                publication.flags &=
                    ~static_cast<uint32_t>(
                        DeviceMoERebalanceDirectoryFlags::
                            CopyComplete);
            }
            ASSERT_EQ(cudaMemcpyAsync(
                          d_transfer_slots +
                              transaction
                                  .slot_indices[command],
                          &publication,
                          sizeof(publication),
                          cudaMemcpyHostToDevice,
                          stream_),
        ...
        *runtime_before = runtime;
        if (delay_last_publication)
        {
            ...
            ASSERT_EQ(
                status.status_code,
                static_cast<uint32_t>(
                    DeviceMoERebalanceApplyStatusCode::
                        InProgress));
            ASSERT_EQ(
            ...
            ASSERT_EQ(
                ...
                transaction.command_count - 1u);
            ASSERT_EQ(status.copy_incomplete, 1u);
            ASSERT_EQ(
                std::memcmp(
                    runtime_before->data(),
                    runtime.data(),
                    sizeof(runtime)),
                0)
                << "an incomplete transaction must not publish any partial runtime bank";
            ASSERT_EQ(
                controller.waves[wave_index].state,
                static_cast<uint32_t>(
                    DeviceMoERebalanceWaveLifecycle::
                        ReadyToApply));
            ASSERT_EQ(
                controller.active_wave,
                wave_index);
            ASSERT_EQ(
                headers[wave_index].command_count,
                transaction.command_count);

            const uint32_t last_command =
                transaction.command_count - 1u;
            ASSERT_EQ(cudaMemcpyAsync(
                          d_transfer_slots +
                              transaction.slot_indices[
                                  last_command],
                          &transaction.completed_slots[
                              last_command],
                          sizeof(
                              DeviceMoEExpertDirectoryEntry),
                          cudaMemcpyHostToDevice,
                          stream_),
                      cudaSuccess);
        }
        ...
        ASSERT_TRUE(
            cuda_kernel_->applyReadyDeviceRebalanceWave(
                launchContext(),
                runtime_table.deviceLayerState(0),
                d_plans,
                nullptr,
        ...
        ASSERT_EQ(
            ...
            transaction.expected_changed_layers);
        ASSERT_EQ(controller.last_error_code, 0u);
        ASSERT_EQ(
            controller.waves[wave_index].state,
            static_cast<uint32_t>(
                DeviceMoERebalanceWaveLifecycle::Applied));
        ASSERT_EQ(headers[wave_index].command_count, 0u);
        ASSERT_EQ(headers[wave_index].epoch, 0u);
        ASSERT_EQ(
            controller.active_wave,
            (wave_index + 1u) %
                command_buffer_count);
        ...
        if ((wave_number % 23u) == 0u)
        {
            ++idempotent_replays;
            *runtime_before = runtime;
            const uint32_t active_wave_before =
                controller.active_wave;
            const uint32_t polls_before =
                controller.decode_apply_polls;
            const uint32_t hits_before =
            ...
            ASSERT_EQ(
                std::memcmp(
                    ...
                    sizeof(runtime)),
                0)
                << "replaying a consumed command buffer must be a byte-exact no-op";
            ASSERT_EQ(
                controller.active_wave,
                active_wave_before);
            ASSERT_EQ(
    ...
    /**
     * A malformed transaction is terminal, but it must still be atomic. Run
     * each corruption against a freshly initialized request controller while
     * preserving the exact runtime image produced by the legal stress corpus.
     */
    auto expect_malformed_transaction_poisoned =
        [&](bool duplicate_physical_slot,
            uint32_t corruption_index)
    {
        const uint32_t command_epoch =
            model::kStressWaveCount +
            corruption_index + 2u;
        auto malformed =
            model::makeAdversarialWave(
                occupants,
                model::kStressWaveCount +
                    corruption_index + 1u,
                command_epoch);
        ASSERT_GE(malformed.command_count, 2u);
        malformed.command_count = 2u;

        if (duplicate_physical_slot)
        {
            /*
             * Two distinct experts cannot claim one physical payload slot.
             * The second command's otherwise-ready directory publication
             * remains in its original slot so duplicate-key validation, not
             * an accidental descriptor mismatch, owns the failure.
             */
            malformed.plans[1].destination_slot =
                malformed.plans[0].destination_slot;
        }
        else
        {
            /*
             * Two physical slots cannot publish the same logical expert.
             * Make the second descriptor internally coherent so the only
             * invalid condition is duplicate logical ownership.
             */
            malformed.plans[1].layer =
                malformed.plans[0].layer;
            malformed.plans[1].expert =
                malformed.plans[0].expert;
            malformed.plans[1].source_participant =
                malformed.plans[0].source_participant;
            malformed.plans[1].source_resident_mask =
                malformed.plans[0].source_resident_mask;
            auto &publication =
                malformed.completed_slots[1];
            publication.layer =
                malformed.plans[0].layer;
            publication.expert =
                malformed.plans[0].expert;
            publication.descriptor.logical_expert_id =
                static_cast<int32_t>(
                    malformed.plans[0].expert);
            publication.descriptor.owner_participant =
                static_cast<int32_t>(
                    malformed.plans[0]
                        .source_participant);
            publication.resident_mask =
                malformed.plans[0]
                    .source_resident_mask |
                (1u << config.participant_id);
        }
        ...
        std::fill(
            ...
            DeviceMoERebalancePlanEntry{});
        std::copy_n(
            malformed.plans.begin(),
            malformed.command_count,
            plans.begin());
        headers = {};
        for (auto &header : headers)
        {
        ...
        headers[0].epoch = command_epoch;
        headers[0].phase = static_cast<uint32_t>(
            DeviceMoERebalancePipelinePhase::
                PlanAssignments);
        headers[0].command_count =
            malformed.command_count;

        controller = {};
        controller.participant_id =
            config.participant_id;
        controller.participant_count =
            config.participant_count;
        controller.wave_count =
            command_buffer_count;
        controller.next_epoch =
            command_epoch + 1u;
        auto &malformed_wave =
            controller.waves[0];
        malformed_wave.epoch = command_epoch;
        malformed_wave.state =
            static_cast<uint32_t>(
                DeviceMoERebalanceWaveLifecycle::
                    ReadyToApply);
        malformed_wave.planned_start_layer =
            malformed.target_layer;
        malformed_wave.planned_layer_count = 1u;
        malformed_wave.command_count =
            malformed.command_count;
        malformed_wave.copied_arrivals =
            malformed.command_count;
        malformed_wave.requested_payload_slots =
            malformed.command_count;
        malformed_wave.payload_bucket_slots =
            malformed.command_count;

        auto malformed_slots = transfer_slots;
        for (uint32_t command = 0u;
             command < malformed.command_count;
             ++command)
        {
            malformed_slots[
                malformed.slot_indices[command]] =
                malformed.completed_slots[command];
        }
        ...
        ASSERT_EQ(cudaMemcpyAsync(
                      d_transfer_slots,
                      malformed_slots.data(),
                      sizeof(malformed_slots),
                      cudaMemcpyHostToDevice,
                      stream_),
                  cudaSuccess);
        ...
        *runtime_before = runtime;
        ...
        EXPECT_EQ(
            ...
            static_cast<uint32_t>(
                DeviceMoERebalanceApplyStatusCode::
                    MissingPlan));
        EXPECT_EQ(status.invalid_plan_entries, 1u);
        EXPECT_EQ(status.applied_arrivals, 0u);
        EXPECT_EQ(status.changed_layers, 0u);
        EXPECT_EQ(
            std::memcmp(
                runtime_before->data(),
                runtime.data(),
                sizeof(runtime)),
            0)
            << "a malformed transaction must not publish any runtime bank";
        EXPECT_EQ(
            controller.last_error_code,
            static_cast<uint32_t>(
                DeviceMoERebalanceApplyStatusCode::
                    MissingPlan));
        EXPECT_EQ(
            controller.waves[0].state,
            static_cast<uint32_t>(
                DeviceMoERebalanceWaveLifecycle::
                    Error));
        EXPECT_EQ(
            controller.waves[0].error_code,
            static_cast<uint32_t>(
                DeviceMoERebalanceApplyStatusCode::
                    MissingPlan));
    };

    expect_malformed_transaction_poisoned(
        /*duplicate_physical_slot=*/true,
        /*corruption_index=*/0u);
    expect_malformed_transaction_poisoned(
        /*duplicate_physical_slot=*/false,
        /*corruption_index=*/1u);

    EXPECT_GT(same_layer_replacements, 0u);
    EXPECT_GT(cross_layer_replacements, 0u);
    EXPECT_GT(delayed_publications, 0u);
    EXPECT_GT(idempotent_replays, 0u);
...
TEST_F(Test__CUDAMoEKernel, DeviceRebalancePackHistogramsFeedsController)
{
    ...
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    EXPECT_EQ(
        llaminar2::moe_rebalance_policy::collectedStateActivationCount(
            packed_counts[0]),
        8u);
    EXPECT_EQ(
        llaminar2::moe_rebalance_policy::collectedStateActivationCount(
    ...
    EXPECT_EQ(
        ...
        0u);
    EXPECT_TRUE(
        llaminar2::moe_rebalance_policy::collectedStatePhysicallyResident(
            packed_counts[2]));
    EXPECT_FALSE(
        llaminar2::moe_rebalance_policy::collectedStateTransferBacked(
            packed_counts[2]));
 ...
 * Its transfer-backed bytes remain the only durable copy and still consume one
 * active claim. The descriptor deliberately resides at slot 1 in a two-entry
 * directory whose active capacity is one. This exercises both the production
 * CUDA collector and controller audit so physical addressability cannot again
 * be confused with the active-claim limit.
 */
TEST_F(Test__CUDAMoEKernel,
       DeviceRebalanceControllerAcceptsPromotedStagingOriginSlot)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
    ...
    EXPECT_TRUE(
        llaminar2::moe_rebalance_policy::collectedStateTransferBacked(
            packed[0]));
    EXPECT_EQ(
        llaminar2::moe_rebalance_policy::collectedStateActiveTransferSlots(
            packed[0]),
        1u);
    EXPECT_EQ(
        status.status_code,
...
TEST_F(Test__CUDAMoEKernel, RouteWithTensorsMatchesCPU)
{
    ...
    EXPECT_TRUE(cuda_host_result.expert_indices.empty());
    EXPECT_TRUE(cuda_host_result.expert_weights.empty());
    EXPECT_TRUE(cuda_host_result.router_logits.empty());
}
...
TEST_F(Test__CUDAMoEKernel, RouteWithTensorsTiledPrefillCapturesAfterWarmup)
{
    ...
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    EXPECT_TRUE(warmup_host_result.expert_indices.empty());
    EXPECT_TRUE(warmup_host_result.expert_weights.empty());
...
TEST_F(Test__CUDAMoEKernel, RouteWithTensorsSingleTokenQwenScaleKeepsOutputsDeviceOwned)
{
#ifndef HAVE_CUDA
    GTEST_SKIP() << "CUDA support not compiled";
#else
 ...
 * exercises every NativeVNNI tensor format through the complete production
 * transaction: grouped Q8 routing, routed expert execution, shared expert
 * execution, and graph capture/replay.  Repeated M=1 table decode remains the
 * serial oracle, while exact layer-31 activation words make Q8 rounding drift
 * reproducible instead of relying on a low-amplitude synthetic coincidence.
 */
TEST_F(Test__CUDAMoEKernel, SharedExpertFFNStageVerifierRowsQwen36AllNativeFormatsMatchSerialGroupedDecode)
{
    ...
    for (size_t i = 0; i < router_values.size(); ++i)
    {
        router_values[i] =
...
#include "../fixtures/Qwen36Layer31M2FFNNormBits.inc"
    ...
    for (const int intermediate : {512, 256})
    {
        ...
        for (size_t format_index = 0; format_index < formats.size(); ++format_index)
        {
            ...
            auto descriptor_probe_down_w = format.create(
                {static_cast<size_t>(d_model), static_cast<size_t>(intermediate)},
                static_cast<uint32_t>(6293 + 10 * format_index));
            auto descriptor_probe_down_prepared = llaminar2::test::makeGpuPreparedGemm(
                descriptor_probe_down_w.get(),
                device,
                std::string("test.cuda_moe.qwen36_shared_stage_verifier.") +
                    format.label + ".routed_down_probe." + std::to_string(intermediate),
                llaminar2::ModelContextId{
                    621000 + 100 * format_index + static_cast<size_t>(intermediate)});

            /*
             * Keep each format's descriptor registry independent.  With the
             * historical variable-width slot arithmetic, routed slot zero and
             * singleton slot one overlap.  Reusing the fixture-wide registry
             * only exercised that exact collision for the first format in the
             * loop, which made the nominal all-format regression misleading.
             */
             ...
             * tensors are allocated once for the maximum verifier bucket and
             * keep stable device addresses across every runtime-M stage below.
             * SharedExpertFFNStage deliberately has no GPU allocation fallback.
             */
            ...
            auto graph_up_scratch = makeZeros(
                {static_cast<size_t>(kVerifierRowCapacity),
                 static_cast<size_t>(intermediate)});
            ASSERT_TRUE(graph_gate_scratch->ensureOnDevice(device, stream_));
            ASSERT_TRUE(graph_up_scratch->ensureOnDevice(device, stream_));
             ...
             * all-format sweep validates the real mixed-width descriptor arena
            ...
            llaminar2::DeviceNativeVNNIMatrixDesc routed_probe_down_desc{};
            ...
            /*
             * Expert 1 swaps the valid gate and up descriptors so its outputs
             * carry a distinct byte signature without introducing a synthetic
             * descriptor format.  A subsequently published singleton shared
             * table must leave this routed table untouched.  Keeping the probe
             * in this loop extends that isolation proof to every CUDA-native
             * codebook in the canonical grouped sweep.
             */
            std::vector<llaminar2::DeviceNativeVNNIMatrixDesc> routed_gate_descs(
                routed_experts,
                serial_gate_desc);
            std::vector<llaminar2::DeviceNativeVNNIMatrixDesc> routed_up_descs(
                routed_experts,
                serial_up_desc);
            ...
            ASSERT_GE(routed_gateup_table, 0)
                << "routed descriptor probe rejected format=" << format.label;
            std::vector<llaminar2::DeviceNativeVNNIMatrixDesc> routed_down_descs(
                routed_experts,
                serial_down_desc);
            routed_down_descs[descriptor_probe_expert] = routed_probe_down_desc;
            const int routed_down_table =
                moe_kernel.uploadGroupedExpertDownDescriptorTable(
                    routed_down_descs.data(),
                    routed_experts,
                    d_model,
                    intermediate);
            ASSERT_GE(routed_down_table, 0)
                << "routed down descriptor probe rejected format=" << format.label;

            std::vector<float> descriptor_probe_values(static_cast<size_t>(d_model));
            for (size_t i = 0; i < descriptor_probe_values.size(); ++i)
            {
                descriptor_probe_values[i] =
                    0.019f * std::sin(0.0071f * static_cast<float>(i + 5)) -
                    0.013f * std::cos(0.0043f * static_cast<float>(i + 17));
            }
            auto descriptor_probe_input = makeTensor(
                {1u, static_cast<size_t>(d_model)},
                descriptor_probe_values);
            ASSERT_TRUE(descriptor_probe_input->ensureOnDevice(device, stream_));
            ...
            {
                auto gate = makeZeros({1u, static_cast<size_t>(intermediate)});
                auto up = makeZeros({1u, static_cast<size_t>(intermediate)});
                auto down = makeZeros({1u, static_cast<size_t>(d_model)});
                EXPECT_TRUE(gate->ensureOnDevice(device, stream_));
                EXPECT_TRUE(up->ensureOnDevice(device, stream_));
                EXPECT_TRUE(down->ensureOnDevice(device, stream_));
                llaminar2::ITensor *gate_outputs[1] = {gate.get()};
                llaminar2::ITensor *up_outputs[1] = {up.get()};
                ...
                EXPECT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
                return {
                    copyCudaFP32TensorToHost(gate, stream_),
                    copyCudaFP32TensorToHost(up, stream_),
                    copyCudaFP32TensorToHost(down, stream_)};
            };
            ...
            for (const int seq_len : llaminar2::test::kGroupedVerifierRuntimeRows)
            {
                ...
                for (size_t i = 0; i < input_values.size(); ++i)
                {
                    ...
                    {
                        /*
                         * Rows zero and one are the exact normalized layer-31
                         * activations from the original model failure.  They are
                         * shared across every format and M bucket so Q8 boundary
                         * values cannot disappear behind friendly synthetic data.
                         */
                        input_values[i] = f32FromBitsForCUDAMoEKernelTest(
                ...
                auto grouped_output = makeZeros(
                    {static_cast<size_t>(seq_len), static_cast<size_t>(d_model)});
                auto routed_output = makeZeros(
                    {static_cast<size_t>(seq_len), static_cast<size_t>(d_model)});
                auto grouped_routing_indices = makeZeros(
                    {static_cast<size_t>(seq_len), static_cast<size_t>(routed_top_k)});
                auto grouped_routing_weights = makeZeros(
                    {static_cast<size_t>(seq_len), static_cast<size_t>(routed_top_k)});
                ASSERT_TRUE(grouped_input->ensureOnDevice(device, stream_));
                ASSERT_TRUE(grouped_output->ensureOnDevice(device, stream_));
                ASSERT_TRUE(routed_output->ensureOnDevice(device, stream_));
                ASSERT_TRUE(grouped_routing_indices->ensureOnDevice(device, stream_));
                ASSERT_TRUE(grouped_routing_weights->ensureOnDevice(device, stream_));

                /*
                 * Follow graph order exactly.  Routing owns the Q8 activation
                 * publication; routed experts consume it first, and the shared
                 * expert must still consume the same device-resident rows after
                 * grouping metadata and descriptor-table state have changed.
                 */
                ...
                for (int row = 0; row < seq_len; ++row)
                {
                    ...
                    auto row_routing_indices = makeZeros(
                        {1u, static_cast<size_t>(routed_top_k)});
                    auto row_routing_weights = makeZeros(
                        {1u, static_cast<size_t>(routed_top_k)});
                    ...
                    /*
                     * Ordinary serial decode also routes before the shared
                     * expert.  The M=1 table decoder deliberately quantizes its
                     * own input, making it the independent byte oracle for the
                     * grouped consumer's router-Q8 reuse.
                     */
                    ASSERT_TRUE(moe_kernel.routeVerifierRowsDecodeEquivalent(
                        row_input.get(),
                        router_gate.get(),
                        ...
                        << "production serial router failed at seq_len="
                ...
                /*
                 * The eager pass above prepares every persistent resource.  The
                 * capture must therefore contain only device work and preserve
                ...
                expectBitwiseFP32RowsEqual(
                    std::string("CUDA graph-captured router-Q8 SharedExpertFFNStage ") +
                        format.label +
                        " grouped verifier vs serial grouped decode seq_len=" +
                        std::to_string(seq_len),
                    replay_host.data(),
                    serial_host.data(),
                    replay_host.size(),
            ...
            for (const auto &record : records)
            {
                ...
                const auto route_it = record.tags.find("route");
                if (route_it != record.tags.end() &&
                    route_it->second == "grouped_table_prefill")
                {
            ...
            for (const int seq_len : llaminar2::test::kGroupedVerifierRuntimeRows)
            {
                ...
                EXPECT_EQ(static_cast<int>(routed_reuse_calls), 2)
                    << "M=" << seq_len
                    << " routed experts must reuse router-owned CUDA Q8 rows in eager execution and capture recording for "
                    << format.label << "\n"
                    << llaminar2::PerfStatsCollector::summaryString({"kernel"});
                EXPECT_EQ(static_cast<int>(shared_reuse_calls), 2)
                    << "M=" << seq_len
                    << " shared expert must reuse router-owned CUDA Q8 rows after routed experts in eager execution and capture recording for "
                    << format.label << "\n"
                    << llaminar2::PerfStatsCollector::summaryString({"kernel"});
            }
...
TEST_F(Test__CUDAMoEKernel, FixedTopologyRuntimeGroupedPrefillMatchesExistingPrefillPath)
{
    ...
    ASSERT_TRUE(reference_output->ensureOnHost());
    ...
    ASSERT_TRUE(runtime_output->ensureOnHost());
...
TEST_F(Test__CUDAMoEKernel, RuntimeVerifierSmallMPrefillAllNativeFormatsMatchRuntimeDecodeRows)
{
    ...
    for (size_t format_index = 0; format_index < formats.size(); ++format_index)
    {
        ...
        /*
         * Keep routing independent of the expert codebook while still using
         * the production optimized contract.  FP32 router weights activate
         * CUDA's Q8 router, which publishes its blockwise-quantized hidden rows
         * for the immediately following grouped gate/up pass.  Sweeping the
         * same router across every expert format proves that the publication is
         * a backend invariant rather than an IQ3-specific coincidence.
         */
        ...
        for (const int seq_len : kGroupedVerifierRuntimeRows)
        {
            ...
            auto routing_indices = makeZeros(
                {static_cast<size_t>(seq_len), static_cast<size_t>(top_k)});
            auto routing_weights = makeZeros(
                {static_cast<size_t>(seq_len), static_cast<size_t>(top_k)});
            ASSERT_TRUE(routing_indices->ensureOnDevice(device, stream_));
            ASSERT_TRUE(routing_weights->ensureOnDevice(device, stream_));
            ...
            for (int row = 0; row < seq_len; ++row)
            {
                ...
                ASSERT_TRUE(moe_kernel.groupedExpertDecodeFromRuntime(
                    ...
                    << seq_len << " row=" << row;
                ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
                ASSERT_TRUE(serial_output->ensureOnHost(stream_));
            ...
            const auto reuse_records =
                llaminar2::PerfStatsCollector::snapshot(
                    {"kernel.cuda_moe_grouped_prefill_router_q8_reuse_calls"});
            ...
            std::string reuse_block_summary;
            for (const auto &record : llaminar2::PerfStatsCollector::snapshot(
                     {"kernel.cuda_moe_grouped_prefill_router_q8_reuse_blocked_calls"}))
            {
                const auto seq_it = record.tags.find("seq_len");
                const auto source_it = record.tags.find("descriptor_source");
                const auto reason_it = record.tags.find("reason");
                if (seq_it != record.tags.end() &&
                    seq_it->second == expected_seq_len &&
                    source_it != record.tags.end() &&
                    source_it->second == "runtime_table" &&
                    reason_it != record.tags.end())
                {
                    if (!reuse_block_summary.empty())
                        reuse_block_summary += ",";
                    reuse_block_summary += reason_it->second;
                }
            }
            EXPECT_NE(reuse_record, reuse_records.end())
                << format.label << " M=" << seq_len
                << " must consume the grouped router's published Q8 rows"
                << " block_reasons=" << reuse_block_summary;
        }
    }
...
TEST_F(Test__CUDAMoEKernel, RoutedOnlyVerifierPrefill_IQ3S_RuntimeMBoundariesMatchRowDecode)
{
    ...
    auto make_routes = [](int capacity_rows,
                          int logical_rows,
                          ...
                          std::vector<float> &weights)
    {
        ...
        indices.assign(
            static_cast<size_t>(capacity_rows * top_k),
            -1.0f);
        weights.assign(
            static_cast<size_t>(capacity_rows * top_k),
            0.0f);
    ...
    {
        const int seq_len = test_case.capacity_rows;
        ...
        std::vector<float> row_by_row_expected(
            static_cast<size_t>(seq_len) * static_cast<size_t>(d_model));
        for (int row = 0; row < test_case.logical_rows; ++row)
        {
    ...
    /*
     * Long-context production prefill does not execute one immutable M forever.
     * The chunk scheduler commonly submits a full 512-row chunk followed by a
     * short tail against the same graph-owned kernel object and workspace.  The
     * all-format verifier sweep above exercises runtime M only through 31, while
     * the production-shape cases in this test historically called the immutable
     * descriptor-table entry point.  That left runtime descriptor
     * materialization, ordered-route metadata, and split-K scratch reuse across
     * a large-to-small M transition completely untested.
     *
     * Establish an M=36 byte witness, execute the production M=512 chunk, then
     * execute the identical M=36 work again through the true runtime-table
     * entry point.  The two tail byte streams must be identical.  This proves
     * that the large chunk cannot leave stale route mappings, active-expert
     * metadata, or split-K partials that contaminate the next chunk.  Comparing
     * two grouped executions deliberately avoids turning the 512-row witness
     * into an expensive serial replay oracle.
     */
    constexpr int kTailRows = 36;
    constexpr int kFullChunkRows = 512;
    llaminar2::DeviceMoERuntimeTable::Config runtime_config;
    runtime_config.device_id = device;
    runtime_config.num_layers = 1;
    runtime_config.num_experts = num_experts;
    runtime_config.top_k = top_k;
    runtime_config.mirror_to_device = true;
    runtime_config.prefill_token_capacity = kFullChunkRows;
    llaminar2::DeviceMoERuntimeTable runtime_table(runtime_config);
    ...
    runtime_update.local_compute_mask.assign(num_experts, 1u);
    runtime_update.replica_role.assign(
        num_experts,
        static_cast<uint8_t>(llaminar2::DeviceMoEReplicaRole::Primary));
    for (int expert = 0; expert < num_experts; ++expert)
    {
        auto &descriptor = runtime_update.experts[static_cast<size_t>(expert)];
        descriptor.logical_expert_id = expert;
    ...
    ASSERT_TRUE(runtime_table.flipActiveBank(0, runtime_update.epoch, stream_));

    auto execute_runtime_prefill =
        [&](int seq_len, std::vector<float> *host_output) -> bool
    {
        auto hidden = make_hidden(seq_len);
        if (!hidden->ensureOnDevice(device, stream_))
            return false;

        std::vector<float> route_indices;
        std::vector<float> route_weights;
        make_routes(seq_len, seq_len, route_indices, route_weights);
        auto routing_indices = llaminar2::test::TestTensorFactory::createFP32(
            {static_cast<size_t>(seq_len), static_cast<size_t>(top_k)});
        auto routing_weights = llaminar2::test::TestTensorFactory::createFP32(
            {static_cast<size_t>(seq_len), static_cast<size_t>(top_k)});
        ...
        std::copy(
            ...
            route_weights.end(),
            routing_weights->mutable_data());
        if (!routing_indices->ensureOnDevice(device, stream_) ||
            !routing_weights->ensureOnDevice(device, stream_))
        {
            return false;
        }

        auto output = llaminar2::test::TestTensorFactory::createFP32(
            {static_cast<size_t>(seq_len), static_cast<size_t>(d_model)});
        ...
        if (!output->ensureOnHost(stream_))
            return false;
        if (host_output)
        {
            host_output->assign(
    ...
    std::vector<float> tail_before_full_chunk;
    std::vector<float> tail_after_full_chunk;
    ASSERT_TRUE(execute_runtime_prefill(kTailRows, &tail_before_full_chunk));
...
TEST_F(Test__CUDAMoEKernel, RoutedAndMaskedLocalTPVerifierPrefill_AllNativeFormats_RuntimeMMatchRowDecode)
{
     ...
     * different min/scale descriptor metadata.  Each gate/up x down format pair
     * executes both the all-local route and the masked LocalTP route.  Their
     * references use the matching production row-by-row table/routing decode
     * entry points, never a CPU implementation or serial-row replay fallback.
     */
    constexpr int d_model = 256;
    constexpr int intermediate = 256;
    constexpr int num_experts = 4;
    constexpr int top_k = 4;
    const std::array<uint8_t, num_experts> local_expert_mask = {1u, 1u, 0u, 0u};
    const auto device = llaminar2::DeviceId::cuda(0);
    ...
    for (const auto &gateup_format : formats)
    {
        ...
        /*
         * LocalTP production tables are global-expert indexed but sparse: only
         * descriptors owned by this participant are populated. A dense table
         * plus a route mask does not exercise blank-descriptor handling and
         * previously let a production-only zero-output failure escape this
         * all-format sweep. Keep an explicit sparse table beside the all-local
         * table so both topology contracts are proven for every format pair.
         */
        auto sparse_gate_descs = gate_descs;
        auto sparse_up_descs = up_descs;
        auto sparse_down_descs = down_descs;
        for (int expert = 0; expert < num_experts; ++expert)
        {
            if (local_expert_mask[static_cast<size_t>(expert)] != 0u)
                continue;
            sparse_gate_descs[static_cast<size_t>(expert)] = {};
            sparse_up_descs[static_cast<size_t>(expert)] = {};
            sparse_down_descs[static_cast<size_t>(expert)] = {};
        }
        const int sparse_gateup_table =
            moe_kernel.uploadGroupedExpertGateUpDescriptorTables(
                sparse_gate_descs.data(),
                sparse_up_descs.data(),
                num_experts,
                d_model,
                intermediate);
        ASSERT_GE(sparse_gateup_table, 0)
            << case_label << " sparse LocalTP gate/up descriptor table";
        const int sparse_down_table =
            moe_kernel.uploadGroupedExpertDownDescriptorTable(
                sparse_down_descs.data(),
                num_experts,
                d_model,
                intermediate);
        ASSERT_GE(sparse_down_table, 0)
            << case_label << " sparse LocalTP down descriptor table";
        ...
        const int active_gateup_table =
            masked_local_tp ? sparse_gateup_table : gateup_table;
        const int active_down_table =
            masked_local_tp ? sparse_down_table : down_table;
        for (size_t row_case = 0; row_case < row_inventory.size(); ++row_case)
        {
            ...
            std::vector<float> routing_index_values(
                static_cast<size_t>(seq_len) * static_cast<size_t>(top_k));
            std::vector<float> routing_weight_values(
                static_cast<size_t>(seq_len) * static_cast<size_t>(top_k));
            auto routing_indices = llaminar2::test::TestTensorFactory::createFP32(
                {static_cast<size_t>(seq_len), static_cast<size_t>(top_k)});
            auto routing_weights = llaminar2::test::TestTensorFactory::createFP32(
            ...
            for (int row = 0; row < seq_len; ++row)
            {
                ...
                for (int route = 0; route < top_k; ++route)
                {
                    ...
                    sum += routing_weight_values[static_cast<size_t>(slot)];
                }
                for (int route = 0; route < top_k; ++route)
                {
                    routing_weight_values[static_cast<size_t>(row * top_k + route)] /=
                        sum;
                }
            }
            std::copy(
                routing_index_values.begin(),
                routing_index_values.end(),
                routing_indices->mutable_data());
            std::copy(
                routing_weight_values.begin(),
                routing_weight_values.end(),
                routing_weights->mutable_data());
            ASSERT_TRUE(routing_indices->ensureOnDevice(device, stream_));
            ASSERT_TRUE(routing_weights->ensureOnDevice(device, stream_));
            ...
            for (int row = 0; row < seq_len; ++row)
            {
                auto hidden_row = llaminar2::test::TestTensorFactory::createFP32(
                    {1u, static_cast<size_t>(d_model)});
                std::copy(hidden_values.data() + static_cast<size_t>(row) * d_model,
                          hidden_values.data() + static_cast<size_t>(row + 1) * d_model,
                          hidden_row->mutable_data());
                ASSERT_TRUE(hidden_row->ensureOnDevice(device, stream_));
                ...
                if (masked_local_tp)
                {
                    ...
                    ASSERT_TRUE(moe_kernel.groupedExpertGateUpDecodeFromRouting(
                        ...
                        << case_label << " masked rowwise gate/up M=" << seq_len
                        << " row=" << row;
                    ASSERT_TRUE(moe_kernel.groupedExpertDownDecodeFromRouting(
                        ...
                        intermediate,
                        local_expert_mask.data()))
                        << case_label << " masked rowwise down M=" << seq_len
                        << " row=" << row;
                }
                else
                {
            ...
            if (masked_local_tp)
            {
                ASSERT_TRUE(moe_kernel.updateGroupedPrefillExpertMask(
                    ...
                    << case_label << " publish fixed-topology device mask M=" << seq_len;
            }
            const bool grouping_ok = masked_local_tp
                                         ? moe_kernel.prepareExpertGroupsAsyncUsingPublishedMask(
                                               routing_indices.get(),
                                               routing_weights.get(),
                                               seq_len,
                                               num_experts,
                                               top_k)
                                         : moe_kernel.prepareExpertGroupsAsync(
                                               routing_indices.get(),
                                               routing_weights.get(),
                                               seq_len,
                                               num_experts,
                                               top_k);
            ASSERT_TRUE(grouping_ok)
                << case_label
            ...
            expectBitwiseFP32RowsEqual(
                case_label +
                    (masked_local_tp ? " CUDA masked LocalTP" : " CUDA routed-only") +
                    " grouped verifier prefill M=" + std::to_string(seq_len),
                grouped_values.data(),
            ...
            if (masked_local_tp)
            {
                ...
                EXPECT_NE(
                    ...
                    << " masked LocalTP output must exclude nonlocal expert contributions";
            }
            else
            {
                ...
                const auto record = std::find_if(
                    ...
                    [&](const llaminar2::PerfStatRecord &candidate)
                    {
                        ...
                        return candidate.name ==
                                   "cuda_moe_masked_prefill_grouping_calls" &&
                               tag_equals("seq_len", expected_seq_len) &&
                               tag_equals("mask_active_experts", "2") &&
                               tag_equals("expert_grid_slots", "2") &&
                               candidate.count > 0;
                    });
                EXPECT_NE(record, records.end())
                    << case_label << " M=" << seq_len
                    << " must record the economical masked LocalTP grouping route\n"
                    << llaminar2::PerfStatsCollector::summaryString(
                           {"kernel.cuda_moe_masked_prefill_grouping_calls"});
            }
        }
...
TEST_F(Test__CUDAMoEKernel, FixedTopologyRuntimeGroupedPrefillGraphReplayClearsInvalidPaddedRouteMappings)
{
    ...
    ASSERT_EQ(cudaMemcpyAsync(
                  ...
                  output->gpu_data_ptr(),
                  replayed_output.size() * sizeof(float),
                  cudaMemcpyDeviceToHost,
                  stream_),
              cudaSuccess);
    ASSERT_EQ(cudaStreamSynchronize(stream_), cudaSuccess);
    ...
    const float *output_data = replayed_output.data();
...
/**
 * @brief Prove request reset retires every CUDA transfer-slot publication.
 *
 * The production failure that motivated this test appeared only after many
 * serving requests: runtime-table epochs were reset while directory occupants
 * survived, so the conservative destination lease allocator eventually found
 * no legal slot. Each iteration below simulates a later maintenance wave by
 * publishing nonempty logical occupants into every live directory entry. The
 * production D2D reset must restore the exact model-setup baseline, including
 * stable payload pointers and empty logical identities, on the caller's stream.
 */
TEST(Test__CUDAMoERequestReset,
     RepeatedRequestResetRestoresImmutableTransferDirectoryBaseline)
{
    ...
    const std::vector<
        llaminar2::DeviceMoETransferSlotDirectory::ProjectionSpec>
        specs{
            {
                .label = "gate",
                .N = 32,
    ...
    const size_t bytes =
        baseline.size() *
        sizeof(llaminar2::DeviceMoEExpertDirectoryEntry);
    std::vector<llaminar2::DeviceMoEExpertDirectoryEntry> restored(
        baseline.size());

    for (uint32_t request_epoch = 1; request_epoch <= 32; ++request_epoch)
    {
        auto published = baseline;
        for (size_t slot = 0; slot < published.size(); ++slot)
        {
            auto &entry = published[slot];
            entry.layer =
                (request_epoch + static_cast<uint32_t>(slot)) % 40u;
            entry.expert =
                (request_epoch * 7u + static_cast<uint32_t>(slot)) % 256u;
            entry.resident_mask = 1u;
            entry.epoch = request_epoch;
            entry.generation = request_epoch * 11u +
            ...
            entry.descriptor.logical_expert_id =
                static_cast<int32_t>(entry.expert);
            entry.descriptor.flags |= llaminar2::toMoEExpertFlags(
                llaminar2::DeviceMoEExpertFlags::Resident |
                llaminar2::DeviceMoEExpertFlags::LocalCompute);
        }
