...
namespace
{
        ...
        SourceView source_view = SourceView::Direct)
    {
        ...
        for (int verifier_rows : kGroupedVerifierRuntimeRows)
        {
            ...
            if (capture)
                ASSERT_EQ(cudaStreamEndCapture(stream, &captured.graph), cudaSuccess);
            ASSERT_TRUE(applied);
            capture_guard.reset();
            if (capture)
            {
                ASSERT_EQ(cudaGraphInstantiate(&captured.executable, captured.graph,
                          nullptr, nullptr, 0), cudaSuccess);
                ASSERT_EQ(cudaGraphLaunch(captured.executable, stream), cudaSuccess);
            }
...
TEST_F(Test__CUDAEmbeddingGroupedVerifier, NestedVocabularySlicesPreserveCapturedRepresentation)
{
    ScopedCudaStream stream;
    ASSERT_TRUE(stream.create());
    ScopedPerfStats perfstats;
