...
TEST_F(Qwen2FP32PrefillVsPytorch, IncrementalDecodeVsPytorch)
{
    ...
    for (size_t step = 0; step < num_decode_steps; ++step)
    {
        ...
        pipeline_->clearSnapshots();

        // Run single-token decode
        std::vector<int> decode_token = {current_token};
        success = pipeline_->forward(decode_token.data(), 1);
        ASSERT_TRUE(success) << "Decode step " << step << " failed";

        // Get Llaminar logits
        size_t decode_logits_size;
        const float *llaminar_decode_logits = pipeline_->getSnapshot("LM_HEAD", decode_logits_size);
        ...
        {
            size_t llaminar_size;
            const float *llaminar_data = pipeline_->getSnapshot("layer0_ATTENTION_NORM", llaminar_size);
            if (llaminar_data != nullptr)
            {
...
TEST_F(Qwen2FP32PrefillVsPytorch, IncrementalDecodeConsistency)
{
    ...
    const float *prefill_logits = pipeline_->getSnapshot("LM_HEAD", prefill_logits_size);
    ...
    for (int step = 0; step < NUM_DECODE_STEPS; ++step)
    {
        ...
        // Get logits
        size_t decode_logits_size;
        const float *decode_logits = pipeline_->getSnapshot("LM_HEAD", decode_logits_size);
