...
namespace llaminar2::test
{
    ...
    /**
     * @brief A stale captured epoch ticket must trap before route publication.
     *
     * `runtime_shape_ok` is intentionally a predicate. If it is evaluated
     * before the fatal ticket boundary, an invalid bank merely makes the
     * predicate false and leaves the preceding token's route scratch live.
     * Resolving the ticket again inside dispatch is also forbidden: epoch
     * publication between the two loads creates a time-of-check/time-of-use
     * split even though each load is individually valid. This source policy
     * locks one ownership snapshot ahead of every CUDA/ROCm decode publication
     * entry point without adding a host-side mirror.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         DecodeEpochTicketIsAuthenticatedBeforeShapeGatedPublication)
    {
        const fs::path root = findRepoRoot();
        const std::array<std::pair<const char *, const char *>, 2> sources{{
            {"CUDA", "src/v2/kernels/cuda/moe/CUDAMoEKernels.cu"},
            {"ROCm", "src/v2/kernels/rocm/moe/ROCmMoEKernels.hip"},
        }};

        for (const auto &[backend, relative_path] : sources)
        {
            const fs::path path = root / relative_path;
            ASSERT_TRUE(fs::exists(path)) << path;
            ...
            ASSERT_FALSE(requirement.empty()) << backend;
            EXPECT_NE(
                requirement.find("runtime_execution_bank(runtime)"),
                std::string::npos)
                << backend << " must resolve the exact ticket-selected bank";
            const std::string fatal_call =
                std::string_view(backend) == "CUDA"
                    ? "FAIL_FAST_INCOMPLETE_LLEP_TRANSFER"
                    : "fail_fast_incomplete_llep_transfer()";
            EXPECT_NE(
                requirement.find(fatal_call),
                std::string::npos)
                << backend << " must make an invalid ticket fatal on device";

            const std::string shape = braceBalancedDeclarationRegion(
                source,
                "runtime_shape_ok(");
            ASSERT_FALSE(shape.empty()) << backend;
            EXPECT_EQ(
                shape.find("runtime_execution_bank(runtime)"),
                std::string::npos)
                << backend
                << " shape validation must consume the already authenticated bank";

            const std::string dispatch = braceBalancedDeclarationRegion(
                source,
                "runtime_resolve_decode_dispatch(");
            ASSERT_FALSE(dispatch.empty()) << backend;
            EXPECT_NE(
                dispatch.find("uint32_t execution_bank"),
                std::string::npos)
                << backend
                << " dispatch must receive the same immutable bank snapshot";
            EXPECT_EQ(
                dispatch.find("runtime_execution_bank(runtime)"),
                std::string::npos)
                << backend
                << " dispatch must not reload a concurrently published ticket";

            const auto expect_requirement_before_shape =
                [&](const std::string &declaration)
            {
                const std::string body =
                    braceBalancedDeclarationRegion(source, declaration);
                ASSERT_FALSE(body.empty()) << backend << ' ' << declaration;
                const size_t require = body.find(
                    "runtime_require_decode_execution_bank(runtime)");
                const size_t shape = body.find("runtime_shape_ok(");
                ASSERT_NE(require, std::string::npos)
                    << backend << ' ' << declaration;
                ASSERT_NE(shape, std::string::npos)
                    << backend << ' ' << declaration;
                EXPECT_LT(require, shape)
                    << backend << " must authenticate epoch ownership before "
                    << "a false shape predicate can suppress publication";
            };

            if (std::string_view(backend) == "CUDA")
            {
                expect_requirement_before_shape(
                    "softmax_topk_decode_runtime_kernel(");
                expect_requirement_before_shape(
                    "decode_route_select_runtime_kernel(");
            }
            else
            {
                expect_requirement_before_shape(
         ...
         GroupedRuntimeComputeConsumesOneCompletePublishedPlan)
    {
        ...
        const fs::path router_stage_path =
         ...
         LocalExpertCompactRoutingBucketsRouteWidthAndUsesInvalidPadding)
    {
        ...
        EXPECT_NE(local_contents.find(".live_rows = static_cast<int>(compact_capacity_)"),
                  std::string::npos)
            << "Retained graphs must execute their immutable padded family, not recapture per live route count";
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, LocalExpertCapturesExactPinnedTransfersOnParticipantStream)
    {
        ...
        const size_t input_publisher = contents.find(
            "class MoELocalExpertInputPublishStage final");
        const size_t captured_h2d = contents.find(
            "transfers.enqueuePinnedHostToDevice(", input_publisher);
        const size_t output_publisher = contents.find(
            "class MoELocalExpertOutputPublishStage final", captured_h2d);
        const size_t captured_d2h = contents.find(
            "enqueueDeviceToPinnedHost(", output_publisher);
        const size_t input_graph_node = contents.find(
            "\"moe_overlay_local_expert_input_publish\"", captured_d2h);
        const size_t compute_graph_node = contents.find(
            "\"moe_overlay_local_expert\"", input_graph_node + 1);
        const size_t output_graph_node = contents.find(
            "\"moe_overlay_local_expert_output_publish\"", compute_graph_node);
        const size_t input_dependency = contents.find(
            "replay->graph.addDependency(\n                \"moe_overlay_local_expert\",\n                \"moe_overlay_local_expert_input_publish\")",
            output_graph_node);
        const size_t output_dependency = contents.find(
            "replay->graph.addDependency(\n                \"moe_overlay_local_expert_output_publish\",\n                \"moe_overlay_local_expert\")",
            input_dependency);
        const size_t exact_producer = contents.find(
            "void *const producer_stream = requireGPUStream()", output_dependency);
        const size_t retained_execute = contents.find(
            "executeWithCachedGraphReplay(", exact_producer);
        const size_t retained_family_publication =
            contents.find(
                "pending_deferred_replay_ = deferred_replay;",
                retained_execute);
        const size_t completion_wait =
            contents.find(
                "pending_deferred_replay_->cache.waitForCaptureStreamFence(",
                retained_family_publication);
        const size_t active_completion_policy = contents.find(
            "HostFenceWaitPolicy::\n                    ActiveProgress",
            completion_wait);
        ASSERT_NE(input_publisher, std::string::npos);
        ASSERT_NE(captured_h2d, std::string::npos);
        ASSERT_NE(output_publisher, std::string::npos);
        ASSERT_NE(captured_d2h, std::string::npos);
        ASSERT_NE(input_graph_node, std::string::npos);
        ASSERT_NE(compute_graph_node, std::string::npos);
        ASSERT_NE(output_graph_node, std::string::npos);
        ASSERT_NE(input_dependency, std::string::npos);
        ASSERT_NE(output_dependency, std::string::npos);
        ASSERT_NE(exact_producer, std::string::npos);
        ASSERT_NE(retained_execute, std::string::npos);
        ASSERT_NE(retained_family_publication, std::string::npos);
        ASSERT_NE(completion_wait, std::string::npos);
        ASSERT_NE(active_completion_policy, std::string::npos);
        EXPECT_LT(input_publisher, captured_h2d);
        EXPECT_LT(captured_h2d, output_publisher);
        EXPECT_LT(output_publisher, captured_d2h);
        EXPECT_LT(captured_d2h, input_graph_node);
        EXPECT_LT(input_graph_node, compute_graph_node);
        EXPECT_LT(compute_graph_node, output_graph_node);
        EXPECT_LT(output_graph_node, input_dependency);
        EXPECT_LT(input_dependency, output_dependency);
        EXPECT_LT(output_dependency, exact_producer);
        EXPECT_LT(exact_producer, retained_execute);
        EXPECT_LT(retained_execute, retained_family_publication);
        EXPECT_LT(retained_family_publication, completion_wait)
            << "The retained graph must retain its exact family before the explicit host completion waits on that graph";
        EXPECT_LT(completion_wait, active_completion_policy)
            << "Latency-critical transaction followers must actively progress the exact completion event";
        EXPECT_EQ(
            contents.find("orderCaptureStreamAfter(", exact_producer),
            std::string::npos)
            << "the exact borrowed stream plus captured H2D nodes need no host-side replay dependency hook";
        EXPECT_EQ(
            contents.find("backend->deviceToHostOnStream("),
            std::string::npos)
            << "the stage must not bypass the canonical transfer authority";

        const size_t inline_bind =
            contents.find("compute_stage->setGPUStream(gpuStream());");
        const size_t inline_execute =
            contents.find("compute_stage->execute(ctx)", inline_bind);
        ASSERT_NE(inline_bind, std::string::npos);
        ASSERT_NE(inline_execute, std::string::npos);
        EXPECT_LT(inline_bind, inline_execute);
    }

    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         EconomyServiceTimingCoversCompleteSparseEndpoint)
    {
        const fs::path root = findRepoRoot();
        const fs::path path =
            root / "src/v2/execution/compute_stages/stages/MoELocalExpertStage.cpp";
        ASSERT_TRUE(fs::exists(path)) << path;
        const std::string contents = readFile(path);
        ASSERT_FALSE(contents.empty()) << path;

        const size_t execute_begin =
            contents.find("bool MoELocalExpertStage::execute(");
        const size_t execute_end = contents.find(
            "bool MoELocalExpertStage::validatePreparedWeights(",
            execute_begin);
        ASSERT_NE(execute_begin, std::string::npos);
        ASSERT_NE(execute_end, std::string::npos);
        const std::string execute_body =
            contents.substr(execute_begin, execute_end - execute_begin);
        ...
        const size_t output_materialization = execute_body.find(
            "pending_deferred_replay_->cache.waitForCaptureStreamFence(",
            service_start);
        const size_t pinned_output_read = execute_body.find(
        ...
        ASSERT_NE(pinned_input, std::string::npos);
        ASSERT_NE(output_materialization, std::string::npos);
        ASSERT_NE(pinned_output_read, std::string::npos);
        ASSERT_NE(aggregation_complete, std::string::npos);
        ASSERT_NE(elapsed_service, std::string::npos);
        ASSERT_NE(publish, std::string::npos);
        ASSERT_NE(diagnostic_publish, std::string::npos);
        EXPECT_LT(service_start, compact_capacity)
            << "Economy timing must include host packet compaction";
        EXPECT_LT(compact_capacity, pinned_input)
            << "Economy timing must include compact packet publication into the captured-transfer source";
        EXPECT_LT(pinned_input, output_materialization)
            << "Economy timing must span device execution and output download";
        EXPECT_LT(output_materialization, pinned_output_read);
        EXPECT_LT(pinned_output_read, aggregation_complete)
            << "Economy timing must include host-visible return aggregation";
        EXPECT_LT(aggregation_complete, elapsed_service);
        EXPECT_LT(elapsed_service, publish);
        EXPECT_LT(publish, diagnostic_publish)
        ...
        EXPECT_EQ(execute_body.find("eventElapsedTimeMs("), std::string::npos)
            << "Kernel-only GPU event timing cannot certify heterogeneous "
               "endpoint economy";
    }
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, PostPrefillHostReclaimIsWorkerOwnedAndAllocationGated)
    {
        ...
        const std::string manager_release_body =
            manager_contents.substr(manager_release, manager_advise - manager_release);
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ExpertOverlayRCUIsTheOnlyTypedDurableAuthority)
    {
        const fs::path root = findRepoRoot();
        const fs::path types_path = root / "src/v2/models/GraphTypes.h";
        const fs::path factory_path =
            root / "src/v2/execution/factory/InferenceRunnerFactory.cpp";
        const fs::path graph_path =
            root / "src/v2/models/qwen35moe/Qwen35MoEGraph.cpp";
        ASSERT_TRUE(fs::exists(types_path)) << types_path;
        ASSERT_TRUE(fs::exists(factory_path)) << factory_path;
        ASSERT_TRUE(fs::exists(graph_path)) << graph_path;

        const std::string types = readFile(types_path);
        const std::string factory = readFile(factory_path);
        const std::string graph = readFile(graph_path);
        ...
        EXPECT_NE(
            types.find("enum class MoEDurableResidencyAuthorityKind"),
            std::string::npos)
            << "Durable placement ownership must be typed rather than inferred "
               "from a generic Dynamic mode.";
        EXPECT_EQ(types.find("LegacyDomainController"), std::string::npos)
            << "the replaced per-domain writer must not remain representable";
        EXPECT_NE(types.find("ExpertOverlayRCU"), std::string::npos);
        EXPECT_NE(
            factory.find(
                "MoEDurableResidencyAuthorityKind::ExpertOverlayRCU"),
            std::string::npos)
            << "The runner factory must bind the arbitrary-tier RCU authority.";
        EXPECT_EQ(types.find("MoERebalanceMode rebalance_mode"),
                  std::string::npos)
            << "Graph configuration must not represent a second durable writer.";
        EXPECT_NE(
            factory.find(
                "validateMoEDurableResidencyAuthorityForGraph("),
            std::string::npos)
            << "Factory construction must enforce universal RCU ownership.";
        EXPECT_EQ(factory.find("LegacyDomainController"), std::string::npos);
        EXPECT_EQ(graph.find("usesLegacyDurableResidencyAuthority"),
                  std::string::npos);
        EXPECT_NE(
            factory.find(
                "Multi-participant MoE reached graph construction without"),
            std::string::npos)
            << "unnormalized multi-device entry must fail before graph lowering";
        EXPECT_NE(
            graph.find("overlay_placement_source = main_it->second.get();"),
            std::string::npos)
            << "Current-batch LLEP must use private child banks whose canonical "
               "placement source is the durable main-decode table.";
        EXPECT_NE(
            graph.find(
                "does not share the canonical ExpertOverlay request ticket"),
            std::string::npos)
            << "Every request-local LLEP child must fail closed unless it shares "
               "the parent ExpertOverlay epoch ticket.";
    }
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, DeviceRebalanceMaintenanceIsOneAtomicGraph)
    {
        ...
        EXPECT_EQ(ffn_body.find("device_side_graph_rebalance_candidate =\n            false"),
                  std::string::npos)
        ...
        EXPECT_NE(ffn_body.find(
                      "PerfStatsCollector::isDomainEnabled(\"moe_rebalance\")"),
                  std::string::npos)
            << "Only an explicit moe_rebalance PerfStats domain may enable "
               "projected load-spread diagnostics; unrelated profiling must "
        ...
        EXPECT_NE(ffn_body.find("!device_side_graph_rebalance_candidate"), std::string::npos);
        EXPECT_EQ(ffn_body.find("usesLegacyDurableResidencyAuthority"),
                  std::string::npos);
        EXPECT_EQ(ffn_body.find("config_.moe.rebalance_mode == MoERebalanceMode::DYNAMIC"),
                  std::string::npos);
        ...
        EXPECT_NE(contents.find("max_hot_replicas_per_participant == 0"), std::string::npos)
            << "A truly resident-only wave may avoid transfer slots when neither ownership nor replicas move.";
        EXPECT_NE(
            contents.find(
                "dynamic_ownership_transfers =\n                    device_side_graph_rebalance_candidate"),
            std::string::npos)
            << "same-tier skew correction must move complete expert payloads even when hot-replica capacity is zero";
        ...
        EXPECT_NE(contents.find("ownerParticipantByExpert"),
                  std::string::npos)
            << "Plain LocalTP masked runtime tables must derive owner/resident metadata from the shared static assignment authority.";
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "Standard LocalTP expert stages must use the participant-local "
               "graph identity instead of shared mutable TP-context state.";
        EXPECT_NE(
            ffn_body.find(
                "expert_params.participant_count =\n"
                "                    config_.tp_ctx && config_.tp_ctx->degree() > 0"),
            std::string::npos)
            << "Expert stages must carry the typed TP context's full domain size "
               "for variable 2+ card domains.";
        EXPECT_NE(ffn_body.find("const auto owner_participants =\n                            routed_expert_ownership::ownerParticipantByExpert"),
                  std::string::npos)
            << "Standard LocalTP graph-side rebalance must know policy-exact remote source owners for missing-arrival planning.";
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "Collective payload slots must use the canonical descriptor-aware wire-stride contract.";
        ...
        const fs::path workspace_contract_header_path =
            root / "src/v2/execution/moe/DeviceMoERebalanceWorkspaceContract.h";
        const fs::path workspace_contract_source_path =
            root / "src/v2/execution/moe/DeviceMoERebalanceWorkspaceContract.cpp";
        ...
        ASSERT_TRUE(fs::exists(workspace_contract_header_path))
            << workspace_contract_header_path;
        ASSERT_TRUE(fs::exists(workspace_contract_source_path))
            << workspace_contract_source_path;
        ...
        const std::string workspace_contract_header =
            readFile(workspace_contract_header_path);
        const std::string workspace_contract_source =
            readFile(workspace_contract_source_path);
        ...
        ASSERT_FALSE(workspace_contract_header.empty())
            << workspace_contract_header_path;
        ASSERT_FALSE(workspace_contract_source.empty())
            << workspace_contract_source_path;
        ...
        EXPECT_NE(controller_header.find("must produce and consume these entries entirely from"),
                  std::string::npos);
        EXPECT_NE(
            workspace_contract_header.find(
                "enum class DeviceMoERebalanceStagePhase"),
            std::string::npos)
            << "The shared phase enum must live beside the capacity identity consumed before stage construction.";
        EXPECT_NE(workspace_contract_header.find("CollectState"), std::string::npos);
        EXPECT_NE(
            workspace_contract_header.find("PlanAndCopyAfterSideband"),
            std::string::npos);
        EXPECT_EQ(workspace_contract_header.find("CollectAndGatherState"), std::string::npos)
            << "The retired probe-only maintenance graph must not remain selectable.";
        EXPECT_EQ(workspace_contract_header.find("PlanAndPreparePayloadAfterSideband"), std::string::npos)
            << "The retired host-separated planning graph must not remain selectable.";
        EXPECT_EQ(workspace_contract_header.find("TransferPreparedPayload"), std::string::npos)
            << "The retired host-selected transport graph must not remain selectable.";
        EXPECT_EQ(workspace_contract_header.find("PlanProbeAfterSideband"), std::string::npos)
            << "A plan-only maintenance phase permits the runtime directory to change before source packing.";
        EXPECT_EQ(workspace_contract_header.find("GatherCommandsAndCopyPreparedPayload"), std::string::npos)
            << "Transport must not gather stale plans or regenerate source payloads.";
        EXPECT_EQ(workspace_contract_header.find("CopyPreparedPayload"), std::string::npos)
            << "The old ambiguous copy phase did not encode immutable payload ownership.";
        EXPECT_NE(workspace_contract_header.find("JoinTransfer"), std::string::npos);
        ...
        EXPECT_NE(workspace_contract_source.find("DeviceMoERebalanceFlags::DeferRuntimeApply"),
                  std::string::npos)
            << "The canonical contract must allocate ready-wave publication for deferred resident apply.";
    ...
    /**
     * @test Per-device graph construction never reads LocalTP's mutable current
     *       participant state.
     *
     * RankOrchestrator builds sibling participant graphs concurrently against
     * one LocalTPContext.  GraphConfig::tp_device_idx is the immutable identity
     * bound to each graph; calling the polymorphic context's myIndex() here
     * either races or fails before any participant has selected a device.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         GraphConstructionUsesParticipantLocalTPIdentity)
    {
        const fs::path root = findRepoRoot();
        const fs::path qwen35_path =
            root / "src/v2/models/qwen35/Qwen35Graph.cpp";
        const fs::path qwen35_moe_path =
            root / "src/v2/models/qwen35moe/Qwen35MoEGraph.cpp";
        ASSERT_TRUE(fs::exists(qwen35_path)) << qwen35_path;
        ASSERT_TRUE(fs::exists(qwen35_moe_path)) << qwen35_moe_path;

        const std::string qwen35 = readFile(qwen35_path);
        const std::string qwen35_moe = readFile(qwen35_moe_path);
        ASSERT_FALSE(qwen35.empty()) << qwen35_path;
        ASSERT_FALSE(qwen35_moe.empty()) << qwen35_moe_path;

        EXPECT_EQ(qwen35.find("config.tp_ctx->myIndex()"), std::string::npos)
            << "GDN graph geometry must use the identity bound to this graph.";
        EXPECT_NE(qwen35.find("GDNHeadAssignment::fromPartition("),
                  std::string::npos)
            << "GDN graph geometry must derive its dependency-closed local heads from the immutable graph partition.";
        EXPECT_NE(qwen35.find("config_.head_start,"), std::string::npos)
            << "GDN head assignment must consume the participant-local graph interval.";
        EXPECT_NE(
            qwen35.find(
                "state_gather_params.tp_device_idx = config_.tp_device_idx;"),
            std::string::npos)
            << "The captured linked-state collective must receive the identity bound to this participant graph.";
        EXPECT_NE(qwen35.find("rec_params.global_v_head_offset = 0;"),
                  std::string::npos)
            << "Dependency-closed local Q/K/V packing must not reintroduce a rank-derived mutable offset.";
        EXPECT_EQ(qwen35_moe.find("config_.tp_ctx->myIndex()"),
                  std::string::npos)
            << "MoE graph construction must not query shared LocalTP current-device state.";
        EXPECT_NE(qwen35_moe.find("std::max(0, config_.tp_device_idx)"),
                  std::string::npos);
        EXPECT_NE(qwen35_moe.find(
                      "config_.tp_device_idx ==\n"
                      "                                    canonical_route_root_participant"),
                  std::string::npos)
            << "Canonical route ownership must be selected from the participant-local graph identity.";
    }

    /**
     * @test ExpertOverlay parity has one production-shaped campaign authority.
     *
     * Plan-only topology smokes and backend-pair fixture copies can pass while
     * the serving graph is broken. Keep topology coverage in the consolidated
     * real-weight fixture, where every cell also proves compact transport and
     * emits the normal Hugging Face comparison CSV artifacts.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ExpertOverlayParityRetiresTopologySmokesAndDuplicateFixtures)
    {
        const fs::path root = findRepoRoot();
        const fs::path parity_dir =
        ...
        for (const char *obsolete : {
                 ...
                 "Test__Qwen35MoE_GraphNative_RocmHotCpuCold_Parity.cpp",
                 "Test__Qwen35MoE_GraphNative_CudaHotRocmWarm_Parity.cpp",
             })
        {
            EXPECT_FALSE(fs::exists(parity_dir / obsolete))
        ...
        EXPECT_EQ(canonical.find("ProfilerCpuFallbackRows"), std::string::npos)
            << "Sparse transport evidence belongs in the real parity forward";
        EXPECT_NE(
            canonical.find("assertSparseTransportPerfStatsEvidence();"),
            std::string::npos)
        ...
        for (const char *obsolete_target : {
                 ...
                 "v2_integration_parity_qwen35moe_graphnative_rocm_hot_cpu_cold",
                 "v2_integration_parity_qwen35moe_graphnative_cuda_hot_rocm_warm\n",
             })
        {
            EXPECT_EQ(cmake.find(obsolete_target), std::string::npos)
                << "Superseded ExpertOverlay CMake target returned: "
                << obsolete_target;
        }
        EXPECT_NE(
            cmake.find(
                "PRODUCTION_PERF_STATS_FILTER \"forward_graph,tp_allreduce,transfer,moe_placement,moe_overlay,"),
            std::string::npos)
            << "The real campaign must retain canonical TP reduction, physical "
               "placement, and sparse transport evidence";
    }

    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, ParityCellsRequireExplicitDynamicMode)
    {
        const fs::path parity_path =
            findRepoRoot() / "tests/v2/integration/parity/ParityTestBase.h";
        ASSERT_TRUE(fs::exists(parity_path)) << parity_path;

        const std::string parity = readFile(parity_path);
        ASSERT_FALSE(parity.empty()) << parity_path;

        const size_t declaration =
            parity.find("MoERebalanceRuntimeConfig moe_rebalance{");
        ASSERT_NE(declaration, std::string::npos);
        const size_t declaration_end = parity.find(";", declaration);
        ASSERT_NE(declaration_end, std::string::npos);
        const std::string initializer =
            parity.substr(declaration, declaration_end - declaration);
        EXPECT_NE(
            initializer.find(".mode = MoERebalanceRuntimeMode::Off"),
            std::string::npos)
         ...
         ProductionParitySeparatesCellControlFromRetainedMPIState)
    {
        ...
        for (const char *contract : {
                 ...
                 "void beginTeardown()",
                 "ParityCellOutcomeConsensus convergeOutcome(",
                 "void finishTeardown()",
        ...
        for (const char *contract : {
                 ...
                 "ParityCellOutcomeConsensus convergeParityCellOutcome(",
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "Every MPI rank must converge the current cell result before fail-fast.";
        EXPECT_NE(
            base.find(
        ...
        const size_t teardown_begin =
            base.find("void TearDown() override");
        const size_t teardown_end =
            base.find("virtual bool regeneratePyTorchSnapshots()", teardown_begin);
        ASSERT_NE(teardown_begin, std::string::npos);
        ASSERT_NE(teardown_end, std::string::npos);
        const std::string teardown =
            base.substr(teardown_begin, teardown_end - teardown_begin);
        for (const char *forbidden_drain : {
                 "pre_destroy_gpu_sync",
                 "final_cuda_sync",
                 "final_rocm_sync",
                 "cudaGetLastError()",
                 "cuda_backend->synchronize(",
                 "rocm_backend->synchronize(",
             })
        {
            EXPECT_EQ(teardown.find(forbidden_drain), std::string::npos)
                << "A parity cell may retire only its typed producer events; "
                   "teardown must not drain unrelated device work: "
                << forbidden_drain;
        }
        EXPECT_NE(
            teardown.find("retireOwnedParityRunners();"),
            std::string::npos)
            << "TearDown must retire runner ownership through its one typed "
               "campaign-overridable boundary.";
        const size_t retirement_begin =
            base.find("virtual void retireOwnedParityRunners()");
        const size_t retirement_end =
            base.find("void releaseBorrowedParityPipeline()", retirement_begin);
        ASSERT_NE(retirement_begin, std::string::npos);
        ASSERT_NE(retirement_end, std::string::npos);
        const std::string retirement =
            base.substr(retirement_begin, retirement_end - retirement_begin);
        EXPECT_NE(retirement.find("runner_.reset();"), std::string::npos);
        EXPECT_NE(retirement.find("orch_runner_.reset();"), std::string::npos);
        EXPECT_NE(
            teardown.find("reset-ready event"),
            std::string::npos)
            << "The surviving borrowed-runner edge must remain explicit at the teardown boundary.";
        EXPECT_EQ(
            countOccurrences(base, "enterParityCellLifecycle();"),
            1u)
            << "Only the common parity SetUp may enter the cell lifecycle.";
        EXPECT_EQ(
            countOccurrences(base, "finishParityCellTeardown();"),
            1u)
            << "Only the common parity TearDown may retire the cell lifecycle.";
        for (const char *retired_api : {
                 "teardownBarrier()",
                 "parityLifecycleBarrier()",
                 "retireParityCellLifecycle()",
             })
        {
            EXPECT_EQ(base.find(retired_api), std::string::npos)
                << "Replaced parity lifecycle API must be removed completely: "
                << retired_api;
            EXPECT_EQ(lifecycle.find(retired_api), std::string::npos)
                << "Replaced parity lifecycle API must be removed completely: "
                << retired_api;
        }
    ...
    /**
     * @brief Participant main/MTP graphs share one runner-lifetime workspace.
     *
     * A builder-local allocator makes each retained graph independently claim
     * the same physical BOM.  Besides wasting VRAM, that split ownership makes
     * setup order determine whether MTP can start.  Keep the ownership visible
     * in the runner type and forbid the former per-build local map.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ParticipantGraphFamiliesHaveOneSerialWorkspaceAuthority)
    {
        const fs::path root = findRepoRoot();
        const fs::path header_path =
            root / "src/v2/execution/moe/"
                   "MoEOverlayParticipantGraphRunner.h";
        const fs::path source_path =
            root / "src/v2/execution/moe/"
                   "MoEOverlayParticipantGraphRunner.cpp";
        ASSERT_TRUE(fs::exists(header_path)) << header_path;
        ASSERT_TRUE(fs::exists(source_path)) << source_path;

        const std::string header = readFile(header_path);
        const std::string source = readFile(source_path);
        EXPECT_NE(
            header.find("serial_graph_family_workspace_allocator_"),
            std::string::npos)
            << "The runner must visibly own the complete serial workspace family";
        EXPECT_GE(
            countOccurrences(
                source,
                "serial_graph_family_workspace_allocator_"),
            3u)
            << "Construction, mapped graphs, and portable graphs must consume the same owner";
        EXPECT_EQ(
            source.find("mapped_workspace_allocators"),
            std::string::npos)
            << "A graph-builder-local allocator duplicates the physical workspace claim";
    }

    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ProductionParityUsesAuthenticatedExactPrefillGraphBucket)
    {
        const fs::path root = findRepoRoot();
        const fs::path parity_path =
            root / "tests/v2/integration/parity/ParityTestBase.h";
        const fs::path qwen_base_path =
            root / "tests/v2/integration/parity/qwen2/Qwen2ParityTestBase.h";
        ASSERT_TRUE(fs::exists(parity_path)) << parity_path;
        ASSERT_TRUE(fs::exists(qwen_base_path)) << qwen_base_path;

        const std::string parity = readFile(parity_path);
        const std::string qwen_base = readFile(qwen_base_path);
        EXPECT_NE(
            parity.find("configureExactProductionParityPrefillGraphBucket()"),
            std::string::npos);
        EXPECT_NE(
            parity.find("LLAMINAR_PREFILL_GRAPH_BUCKET_SIZES"),
            std::string::npos)
            << "The bounded campaign must capture the authenticated prompt geometry instead of exporting padded rows";
        EXPECT_NE(
            qwen_base.find(
                "configureExactProductionParityPrefillGraphBucket();"),
            std::string::npos)
            << "Every Qwen-family production parity fixture must install the exact bucket after authenticating token_ids";
    }
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ExpertOverlayMaintenanceRunsAtCommittedDecodeBoundary)
    {
        const fs::path root = findRepoRoot();
        const fs::path runner_path = root / "src/v2/execution/runner/OrchestrationRunner.cpp";
        ...
        const fs::path device_rebalance_stage_path =
            root / "src/v2/execution/compute_stages/stages/MoEDeviceRebalanceStage.cpp";
        const fs::path decode_commit_stage_path =
            root / "src/v2/execution/compute_stages/stages/"
                   "MoEDeviceDecodeCommitBoundaryStage.cpp";
        const fs::path debug_env_path = root / "src/v2/utils/DebugEnv.h";
        ...
        ASSERT_TRUE(fs::exists(decode_commit_stage_path)) << decode_commit_stage_path;
        ...
        const std::string decode_commit_stage = readFile(decode_commit_stage_path);
        const std::string debug_env = readFile(debug_env_path);
        ...
        ASSERT_FALSE(decode_commit_stage.empty()) << decode_commit_stage_path;
        ...
        EXPECT_NE(
            maybe_body.find("moe_expert_overlay_maintenance_service_"),
            std::string::npos)
            << "Every dynamic placement boundary must address the one ExpertOverlay authority service.";
        EXPECT_NE(
            maybe_body.find("notifyHostMoEOverlayInferenceProgress("),
            std::string::npos)
            << "Host-coordinated inference must publish exact logical token "
               "progress to its non-blocking background worker.";
        EXPECT_NE(maybe_body.find("committed_tokens"), std::string::npos)
            << "Decode cadence must be derived from the committed transaction, "
               "not a synthetic maintenance tick.";
        EXPECT_EQ(
            maybe_body.find("notifyMaintenanceProgress()"),
            std::string::npos)
            << "An untyped wake cannot unlock a device-owned histogram whose "
               "host view is still underfull.";
        EXPECT_NE(
            maybe_body.find(
        ...
        const size_t stable_predicate_start =
            dgo.find("bool DeviceGraphOrchestrator::usesGraphStableMoEOverlayResidency() const");
        ...
        EXPECT_NE(stable_predicate.find("usesExpertOverlayDurableResidencyAuthority"),
                  std::string::npos);
        EXPECT_NE(stable_predicate.find("MoEOverlayAuthorityExecutionKind::Unresolved"),
                  std::string::npos)
            << "graph stability must come from the frozen epoch-address contract";
        EXPECT_EQ(stable_predicate.find("usesLegacyDurableResidencyAuthority"),
                  std::string::npos);

        const size_t device_controller_start = dgo.find(
        ...
        const std::string device_controller_predicate = dgo.substr(
            device_controller_start,
            device_controller_end - device_controller_start);
        EXPECT_NE(
            device_controller_predicate.find(
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "the runner must report the topology-frozen sole authority executor";
        EXPECT_EQ(
            device_controller_predicate.find(
                "usesLegacyDurableResidencyAuthority"),
            std::string::npos);
        ...
        EXPECT_LT(maintenance_dynamic_gate, maintenance_boundary_timer)
            << "Static/off placement must return before any maintenance timing or enqueue work.";
        ...
        for (const char *hosted_fragment : {
                 "publication->instantiate()",
                 "acknowledgement->instantiate()",
             })
        {
            EXPECT_NE(dgo.find(hosted_fragment), std::string::npos)
                << "HIP must instantiate every standalone ticket-scheduler fragment during setup: "
                << hosted_fragment;
        }
        EXPECT_NE(
            dgo.find(
            ...
            std::string::npos)
            << "Impossible-due HIP boundaries must remain graph-embedded and D2H-free";
        EXPECT_EQ(dgo.find("known_non_due->instantiate()"), std::string::npos)
            << "HIP must not retain a second per-token cadence graph after the complete decode graph owns the edge";
        const size_t embedded_cadence_append = dgo.find(
        ...
        EXPECT_NE(
            dgo.find(
                ...
                embedded_cadence_append),
            std::string::npos)
            << "The HIP cadence kernels must be a typed production graph stage";
        const size_t embedded_publish = decode_commit_stage.find(
            "enqueuePublishSerialDecodeCommitBoundary(");
        const size_t embedded_ack = decode_commit_stage.find(
            "enqueueAcknowledgeDecodeCommitBoundary(");
        ASSERT_NE(embedded_publish, std::string::npos);
        ASSERT_NE(embedded_ack, std::string::npos);
        EXPECT_LT(embedded_publish, embedded_ack)
            << "The complete HIP decode graph must publish before acknowledging a non-due edge";
        ...
        const size_t live_state_reason = maintenance_scheduler.find(
            "\"moe_device_rebalance_maintenance_before_executable_launch\"",
            live_state_wait);
        const size_t live_state_role = maintenance_scheduler.find(
            "DeviceTimelineRole::MoERebalanceMaintenance",
            live_state_wait);
        ASSERT_NE(live_state_reason, std::string::npos);
        ASSERT_NE(live_state_role, std::string::npos);
        EXPECT_LT(live_state_wait, live_state_reason);
        EXPECT_LT(live_state_reason, live_state_role)
            << "The captured launch dependency must retain the typed MoE timeline role";
        ...
        const size_t hosted_observer =
            dgo.find("bool DeviceGraphOrchestrator::observeDeviceMoERebalanceDispatchTicket(");
        const size_t hosted_observer_wait = dgo.find(
            "waitForLiveInferenceStateReadyForObservation(",
            hosted_observer);
        const size_t hosted_ticket_launch = dgo.find(
            "cadence_boundary_publication_capture->launchOnStream(",
            hosted_observer);
        ASSERT_NE(hosted_observer, std::string::npos);
        ASSERT_NE(hosted_observer_wait, std::string::npos);
        ASSERT_NE(hosted_ticket_launch, std::string::npos);
        EXPECT_LT(hosted_observer_wait, hosted_ticket_launch)
            << "The authenticated HIP ticket publisher must join committed inference state before sampling cadence";
        const size_t fast_boundary_begin = dgo.find(
            "submitHostScheduledDeviceMoERebalanceKnownNonDueBoundary()");
        const size_t fast_boundary_end = dgo.find(
            "bool DeviceGraphOrchestrator::observeDeviceMoERebalanceDispatchTicket(",
            fast_boundary_begin);
        ASSERT_NE(fast_boundary_begin, std::string::npos);
        ASSERT_NE(fast_boundary_end, std::string::npos);
        const std::string fast_boundary = dgo.substr(
            fast_boundary_begin,
            fast_boundary_end - fast_boundary_begin);
        EXPECT_NE(
            fast_boundary.find(
                "hosted_decode_commit_boundary_embedded"),
            std::string::npos)
            << "The host no-op requires immutable proof that the decode graph owns cadence";
        for (const char *forbidden_fast_boundary_work : {
                 "launchOnStream(",
                 "recordEvent(",
                 "waitForLiveInferenceStateReadyForObservation(",
        ...
        {
            EXPECT_EQ(
                fast_boundary.find(forbidden_fast_boundary_work),
                std::string::npos)
                << "A proven non-due HIP boundary must do no standalone work: "
                << forbidden_fast_boundary_work;
        }
        EXPECT_EQ(
            fast_boundary.find(
                "releaseMoEOverlayEpochForExternalTransaction"),
            std::string::npos)
            << "A cadence-only HIP boundary does not write placement and must keep the RCU reader acquired";
        EXPECT_NE(
        ...
        EXPECT_NE(
            dgo.find("DeviceMoERebalanceDispatchAction::None"),
            std::string::npos)
            << "An already-acknowledged serial boundary must be a typed no-op";
        EXPECT_NE(
            dgo.find(
                "device_moe_rebalance_idle_ticket_acknowledgements_elided"),
            std::string::npos)
            << "PerfStats must prove that idle ticket observations launch no acknowledgement graph";
        ...
        EXPECT_EQ(generate_body.find("usesDeviceSideMoERebalanceController"),
                  std::string::npos);
        EXPECT_EQ(generate_body.find("publishPendingMoERebalanceUpdate"),
                  std::string::npos)
            << "Generation epilogues must not publish a second placement authority.";
        ...
        const size_t maintenance_boundary_end = runner.find(
            "const RankExecutionPlan &OrchestrationRunner::executionPlan()",
            maintenance_boundary_start);
        ASSERT_NE(maintenance_boundary_end, std::string::npos);
        const std::string maintenance_boundary_body = runner.substr(
            maintenance_boundary_start,
        ...
        ASSERT_NE(device_maintenance_launch, std::string::npos)
            << "The topology-selected captured authority must own the ordinary "
               "committed-boundary launch.";
        EXPECT_LT(device_authority_selection, device_maintenance_launch);
        EXPECT_NE(
            maintenance_boundary_body.find(
                "device_generation_embedded_moe_maintenance_pending_ack_"),
            std::string::npos)
        ...
        EXPECT_EQ(benchmark.find("benchmark.setPostWarmupCallback"),
                  std::string::npos)
        ...
        EXPECT_NE(runner.find("resetUnderlyingRunnerRequestState"), std::string::npos)
            << "Request-owned data still needs an explicit non-placement reset contract.";
        EXPECT_NE(
            qwen36_parity.find("applyMoEOverlayMaintenanceIfNeeded"),
            std::string::npos)
            << "Production parity loops must invoke the universal authority boundary.";
        EXPECT_EQ(qwen36_parity.find("applyHostMoERebalanceIfNeeded"),
                  std::string::npos);
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         Qwen35MoEResetPreservesDurableAndRestoresTransientPlacementBanks)
    {
        const fs::path root = findRepoRoot();
        const fs::path graph_path = root / "src/v2/models/qwen35moe/Qwen35MoEGraph.cpp";
        ...
        EXPECT_NE(reset_body.find("!table->usesOverlayEpochTicket()"),
                  std::string::npos)
            << "Request-boundary reset must distinguish request-transient "
               "placement from model-lifetime ExpertOverlay RCU state.";
        EXPECT_NE(
            reset_body.find(
                "table->overlayPlacementSource() != nullptr"),
            std::string::npos)
            << "A ticket-pinned CurrentBatchLLEP child remains request-local "
               "even though its durable parent survives reset.";
        EXPECT_NE(reset_body.find("restoreInitialRuntimeState(execution_stream)"),
                  std::string::npos)
            << "Request-boundary reset must restore the immutable baseline for "
               "request-transient CurrentBatchLLEP placement.";
        EXPECT_EQ(reset_body.find("restoreInitialRuntimeState()"), std::string::npos)
            << "GPU model reset must never select an implicit stream.";
        EXPECT_EQ(reset_body.find("resetRequestPublications(execution_stream)"),
                  std::string::npos)
            << "A request reset must not erase a transfer-directory descriptor "
               "that is still selected by the durable placement epoch.";
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, KernelDynamicResetInvalidatesCachedMoEDescriptorHandles)
    {
        ...
        const size_t fixed_prefill_end =
            moe_cpp.find("const bool forced_verifier_decode_replay =",
                         fixed_prefill_start);
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, GraphCaptureFailureCannotEnterEagerRecovery)
    {
        ...
        const size_t capture_recorder_start =
            executor.find(
                "bool DeviceGraphExecutor::recordGraphCaptureBody(");
        const size_t retained_capture_start =
            executor.find(
                "bool DeviceGraphExecutor::captureRetainedGraphFragment(",
                capture_recorder_start);
        ASSERT_NE(capture_recorder_start, std::string::npos);
        ASSERT_NE(retained_capture_start, std::string::npos);
        const std::string capture_recorder =
            executor.substr(
                capture_recorder_start,
                retained_capture_start - capture_recorder_start);
        const size_t finish =
            capture_recorder.find("capture_transaction.finish();");
        const size_t execution_failure =
            capture_recorder.find("if (!exec_success)", finish);
        ...
        EXPECT_EQ(capture_recorder.find(
                      "if (!exec_success || !capture->endCapture())"),
                  std::string::npos)
            << "Short-circuit evaluation must never skip endCapture after a stage failure";
        EXPECT_NE(
            capture_recorder.find(
                "exec_success && capture->nodeCount() != 0u"),
            std::string::npos)
            << "The typed terminal capture transition must classify a "
               "zero-node native graph as aborted.";
        EXPECT_NE(capture_recorder.find("if (!usable_capture)"),
                  std::string::npos)
            << "A zero-node capture must be rejected after the backend and "
               "stage capture lifecycles have both been closed.";
        EXPECT_NE(capture_recorder.find(
        ...
        EXPECT_EQ(capture_recorder.find("Skipping graph replay"),
                  std::string::npos)
            << "Zero-node capture must never be treated as successful execution";

        const size_t immediate_capture_start = executor.find(
            "bool DeviceGraphExecutor::executeWithGraphCapture(",
            retained_capture_start);
        const size_t decode_policy_start = executor.find(
            "bool DeviceGraphExecutor::executeDecodeWithCapturePolicy(",
            immediate_capture_start);
        ASSERT_NE(immediate_capture_start, std::string::npos);
        ASSERT_NE(decode_policy_start, std::string::npos);
        const std::string immediate_capture = executor.substr(
            immediate_capture_start,
            decode_policy_start - immediate_capture_start);
        EXPECT_NE(immediate_capture.find("recordGraphCaptureBody("),
                  std::string::npos)
            << "Immediate replay must use the same structural recorder as retained child composition";
    ...
    /**
     * @brief Keep broad telemetry enablement out of production execution code.
     *
     * `PerfStatsCollector::isEnabled()` answers whether any output was
     * requested. Using it to guard work in an unrelated instrumentation family
     * makes a focused diagnostic alter maintenance scheduling, and can thereby
     * change the histogram window consumed by an asynchronous controller.
     * Producers must use their exact `isDomainEnabled()` gate instead. The
     * collector implementation itself and BenchmarkRunner's passive output
     * metadata are the only legitimate broad queries.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ProductionTelemetryUsesExactDomainGates)
    {
        const fs::path root = findRepoRoot();
        const fs::path source_root = root / "src/v2";
        ASSERT_TRUE(fs::exists(source_root)) << source_root;

        const std::array<fs::path, 2> broad_gate_owners{
            fs::path{"utils/PerfStatsCollector.cpp"},
            fs::path{"utils/BenchmarkRunner.cpp"},
        };
        ...
        for (const auto &entry : fs::recursive_directory_iterator(source_root))
        {
            ...
            if (extension != ".cpp" && extension != ".cu" &&
                extension != ".hip" && extension != ".h" &&
                extension != ".inl")
            {
                continue;
            }
            ...
            if (std::find(
                    broad_gate_owners.begin(),
                    broad_gate_owners.end(),
                    relative) != broad_gate_owners.end())
            {
                continue;
            }
            if (readFile(entry.path()).find(
                    "PerfStatsCollector::isEnabled()") != std::string::npos)
            {
        ...
        EXPECT_TRUE(violations.empty())
            << "Production instrumentation must use an exact PerfStats domain; "
               "broad enablement changes unrelated execution scheduling. First "
               "violation: "
            << (violations.empty() ? fs::path{} : violations.front());
    }
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, PrefillWorkRedistributionEvidenceIsDeviceResidentAndBackendSymmetric)
    {
        ...
        const fs::path expert_stage_path =
            root / "src/v2/execution/compute_stages/stages/MoEExpertComputeStage.cpp";
        const fs::path current_batch_stage_path =
            root / "src/v2/execution/compute_stages/stages/MoEGPUCurrentBatchLLEPStage.cpp";
        const fs::path orchestrator_path =
            root / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp";
        ...
        ASSERT_TRUE(fs::exists(current_batch_stage_path))
            << current_batch_stage_path;
        ASSERT_TRUE(fs::exists(orchestrator_path)) << orchestrator_path;
        ...
        const std::string current_batch_stage =
            readFile(current_batch_stage_path);
        const std::string orchestrator = readFile(orchestrator_path);
        ...
        EXPECT_NE(orchestrator.find(
                      "\"device_rebalance_request_copied_payload_lower_bound\""),
                  std::string::npos)
            << "Request-final PerfStats must retain copied payload evidence after rolling wave reuse";
        EXPECT_NE(orchestrator.find(
                      "\"device_rebalance_request_applied_payload_lower_bound\""),
                  std::string::npos)
            << "Request-final PerfStats must independently retain apply evidence";
        EXPECT_NE(orchestrator.find(
                      "\"device_rebalance_request_useful_payload_bytes_lower_bound\""),
                  std::string::npos)
            << "Request-final PerfStats must prove nonzero whole-expert payload bytes";
        EXPECT_NE(
        ...
        EXPECT_NE(
            graph_interface.find("uint64_t expert_payload_slot_bytes = 0;"),
            std::string::npos)
            << "The typed LLEP evidence source must carry an immutable whole-expert byte extent";
        EXPECT_NE(
            orchestrator.find(
                "llep_evidence_source.expert_payload_slot_bytes"),
            std::string::npos)
            << "Terminal LLEP evidence must convert applied layer markers into physical-byte lower bounds";
        EXPECT_NE(
            qwen_graph.find(
        ...
        EXPECT_NE(current_batch_stage.find(
                      "params_.config.min_wave_spread_improvement_per_payload_slot"),
                  std::string::npos)
            << "The explicit current-batch planner must use graph-owned economy policy";
        EXPECT_EQ(current_batch_stage.find(
                      "moe_env\n"
        ...
        EXPECT_NE(
            diagnostic.find(
                "!PerfStatsCollector::isDomainEnabled(\"moe_rebalance\")"),
            std::string::npos)
            << "The terminal diagnostic readback must require its exact "
               "PerfStats domain";
        EXPECT_EQ(diagnostic.find("snapshot_enabled_"), std::string::npos)
            << "Release PerfStats must prove LLEP movement without a snapshot build";
        EXPECT_NE(
            diagnostic.find("DeviceTimelineRole::Diagnostics"),
            std::string::npos)
            << "Terminal evidence must consume the explicit forward-output event";
    }
         ...
         TransferSlotCapacityContractSeparatesOccupancyFromAddressability)
    {
        ...
        EXPECT_EQ(
            ...
            3u)
            << "Both configs use total addressability and the directory boundary derives its slot count from the typed capacity";
        EXPECT_EQ(
            countOccurrences(
                graph,
                "transfer_capacity,\n"),
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, GPUTransferSlotsHaveSingleDeviceOwnedLogicalOccupants)
    {
                 ...
                 std::pair{"ROCm", &rocm}})
        {
            ...
            EXPECT_NE(source->find(
                          "build_transfer_slot_claim_index_kernel("),
                      std::string::npos)
                << backend
                << " projection must build one fresh reverse index over every "
                   "active runtime claim";
            EXPECT_NE(source->find(
                          "rebalance_transfer_slot_claim_index_ok("),
                      std::string::npos)
                << backend
                << " projection must reject incomplete, malformed, or duplicate "
                   "transfer-slot claims before leasing storage";
            EXPECT_EQ(source->find(
                          "rebalance_any_active_runtime_claims_slot("),
                      std::string::npos)
                << backend
                << " slot leasing must not rescan every layer and expert for "
                   "each candidate slot";
            EXPECT_NE(source->find(
         ...
         PrefillLLEPPolicyIsTypedAndAlwaysTransferBacked)
    {
        ...
        ASSERT_FALSE(graph.empty()) << graph_path;
        ASSERT_FALSE(stage.empty()) << stage_path;
        ASSERT_FALSE(current_batch_stage.empty()) << current_batch_stage_path;
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "Requested GPU LLEP must fail hard when full graph transport is unavailable.";
        EXPECT_NE(
            graph.find(
            ...
            std::string::npos)
            << "Requested CPU or GPU LLEP must fail hard when no production "
        ...
        EXPECT_EQ(stage.find("assignPrefillRoutesFromLeastLoadedCurrentBatchPlanAfterTransfers"),
                  std::string::npos)
            << "The expert stage must not retain a hidden current-batch transport path.";
        EXPECT_NE(stage.find("usesGraphPhasedCurrentBatchPrefillLLEP"),
                  std::string::npos)
            << "Expert compute must consume, but never produce, graph-phased assignment.";
        const size_t movement_begin = stage.find(
            "executePrefixRuntimeRehydrationPayloadMovement(");
        const size_t movement_end = stage.find(
            "bool MoEExpertComputeStage::executeFixedTopologyGroupedPrefill(",
            movement_begin);
        ASSERT_NE(movement_begin, std::string::npos);
        ASSERT_NE(movement_end, std::string::npos);
        const std::string movement_body =
            stage.substr(movement_begin, movement_end - movement_begin);
        EXPECT_EQ(
            movement_body.find(
                "materializePrefillLeastLoadedMirroredDomainCommands("),
            std::string::npos)
            << "The prefix-only composite must not retain ordinary current-batch planning.";
        EXPECT_EQ(
            countOccurrences(movement_body, "allgatherRawOnStream("),
            3u)
            << "Prefix rehydration owns its two metadata gathers and one payload gather.";

        EXPECT_NE(
            current_batch_stage.find(
                "materializePrefillLeastLoadedMirroredDomainCommands("),
            std::string::npos)
            << "Symmetric current-batch plans must materialize the complete "
               "participant-major command envelope locally.";
        EXPECT_NE(
            current_batch_stage.find(
                "assignPrefillRoutesFromLeastLoadedCurrentBatchPlanAfterTransfers("),
            std::string::npos)
            << "The explicit apply phase must publish assignment after arrivals.";
        EXPECT_EQ(
            current_batch_stage.find("allgatherRawOnStream("),
            std::string::npos)
            << "Current-batch plan/apply phases must never launch a hidden collective.";
        EXPECT_NE(
            graph.find("moe_current_batch_llep_payload"),
            std::string::npos)
            << "The graph must carry current-batch payloads through a named TP sideband.";
        EXPECT_NE(
            graph.find("takeCurrentBatchLLEPPayloadSideband()"),
            std::string::npos)
            << "The sideband must attach to the production shared-expert allreduce.";
         ...
         RebalanceAndPrefillLLEPUseImmutableStageOwnedLaunchContexts)
    {
        ...
        const std::string current_batch_stage_source =
            readFile(current_batch_stage_path);
        ...
        ASSERT_FALSE(current_batch_stage_source.empty());
        ...
        EXPECT_NE(
            current_batch_stage_source.find(
                "const MoEKernelLaunchContext launch"),
            std::string::npos)
            << "Both explicit current-batch phases must launch on their exact graph stream.";
        EXPECT_EQ(
            current_batch_stage_source.find("allgatherRawOnStream("),
            std::string::npos)
            << "The model TP stage, not either local phase, owns the sideband collective.";
        EXPECT_EQ(
            current_batch_stage_source.find("recordEventChecked"),
            std::string::npos)
            << "Graph dependencies already provide plan/collective/apply ordering.";
        EXPECT_EQ(
            current_batch_stage_source.find("synchronizeStream"),
            std::string::npos);
        EXPECT_EQ(
            current_batch_stage_source.find("DeviceSynchronize"),
            std::string::npos);
        EXPECT_EQ(
            current_batch_stage_source.find("StreamSynchronize"),
            std::string::npos);
        ...
        EXPECT_NE(
            ...
            std::string::npos)
            << "Prefix rehydration must name its distinct transfer-resource authority.";
        EXPECT_NE(
            llep_prepare_body.find("isMaterializedFor("),
            std::string::npos)
            << "Prefix rehydration graph preparation must validate persistent resources.";
        EXPECT_NE(
            llep_prepare_body.find(
         ...
         GroupedVerifierPublishesHistogramsOnlyAtAcceptedStateCommit)
    {
        ...
        const std::string router_stage_source = readFile(router_stage_path);
        ...
        EXPECT_EQ(
            ...
            std::string::npos);
        EXPECT_EQ(
            router_stage_source.find("snapshotGroupedVerifierRoutes("),
            std::string::npos);
        EXPECT_NE(
            interface_source.find("deferred_selected_route_ledger"),
            std::string::npos)
            << "The explicit heterogeneous ticket boundary must fuse selected "
                 ...
                 std::pair{"ROCm", &rocm_source}})
        {
            ...
            const std::string verifier_topk_kernel =
                backend == std::string("CUDA")
                    ? "softmax_topk_kernel("
                    : "rocm_moe_softmax_topk_decode_equivalent_rows_kernel(";
            const size_t verifier_topk_begin =
                source->find(verifier_topk_kernel);
            const size_t grouping_begin =
                source->find(grouping_kernel);
            const size_t commit_begin =
                source->find(commit_kernel);
            ASSERT_NE(verifier_topk_begin, std::string::npos) << backend;
            ASSERT_NE(grouping_begin, std::string::npos) << backend;
            ASSERT_NE(commit_begin, std::string::npos) << backend;
            ASSERT_LT(verifier_topk_begin, grouping_begin) << backend;
            ASSERT_LT(grouping_begin, commit_begin) << backend;
            const std::string verifier_topk =
                source->substr(
                    verifier_topk_begin,
                    grouping_begin - verifier_topk_begin);
            EXPECT_NE(
                verifier_topk.find("deferred_selected_route_ledger"),
                std::string::npos)
                << backend;
            EXPECT_NE(
                verifier_topk.find(
                ...
                std::string::npos)
                << backend
                << " decode-equivalent top-k must retain overlay selections "
            ...
            EXPECT_EQ(
                speculative_grouping.find(
                    "runtime_selected_histogram(*runtime, 2u)"),
                std::string::npos)
                << backend
                << " speculative grouping mutated external grouped-verifier history.";
            EXPECT_EQ(
                speculative_grouping.find(
                    "runtime_local_histogram(*runtime, 2u)"),
                std::string::npos)
                << backend
                << " speculative grouping mutated external grouped-verifier local history.";
            EXPECT_NE(
                speculative_grouping.find(
            ...
            EXPECT_NE(
                ...
                std::string::npos)
                << backend
                << " must publish accepted routes through the active persistent bank.";
            EXPECT_NE(
                committed_publication.find(
                    "runtime_local_histogram(*runtime, 2u)[expert_id]"),
                std::string::npos)
                << backend
                << " must publish accepted local routes through the active persistent bank.";
        }
    }
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, PrefixRuntimeStateDoesNotSerializeMoEPlacementPointers)
    {
        ...
        EXPECT_NE(capture_body.find("if (table->usesOverlayEpochTicket())"),
                  std::string::npos)
            << "Prefix capture must exclude model-lifetime ExpertOverlay RCU "
               "placement instead of serializing a stale residency epoch.";
        EXPECT_NE(capture_body.find(
                      "moe_durable_runtime_state_excluded_from_prefix"),
                  std::string::npos)
            << "PerfStats must prove that durable placement was deliberately "
               "excluded from the prefix payload.";
        EXPECT_NE(graph.find(
                      "Prefix-cache MoE runtime state attempted to replace model-lifetime ExpertOverlay placement"),
                  std::string::npos)
            << "Restore must reject a payload that names an epoch-ticketed table.";
        ...
        EXPECT_NE(
            prefix_reset_body.find("!table->usesOverlayEpochTicket()"),
            std::string::npos)
            << "No-payload prefix restore must reset only request-transient "
               "placement and preserve the live model-lifetime RCU family.";
        EXPECT_NE(
            prefix_reset_body.find(
                "table->overlayPlacementSource() != nullptr"),
            std::string::npos)
            << "Prefix reset must also restore a request-local LLEP child that "
               "pins, but does not own, its durable ExpertOverlay parent.";
        EXPECT_NE(prefix_reset_body.find("restoreInitialRuntimeState(execution_stream)"),
                  std::string::npos)
            << "No-payload prefix restore must republish the immutable baseline "
               "for request-transient placement on the transaction stream.";
        ...
        EXPECT_EQ(prefix_reset_body.find("resetRequestPublications(execution_stream)"),
                  std::string::npos)
            << "The transfer directory may back a live durable bank; prefix "
               "reset must not invalidate descriptors selected by the RCU ticket.";
    ...
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, GraphCapturedSnapshotsPublishOnlyFromImmutableDeviceManifest)
    {
        ...
        EXPECT_NE(manifest_publish.find(
                      "output.data = arena_host_bytes + copy.storage_offset_bytes"),
                  std::string::npos)
            << "Published descriptors must refer directly to their graph-stable arena subrange.";
        EXPECT_NE(manifest_publish.find("output.tensor = nullptr"),
                  std::string::npos)
            << "An acquired arena subrange must not be rebound to the arena base by StageDumpInfo.";
        EXPECT_EQ(manifest_publish.find("ensureOutputsOnHost"),
                  std::string::npos)
            << "The graph-owned arena must be acquired once before per-stage publication.";
        ...
        EXPECT_EQ(copy_body.find("FP32Tensor::createMapped("),
                  std::string::npos)
            << "Per-stage snapshot recording must never allocate mapped storage.";
        EXPECT_NE(copy_body.find("snapshot_manifest.storage_arena->isMapped()"),
                  std::string::npos)
            << "Recording must reject mapped-host snapshot destinations.";
        EXPECT_EQ(copy_body.find("std::make_unique<FP32Tensor>("),
                  std::string::npos)
            << "Graph snapshot manifests must not allocate ordinary device tensors.";
        EXPECT_EQ(copy_body.find("allocateOnDevice("),
                  std::string::npos)
        ...
        const size_t capture_event_branch =
            copy_body.rfind("if (capture_active)", event_contract);
        ASSERT_NE(capture_event_branch, std::string::npos)
            << "Graph-captured snapshot copies need a capture-specific "
               "coherence branch.";
        const size_t arena_acquire_start = source.find(
            "bool DeviceGraphExecutor::acquireGraphSnapshotArenaOnHost(",
            copy_start);
        ASSERT_NE(arena_acquire_start, std::string::npos);
        ASSERT_GT(arena_acquire_start, capture_event_branch);
        const std::string capture_event_only =
            source.substr(
                capture_event_branch,
                arena_acquire_start - capture_event_branch);
        EXPECT_NE(
        ...
        const size_t arena_bind_start =
            source.find("bool DeviceGraphExecutor::bindGraphSnapshotArena(");
        ASSERT_NE(arena_bind_start, std::string::npos);
        ASSERT_LT(arena_bind_start, copy_start);
        const std::string arena_bind_body =
            source.substr(arena_bind_start, copy_start - arena_bind_start);
        EXPECT_EQ(arena_bind_body.find("FP32Tensor::createMapped("),
                  std::string::npos)
            << "Captured snapshot payloads must never target mapped host pages.";
        EXPECT_NE(arena_bind_body.find("std::make_shared<FP32Tensor>("),
                  std::string::npos)
            << "Descriptor completion must bind one ordinary device arena whose shared owner preserves captured addresses.";
        EXPECT_NE(arena_bind_body.find(
                      "allocateOnDevice(arena_device, setup_stream)"),
                  std::string::npos)
            << "The snapshot destination must be materialized in VRAM before capture.";
        EXPECT_NE(arena_bind_body.find("storage_offset_bytes = aligned_offset"),
                  std::string::npos)
            << "Every immutable checkpoint needs an explicit disjoint arena offset.";
        EXPECT_EQ(arena_bind_body.find("for (auto &copy"),
                  arena_bind_body.rfind("for (auto &copy"))
            << "One descriptor loop must size slots before one arena allocation.";

        ASSERT_LT(arena_acquire_start, manifest_publish_start);
        const std::string arena_acquire_body = source.substr(
            arena_acquire_start,
            manifest_publish_start - arena_acquire_start);
        EXPECT_NE(arena_acquire_body.find(
                      "TransferEngine::publishDeviceWrite("),
                  std::string::npos)
            << "The complete arena needs one exact post-launch producer event.";
        EXPECT_NE(arena_acquire_body.find(
                      "storage_arena->ensureOnHost(producer_stream)"),
                  std::string::npos)
            << "The complete device arena must be downloaded once before callbacks read subranges.";
    }

    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         SnapshotPolicyCannotChangeGraphActivationPlacement)
    {
        const fs::path root = findRepoRoot();
        const std::vector<fs::path> public_config_paths = {
            root / "src/v2/execution/factory/InferenceRunnerFactory.h",
            root / "src/v2/execution/local_execution/orchestrators/RankOrchestrator.h",
            root / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.h",
        };
        for (const auto &path : public_config_paths)
        {
            ASSERT_TRUE(fs::exists(path)) << path;
            const std::string source = readFile(path);
            ASSERT_FALSE(source.empty()) << path;
            EXPECT_EQ(source.find("use_mapped_memory"), std::string::npos)
                << path
                << " must not expose a flag that relocates captured activation "
                   "storage into GPU-visible host memory";
        }

        const fs::path arena_path = root / "src/v2/memory/BufferArena.cpp";
        const fs::path factory_path = root / "src/v2/tensors/TensorFactory.cpp";
        const fs::path orchestrator_path =
            root / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp";
        const fs::path parity_path =
            root / "tests/v2/integration/parity/ParityTestBase.h";
        for (const auto &path : {arena_path, factory_path, orchestrator_path, parity_path})
            ASSERT_TRUE(fs::exists(path)) << path;

        const std::string arena = readFile(arena_path);
        const std::string factory = readFile(factory_path);
        const std::string orchestrator = readFile(orchestrator_path);
        const std::string parity = readFile(parity_path);
        EXPECT_EQ(arena.find("FP32Tensor::createMapped("), std::string::npos)
            << "BufferArena owns graph activations, never mapped transport pages";
        EXPECT_NE(arena.find("tensor->isMapped()"), std::string::npos)
            << "BufferArena must reject a future factory that attempts to return "
               "mapped storage for an owned GPU activation";
        EXPECT_EQ(factory.find("FP32Tensor::createMapped("), std::string::npos)
            << "Generic TensorFactory construction must have no stateful mapped mode";
        EXPECT_EQ(factory.find("use_mapped_memory_for_gpu_"), std::string::npos);
        EXPECT_EQ(orchestrator.find("setUseMappedMemoryForGPU"), std::string::npos);
        EXPECT_EQ(orchestrator.find("arena_config.use_mapped_memory"),
                  std::string::npos);
        EXPECT_EQ(parity.find("use_mapped_memory"), std::string::npos)
            << "Parity diagnostics must exercise the same device-local graph "
               "activation placement as production inference";
    }

    TEST(Test__MoEGraphNative_ForbiddenDependencyScan, FastCollectivePathUsesTypedSnapshotRecordingAuthority)
    {
        const fs::path root = findRepoRoot();
        const fs::path executor_path =
            root / "src/v2/execution/local_execution/graph/DeviceGraphExecutor.cpp";
        ...
        EXPECT_NE(fast_collective_body.find(
                  ...
                  std::string::npos)
            << "Fast collectives must enter the graph-stable D2D snapshot contract whenever snapshotting is enabled.";
        EXPECT_NE(fast_collective_body.find(
                      "policy.snapshot_recording_authority =="),
                  std::string::npos)
            << "Exactly one typed lifecycle owner must record each fast-collective snapshot copy.";
        EXPECT_NE(fast_collective_body.find(
                      "StageRunPolicy::SnapshotRecordingAuthority::"),
                  std::string::npos);
        EXPECT_NE(fast_collective_body.find("StageExecutor"),
                  std::string::npos)
            << "The fast stage executor may record only when it owns snapshot recording; segmented capture delegates to the graph controller.";
    ...
    /**
     * @brief Keep aggregate parity cells isolated to their claimed GPU runtime.
     *
     * The aggregate scheduler intentionally overlaps CPU, CUDA, and ROCm cells
     * only when their typed resource signatures are disjoint. Merely asking an
     * unclaimed backend for its device count initializes that runtime and its
     * pinned-host allocator, which makes otherwise independent CUDA and ROCm
     * processes interfere. Hardware admission must therefore calculate the
     * required device families first and probe only the families the cell owns.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ProductionParityProbesOnlyClaimedGpuRuntimes)
    {
        const fs::path parity_path =
            findRepoRoot() / "tests/v2/integration/parity/ParityTestBase.h";
        ASSERT_TRUE(fs::exists(parity_path)) << parity_path;
        ...
        const size_t admission_start = parity.find(
            "inline std::optional<std::string> checkHardwareAvailability(");
        const size_t next_declaration = parity.find(
            "// Metric Computation Functions",
            admission_start);
        ASSERT_NE(admission_start, std::string::npos);
        ASSERT_NE(next_declaration, std::string::npos);
        const std::string admission = parity.substr(
            admission_start, next_declaration - admission_start);

        EXPECT_NE(
            admission.find(
                "required_cuda > 0 ? getCudaDeviceCount() : 0"),
            std::string::npos)
            << "A non-CUDA parity cell must not initialize the CUDA runtime.";
        EXPECT_NE(
            admission.find(
                "required_rocm > 0 ? getRocmDeviceCount() : 0"),
            std::string::npos)
            << "A non-ROCm parity cell must not initialize the ROCm runtime.";

        const size_t requirement_scan = admission.find(
            "for (auto dt : cfg.devices)");
        const size_t cuda_probe = admission.find("getCudaDeviceCount()");
        const size_t rocm_probe = admission.find("getRocmDeviceCount()");
        ASSERT_NE(requirement_scan, std::string::npos);
        ASSERT_NE(cuda_probe, std::string::npos);
        ASSERT_NE(rocm_probe, std::string::npos);
        EXPECT_LT(requirement_scan, cuda_probe)
    ...
    /**
     * @brief Keep background device work outside every serving-graph capture.
     *
     * CUDA and HIP capture are process-wide ordering domains even when the
     * controller owns a nominally independent stream. Starting Dynamic from
     * its constructor let economy calibration race the later lean serving
     * topology, producing intermittent capture invalidation under model reuse.
     * The controller must therefore remain `Prepared` until the runner has
     * materialized both variants and enters its explicit activation phase.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         DeviceControllerActivationFollowsEveryProductionGraphCapture)
    {
        const fs::path root = findRepoRoot();
        const fs::path runner_path =
            root / "src/v2/execution/runner/OrchestrationRunner.cpp";
        ...
        const std::string constructor = braceBalancedDeclarationRegion(
            service,
            "MoEOverlayDeviceControllerGraphService::\n        MoEOverlayDeviceControllerGraphService(Config config)");
        const std::string activation = braceBalancedDeclarationRegion(
            service,
            "void MoEOverlayDeviceControllerGraphService::start()");
        ASSERT_FALSE(constructor.empty());
        ASSERT_FALSE(activation.empty());
        EXPECT_EQ(constructor.find("std::jthread("), std::string::npos)
            << "Service construction must remain dormant during later graph capture";
        EXPECT_NE(activation.find("std::jthread("), std::string::npos)
            << "The explicit activation edge must own Dynamic worker creation";
        EXPECT_NE(
            activation.find(
                "MoEOverlayDeviceControllerActivationState::Prepared"),
            std::string::npos)
            << "Activation must reject every state except Prepared";

        const std::string initialize = braceBalancedDeclarationRegion(
            runner, "bool OrchestrationRunner::initialize()");
        const std::string start_maintenance =
            braceBalancedDeclarationRegion(
                runner,
                "bool OrchestrationRunner::\n        startMoEExpertOverlayResidencyMaintenance()");
        ASSERT_FALSE(initialize.empty());
        ASSERT_FALSE(start_maintenance.empty());
        const size_t lean_capture = initialize.find(
            "deactivatePreparedSnapshotCaptureAndMaterializeLeanServingFamily");
        const size_t transaction_capture = initialize.find(
            "initializeMoEOverlayInferenceTransactions");
        const size_t activate = initialize.find(
            "startMoEExpertOverlayResidencyMaintenance");
        ASSERT_NE(lean_capture, std::string::npos);
        ASSERT_NE(transaction_capture, std::string::npos);
        ASSERT_NE(activate, std::string::npos);
        EXPECT_LT(lean_capture, transaction_capture)
            << "MTP transactions are materialized only after the serving topology";
        EXPECT_LT(transaction_capture, activate)
            << "Every serving and MTP transaction graph must exist before background activation";
        EXPECT_NE(
            start_maintenance.find(
                "moe_overlay_device_controller_graph_service_->start()"),
            std::string::npos)
            << "The rank-synchronized activation phase must start the device authority";
    }

    /**
     * @brief Make the terminal physical seal the sole registry-rebind authority.
     *
     * Device-owned RCU intentionally leaves the host residency snapshot at its
     * setup epoch. Rebinding from that snapshot after a restored device epoch
     * creates a split authority and strands the model lifecycle in `Sealing`.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ReusableContextRegistryRebindCannotConsultHostResidencySnapshot)
    {
        const fs::path root = findRepoRoot();
        const fs::path runner_path =
            root / "src/v2/execution/runner/OrchestrationRunner.cpp";
        const fs::path fabric_header_path =
            root /
            "src/v2/execution/moe/MoEOverlayPhysicalResidencyFabric.h";
        ASSERT_TRUE(fs::exists(runner_path)) << runner_path;
        ASSERT_TRUE(fs::exists(fabric_header_path)) << fabric_header_path;

        const std::string runner = readFile(runner_path);
        const std::string fabric_header = readFile(fabric_header_path);
        ASSERT_FALSE(runner.empty());
        ASSERT_FALSE(fabric_header.empty());

        const std::string rebind = braceBalancedDeclarationRegion(
            runner,
            "bool OrchestrationRunner::rebindModelRegistryFromReusableContextSeal(");
        ASSERT_FALSE(rebind.empty());
        EXPECT_NE(
            fabric_header.find("MoEExpertOwnerMap canonical_owner_map;"),
            std::string::npos)
            << "The terminal seal must carry the owner map represented by its banks";
        EXPECT_NE(
            rebind.find("seal.canonical_owner_map"),
            std::string::npos)
            << "Registry rebinding must consume only the self-contained physical seal";
        EXPECT_EQ(
            rebind.find("moe_expert_overlay_residency_authority_"),
            std::string::npos)
            << "Device-owned teardown cannot reconstruct authority from a stale host snapshot";
    }
         ...
         TerminalDrainRetainsDeviceMovementLedgerForRunnerLifetime)
    {
        ...
        const size_t worker_release = service_drain.find(
            "dynamic_worker_.reset()", service_retain);
        ASSERT_NE(worker_release, std::string::npos);
        EXPECT_LT(service_retain, worker_release)
            << "A live worker's complete ledger must survive its resource owner; Prepared setup rollback owns no movement ledger";
         ...
         MappedTransferProgressUsesBoundedTypedExecutionLanes)
    {
        ...
        EXPECT_NE(materialize.find("index %"), std::string::npos);
        EXPECT_NE(
            materialize.find("config_.execution_streams.size()"),
            std::string::npos)
            << "Logical event lanes must share the typed physical stream pool";
        EXPECT_EQ(
            materialize.find("getOrCreateAuxiliaryStream"),
            std::string::npos)
            << "Mapped progress must consume TransferEngine-owned streams";
        ...
        EXPECT_NE(
            ...
            std::string::npos);
        EXPECT_NE(
            integration.find("executionStreamCapacity(), 4u"),
            std::string::npos);
        EXPECT_NE(
            integration.find(
    ...
    /**
     * @brief Prevent logical ExpertOverlay edges from recreating GPU queues.
     *
     * The physical fabric owns one participant/cycle execution-stream pool.
     * Projection, direction, and remote-role lanes receive typed leases on
     * those streams and retain only their own staging slice and completion
     * event. This keeps setup proportional to real concurrency and also makes
     * an interior pointer into a shared staging slab impossible to free.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         ExpertOverlayTransferLanesUseOneTypedPhysicalExecutionPool)
    {
        const fs::path root = findRepoRoot();
        const fs::path transfer_header =
            root / "src/v2/transfer/TransferEngine.h";
        const fs::path transfer_source =
            root / "src/v2/transfer/TransferEngine.cpp";
        const fs::path fabric_path =
            root /
            "src/v2/execution/moe/MoEOverlayPhysicalResidencyFabric.cpp";
        const std::array<fs::path, 3> lane_paths{
            root /
                "src/v2/execution/moe/ExpertTierWeightTransferLane.cpp",
            root /
                "src/v2/execution/moe/ExpertTierGpuPeerTransferLane.cpp",
            root /
                "src/v2/execution/moe/MoEOverlayGpuRemoteProjectionEndpoint.cpp",
        };
        const fs::path cuda_test_path =
            root /
            "tests/v2/integration/kernels/cuda/Test__CUDAExpertTierWeightKernels.cpp";
        ...
        for (const auto &path : lane_paths)
            ASSERT_TRUE(fs::exists(path)) << path;

        const std::string header = readFile(transfer_header);
        const std::string transfer = readFile(transfer_source);
        const std::string fabric = readFile(fabric_path);
        const std::string cuda_test = readFile(cuda_test_path);
        const std::string rocm_test = readFile(rocm_test_path);
        const std::string cmake = readFile(cmake_path);
        ...
        EXPECT_NE(
            header.find("class PersistentTransferExecutionLane final"),
            std::string::npos);
        EXPECT_NE(
            header.find("allocatePersistentTransferExecutionLanes("),
            std::string::npos);
        const std::string allocate_pool = braceBalancedDeclarationRegion(
            transfer,
            "TransferEngine::allocatePersistentTransferExecutionLanes(");
        ASSERT_FALSE(allocate_pool.empty());
        EXPECT_NE(
            allocate_pool.find("BackgroundMaintenance"),
            std::string::npos)
            << "The pooled transfer queues must remain background maintenance streams";
        ...
        EXPECT_EQ(
            ...
            1u)
            << "One physical pool per GPU must precede logical lane expansion";
        EXPECT_NE(
            materialize.find("parallelLaneCount(multiplicity)"),
            std::string::npos);
        EXPECT_NE(
            materialize.find(".execution = executionLane("),
            std::string::npos);

        for (const auto &path : lane_paths)
        {
            const std::string lane = readFile(path);
            EXPECT_EQ(lane.find("getOrCreateAuxiliaryStream"), std::string::npos)
                << path << " must consume its typed execution lane";
            EXPECT_EQ(lane.find("free(device_chunk_"), std::string::npos)
                << path << " must not free a shared-slab interior pointer";
            EXPECT_EQ(lane.find("freePinned(pinned_chunk_"), std::string::npos)
                << path << " must not free a shared-slab interior pointer";
        }

        constexpr const char *contract_test =
            "PersistentTransferExecutionPoolIsBoundedAndIdempotent";
        EXPECT_NE(cuda_test.find(contract_test), std::string::npos);
        EXPECT_NE(rocm_test.find(contract_test), std::string::npos);
        EXPECT_GE(
            countOccurrences(
                cmake,
                "V2_Integration_CUDA_ExpertTierWeightKernels"),
            2u)
            << "The CUDA pool contract must remain in parity preflight";
        EXPECT_GE(
            countOccurrences(
                cmake,
                "V2_Integration_ROCm_ExpertTierWeightKernels"),
            2u)
            << "The ROCm pool contract must remain in parity preflight";
    }

    /**
     * @brief Restrict late host-page registration to true shared-memory owners.
     *
     * Exact-one-GPU graph storage must originate in TransferEngine's native
    ...
    {
        const fs::path root = findRepoRoot();
        const std::array<std::pair<fs::path, size_t>, 4> allowed{
            std::pair{
                fs::path{"src/v2/transfer/TransferEngine.cpp"}, 2u},
            std::pair{
                fs::path{"src/v2/execution/moe/MoEOverlayNodeLocalDeviceControllerFabric.cpp"},
                1u},
            std::pair{
                fs::path{"src/v2/execution/moe/MoEOverlayNodeLocalRankBatchTransport.cpp"},
                1u},
            std::pair{
        ...
        const fs::path source_root = root / "src/v2";
        ASSERT_TRUE(fs::exists(source_root));
        for (const auto &entry :
             fs::recursive_directory_iterator(source_root))
        {
            ...
            const auto allowed_it = std::find_if(
                ...
                {
                    return candidate.first == relative;
                });
            const size_t expected =
                allowed_it == allowed.end() ? 0u : allowed_it->second;
            EXPECT_EQ(
                countOccurrences(readFile(entry.path()), std::string{needle}),
                expected)
                << relative
                << " must allocate exact-device mapped pages through TransferEngine; only declared cross-process/multi-device shared mappings may register external pages";
        }
    }

    /**
     * @brief Keep maintenance graph storage under one typed memory contract.
     *
     * Setup-time admission runs before live directory pointers exist, whereas
     * the compute stage publishes concrete workspace descriptors later.  Both
     * lifecycle phases must consume the same pointer-free contract or Dynamic
     * LocalTP can pass preflight and overrun its admitted workspace at graph
     * materialization time.
     */
    TEST(Test__MoEGraphNative_ForbiddenDependencyScan,
         DeviceRebalanceWorkspaceHasOneAdmissionAndRuntimeAuthority)
    {
        const fs::path root = findRepoRoot();
        const fs::path contract_header_path =
            root / "src/v2/execution/moe/DeviceMoERebalanceWorkspaceContract.h";
        const fs::path contract_source_path =
            root / "src/v2/execution/moe/DeviceMoERebalanceWorkspaceContract.cpp";
        const fs::path stage_source_path =
            root / "src/v2/execution/compute_stages/stages/MoEDeviceRebalanceStage.cpp";
        const fs::path admission_source_path =
        ...
        const std::string contract_header = readFile(contract_header_path);
        const std::string contract_source = readFile(contract_source_path);
        const std::string stage_source = readFile(stage_source_path);
        ...
        EXPECT_NE(
            ...
            std::string::npos);
        EXPECT_GE(
            countOccurrences(contract_source, "append(\n"),
            19u)
            << "The contract must continue to own every maintenance descriptor.";
        EXPECT_NE(
            stage_source.find(
                "DeviceMoERebalanceWorkspaceContract::requirements("),
            std::string::npos)
            << "The live stage must project descriptors from the canonical contract.";
        EXPECT_NE(
            stage_source.find(
                "DeviceMoERebalanceWorkspaceContract::logicalBytes("),
            std::string::npos)
            << "Runtime diagnostics must use the same buffer inventory.";
        EXPECT_NE(
            admission_source.find(
                "DeviceMoERebalanceWorkspaceContract::allocationBytes("),
            std::string::npos)
            << "Preflight must admit the contract's physical extents.";
        EXPECT_NE(
            admission_source.find(
        ...
        EXPECT_GE(
            ...
            1u)
            << "Graph construction must use the canonical codebook-aware slot stride.";
        EXPECT_EQ(
            graph_source.find(
