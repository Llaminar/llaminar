/**
 * @file Test__Q4_1Tensor_Views.cpp
 * @brief Unit tests for Q4_1Tensor view support
...
namespace
{
    ...
    // Helper: Create Q4_1 tensor with unique scale/min per block for verification
    std::shared_ptr<Q4_1Tensor> createTestTensor(size_t rows, size_t cols)
    {
        constexpr size_t BLOCK_SIZE = Q4_1Block::BLOCK_SIZE;
        ...
        {
            // Unique scale and min per block for verification
        ...
        return std::make_shared<Q4_1Tensor>(std::vector<size_t>{rows, cols}, raw_data);
    }

} // namespace

// Test 1: Basic view creation
TEST(Q4_1TensorViews, BasicViewCreation)
{
...
TEST(Q4_1TensorViews, ViewWithOffset)
{
    auto parent = createTestTensor(64, 128);
    ASSERT_NE(parent, nullptr);
...
TEST(Q4_1TensorViews, KDimensionMustMatch)
{
    auto parent = createTestTensor(64, 128);
    ASSERT_NE(parent, nullptr);
...
TEST(Q4_1TensorViews, OffsetMustBeRowAligned)
{
    auto parent = createTestTensor(64, 128);
    ASSERT_NE(parent, nullptr);
...
TEST(Q4_1TensorViews, ViewBoundsChecking)
{
    auto parent = createTestTensor(64, 128);
    ASSERT_NE(parent, nullptr);
...
TEST(Q4_1TensorViews, ViewLifetime)
{
...
TEST(Q4_1TensorViews, ViewChaining)
{
...
TEST(Q4_1TensorViews, IBlockDecoderInterface)
{
    auto parent = createTestTensor(64, 128);
    ASSERT_NE(parent, nullptr);
    ...
    EXPECT_EQ(decoder->block_size(), Q4_1Block::BLOCK_SIZE);

    // Decode a block from the view
    float output[Q4_1Block::BLOCK_SIZE];
    decoder->decode_block_at(0, 0, output);

    // Output should be non-zero (pattern-based data with scale + min)
