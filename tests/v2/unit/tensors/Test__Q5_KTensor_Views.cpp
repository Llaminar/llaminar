/**
 * @file Test__Q5_KTensor_Views.cpp
 * @brief Unit tests for Q5_KTensor row-slice view support
...
namespace
{
    ...
    // Helper to create a test Q5_K tensor with unique values
    std::shared_ptr<Q5_KTensor> createTestTensor(size_t rows, size_t cols)
    {
        // Fill with unique block values for verification
        // Q5_K has 256 elements per block (super-block)
        ...
        {
            // Set unique delta and dmin values for each block
            ...
            // Set pattern in high bits (32 bytes)
            for (size_t j = 0; j < 32; ++j)
            {
                blocks[i].qh[j] = static_cast<uint8_t>((i * 32 + j) & 0xFF);
            }

            // Set pattern in quantized data (128 bytes, 4 low bits × 2 = 256 elements)
        ...
        return std::make_shared<Q5_KTensor>(std::vector<size_t>{rows, cols}, raw_data);
    }

} // anonymous namespace

TEST(Test__Q5_KTensor, BasicViewCreation)
{
...
TEST(Test__Q5_KTensor, ViewWithOffset)
{
...
TEST(Test__Q5_KTensor, KDimensionMustMatch)
{
...
TEST(Test__Q5_KTensor, OffsetMustBeRowAligned)
{
    auto parent = createTestTensor(128, 512);
...
TEST(Test__Q5_KTensor, ViewBoundsChecking)
{
    auto parent = createTestTensor(128, 512);
...
TEST(Test__Q5_KTensor, ViewLifetime)
{
    std::weak_ptr<Q5_KTensor> weak_parent;
...
TEST(Test__Q5_KTensor, ViewChaining)
{
...
TEST(Test__Q5_KTensor, IBlockDecoderInterface)
{
...
TEST(Test__Q5_KTensor, SuperBlockAlignment)
{
    // Q5_K uses 256-element super-blocks
