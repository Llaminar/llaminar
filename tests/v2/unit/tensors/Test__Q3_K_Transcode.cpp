...
class Q3_K_TranscodeTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
    ...
    void RunScalar(const Q3_KBlock &block, int sub_block_idx, float *out_scale, float *out_min)
    {
        llaminar2::simd::transcode_q3_k_to_int8_scalar(block, sub_block_idx, expected_int8, out_scale, out_min);
    }
...
TEST_F(Q3_K_TranscodeTest, ScalarVsAVX2_Random)
{
#if defined(__AVX2__)
    ...
    for (int i = 0; i < 100; ++i)
    {
        ...
        for (int j = 0; j < 32; ++j)
            block.hmask[j] = dist_q(rng);

        int sub_block_idx = dist_sub(rng);

        RunScalar(block, sub_block_idx, &expected_scale, &expected_min);
        llaminar2::simd::transcode_q3_k_to_int8_avx2(block, sub_block_idx, actual_int8, &actual_scale, &actual_min);
