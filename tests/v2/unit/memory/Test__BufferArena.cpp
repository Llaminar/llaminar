...
// ============================================================================
// Phase 2: SwiGLUStage bufferContract() Tests
// ============================================================================

#include "execution/compute_stages/stages/SwiGLUStage.h"

TEST(Test__SwiGLUContract, EmptyWhenNoBufferIds)
{
    // SwiGLU without BufferIds should return empty contract (old path)
    auto gate = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());
    auto up = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());

    SwiGLUStage::Params params;
    params.gate = gate.get();
    params.up = up.get();
    params.output = up.get(); // in-place
    params.seq_len = 4;
...
TEST(Test__SwiGLUContract, InPlaceContractHasInOut)
{
    // SwiGLU with in-place output (output_id == up_id) should use addInOut
    auto gate = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());
    auto up = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());
    ...
    params.seq_len = 4;
    params.device_id = DeviceId::cpu();
    params.gate_buffer_id = BufferId::GATE_PROJ;
    params.up_buffer_id = BufferId::UP_PROJ;
    params.output_buffer_id = BufferId::UP_PROJ; // same as up → in-place

    SwiGLUStage stage(params);
    auto contract = stage.bufferContract();

    EXPECT_FALSE(contract.empty());
    // 1 input (gate) + 0 outputs + 1 inout (up/output)
    EXPECT_EQ(contract.inputs.size(), 1u);
    EXPECT_EQ(contract.outputs.size(), 0u);
    EXPECT_EQ(contract.inouts.size(), 1u);
    EXPECT_EQ(contract.weight_tensors.size(), 0u);

    EXPECT_EQ(contract.inputs[0].id, BufferId::GATE_PROJ);
    EXPECT_EQ(contract.inputs[0].access, BufferAccess::READ);
    EXPECT_EQ(contract.inouts[0].id, BufferId::UP_PROJ);
    EXPECT_EQ(contract.inouts[0].access, BufferAccess::READWRITE);

    // allArenaReads should include both input and inout
    auto reads = contract.allArenaReads();
    EXPECT_EQ(reads.size(), 2u);

    // allWrites should include only inout
    auto writes = contract.allWrites();
    EXPECT_EQ(writes.size(), 1u);
    EXPECT_EQ(writes[0].id, BufferId::UP_PROJ);
}

TEST(Test__SwiGLUContract, OutOfPlaceContractSeparatesIO)
{
    // SwiGLU with separate output should have 2 inputs + 1 output
    auto gate = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());
    auto up = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());
    auto out = std::make_shared<FP32Tensor>(std::vector<size_t>{4, 128}, DeviceId::cpu());

    SwiGLUStage::Params params;
    params.gate = gate.get();
    params.up = up.get();
    params.output = out.get(); // separate buffer
    params.seq_len = 4;
    params.device_id = DeviceId::cpu();
    params.gate_buffer_id = BufferId::GATE_PROJ;
    params.up_buffer_id = BufferId::UP_PROJ;
    params.output_buffer_id = BufferId::FFN_OUTPUT; // different → out-of-place

    SwiGLUStage stage(params);
    auto contract = stage.bufferContract();

    EXPECT_FALSE(contract.empty());
    EXPECT_EQ(contract.inputs.size(), 2u);  // gate + up
    EXPECT_EQ(contract.outputs.size(), 1u); // ffn_output
    EXPECT_EQ(contract.inouts.size(), 0u);
    EXPECT_EQ(contract.weight_tensors.size(), 0u);

    EXPECT_EQ(contract.inputs[0].id, BufferId::GATE_PROJ);
    EXPECT_EQ(contract.inputs[1].id, BufferId::UP_PROJ);
    EXPECT_EQ(contract.outputs[0].id, BufferId::FFN_OUTPUT);
}
...
TEST(Test__ArenaContractCoherence, ContractEmptynessDeterminesPath)
{
    // Verify the executor's branching logic:
    // - Empty contract → fall back to getDumpInfo path
    // - Non-empty contract + arena → use contract path

    auto gate = std::make_shared<FP32Tensor>(std::vector<size_t>{2, 4}, DeviceId::cpu());

    // Stage without BufferIds → empty contract
    SwiGLUStage::Params params_no_ids;
    params_no_ids.gate = gate.get();
    params_no_ids.up = gate.get();
    params_no_ids.output = gate.get();
    params_no_ids.seq_len = 2;
    params_no_ids.device_id = DeviceId::cpu();
    SwiGLUStage stage_no_ids(params_no_ids);

    EXPECT_TRUE(stage_no_ids.bufferContract().empty());

    // Stage with BufferIds → non-empty contract
    SwiGLUStage::Params params_with_ids = params_no_ids;
    params_with_ids.gate_buffer_id = BufferId::GATE_PROJ;
    params_with_ids.up_buffer_id = BufferId::UP_PROJ;
    params_with_ids.output_buffer_id = BufferId::UP_PROJ;
    SwiGLUStage stage_with_ids(params_with_ids);

    auto contract = stage_with_ids.bufferContract();
    EXPECT_FALSE(contract.empty());
    EXPECT_EQ(contract.bindingCount(), 2u); // 1 input + 1 inout
}
