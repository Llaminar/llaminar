...
namespace llaminar2::test
{
    namespace
    {
        ...
        {
        public:
            explicit EnvVarUnsetGuard(std::string name)
                : name_(std::move(name))
            {
                const char *previous = std::getenv(name_.c_str());
                if (previous)
    ...
    TEST(Test__Qwen35MoEExpertOverlayGraph, RawExpertReleaseDefaultsOnAndCanBeDisabled)
    {
        {
            EnvVarUnsetGuard release_unset("LLAMINAR_MOE_RELEASE_RAW_WEIGHTS");
            EXPECT_TRUE(debugEnv().moe_rebalance.release_raw_weights);
        }

        {
            EnvVarGuard release_disabled("LLAMINAR_MOE_RELEASE_RAW_WEIGHTS", "0");
            EXPECT_FALSE(debugEnv().moe_rebalance.release_raw_weights);
        }

        {
            EnvVarGuard release_enabled("LLAMINAR_MOE_RELEASE_RAW_WEIGHTS", "1");
            EXPECT_TRUE(debugEnv().moe_rebalance.release_raw_weights);
        }
    }
