...
namespace llaminar2::test
{
    ...
    TEST(Test__Qwen35MoEExpertOverlayGraph, CpuNodeLocalTierLowersToDomainFallbackStageWithoutPrimaryOnlyGpuTiers)
    {
        ...
        const auto *fallback_stage = dynamic_cast<const MoEExpertOverlayCPUFallbackStage *>(cpu_node->stage.get());
        ASSERT_NE(fallback_stage, nullptr);
        EXPECT_EQ(fallback_stage->params().domain.name, "cpu_domain");
        EXPECT_EQ(fallback_stage->params().domain.kind, ExpertDomainKind::NodeLocalTP);
        EXPECT_EQ(fallback_stage->params().domain.compute_kind, ExpertDomainComputeKind::TensorParallelExperts);
        EXPECT_EQ(fallback_stage->params().domain.participants.size(), 2u);
        EXPECT_EQ(fallback_stage->params().root_world_rank, 0);
    }
    ...
    TEST(Test__Qwen35MoEExpertOverlayGraph, ActiveRocmLocalTPTierBuildsAndRunsWithMockContextAndKeepsCpuFallbackStage)
    {
        ...
        const auto *local_tp_stage = dynamic_cast<const MoEExpertOverlayLocalTPStage *>(rocm_node->stage.get());
        ASSERT_NE(local_tp_stage, nullptr);
        EXPECT_EQ(local_tp_stage->coherencePolicy(), CoherencePolicy::NONE);
        EXPECT_TRUE(local_tp_stage->bufferContract().empty());
        EXPECT_EQ(local_tp_stage->params().domain.name, "rocm_domain");
        EXPECT_EQ(local_tp_stage->params().domain.kind, ExpertDomainKind::LocalTP);
        EXPECT_EQ(local_tp_stage->params().domain.compute_kind, ExpertDomainComputeKind::TensorParallelExperts);
        EXPECT_EQ(local_tp_stage->params().local_tp_context, &rocm_tp_context);
        EXPECT_NE(local_tp_stage->params().dispatch_output, nullptr);
        EXPECT_EQ(local_tp_stage->params().dispatch_tier_index, 0);
        EXPECT_NE(local_tp_stage->params().diagnostics, nullptr);
        ...
        EXPECT_EQ(fallback_stage->params().domain.compute_kind, ExpertDomainComputeKind::TensorParallelExperts);
        EXPECT_EQ(fallback_stage->params().domain.participants.size(), 2u);
        EXPECT_NE(fallback_stage->params().dispatch_output, nullptr);
        EXPECT_EQ(fallback_stage->params().dispatch_tier_index, 1);
        EXPECT_EQ(fallback_stage->params().transfer_mode, MoEExpertTransferMode::Auto);
        EXPECT_EQ(local_tp_stage->params().dispatch_output, fallback_stage->params().dispatch_output);
        ...
        const auto *dispatch = local_tp_stage->params().dispatch_output;
        ...
        const auto *diagnostics = local_tp_stage->params().diagnostics;
        ...
        EXPECT_TRUE(hasNonZeroValue(local_tp_stage->params().output));

        auto reference = activation_arena.fp32({kSeqLen, kDModel});
        ASSERT_TRUE(runDenseLocalTPReference(local_tp_stage->params().domain,
                                             rocm_tp_context,
                                             layer,
                                             buffers,
                                             {true, true, false, false, true, false},
                                             reference));
        expectTensorNear(local_tp_stage->params().output, reference);
    }
    ...
    TEST(Test__Qwen35MoEExpertOverlayGraph, DenseTransferCompatibilityModeIsExplicitDebugEnvControl)
    {
        ...
        const auto *fallback_stage = dynamic_cast<const MoEExpertOverlayCPUFallbackStage *>(cpu_node->stage.get());
        ASSERT_NE(fallback_stage, nullptr);
        EXPECT_EQ(fallback_stage->params().transfer_mode, MoEExpertTransferMode::DenseFullSequence);
        EXPECT_NE(fallback_stage->params().dispatch_output, nullptr);
