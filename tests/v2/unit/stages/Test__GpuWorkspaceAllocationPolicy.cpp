...
TEST(Test__GpuWorkspaceAllocationPolicy, MirroredMTPDiagnosticsUseTypedForwardRoles)
{
    ...
    EXPECT_NE(header.find("enum class ForwardExecutionRole"),
              std::string::npos);
    EXPECT_NE(header.find("GroupedMTPVerifier"),
              std::string::npos);
    EXPECT_NE(header.find("MTPRequestBatchCondition"),
              std::string::npos);
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "Single-request device-token verifier forwards must declare their role.";
...
TEST(Test__GpuWorkspaceAllocationPolicy, MoERebalanceMaintenanceAlwaysPublishesDeviceEvent)
{
    ...
    EXPECT_NE(executable_fn.find("maintenance_graph_uses_decode_collective_lane()"), std::string::npos)
        << "PerfStats should retain whether maintenance shares the decode LocalTP communicator.";
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPShiftedKVAsyncHandoffUsesEventBeforeConsumers)
{
    ...
    const size_t live_epoch_advance =
        publish_body.find("handleLivePrefixReplayStateAfterMutation");
    const size_t logical_mailbox_publish =
        publish_body.find("recordDeviceResidentLogicalSequenceStateMailbox");
    ...
    EXPECT_NE(shifted_mutation_body.find("shifted_mtp_kv_sidecar_only"),
              std::string::npos)
        << "PerfStats must identify the narrow shifted-cache mutation scope.";
    ...
    expectNeedleBefore(
        ...
        "Resident correction shifted commits must consume the mailbox after the shifted-KV epoch advances.");
    EXPECT_NE(resident_logical_body.find(
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPTerminalHiddenMailboxUsesExplicitDeviceOwnership)
{
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "Terminal refresh must wait on the durable main-forward publication.";
    EXPECT_EQ(
...
TEST(Test__GpuWorkspaceAllocationPolicy, LivePrefixRestoreAndTruncatePublishEventBackedMutationHandoffs)
{
    ...
    EXPECT_NE(
        compact_device_publication.find(
...
TEST(Test__GpuWorkspaceAllocationPolicy, StochasticOutcomeHostBridgeWaitsOnResponseReadyEvent)
{
    ...
    EXPECT_NE(compact_handle.find(
                  "boolresponse_ready_after_rank_collective=false"),
              std::string::npos)
        << "The compact outcome handle must identify whether readiness belongs "
           "to the child verifier or the completed rank collective.";
    ...
    EXPECT_NE(compact_bridge.find(
                  "handle.response_ready_after_rank_collective?DeviceTimelinePoint::RankCompactSpeculativeResponseReady:DeviceTimelinePoint::CompactSpeculativeResponseReady"),
              std::string::npos)
        << "The bridge must select the exact ownership edge carried by the handle.";
    EXPECT_NE(compact_bridge.find(
                  "DeviceEventEdge::at(response_point).from(response_producer).to(DeviceTimelineRole::HostResultBridge).enqueueWait(*backend,state_.device_id,handle.response_ready_event.get(),handle.stream,copy_stream)"),
              std::string::npos)
        << "The explicit D2H bridge stream must consume the named response "
           "timeline point as a device-side event dependency.";
    EXPECT_NE(compact_bridge.find(
 ...
 * per-kernel timing events must therefore be queried and reclaimed without a
 * stream/device synchronization before the fixed pool can be reused. The rank
 * result event must also be republished after the NCCL/RCCL collective, making
 * that nonblocking reclamation ordering explicit rather than timing-dependent.
 */
TEST(Test__GpuWorkspaceAllocationPolicy, MirroredMTPTimingEventsReclaimAfterRankCollective)
{
    const auto dgo_source =
        readFile(repoRoot() / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto rank_source =
    ...
    const auto acquire_body = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            ...
            "bool DeviceGraphOrchestrator::recordShiftedMTPKVReady(")));
    const auto broadcast_body = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            rank_source,
    ...
    expectNeedleBefore(
        broadcast_body,
        "tp_ctx_->collectiveSidebandsMultiOnStreams(",
        "publishRankCompactSpeculativeResponseReady(",
        "Rank response readiness must be published after NCCL/RCCL submission.");
    EXPECT_NE(broadcast_body.find(
                  "runner->publishRankCompactSpeculativeResponseReady(&child)"),
              std::string::npos)
        << "Rank orchestration must ask the child that owns the event, stream, "
           "and backend to strengthen the response-ready edge.";
    EXPECT_EQ(broadcast_body.find("getBackendFor("), std::string::npos)
        << "Rank orchestration must not manipulate child-owned event resources.";
    EXPECT_EQ(broadcast_body.find("synchronizeStream("), std::string::npos);
    EXPECT_EQ(broadcast_body.find("synchronizeDevice("), std::string::npos);
}
    ...
    MTPSidecarPreservationProbeUsesProductionDeviceVerifierInput)
{
    ...
    const size_t gpu_branch =
        compact.find("if(runner_->primaryDeviceId().is_gpu())");
    const size_t device_first =
        compact.find(
            "prepareMTPVerifierInputTokensOnDeviceFromDeviceFirstToken(",
            gpu_branch);
    const size_t device_host_first =
        compact.find(
            "prepareMTPVerifierInputTokensOnDevice(",
            device_first);
    const size_t host_row =
        compact.find(
    ...
    ASSERT_NE(gpu_branch, std::string::npos);
    ASSERT_NE(device_first, std::string::npos);
    ASSERT_NE(device_host_first, std::string::npos);
    ASSERT_NE(host_row, std::string::npos);
    ASSERT_NE(bind_forward_options, std::string::npos);
    ASSERT_NE(verifier_forward, std::string::npos);
    EXPECT_LT(gpu_branch, device_first);
    EXPECT_LT(device_first, device_host_first);
    EXPECT_LT(device_host_first, host_row);
    EXPECT_LT(host_row, bind_forward_options);
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPTerminalHiddenDeviceAcceptedRowsUseExternalMetadataWorkspace)
{
    ...
    EXPECT_NE(compact_external_rows.find("cache.stage->setGPUStream(stream)"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPDeviceResidentPublicationRequiresAtomicKVAndLogicalState)
{
    ...
    EXPECT_NE(compact_publish.find("DeviceSequenceStatePublicationRequestkv_request;"),
              std::string::npos);
    EXPECT_NE(compact_publish.find("kv_request.target_cached_tokens_device=publication_ptrs.target_cached_tokens"),
              std::string::npos);
    EXPECT_NE(compact_publish.find("kv_request.accepted_state_counts_device=publication_ptrs.accepted_state_counts"),
              std::string::npos);
    EXPECT_NE(compact_publish.find("kv_request.publication_ok_flags_device=publication_ptrs.publication_ok_flags"),
              std::string::npos);
    ...
    EXPECT_NE(compact_publish.find("workspace_ptrs.shifted_target_cached_tokens"),
              std::string::npos);
    EXPECT_NE(compact_publish.find("workspace_ptrs.shifted_accepted_state_counts"),
              std::string::npos);
    ...
    EXPECT_NE(compact_publish.find("workspace_ptrs.accepted_state_slot_indices"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, DGODeviceLogicalStateMailboxOwnsArenaRowsAcrossWorkspaceRebind)
{
    ...
    EXPECT_NE(
        compact_diagnostic_rebind.find(
            "recordDeviceResidentLogicalSequenceStateMailbox(request_count,stream,&mailbox_error)"),
        std::string::npos)
    ...
    EXPECT_NE(
        compact_resident_target_publication.find(
            "waitForDeviceResidentLogicalSequenceStateMailbox("),
        std::string::npos)
        << "The D2D snapshot must wait on the resident producer event.";
    ...
    EXPECT_NE(compact_prepare.find("publication_ptrs.target_cached_tokens"),
              std::string::npos);
    EXPECT_NE(compact_prepare.find("publication_ptrs.next_condition_tokens"),
              std::string::npos);
     ...
     ScalarDeviceTargetSidecarRepublishesCanonicalLogicalState)
{
    ...
    const size_t prior_mailbox_wait = compact_publication.find(
        "waitForDeviceResidentLogicalSequenceStateMailbox("
        "producer_stream,");
    ...
    const size_t mailbox_event = compact_publication.find(
        "recordDeviceResidentLogicalSequenceStateMailbox(");
    ASSERT_NE(prior_mailbox_wait, std::string::npos);
    ASSERT_NE(target_wait, std::string::npos);
    ASSERT_NE(initialize, std::string::npos);
    ASSERT_NE(mailbox_event, std::string::npos);
    EXPECT_LT(prior_mailbox_wait, initialize)
        << "Single-buffered durable rows cannot be overwritten before their prior publication.";
     ...
     GPUCheckpointObservationUsesCanonicalKVCountWithoutMailbox)
{
    ...
    const auto compact_probe_fallback =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(sliceBetween(
            probe_body,
            "else if (state_.device_id.is_gpu() && state_.kv_cache)",
     ...
     RequestBatchedGPUMTPSidecarsRemainDeviceOwnedAcrossEveryDepth)
{
    ...
    EXPECT_NE(condition_advance.find(
              ...
              std::string::npos);
    EXPECT_NE(condition_advance.find(
                  "request_batch,ForwardExecutionRole::MTPRequestBatchCondition,false,true,logical_state.target_positions_device,logical_state.target_sequence_lengths_device"),
              std::string::npos)
     ...
     RequestBatchedStochasticThresholdPositionsRemainDeviceOwned)
{
    ...
    EXPECT_NE(main_forward.find(
                  "admitRequestInputsOnDevice(tokens,position_ids.data(),total_tokens,batch_size,seq_len)"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPBudgetLimitedDirectEmitUsesCheckpointShiftedSidecarAnchor)
{
    ...
    EXPECT_NE(compact_direct_emit.find(
                  "commitMTPShiftedRowFromDeviceTargetSample(0,0,true,base_sidecar_position)"),
              std::string::npos)
        << "GPU direct emit must publish shifted MTP state from its persistent target-sample slot.";
    EXPECT_NE(compact_direct_emit.find(
    ...
    EXPECT_NE(compact_direct_emit.find(
                  "forwardWithDeviceTokenIds(&first_token,direct_emit_token_device,1)"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPSpecStatePublicationSeparatesSidecarReplaySafety)
{
    ...
    EXPECT_NE(executable_mutation_body.find(
                  "if(!preserve_gpu_replay_state&&preserves_live_graph_replay)"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, Qwen35MoEDeviceRoutedDecodeTableGuardsOwnershipShape)
{
    ...
    EXPECT_NE(compact_graph.find("config_.moe.routed_compute_policy==RoutedExpertComputePolicy::Apportioned||use_expert_overlay"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, LiveCheckpointStorageUsesReusableDomainCorrectPool)
{
    ...
    EXPECT_NE(initialize_body.find("kConcurrentCheckpointSlots = 4"),
              std::string::npos)
        << "Initialization must cover every concurrently retained production checkpoint plus diagnostics.";
    EXPECT_NE(initialize_body.find(
                  "kConcurrentCheckpointSlots * checkpoint_handles_per_snapshot"),
              std::string::npos)
        << "Each retained checkpoint needs independent main and shifted-cache storage handles.";
