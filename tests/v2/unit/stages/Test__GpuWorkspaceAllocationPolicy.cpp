     ...
     MoERouteScratchIsSharedPlannedAndImmutableBeforeCapture)
{
    ...
    EXPECT_NE(
        factory.find("resolveMTPRetainedTargetQueryRows(config_.mtp)"),
        std::string::npos)
        << "grouped verifier and request-batched MTP rows must contribute the "
           "complete retained setup capacity even while execution is disabled";
    EXPECT_EQ(
        factory.find("resolveMTPMaxTargetQueryRows(config_.mtp)"),
        std::string::npos)
        << "immutable route scratch must not regress from setup capacity to the active execution-policy width";
    EXPECT_NE(
...
/**
 * @brief Keep model-lifetime MTP workspace accounting independent of execution.
 *
 * A process campaign can deliberately execute an MTP-off control while
 * retaining the same depth-capable graph envelope as later MTP cells. Memory
 * admission already uses `retainsMTPGraphCapacity`; graph-family workspace
 * construction must use that same authority or the first runner seals a block
 * that is smaller than the capacity charged by preflight.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     RetainedMTPWorkspaceUsesTheCanonicalCapacityAuthority)
{
    const auto source = readFile(
        repoRoot() /
        "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto allocation = removeAsciiWhitespace(
    ...
    EXPECT_NE(
        allocation.find(
            "constboolretains_mtp_graph_capacity=retainsMTPGraphCapacity(config.mtp);"),
        std::string::npos)
        << "Workspace materialization and memory admission need one typed MTP-capacity authority.";
    EXPECT_NE(
        allocation.find(
            "retains_mtp_graph_capacity){hints.serial_family_max_compact_rows="),
        std::string::npos)
        << "The retained verifier-row envelope must contribute while MTP execution is off.";
    EXPECT_NE(
        allocation.find(
            "state_.device_id.is_gpu()&&retains_mtp_graph_capacity){"),
        std::string::npos)
        << "Fixed-shape verifier metadata belongs to retained setup capacity, not request execution.";
    EXPECT_EQ(allocation.find("config.mtp.enabled"), std::string::npos)
        << "Workspace capacity must not be inferred from the active execution switch.";

    const auto sidecar_manifest = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::buildMTPWorkspaceFamilyManifest(",
            "bool DeviceGraphOrchestrator::materializeForwardGraphForShape(")));
    EXPECT_NE(
        sidecar_manifest.find(
            "!retainsMTPGraphCapacity(graph_builder_->config().mtp)"),
        std::string::npos)
        << "Capacity-only runners must contribute the exact MTP sidecar participants.";
    EXPECT_EQ(sidecar_manifest.find("mtp.enabled"), std::string::npos)
        << "The sidecar workspace manifest is setup capacity, never an execution decision.";

    const auto family_manifest = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::materializeForwardGraphForShape(",
            "ExpertTransferResult DeviceGraphOrchestrator::transferExpertWeights(")));
    const std::string retained_manifest_guard =
        "retainsMTPGraphCapacity(graph_builder_->config().mtp)";
    const size_t first_guard = family_manifest.find(retained_manifest_guard);
    ASSERT_NE(first_guard, std::string::npos)
        << "The grouped verifier must be present in a retained MTP family.";
    EXPECT_NE(
        family_manifest.find(retained_manifest_guard,
                             first_guard + retained_manifest_guard.size()),
        std::string::npos)
        << "The live MTP condition must be present in the same retained family.";
    ...
    EXPECT_NE(
        shifted_prefill_binding.find(
            "constbooldeclaration_only=purpose==ShiftedMTPPrefillGraphBinding::Purpose::WorkspaceFamilyDeclaration;"),
        std::string::npos)
        << "Shifted-KV topology needs an explicit declaration-only lifecycle.";
    EXPECT_NE(
        shifted_prefill_binding.find(
            "declaration_only&&retainsMTPGraphCapacity(graph_builder_->config().mtp)"),
        std::string::npos)
        << "An MTPOff runner retaining capacity must declare the same shifted-KV workspace participant as an enabled runner.";
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, GraphCaptureControllerLimitsStreamSyncToDiagnostics)
{
    ...
    EXPECT_EQ(compact_replay_phase.find("createEvent("), std::string::npos)
        << "Every replay timing event must be preallocated during capture setup.";
...
/**
 * @brief Publish every retained runtime layer before local maintenance begins.
 *
 * A homogeneous single-domain Dynamic topology deliberately has no external
 * topology-wide controller service. Its retained maintenance stream is the
 * sole controller and must therefore perform the same typed initial-runtime
 * handoff after the complete main/MTP graph family has been declared. Without
 * this edge, a dormant physical NextN layer can remain the epoch-zero template
 * and make the first maintenance scan reject an otherwise valid plan.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     ParticipantLocalMaintenancePublishesCompleteRuntimeFamily)
{
    const auto source = readFile(
        repoRoot() /
        "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto family = sliceBetween(
        source,
        "bool DeviceGraphOrchestrator::materializeForwardGraphForShape(",
        "ExpertTransferResult DeviceGraphOrchestrator::transferExpertWeights(");
    const auto compact = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(family));

    const size_t mtp_family = compact.find(
        "buildMTPWorkspaceFamilyManifest(");
    const size_t maintenance_family = compact.find(
        "materializeDeviceMoERebalanceMaintenanceGraphForFamily()");
    const size_t maintenance_stream = compact.find(
        "void*constmaintenance_stream=");
    const size_t initial_publication = compact.find(
        "publishMoEOverlayDeviceInitialRuntime(maintenance_stream)");
    const size_t transaction_initialization = compact.find(
        "resetRequestOwnedDeviceTransaction(",
        initial_publication);

    ASSERT_NE(mtp_family, std::string::npos);
    ASSERT_NE(maintenance_family, std::string::npos);
    ASSERT_NE(maintenance_stream, std::string::npos);
    ASSERT_NE(initial_publication, std::string::npos);
    ASSERT_NE(transaction_initialization, std::string::npos);
    EXPECT_LT(mtp_family, initial_publication)
        << "The immutable initial template must include every retained MTP/NextN layer.";
    EXPECT_LT(maintenance_family, initial_publication)
        << "The controller graph declaration may contribute runtime bindings.";
    EXPECT_LT(maintenance_stream, initial_publication)
        << "Initial publication must use the exact retained controller stream.";
    EXPECT_LT(initial_publication, transaction_initialization)
        << "No request-owned maintenance state may observe the partial runtime family.";

    const auto publication_region = compact.substr(
        maintenance_stream,
        transaction_initialization - maintenance_stream);
    EXPECT_EQ(
        publication_region.find("synchronizeStream"),
        std::string::npos)
        << "The setup handoff must remain an event-ordered device operation.";
    EXPECT_EQ(
        publication_region.find("synchronizeDevice"),
        std::string::npos)
        << "Publishing the runtime family must never drain the GPU.";
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPFirstUseCaptureAndParentPublicationAreAtomic)
{
    ...
    EXPECT_EQ(first_use.find("runStages("), std::string::npos)
        << "Transaction zero must never execute through an eager stage loop.";
    EXPECT_EQ(first_use.find("MaterializeOnly"), std::string::npos)
        << "Capture must launch transaction zero; metadata-only capture is a retired fallback.";
    expectNeedleBefore(
        first_use,
        "DeviceGraphCaptureController::buildCapturePlan(",
        "DeviceGraphCaptureController::executeCapturePhase(",
        "Launch topology must be planned before transaction-zero capture begins.");
    expectNeedleBefore(
        first_use,
        "DeviceGraphCaptureController::executeCapturePhase(",
        "segment_cache.initialized = true;",
        "The cache becomes initialized only after transaction zero launches successfully.");
    expectNeedleBefore(
        first_use,
        "segment_cache.initialized = true;",
        "segment_cache.needs_capture = false;",
        "Executable ownership and replay readiness must publish atomically.");
    EXPECT_EQ(executor_source.find("CapturePublicationPolicy"), std::string::npos);
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPShiftedKVAsyncHandoffUsesEventBeforeConsumers)
{
    ...
    const size_t publication_launch =
        publish_body.find("executeMTPStatePublicationGraph");
    ...
    const auto sidecar_executable = stripCommentsAndStringLiterals(sidecar_body);

    // Metadata checks borrow on a setup stream; only the actual sidecar or
    // mailbox writer may retire the publication on its execution stream. The
    // matching CUDA/HIP held-reader proof tests the resulting event DAG.
    const auto batch_condition_executable = stripCommentsAndStringLiterals(sliceBetween(
        source,
        "bool DeviceGraphOrchestrator::advanceMTPRequestBatchConditionOnDevice(",
        "bool DeviceGraphOrchestrator::forwardMTPBatchFromDeviceResidentLogicalStateAndSampleGreedyToDeviceDraftSlots("));
    for (const auto *body : {&batch_condition_executable,
                             &sequential_executable,
                             &initial_device_outcome_executable,
                             &device_target_executable,
                             &resident_logical_executable,
                             &device_outcome_executable,
                             &partial_executable})
    {
        EXPECT_NE(body->find("waitForPendingShiftedMTPKVReadyForObservation("),
                  std::string::npos)
            << "Shifted-cache metadata must preserve the writer's publication.";
        EXPECT_EQ(body->find("waitForPendingShiftedMTPKVReady("), std::string::npos)
            << "Metadata cannot consume an edge on an unrelated stream.";
    }
    ...
    expectNeedleBefore(
        ...
        "Every terminal-hidden sidecar must acquire the exact publication lease immediately before graph submission.");
    expectNeedleBefore(
        sidecar_executable,
        "sidecar_cache.graph->reset()",
        "stillOwns",
        "The common sidecar boundary must verify terminal-hidden ownership after submitting the graph read.");
    EXPECT_NE(sidecar_body.find("sidecar_terminal_hidden_read_leases"),
              std::string::npos)
        << "Common sidecar terminal-hidden consumption must remain visible in perf counters.";
    EXPECT_EQ(resident_logical_executable.find("acquireReadLease"),
              std::string::npos)
        << "Caller-specific terminal-hidden leases must not duplicate the common sidecar ownership boundary.";
    EXPECT_EQ(
        resident_logical_executable.find(
            "selectMTPTerminalHiddenRowsFromDeviceAcceptedState"),
        std::string::npos)
        << "A read-only sidecar must not reconstruct durable terminal hidden from transient verifier geometry.";
    EXPECT_EQ(resident_logical_executable.find("last_forward_seq_len"),
              std::string::npos)
        << "Resident correction must not infer terminal-hidden provenance from the latest host-visible forward shape.";
    EXPECT_EQ(resident_logical_executable.find("last_forward_batch_size"),
              std::string::npos)
        << "Resident correction must not infer terminal-hidden provenance from the latest host-visible forward shape.";
    expectNeedleBefore(
        resident_logical_executable,
...
TEST(Test__GpuWorkspaceAllocationPolicy, LivePrefixRestoreAndTruncatePublishEventBackedMutationHandoffs)
{
    ...
    const auto published_join_body = sliceBetween(
        ...
        "bool DeviceGraphOrchestrator::joinPublishedLiveStateHandoffs(",
        "void DeviceGraphOrchestrator::clearPendingLivePrefixMutationReady()");
    const auto clear_prefix_mutation_body = sliceBetween(
        source,
        "void DeviceGraphOrchestrator::clearPendingLivePrefixMutationReady()",
        "bool DeviceGraphOrchestrator::orderCheckpointMetadataPoolReuse(");
    ...
    const auto compact_clear_prefix_mutation =
        removeAsciiWhitespace(
            stripCommentsAndStringLiterals(clear_prefix_mutation_body));
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        compact_clear_prefix_mutation.find(
            "ready.valid||!ready.retained_payload_sources.empty()||!ready.retained_device_sources.empty()"),
        std::string::npos)
        << "A valid prefix-mutation event must survive consumption even when "
           "the mutation owns no auxiliary payload aliases; otherwise the next "
           "prefix archive can observe live GPU state before its producer.";
    EXPECT_EQ(
...
TEST(Test__GpuWorkspaceAllocationPolicy, ResidentGroupedOutcomeControlIsDeviceAuthoritative)
{
    ...
    const auto greedy_outcome = sliceBetween(
        orchestrator,
        "bool DeviceGraphOrchestrator::verifyGreedyAllPositionBatchOutcomeOnDeviceResident(",
    ...
    EXPECT_EQ(
        reducer.find("consumeDeviceGenerationStateReady("),
        std::string::npos)
        << "Verifier preparation, not a sampling-specific reducer, must own "
           "the controller handoff.";
    EXPECT_EQ(
        reducer.find("enqueuePrepareDeviceGenerationTransactionBudget("),
        std::string::npos)
        << "The transaction budget must be prepared before the common reducer.";

    EXPECT_NE(
        greedy_outcome.find(
            "out_handle->device_generation_controller_owned = true"),
        std::string::npos)
        << "Greedy and stochastic grouped outcomes must return the same "
           "controller-owned publication contract.";
    EXPECT_EQ(
        greedy_outcome.find("deviceToHost"),
        std::string::npos);
    EXPECT_EQ(
        greedy_outcome.find("synchronize"),
        std::string::npos);
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, TerminalDeviceGenerationBridgeIsSingleAndPreallocated)
{
    ...
    EXPECT_NE(
        rank_bridge.find("finishDeviceResidentGeneration("),
        std::string::npos);
    EXPECT_NE(
...
/**
 * @brief Keep the generation ledger and graph frontier on one producer edge.
 *
 * DeviceGenerationStateReady is both an exclusive lifecycle token and an
 * ordering edge. The captured publication graph independently validates its
 * arena inputs, so publishing only the private token would leave the graph
 * frontier observing stale host authority. Setup also initializes the full
 * persistent storage on device to allocate completion events before admission.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     DeviceGenerationPublicationIncludesArenaAuthority)
{
    const auto orchestrator = readFile(
        repoRoot() /
        "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto setup = sliceBetween(
        orchestrator,
    ...
    const auto arena_publication = sliceBetween(
        orchestrator,
        "bool DeviceGraphOrchestrator::publishDeviceGenerationArenaState(",
        "bool DeviceGraphOrchestrator::materializePendingMTPVerifierInputTokensOnDevice(");
    const auto lifecycle_publication = sliceBetween(
        orchestrator,
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_EQ(setup.find("hostToDevice"), std::string::npos);
    EXPECT_EQ(setup.find("synchronize"), std::string::npos);
    ...
    const size_t lifecycle_edge = lifecycle_publication.find(
        "DeviceTimelinePoint::DeviceGenerationStateReady");
    ASSERT_NE(arena_edge, std::string::npos);
    ASSERT_NE(lifecycle_edge, std::string::npos);
    EXPECT_LT(arena_edge, lifecycle_edge)
        << "The private readiness token cannot outrun arena authority.";
}

/**
 * @brief Keep controller reset legal after a verifier borrows then fails.
 *
 * Published and borrowed are distinct lifecycle phases. Reset must join either
 * exact frontier with events; treating both as `valid == false` recreates the
 * teardown abort that hid the original heterogeneous verifier-pairing defect.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     DeviceGenerationResetRetiresTypedPublishedOrBorrowedFrontier)
{
    const auto root = repoRoot();
    const auto header = readFile(
        root /
    ...
    const auto state = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            header,
            "struct PendingDeviceGenerationStateReadyState",
            "struct PendingRequestInputReuseReadyState")));
    const auto consume = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::consumeDeviceGenerationStateReady(",
            "retireDeviceGenerationStateForRequestReset(")));
    const auto retirement = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "retireDeviceGenerationStateForRequestReset(",
            "DeviceGraphOrchestrator::deviceGenerationExecutionPolicy(")));
    const auto terminal = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::finishDeviceResidentGeneration(",
            "bool DeviceGraphOrchestrator::joinPriorDeviceWorkForRequestStateReset(")));

    EXPECT_NE(state.find("DeviceGenerationStateHandoffhandoff"),
              std::string::npos);
    EXPECT_EQ(state.find("boolvalid"), std::string::npos)
        << "A boolean cannot distinguish published from borrowed ownership.";
    EXPECT_NE(consume.find("ready.handoff.borrow("), std::string::npos);
    EXPECT_NE(retirement.find("ready.handoff.retireForReset("),
              std::string::npos);
    EXPECT_NE(
        retirement.find(
            "phase==DeviceGenerationStateHandoffPhase::Borrowed"),
        std::string::npos)
        << "A failed borrower must publish its exact stream frontier before reset.";
    EXPECT_NE(terminal.find("handoff.retireAfterHostCompletion("),
              std::string::npos)
        << "The terminal D2H observer must close its borrow after the event wait.";
    EXPECT_EQ(retirement.find("synchronizeStream("), std::string::npos);
    EXPECT_EQ(retirement.find("synchronizeDevice("), std::string::npos);
}
...
/**
 * @brief Forbids backend event allocation from resident-generation hot paths.
 *
 ...
 */
TEST(Test__GpuWorkspaceAllocationPolicy, ResidentGenerationHotPathsUseOnlyPreallocatedEvents)
{
    const auto source =
        readFile(repoRoot() / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto initialization_body = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::initializePersistentDeviceGenerationEvents()",
    ...
    const auto ordinary_boundary = initialization_body.find("if(!graph_builder_->config().mtp.enabled)");
    ASSERT_NE(ordinary_boundary, std::string::npos);
    EXPECT_LT(initialization_body.find("device_resident_logical_sequence_state_ready_event_"),
              ordinary_boundary);
    EXPECT_LT(initialization_body.find("device_resident_logical_state_reader_completion_events_[lane]"),
              ordinary_boundary)
        << "Logical-state readers and writers exist without speculative execution.";
        ...
        hot_paths = {{
            {
                ...
                "bool DeviceGraphOrchestrator::materializeDeviceResidentGeneration(",
                "ordinary and speculative admission",
            },
            {
                "bool DeviceGraphOrchestrator::recordDeviceResidentMTPTransactionMutation(",
                "bool DeviceGraphOrchestrator::recordRestoredDeviceResidentMTPTransaction(",
...
/**
 * @brief Require sample readiness to publish both range and arena authority.
 *
 * Target and draft slots carry independent preallocated readiness events, but
 * verifier graph frontier validation observes their arena-owned backing
 * tensors. Every producer must therefore publish both views on the same exact
 * stream. Setup initializes the banks once so those tensor-level completion
 * events can only be re-recorded, never allocated, in decode.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     ResidentMTPSampleReadinessPublishesArenaAuthority)
{
    const auto source = readFile(
        repoRoot() /
        "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");

    const auto initialize_state = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::initializeInferenceStateFromArena(",
            "bool DeviceGraphOrchestrator::initializeKVCaches(")));
    const auto initialize_banks = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::initializeAndPublishMTPSampleBanksOnStream(",
            "bool DeviceGraphOrchestrator::materializePendingMTPVerifierInputTokensOnDevice(")));
    const auto draft_ready = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::recordStochasticDraftSampleReady(",
            "bool DeviceGraphOrchestrator::recordStochasticTargetSampleReady(")));
    const auto target_ready = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::recordStochasticTargetSampleReady(",
            "bool DeviceGraphOrchestrator::beginPendingGpuTimingMeasurement(")));
    ...
    EXPECT_NE(
        initialize_state.find(
            "initializeAndPublishMTPSampleBanksOnStream(initialization_stream,"),
        std::string::npos)
        << "Persistent sample-bank completion events must be established before decode.";
    EXPECT_NE(
        initialize_banks.find(
            "publishPreparedArenaGraphInput(BufferId::STOCHASTIC_TARGET_SAMPLE_TOKENS,stochastic_target_sample_tokens_dev_,producer_stream,state_.device_id"),
        std::string::npos);
    EXPECT_NE(
        initialize_banks.find(
            "publishPreparedArenaGraphInput(BufferId::STOCHASTIC_DRAFT_SAMPLE_TOKENS,stochastic_draft_sample_tokens_dev_,producer_stream,state_.device_id"),
        std::string::npos);
    EXPECT_NE(
        draft_ready.find(
            "publishPreparedArenaGraphInput(BufferId::STOCHASTIC_DRAFT_SAMPLE_TOKENS,stochastic_draft_sample_tokens_dev_,producer_stream,state_.device_id"),
        std::string::npos)
        << "Draft readiness must publish the arena tensor on the exact sampler stream.";
    EXPECT_NE(
        target_ready.find(
            "publishPreparedArenaGraphInput(BufferId::STOCHASTIC_TARGET_SAMPLE_TOKENS,stochastic_target_sample_tokens_dev_,producer_stream,state_.device_id"),
        std::string::npos)
        << "Target readiness must publish the arena tensor on the exact sampler stream.";
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPDeviceResidentPublicationMetadataStaysOnVerifierStream)
{
    const auto source =
        readFile(repoRoot() / "src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.cpp");
    const auto publication_source = readFile(repoRoot() /
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPDeviceResidentPublicationRequiresAtomicKVAndLogicalState)
{
    ...
    EXPECT_EQ(compact_publish.find("request.requestCount()!=1&&mtpSpecStatePublicationRequiresCapturedStage()"),
              std::string::npos)
    ...
    const size_t graph_preflight = compact_prepare.find(
        "materializeMTPSpeculativeStatePublicationGraph(");
    const size_t transaction_wait = compact_prepare.find(
        "waitForDeviceResidentMTPTransaction(");
    const size_t logical_row_writer = compact_prepare.find(
        "waitForDeviceResidentLogicalSequenceStateRowReuse(");
    ASSERT_NE(graph_preflight, std::string::npos);
    ASSERT_NE(transaction_wait, std::string::npos);
    ASSERT_NE(logical_row_writer, std::string::npos);
    EXPECT_LT(transaction_wait, graph_preflight)
        << "The producer lease must be validated and its latest mutation event "
           "joined before a publication stage freezes shifted-cache bindings.";
    EXPECT_LT(graph_preflight, logical_row_writer)
        << "The exclusive reusable-row writer must be the final acquisition so "
           "a failed preflight cannot leave an unfinished replacement transaction.";
    ...
    EXPECT_NE(compact_publish.find("executeMTPStatePublicationGraph("),
              std::string::npos)
    ...
    EXPECT_NE(compact_publish.find("finalizeMTPSpeculativeStatePublicationLaunch("),
              std::string::npos);
    EXPECT_LT(compact_publish.find("executeMTPStatePublicationGraph("),
...
/**
 * @brief Keep logical verifier width and captured storage stride producer-owned.
 *
 * Dynamic scalar MTP reuses a bounded physical graph bucket for several logical
 * depths. The compact outcome producer is the only component that can prove
 * which graph actually ran, so later publication callers must not be able to
 * redeclare either cardinality. This source contract catches the exact class of
 * stale geometry handoff that previously opened a mailbox writer transaction
 * before publication discovered a row mismatch.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     MTPCompactOutcomeIsTheSolePublicationGeometryAuthority)
{
    const auto root = repoRoot();
    const auto metadata_header = readFile(
        root / "src/v2/execution/mtp/MTPSpecDecodeMetadata.h");
    const auto runner_interface = readFile(
        root /
        "src/v2/execution/local_execution/orchestrators/IInferenceRunner.h");
    const auto orchestrator_source = readFile(
    ...
    const auto outcome = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            runner_interface,
            "struct DeviceSpeculativeOutcomeHandle",
            "struct DeviceGenerationTerminalRequestResult")));
    const auto publication_request = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            runner_interface,
            "struct DeviceSpeculativePublicationRequest",
            "struct DeviceResidentLogicalSequenceStateHandle")));
    const auto seal = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            orchestrator_source,
            "bool DeviceGraphOrchestrator::sealDeviceSpeculativeOutcomeVerifierGeometry(",
            "bool DeviceGraphOrchestrator::finalizeMTPSpeculativeStatePublicationLaunch(")));
    const auto prepare = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            orchestrator_source,
            "bool DeviceGraphOrchestrator::prepareDeviceResidentMTPSpecPublicationMetadata(",
            "std::vector<ForwardExecutionEngine::ReplayCacheObservation>")));
    const auto set_plan = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            orchestrator_source,
            "bool DeviceGraphOrchestrator::setMTPSpecVerifierInputPlan(",
            "void DeviceGraphOrchestrator::clearMTPSpecVerifierInputPlan(")));

    EXPECT_NE(outcome.find("intlogical_verifier_rows_per_request=0;"),
              std::string::npos);
    EXPECT_NE(outcome.find("intphysical_verifier_rows_per_request=0;"),
              std::string::npos);
    EXPECT_EQ(publication_request.find("intrequest_count="), std::string::npos);
    EXPECT_EQ(publication_request.find("intmax_draft_tokens="),
              std::string::npos);
    EXPECT_NE(publication_request.find("returnoutcome.request_count;"),
              std::string::npos);
    EXPECT_NE(
        publication_request.find(
            "returnoutcome.logical_verifier_rows_per_request;"),
        std::string::npos);
    EXPECT_NE(
        publication_request.find(
            "returnoutcome.physical_verifier_rows_per_request;"),
        std::string::npos);

    EXPECT_NE(seal.find("lastAllPositionVerifierForwardGraph()"),
              std::string::npos);
    EXPECT_NE(
        seal.find("mtpVerifierPhysicalWidthPolicyForRequest("),
        std::string::npos)
        << "Outcome sealing must resolve graph width through the same typed "
           "controller-ownership policy as verifier capture.";
    EXPECT_NE(seal.find("mtpVerifierPhysicalPaddedSeqLen("),
              std::string::npos);
    EXPECT_NE(
        seal.find(
            "handle->logical_verifier_rows_per_request=logical_rows_per_request;"),
        std::string::npos);
    EXPECT_NE(
        seal.find(
            "handle->physical_verifier_rows_per_request=graph_physical_rows;"),
        std::string::npos);
    EXPECT_GE(
        countOccurrences(
            orchestrator_source,
            "sealDeviceSpeculativeOutcomeVerifierGeometry("),
        4U)
        << "Every GPU compact outcome family must seal retained graph geometry.";

    EXPECT_NE(
        prepare.find("mtpVerifierPhysicalWidthPolicyForRequest("),
        std::string::npos)
        << "Publication must not reconstruct dynamic graph geometry from a "
           "logical row count.";
    EXPECT_NE(prepare.find("mtpVerifierPhysicalPaddedSeqLen("),
              std::string::npos);
    EXPECT_NE(
        prepare.find(
            "verifier_graph->signature.seq_len!=request.physicalVerifierRowsPerRequest()"),
        std::string::npos)
        << "Publication must address the retained graph by its physical stride.";
    EXPECT_EQ(metadata_header.find("setShape("), std::string::npos)
        << "Request code must not have an API capable of replacing workspace capacity.";
    EXPECT_NE(metadata_header.find("ensureCapacity("), std::string::npos)
        << "Graph-family setup must reserve metadata through a monotonic API.";
    EXPECT_EQ(
        set_plan.find("mtp_spec_decode_metadata_binding_.ensureCapacity("),
        std::string::npos)
        << "Installing active request geometry must never resize metadata storage.";
    EXPECT_NE(
        set_plan.find(
            "metadata_capacity.max_draft_tokens<physical_row_count"),
        std::string::npos)
        << "A bounded verifier request must prove its physical bucket fits the setup-owned capacity.";
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, DGODeviceLogicalStateMailboxOwnsArenaRowsAcrossWorkspaceRebind)
{
    ...
    const auto condition_mailbox_retirement_body = sliceBetween(
        source,
        "bool DeviceGraphOrchestrator::retireConsumedMTPConditionInputMailbox(",
        "void DeviceGraphOrchestrator::retireDeviceResidentMTPTransaction()");
    const auto compact_clear_mailbox =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(clear_mailbox_body));
    const auto compact_condition_mailbox_retirement =
        removeAsciiWhitespace(
            stripCommentsAndStringLiterals(condition_mailbox_retirement_body));
    const auto retire_transaction_body = sliceBetween(
        source,
        "void DeviceGraphOrchestrator::retireDeviceResidentMTPTransaction()",
        "DeviceResidentMTPTransactionLease");
    ...
    EXPECT_NE(
        compact_condition_mailbox_retirement.find(
            "mailbox.publication_generation!=publication_generation"),
        std::string::npos)
        << "Condition retirement must name the exact admitted mailbox generation.";
    EXPECT_NE(
        compact_condition_mailbox_retirement.find(
            "completion.stream==completed_stream&&completion.validFor("
            "publication_generation)"),
        std::string::npos)
        << "A condition input cannot retire before its exact graph reader has "
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "A consumed condition input must stop masquerading as post-graph "
           "logical cursor; canonical device KV owns that count.";
    EXPECT_EQ(
        compact_condition_mailbox_retirement.find("synchronize"),
        std::string::npos)
        << "Typed condition retirement is metadata-only and must not block the host.";
    EXPECT_NE(
        condition_mailbox_retirement_body.find(
            "device_resident_mtp_condition_input_mailbox_retirements"),
        std::string::npos)
        << "The condition-consumption transition must remain observable.";
    EXPECT_NE(
        compact_live_prepare.find(
            "input.execution_role==ForwardExecutionRole::MTPCondition&&!retireConsumedMTPConditionInputMailbox("),
        std::string::npos)
        << "Every MTP-condition graph must retire its consumed input from the "
           "shared forward-completion lifecycle.";
    EXPECT_NE(compact_retire_transaction.find(
                  "device_resident_logical_sequence_state_storage_.retirePublication()"),
              std::string::npos)
        << "The ordinary request-scoped boundary must continue to retire durable logical values.";
    ...
    EXPECT_NE(compact_prepare.find("bindPublicationOutputs(&publication_ptrs,request.requestCount())"),
              std::string::npos)
    ...
    EXPECT_NE(compact_publication_lifecycle.find("recordDeviceResidentLogicalSequenceStateMailbox("),
              std::string::npos);
    EXPECT_LT(compact_direct_publish.find("executeMTPStatePublicationGraph("),
     ...
     LogicalStateMailboxReadersFanOutAndReplacementWritersFanIn)
{
    ...
    const auto compact_prefill_execution =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(sliceBetween(
            engine_source,
            "bool ForwardExecutionEngine::executePrefillWithGraphCache(",
            "bool ForwardExecutionEngine::executeCacheMiss(")));
    ...
    EXPECT_NE(
        compact_cache_hit.find(
            "void*prefill_producer_stream=nullptr;"),
        std::string::npos)
        << "Prefill must carry concrete producer provenance out of its launch state machine.";
    EXPECT_NE(
        compact_cache_hit.find("&prefill_producer_stream"),
        std::string::npos);
    EXPECT_NE(
        compact_cache_hit.find(":prefill_producer_stream)"),
        std::string::npos)
        << "Live-state completion must consume the actual prefill launch stream, not reconstruct one from a cache kind.";
    EXPECT_NE(
        compact_prefill_execution.find("*out_producer_stream=stream;"),
        std::string::npos)
        << "Captured monolithic and heterogeneous prefill must return their exact launch stream.";
    EXPECT_NE(
        compact_prefill_execution.find(
            "*out_producer_stream=forward_cache.prefill_capture_stream.stream;"),
        std::string::npos)
        << "Direct warmup must return the explicit stream installed before eager launch.";
    ...
    const size_t replacement_write = compact_batch_advance.find(
        "backend->enqueueInitializeGenerationLogicalState(",
        writer_wait);
     ...
     ScalarDeviceTargetSidecarRepublishesCanonicalLogicalState)
{
    ...
    const size_t initialize = compact_publication.find(
        ...
        ".positions=live_position_device,"
        ".sampled_tokens=condition_token_device,");
     ...
     RequestBatchedGPUMTPSidecarsRemainDeviceOwnedAcrossEveryDepth)
{
    ...
    EXPECT_NE(condition_advance.find(
              ...
              std::string::npos);
    EXPECT_NE(condition_advance.find(
                  "request_batch,ForwardExecutionRole::MTPCondition,"
    ...
    EXPECT_NE(compact_backend.find("enqueuePrepareMTPBatchedSidecarInputs("),
              std::string::npos);
    EXPECT_NE(compact_backend.find("enqueueInitializeGenerationLogicalState("),
              std::string::npos);
}
     ...
     RequestBatchedStochasticThresholdPositionsRemainDeviceOwned)
{
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "Decode must bind the exact cache and arena rows into its captured position root.";
    EXPECT_NE(main_forward.find(
    ...
    EXPECT_EQ(
        ...
        std::string::npos)
        << "The host prelude must not materialize position outside the captured graph.";
    EXPECT_NE(
        forward_live_state_prelude.find(
            "DeviceTimelinePoint::RequestInputReuseReady"),
        std::string::npos)
    ...
    EXPECT_NE(initial_publication.find(
                  ".source=GenerationInitialFrontier::SampledCondition,"
                  ".positions=static_cast<constint32_t*>(request_sequence_lengths_dev_),"
                  ".sampled_tokens=static_cast<constint32_t*>(stochastic_target_sample_tokens_dev_),"
    ...
    EXPECT_NE(compact_backend.find("threshold_position_device"),
              std::string::npos);
    EXPECT_NE(compact_backend.find("constGenerationLogicalStateInitialization&initialization"),
              std::string::npos);
    ...
    for (const auto &source : {cuda_sampling, rocm_sampling})
    {
        ...
        EXPECT_NE(compact.find("mtp_spec_threshold_from_seed("),
                  std::string::npos);
        EXPECT_NE(compact.find("initialization.initializeRequest(static_cast<int>(request))"),
                  std::string::npos)
            << "Both backends must use the shared device-owned frontier initializer.";
        EXPECT_EQ(compact.find("intposition_0"), std::string::npos)
            << "Host-derived position launch scalars are obsolete.";
    }
    const auto logical_initialization = removeAsciiWhitespace(stripCommentsAndStringLiterals(
        readFile(root / "src/v2/kernels/common/GenerationLogicalState.h")));
    EXPECT_NE(logical_initialization.find("constint32_tposition=positions[request];"),
              std::string::npos)
...
TEST(Test__GpuWorkspaceAllocationPolicy, MTPConditionForwardPublishesShiftedSidecarBeforeMainForward)
{
    ...
    const size_t resident_state_pos = compact_condition_forward.find(
        "condition_state=runner_->deviceResidentLogicalSequenceState()");
    const size_t target_publication_pos = compact_condition_forward.find(
        "publishDeviceResidentConditionTokenToTargetSampleSlot("
        "condition_state,0,kConditionTargetSampleSlot)");
    const size_t resident_commit_pos = compact_condition_forward.find(
    ...
    const size_t cpu_forward_pos = compact_condition_forward.find(
        "runner_->forward(&condition_token,1)");

    ASSERT_NE(resident_state_pos, std::string::npos);
    ASSERT_NE(target_publication_pos, std::string::npos)
        << "GPU condition-forward must preserve the mailbox token D2D before "
           "the shifted-cache mutation consumes the mailbox.";
    ASSERT_NE(resident_commit_pos, std::string::npos)
        << "GPU shifted-cache publication must consume the resident token and "
           "position as one transaction.";
    ASSERT_NE(target_advance_pos, std::string::npos)
        << "GPU main condition replay must consume the durable target slot.";
    EXPECT_LT(resident_state_pos, target_publication_pos);
    EXPECT_LT(target_publication_pos, resident_commit_pos);
    EXPECT_LT(resident_commit_pos, target_advance_pos);

    ASSERT_NE(cpu_commit_pos, std::string::npos)
        << "CPU condition-forward retains its host-owned serial contract.";
    ASSERT_NE(cpu_forward_pos, std::string::npos);
    EXPECT_LT(cpu_commit_pos, cpu_forward_pos)
        << "The CPU shifted sidecar row must be committed before its main forward.";
    EXPECT_EQ(
        compact_condition_forward.find(
            "advanceMTPMainConditionFromDeviceResidentLogicalState("),
        std::string::npos)
        << "The original mailbox is consumed by shifted-KV publication and "
           "must never be replayed afterward.";
...
TEST(Test__GpuWorkspaceAllocationPolicy, MoERuntimeRefreshPublishesInitializedFlag)
{
    ...
    EXPECT_NE(compact_refresh.find("moe_runtime_table_initialized_=false;"),
        ...
        << "Failed graph-stable refreshes must clear the stage-local readiness "
           "flag before the next capture/replay observes stale placement state.";
    ...
    expectNeedleBefore(
        ...
        "runtime_grouped_decode_launch_state_prepared_=preserve_capture_ready",
...
TEST(Test__GpuWorkspaceAllocationPolicy, TypedMTPLogitsReuseOnlyPreplannedDeviceOutputStorage)
{
    ...
    const auto binding_body = sliceBetween(
        orchestrator_source,
        "bool DeviceGraphOrchestrator::bindAllPositionLogitsOutputs(",
    ...
    const auto compact_helper =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(helper_body));
    const auto compact_binding =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(binding_body));
    const auto compact_initialization =
        removeAsciiWhitespace(
    ...
    EXPECT_NE(compact_binding.find("preplannedMTPLogitsBuffer("),
              std::string::npos);
    EXPECT_NE(compact_binding.find("BufferId::MTP_LOGITS_GATHERED"),
              std::string::npos)
        << "Full-vocabulary grouped outputs should reuse a preplanned gathered "
           "sidecar allocation where that policy owns one.";
    EXPECT_NE(compact_forward.find("bindAllPositionLogitsOutputs("),
              std::string::npos)
        << "Live execution and setup-only graph materialization must share one "
           "typed logits binding authority.";
    EXPECT_EQ(compact_forward.find("tensor_factory_->createFP32("),
              std::string::npos)
        << "forwardImpl must not bypass the typed logits binding authority.";
}
...
/**
 * @brief Keep variable-row MoE graph execution owned by one device scalar.
 *
 * Request admission and grouped-verifier preparation already publish stable
 * device geometry. Routing, resident expert assignment, and shared gating must
 * consume that exact address;
 * recreating the value in pinned host memory introduces a second authority and
 * makes the captured launch depend on whichever request happened to run first.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     MoEVariableRowStagesConsumeDeviceGeometryWithoutHostMirrors)
{
    const auto root = repoRoot();
    const auto routing_header = readFile(
        root / "src/v2/execution/compute_stages/stages/MoERoutingStage.h");
    const auto routing_source = readFile(
        root / "src/v2/execution/compute_stages/stages/MoERoutingStage.cpp");
    const auto expert_header = readFile(
        root / "src/v2/execution/compute_stages/stages/MoEExpertComputeStage.h");
    const auto expert_source = readFile(
        root / "src/v2/execution/compute_stages/stages/MoEExpertComputeStage.cpp");
    const auto workspace_source = readFile(
        root / "src/v2/execution/moe/MoEWorkspaceRequirements.h");
    const auto graph_source = readFile(
        root / "src/v2/models/qwen35moe/Qwen35MoEGraph.cpp");
    const auto shared_gate_source = sliceBetween(
        expert_source,
        "SharedExpertGateStage::SharedExpertGateStage(",
        "StageBufferRequirements SharedExpertGateStage::getBufferRequirements() const");
    const auto shared_gate_execute = sliceBetween(
        expert_source,
        "bool SharedExpertGateStage::execute(",
        "IMoEKernel *SharedExpertGateStage::ensureMoEKernel() const");
    const auto resident_assignment = sliceBetween(
        expert_source,
        "kernel->assignPrefillRoutesLeastLoadedResident(",
        "if (groups_prepared)");
    const auto combined_gate_input = sliceBetween(
        expert_source,
        "TensorBase *MoEExpertComputeStage::effectiveSafeCompositeSharedGateInput() const",
        "bool MoEExpertComputeStage::executeSafeCombinedSharedVerifierComposite(");

    EXPECT_NE(routing_header.find("active_row_count_device"), std::string::npos);
    EXPECT_NE(expert_header.find("active_row_count_device"), std::string::npos);
    EXPECT_NE(routing_source.find("params_.active_row_count_device"), std::string::npos);
    EXPECT_NE(routing_source.find("routeWithTensorsEffectiveSeqLen"), std::string::npos);
    EXPECT_NE(shared_gate_source.find("params_.active_row_count_device"), std::string::npos);
    EXPECT_NE(shared_gate_source.find("sharedExpertGateFromTensorsEffectiveSeqLen"), std::string::npos);
    EXPECT_NE(shared_gate_source.find("sharedExpertGateAddFromTensorsEffectiveSeqLen"), std::string::npos);
    EXPECT_NE(
        resident_assignment.find("params_.active_row_count_device"),
        std::string::npos)
        << "Resident route assignment must consume the same device geometry "
           "as its routing producer.";
    EXPECT_GE(countOccurrences(graph_source, "active_row_count_device ="), 3u)
        << "The declarative graph must wire one device row-count owner into "
           "routing, resident assignment, and shared gating.";

    for (const auto &[name, source] :
         std::vector<std::pair<std::string, std::string>>{
             {"MoERoutingStage", routing_source},
             {"SharedExpertGateStage", shared_gate_source}})
    {
        EXPECT_EQ(source.find("GpuEffectiveSeqLenState"), std::string::npos) << name;
        EXPECT_EQ(source.find("allocateRowSelectHostParam"), std::string::npos) << name;
        EXPECT_EQ(source.find("uploadRowSelectParam"), std::string::npos) << name;
        EXPECT_EQ(source.find("freeRowSelectHostParam"), std::string::npos) << name;
        EXPECT_EQ(source.find("host_effective_seq_len"), std::string::npos) << name;
        EXPECT_EQ(source.find("PREFILL_EFFECTIVE_SEQ_LEN"), std::string::npos) << name;
    }
    EXPECT_EQ(workspace_source.find("PREFILL_EFFECTIVE_SEQ_LEN"), std::string::npos)
        << "A removed host-mirror protocol must not retain a dead arena allocation.";
    for (const auto &[name, source] :
         std::vector<std::pair<std::string, std::string>>{
             {"SharedExpertGateStage::execute", shared_gate_execute},
             {"MoEExpertComputeStage::effectiveSafeCompositeSharedGateInput",
              combined_gate_input}})
    {
        EXPECT_EQ(source.find("make_shared<FP32Tensor>"), std::string::npos) << name;
        EXPECT_EQ(source.find("to_fp32("), std::string::npos) << name;
    }
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, CUDAMoEWorkspaceRebindPreservesCaptureMetadata)
{
    ...
    EXPECT_NE(bind_workspace.find("workspace_ == workspace"), std::string::npos)
        << "Same-workspace rebinds happen as sibling MoE stages touch the singleton kernel. "
           "They must not clear runtime pointer arrays populated by launch preparation.";
...
TEST(Test__GpuWorkspaceAllocationPolicy, CachedForwardReplayRefreshesResidentPositionRows)
{
    ...
    const auto compact_forward_engine =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(forward_engine));
    const auto compact_dynamic_input_helper =
        removeAsciiWhitespace(stripCommentsAndStringLiterals(sliceBetween(
            forward_engine,
    ...
    EXPECT_NE(compact_forward_types.find("constvoid*device_position_ids=nullptr"),
              std::string::npos)
        << "Forward graph signatures must name the exact resident-position owner.";
    EXPECT_NE(compact_forward_types.find("device_position_ids==other.device_position_ids"),
              std::string::npos);
    EXPECT_NE(compact_forward_types.find("std::hash<constvoid*>{}(sig.device_position_ids)"),
              std::string::npos);
    EXPECT_NE(compact_forward_engine.find("usesDevicePositionIds()"),
              std::string::npos)
        << "Perf tags should expose whether a cached graph used resident positions.";
    EXPECT_NE(compact_forward_engine.find(".device_position_ids=effective_input.position_ids_device"),
              std::string::npos);
    EXPECT_NE(compact_forward_engine.find(".device_token_ids=effective_input.token_ids_device"),
              std::string::npos)
        << "Both resident input owners must participate in complete capture identity.";
    ...
    EXPECT_NE(compact_dynamic_input_helper.find("stage->updateDynamicDevicePositionIds(input.position_ids_device,position_row_count)"),
              std::string::npos)
        << "The shared graph-input publisher must refresh every resident RoPE row.";
    EXPECT_NE(compact_dynamic_input_helper.find("stage->updateDynamicPositionIds(host_position_ids,position_row_count)"),
              std::string::npos)
        << "The shared graph-input publisher must refresh every host-owned RoPE row.";
    EXPECT_NE(compact_forward_engine.find("updateDynamicParamStages(input,forward_cache.dynamic_param_stages,replay_position_ids,"),
              std::string::npos)
        << "Cache-hit replay must publish current position rows through the shared helper.";
    EXPECT_NE(compact_forward_engine.find("updateDynamicParamStages(effective_input,cache_miss_dynamic_param_stages,effective_input.position_ids,"),
              std::string::npos)
        << "Cache-miss execution must publish exact position ownership after binding its stream.";
}

TEST(Test__GpuWorkspaceAllocationPolicy, PrefillCapturePreparesArenaStorageBeforeBeginCapture)
{
    const auto source =
        readFile(repoRoot() / "src/v2/execution/local_execution/engine/ForwardExecutionEngine.cpp");
...
/**
 * @brief Require one frozen dependency ledger and canonical stage runner per capture.
 *
 * A recorded producer has not executed yet, so its output cannot be made
 * globally device-authoritative while the native graph is being constructed.
 * Production capture must instead prove the internal edge from the graph's
 * BufferId topology, advance that proof through the canonical stage runner,
 * and publish global authority only after graph launch. This source gate keeps
 * direct, cached, diagnostic-recapture, and monolithic-prefill paths on that
 * same typed transaction and prevents a future direct `stage->execute()` loop
 * from bypassing the producer/consumer ledger.
 */
TEST(Test__GpuWorkspaceAllocationPolicy, NativeCaptureUsesFrozenDependencyTransactions)
{
    const auto root = repoRoot();
    const auto guard_source = readFile(
        root /
        "src/v2/execution/local_execution/graph/GraphCaptureGuard.h");
    const auto executor_source = readFile(
        root /
        "src/v2/execution/local_execution/graph/DeviceGraphExecutor_GraphCapture.cpp");
    const auto controller_source = readFile(
        root /
        "src/v2/execution/local_execution/graph/DeviceGraphCaptureController.cpp");
    const auto forward_source = readFile(
        root /
        "src/v2/execution/local_execution/engine/ForwardExecutionEngine.cpp");
    const auto prefill_cache_source = readFile(
        root /
        "src/v2/execution/local_execution/engine/PrefillGraphCache.cpp");
    const auto transfer_source = readFile(
        root / "src/v2/transfer/TransferEngine.cpp");

    EXPECT_NE(guard_source.find("class GraphCaptureDependencyLedger final"),
              std::string::npos)
        << "Capture topology must have one typed, frozen transaction owner.";
    EXPECT_NE(guard_source.find("class ScopedGraphCaptureStage final"),
              std::string::npos)
        << "Stage-order advancement must be structural and exception-safe.";
    EXPECT_NE(guard_source.find("currentGraphCaptureDependencyLedger()"),
              std::string::npos)
    ...
    const auto capture_phase = sliceBetween(
        controller_source,
        "DeviceGraphCaptureController::CapturePhaseResult DeviceGraphCaptureController::executeCapturePhase(",
        "DeviceGraphCaptureController::ReplayPhaseResult DeviceGraphCaptureController::executeReplayPhase(");
    expectNeedleBefore(
        ...
        "ScopedBackendGraphCapture capture_transaction(",
        "Cached capture must freeze its segment topology before beginCapture().");
    EXPECT_NE(capture_phase.find("hooks.execute_node(*node)"),
              std::string::npos)
        << "Cached capture must use the executor-owned stage hook.";
    EXPECT_EQ(capture_phase.find("node->stage->execute(ctx)"),
              std::string::npos)
        << "Cached native capture must not bypass the canonical executor.";
    ...
    expectNeedleBefore(
        ...
        "planCaptureDependencies(",
        "ScopedBackendGraphCapture capture_transaction(",
        "Diagnostic recapture must obey the production dependency transaction.");
    EXPECT_NE(diagnostic_recapture.find("execute_node_cb(*node)"),
              std::string::npos)
        << "Diagnostic recapture must use the canonical stage callback.";
    EXPECT_EQ(diagnostic_recapture.find("node->stage->execute(ctx)"),
              std::string::npos)
        << "Diagnostics cannot introduce a second native-capture execution path.";

    const auto prefill_path = sliceBetween(
        forward_source,
        "if (can_attempt_capture && capture_ready_reason == PrefillGraphRejectReason::None)",
        "const std::string capture_launch_boundary");
    expectNeedleBefore(
        prefill_path,
        "executor_.planGraphCaptureDependencies(",
        "cache.captureAndInstantiate(",
        "Monolithic prefill must freeze the complete stage topology before capture.");
    EXPECT_NE(prefill_path.find("dependency_ledger.get()"),
              std::string::npos)
        << "The prefill cache must receive the exact executor-owned ledger.";

    const auto prefill_capture = sliceBetween(
        prefill_cache_source,
        "bool PrefillGraphCache::captureAndInstantiate(",
        "size_t PrefillGraphCache::size() const");
    EXPECT_NE(prefill_capture.find("dependency_ledger"), std::string::npos);
    EXPECT_NE(prefill_capture.find("ScopedBackendGraphCapture capture_transaction("),
              std::string::npos)
        << "Prefill begin/body/end must be one dependency-aware transaction.";

    const auto require_input = sliceBetween(
        transfer_source,
        "void TransferEngine::requireDeviceInput(",
        "void TransferEngine::prepareDeviceOutput(");
    EXPECT_NE(require_input.find("currentGraphCaptureDependencyLedger()"),
              std::string::npos)
        << "Internal recorded reads need the active transaction proof.";
    EXPECT_NE(require_input.find("InputDisposition::InternalRecorded"),
              std::string::npos)
        << "Only an exact earlier producer may satisfy an internal read.";

    const auto publish_write = sliceBetween(
        transfer_source,
...
/**
 * @brief Forbid placement-capable transfer calls from GPU kernel code.
 *
 * Kernel launchers execute after graph storage planning and therefore may only
 * validate exact input residency or output storage. Allowing a kernel to call
 * prepareDeviceInput() or prepareDeviceOutput() reintroduces hidden allocation,
 * migration, upload, and stale-generation event import into the hot path. The
 * backend trees are scanned as one policy surface so CUDA and ROCm cannot drift.
 */
TEST(Test__GpuWorkspaceAllocationPolicy, GPUKernelsCannotOwnTensorPlacement)
{
    const auto root = repoRoot();
    const std::array<std::filesystem::path, 2> backend_roots = {
        root / "src/v2/kernels/cuda",
        root / "src/v2/kernels/rocm",
    };

    for (const auto &backend_root : backend_roots)
    {
        ASSERT_TRUE(std::filesystem::exists(backend_root));
        for (const auto &entry :
             std::filesystem::recursive_directory_iterator(backend_root))
        {
            if (!entry.is_regular_file())
                continue;
            const auto extension = entry.path().extension().string();
            ...
            const auto executable_source =
                stripCommentsAndStringLiterals(readFile(entry.path()));
            EXPECT_EQ(
                executable_source.find("TransferEngine::prepareDeviceInput("),
                std::string::npos)
                << entry.path()
                << " must require executor-prepared input residency.";
            EXPECT_EQ(
                executable_source.find("TransferEngine::prepareDeviceOutput("),
                std::string::npos)
                << entry.path()
                << " must require preallocated output storage.";
        }
    }
}

/**
 * @brief Keep every production routed-expert exit on one publication policy.
 *
 * Canonical LocalTP producers write route-slot contributions while a later
 * reducer owns the final MoE output. A direct publication of `params_.output`
 * after otherwise-correct grouped work would manufacture an event for an
 * inactive tensor with no device storage. The stage API centralizes this
 * choice; this source gate prevents M=1, runtime-table, fixed-table, and
 * grouped-M exits from bypassing it independently.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     MoECanonicalProducersShareOnePublicationTarget)
{
    const auto source = readFile(
        repoRoot() /
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_EQ(
        grouped_prefill.find("gpuExecution().publish(params_.output)"),
        std::string::npos)
        << "Ordinary grouped prefill must publish the active canonical or "
           "direct target through one typed policy.";
    ...
    const auto explicit_route = sliceBetween(
        ...
        "if (is_gpu)");
    EXPECT_NE(
        runtime_route.find("publishPublicationTarget();"),
        std::string::npos);
    EXPECT_NE(
        explicit_route.find("publishPublicationTarget();"),
        std::string::npos);

    const auto verifier_prefill = sliceBetween(
        source,
        "bool MoEExpertComputeStage::executeDecodeEquivalentVerifierPrefill(",
        "void MoEExpertComputeStage::ensureGemmEnginesCached()");
    EXPECT_NE(
        verifier_prefill.find("publishPublicationTarget();"),
        std::string::npos);
    EXPECT_EQ(
        verifier_prefill.find("gpuExecution().publish(params_.output)"),
        std::string::npos)
        << "Grouped verifier M must use the same publication target as M=1.";
}
...
TEST(Test__GpuWorkspaceAllocationPolicy, PrefillReplayStagesPreserveCapturedResetState)
{
    ...
    const auto moe_expert_header =
        readFile(repoRoot() / "src/v2/execution/compute_stages/stages/MoEExpertComputeStage.h");
    const auto shared_gate_header = sliceBetween(
        moe_expert_header,
    ...
    EXPECT_EQ(shared_gate_header.find("hasPrefillReplayParams() const override"),
              std::string::npos)
        << "SharedExpertGateStage consumes device request geometry and owns no host replay state to reset.";
     ...
     PersistentMTPGeneratedTokenHistoryIsPublishedBeforeFirstRequest)
{
    ...
    const auto publication_body = removeAsciiWhitespace(sliceBetween(
        source,
        "bool DeviceGraphOrchestrator::zeroAndPublishGenerationSamplingInputsOnStream(",
        "bool DeviceGraphOrchestrator::materializePendingMTPVerifierInputTokensOnDevice("));
    ...
    EXPECT_NE(
        initialization_body.find(
            "explicitGPUStreamForOperation(\"initialize_generation_persistent_state\")"),
        std::string::npos)
        << "Fresh GPU runners need one explicit persistent-state initialization stream.";
    EXPECT_NE(
        initialization_body.find(
            "zeroAndPublishGenerationSamplingInputsOnStream("),
        std::string::npos)
        << "Construction must publish initialized histogram bytes before the first request.";
    EXPECT_NE(
        initialization_body.find("initializeAndPublishDeviceGenerationStorageOnStream("),
        std::string::npos)
        << "Ordinary and speculative generation initialize the same resident controller owner.";
    ...
    EXPECT_NE(
        reset_body.find(
            "zeroAndPublishGenerationSamplingInputsOnStream(reset_transaction.executionStream(),reset_reason)"),
        std::string::npos)
 ...
 * path if allocation fails. Device-hot promotion now reports a typed lease
 * result rather than a boolean, so this source-level policy test deliberately
 * anchors on the method name while making the required ordering explicit and
 * forbidding exceptional stream synchronization.
 */
     ...
     ForwardWorkspaceFamilyManifestDeclaresMTPConditionParticipant)
{
    ...
    EXPECT_NE(
        compact.find(
            "ScopedLiveMTPRequestBatchConditionPolicycondition_scope("
            "graph_builder_.get(),&live_mtp_request_batch_condition_,true)"),
        std::string::npos)
        << "Manifest construction must use the typed RAII policy transition so "
           "the builder and orchestrator flag cannot diverge.";
    EXPECT_NE(compact.find("if(!condition_scope.ready())"),
              std::string::npos);
    EXPECT_EQ(compact.find("builder->setLiveMTPRequestBatchCondition(true)"),
              std::string::npos)
        << "Callers must not open-code half of the paired condition-policy transition.";
    EXPECT_NE(
        compact.find(
     ...
     TypedGPUMTPLogitsCannotReachDynamicTensorAllocation)
{
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "Live execution must enter the same fail-closed binding authority.";
    EXPECT_EQ(
        live_forward_body.find("tensor_factory_->createFP32("),
        std::string::npos)
        << "Dynamic allocation must remain encapsulated behind the typed role gate.";
}
     ...
     GroupedVerifierPreparationIsOneStrictDeviceGraph)
{
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        stage_execute.find("enqueuePrepareMTPVerifierControlledRow("),
        std::string::npos)
        << "Controller-owned preparation must fuse token composition, logical "
           "width, positions, and base snapshot into one replay-time kernel.";
    EXPECT_NE(
        stage_execute.find(
            "enqueuePrepareDeviceGenerationTransactionBudget("),
        std::string::npos)
        << "Verifier preparation must publish the exact transaction budget "
           "before either greedy or stochastic reduction reads it.";
    EXPECT_EQ(
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        captured_execute.find("consumeDeviceGenerationStateReady("),
        std::string::npos)
        << "Preparation is the sole first reader of the resident controller.";
    EXPECT_NE(
        captured_execute.find("DeviceTimelineRole::AllPositionVerifier"),
        std::string::npos);
    EXPECT_NE(
    ...
    EXPECT_NE(
        ...
        std::string::npos)
        << "Parent-loop export must reject any preparation graph that does not "
           "consume the canonical device-generation controller.";
     ...
     MTPPenaltyHistoryHasOneDeviceWriterAndNoPolicyGraphIdentity)
{
    ...
    EXPECT_NE(request.find("MTPRequestPenaltyPolicypenalty_policy{}"),
              std::string::npos);
    EXPECT_EQ(request.find("intrequest_count="), std::string::npos)
        << "Publication requests must not redeclare producer-owned request geometry.";
    EXPECT_EQ(request.find("intmax_draft_tokens="), std::string::npos)
        << "Publication requests must not redeclare producer-owned verifier geometry.";
    EXPECT_NE(request.find("logicalVerifierRowsPerRequest()constnoexcept"),
              std::string::npos);
    EXPECT_NE(request.find("physicalVerifierRowsPerRequest()constnoexcept"),
              std::string::npos);
    ...
    EXPECT_NE(
        lifecycle_finalize.find("if(request.requestCount()==1)"),
        std::string::npos);
    EXPECT_EQ(
        countOccurrences(
            lifecycle_finalize,
...
/**
 * @brief Address every verifier prelude through one exact bounded registry key.
 *
 * Parent composition receives controller policy, request count, and physical
 * verifier width. Those values must directly address one immutable slot;
 * recency, nearest-geometry scans, and same-generation replacement would make
 * pointer ownership ambiguous. The paired forward signature lives on that
 * exact slot as well.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     GroupedVerifierPreparationUsesExactBoundedRegistryKeys)
{
    const auto root = repoRoot();
    const auto header = readFile(
        root /
    ...
    const auto row_mode_setter = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::setComputeRowIndexedAllPositionLogits(",
            "bool DeviceGraphOrchestrator::setMTPSpecVerifierInputPlan(")));
    const auto verifier_plan_setter = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "bool DeviceGraphOrchestrator::setMTPSpecVerifierInputPlan(",
            "void DeviceGraphOrchestrator::clearMTPSpecVerifierInputPlan(")));
    ...
    EXPECT_NE(
        compact_header.find("structMTPVerifierPreparationGraphKey"),
        std::string::npos);
    EXPECT_NE(
        compact_header.find(
            "enumclassMTPVerifierPreparationControlPolicy:uint8_t"),
        std::string::npos);
    EXPECT_NE(
        compact_header.find(
            "MTPVerifierPreparationControlPolicycontrol_policy="),
        std::string::npos);
    EXPECT_NE(
        compact_header.find("intrequest_count=0;"),
        std::string::npos);
    EXPECT_NE(
        compact_header.find("intpadded_seq_len=0;"),
        std::string::npos);
    EXPECT_NE(
        compact_header.find("MTPVerifierPreparationGraphKeykey"),
        std::string::npos)
        << "Each cache owner must retain the exact key assigned to its slot.";
    EXPECT_EQ(
        compact_header.find("ActiveMTPVerifierPreparationGraph"),
        std::string::npos)
        << "Mutable last-transaction identity must not select parent fragments.";
    EXPECT_EQ(
        source.find("active_mtp_verifier_preparation_graph_"),
        std::string::npos);

    EXPECT_NE(
        registry_lookup.find(
            "mtpVerifierPhysicalRowBucket(key.padded_seq_len,mtp_max_verifier_rows_)!=key.padded_seq_len"),
        std::string::npos)
        << "Scalar registry keys must already name a canonical physical bucket.";
    EXPECT_EQ(
        countOccurrences(source, "mtpVerifierPhysicalRowBucket("),
        1U)
        << "Production graph geometry must resolve through the typed "
           "controller-owned width policy; the raw bucketer is reserved for "
           "validating canonical registry addresses.";
    EXPECT_NE(
        registry_lookup.find(
            "(control_policy_index*static_cast<size_t>(stochastic_batch_output_request_capacity_)+static_cast<size_t>(key.request_count-1))*static_cast<size_t>(mtp_max_verifier_rows_)+static_cast<size_t>(key.padded_seq_len-1)"),
        std::string::npos)
        << "Policy and geometry must map directly to one deterministic flat slot.";
    EXPECT_NE(
        registry_lookup.find(
            "mtp_verifier_preparation_graphs_.size()!=expected_registry_size"),
        std::string::npos);

    EXPECT_NE(
        materializer.find(
            "mtpVerifierPreparationRegistryIndex(requested_key,&registry_error)"),
        std::string::npos);
    EXPECT_NE(
        materializer.find("cache.key!=requested_key"),
        std::string::npos);
    EXPECT_NE(
        materializer.find("cache.stage->hasSameCaptureIdentity(params)"),
        std::string::npos)
        << "A populated exact slot must still reject changed pointer topology.";
    EXPECT_EQ(materializer.find("available_index"), std::string::npos);
    EXPECT_EQ(
        materializer.find(
            "for(size_tindex=0;index<mtp_verifier_preparation_graphs_.size()"),
        std::string::npos)
        << "Materialization must never scan for a recent, nearest, or empty slot.";

    EXPECT_NE(
        metadata_prelude.find("MTPVerifierPreparationGraphKeypreparation_key"),
        std::string::npos);
    EXPECT_NE(
        metadata_prelude.find(
            "mtpVerifierPreparationControlPolicyForRequest("),
        std::string::npos)
        << "Preparation must fail on malformed admission rather than silently selecting standalone mode.";
    EXPECT_NE(
        metadata_prelude.find(
            "executeMTPVerifierPreparationCaptured(execution_stream,preparation_key"),
        std::string::npos);

    EXPECT_NE(
        metadata_prelude.find(
            "mtpVerifierPhysicalWidthPolicyForRequest(batch_plan.request_count,&width_policy_error)"),
        std::string::npos)
        << "Verifier preparation must resolve the immutable width policy from "
           "the admitted controller lifecycle.";
    EXPECT_NE(
        metadata_prelude.find(
            "mtpVerifierPhysicalPaddedSeqLen(batch_plan.request_count,batch_plan.logical_padded_seq_len,mtp_max_verifier_rows_,*width_policy)"),
        std::string::npos)
        << "Verifier token plans must validate logical width against the exact "
           "typed physical graph policy.";
    EXPECT_NE(
        metadata_prelude.find(
    ...
    EXPECT_EQ(
        metadata_prelude.find(
            "batch_plan.logical_padded_seq_len!=input.seq_len"),
        std::string::npos)
        << "Logical depth must not become part of the physical graph identity.";

    EXPECT_NE(
        row_mode_setter.find(
            "mtpVerifierPhysicalWidthPolicyForRequest("),
        std::string::npos)
        << "Row-indexed LM-head geometry must use the same controller-owned "
           "policy as the forward graph.";
    EXPECT_NE(
        row_mode_setter.find("mtpVerifierPhysicalPaddedSeqLen("),
        std::string::npos);
    EXPECT_NE(
        verifier_plan_setter.find(
            "mtpVerifierPhysicalWidthPolicyForRequest("),
        std::string::npos)
        << "Verifier metadata selection must not independently reconstruct a "
           "power-of-two row bucket.";
    EXPECT_NE(
        verifier_plan_setter.find("mtpVerifierPhysicalPaddedSeqLen("),
        std::string::npos);

    EXPECT_NE(
        exporter.find("MTPVerifierPreparationGraphKeykey"),
        std::string::npos);
    EXPECT_NE(
        exporter.find(
            "MTPVerifierPreparationControlPolicy::DeviceGenerationControlled"),
        std::string::npos)
        << "Only the resident-controller capture may be cloned into the device loop.";
    EXPECT_NE(
        exporter.find(
            "mtpVerifierPreparationRegistryIndex(key,&registry_error)"),
        std::string::npos);
    EXPECT_NE(exporter.find("cache.key!=key"),
              std::string::npos);
    ...
    EXPECT_NE(
        verifier_exporter.find(
            "forward_engine_->deviceLoopGraphTemplate("),
        std::string::npos)
        << "Parent composition must resolve the immutable paired signature exactly.";
    EXPECT_EQ(
        verifier_exporter.find(
            "lastAllPositionVerifierDeviceLoopGraphTemplate("),
        std::string::npos)
        << "Mutable last-executed state must not choose a parent-loop verifier capture.";
    ...
    EXPECT_NE(
        verifier_exporter.find(
            "signature.all_position_logit_rows!=request_count*padded_seq_len"),
        std::string::npos)
        << "The expensive forward capture must use the bounded physical bucket.";
    EXPECT_EQ(
        verifier_exporter.find(
            "signature.all_position_logit_rows!=preparation.valid_graph_row_count"),
        std::string::npos)
        << "Logical preparation rows may be shorter than the reusable physical forward bucket.";
    EXPECT_NE(verifier_exporter.find("!signature.usesDeviceTokenIds()"),
              std::string::npos);
    EXPECT_NE(verifier_exporter.find("!signature.usesDevicePositionIds()"),
              std::string::npos);
    EXPECT_NE(
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        verifier_exporter.find("cache.key!=key"),
        std::string::npos)
        << "Forward pairing must resolve the same exact registry owner.";
}

/**
 * @brief Lock both grouped sampling topologies into typed backend loop policies.
 *
 * Homogeneous CUDA requires monolithic child captures. A topology containing a
 * CPU, ROCm, or remote participant instead retains the exact segmented sparse
 * transaction and selects it from a device-authenticated ticket. Depth D has D
 * draft rows and D+1 verifier/bonus rows. Greedy uses 2D+4 common fragments
 * because its compact reducer is inside the verifier forward; stochastic adds
 * exactly two child graphs for target-distribution preparation and serial
 * rejection. Dynamic placement may append one device-gated maintenance
 * transaction. Sampling mode and child replacement are cache identity, while
 * unchanged executables remain reusable across request-content reset. This is
 * a typed construction policy, never a redirect after native composition fails.
 */
TEST(Test__GpuWorkspaceAllocationPolicy,
     DeviceGenerationPolicyOwnsTheCompleteOrderedTransaction)
{
    ...
    const auto runner_source = readFile(
        root / "src/v2/execution/runner/OrchestrationRunner.cpp");
    const auto program = removeAsciiWhitespace(stripCommentsAndStringLiterals(
        readFile(root / "src/v2/execution/mtp/DeviceGenerationGraphProgram.cpp")));
    ...
    const auto execution_policy = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
            "DeviceGraphOrchestrator::deviceGenerationExecutionPolicy(",
            "bool DeviceGraphOrchestrator::beginDeviceResidentGeneration(")));
    const auto verifier_pairing = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            source,
    ...
    EXPECT_NE(
        execution_policy.find("hasHeterogeneousCollectiveExecutionDomain()"),
        std::string::npos)
        << "Process-local CPU/GPU and cross-vendor overlays must select the "
           "same hosted policy as their segmented decode graph.";
    EXPECT_NE(
        execution_policy.find("moe_overlay_inference_transaction_coordinator_"),
        std::string::npos)
        << "A remote sparse participant is independently sufficient to require "
           "hosted retained-transaction submission.";
    EXPECT_NE(
        verifier_pairing.find("deviceGenerationExecutionPolicy(topology)"),
        std::string::npos)
        << "Verifier pairing must consume the one typed generation policy.";
    EXPECT_NE(
        verifier_pairing.find("forward_engine_->retainedDecodeGraph("),
        std::string::npos)
        << "Hosted pairing must admit the exact segmented production plan.";
    EXPECT_NE(
        verifier_pairing.find("forward_engine_->deviceLoopGraphTemplate("),
        std::string::npos)
        << "Native CUDA pairing must retain its stricter composable child.";
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        composer.find("mtpVerifierPhysicalWidthPolicyForRequest("),
        std::string::npos)
        << "Parent composition must address verifier fragments through the "
           "same controller-owned width policy as forward capture.";
    EXPECT_NE(
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(composer.find("DeviceGenerationGraphProgram::native("), std::string::npos);
    EXPECT_NE(composer.find("DeviceGenerationGraphProgram::ticketPublisher("), std::string::npos);
    EXPECT_EQ(composer.find("buildDeviceControlledWhileLoop("), std::string::npos)
        << "Algorithms declare fragments; only the shared program compiler lowers native loops.";
    EXPECT_NE(program.find("destination.buildDeviceControlledWhileLoop("), std::string::npos);
    EXPECT_NE(program.find("destination.buildDeviceControlledSelectorWhileLoop("), std::string::npos);
    EXPECT_NE(program.find("destination.instantiate()&&destination.hasExecutable()"), std::string::npos);
    EXPECT_NE(composer.find("capture->hasExecutable()"), std::string::npos);
    EXPECT_NE(
        program.find("healthy_index=kDeviceGenerationControlOk"),
        std::string::npos);
    EXPECT_NE(
        program.find(
    ...
    for (const char *needle : forbidden)
    {
        EXPECT_EQ(composer.find(needle), std::string::npos)
            ...
            << needle;
        EXPECT_EQ(program.find(needle), std::string::npos)
            << "The shared generation compiler inherits the same no-transfer/no-sync contract: "
            << needle;
    }
    ...
    EXPECT_EQ(
        ...
        std::string::npos)
        << "Request-content admission must preserve a parent whose complete child identity is unchanged.";
    EXPECT_NE(
        admission.find("mtp_device_generation_loop_graph_.launched"),
        std::string::npos)
        << "Admission must reject a parent launch that did not cross terminal retirement.";
    EXPECT_NE(
        terminal.find(
            "mtp_device_generation_loop_graph_.retireCompletedLaunch()"),
        std::string::npos)
        << "Only successful terminal materialization may release parent launch ownership for request reuse.";
    EXPECT_NE(
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    EXPECT_NE(
        conditional_builder.find("initial_predicate_node=prelude.tail"),
        std::string::npos);
    EXPECT_NE(
        conditional_builder.find(
    ...
    EXPECT_NE(
        conditional_builder.find(
            "validateCudaConditionalBodyFragmentGraph("),
        std::string::npos)
    ...
    EXPECT_NE(
        ...
        std::string::npos);
    // Local sampling belongs behind request health; collective arrival cannot
    // be guarded by the follower's stale prior command. The shared prelude
    // performs that exchange before the loop's live continuation check.
    ...
    const auto initialization_builder = removeAsciiWhitespace(
        stripCommentsAndStringLiterals(sliceBetween(
            cuda_capture,
            "DeviceControlledFragmentAppendResult appendGuardedLoopInitialization(",
            "#endif")));
    EXPECT_NE(initialization_builder.find("cudaGraphCondTypeIf"), std::string::npos);
    EXPECT_NE(initialization_builder.find(
        "cudaGraphConditionalHandleCreate(&initialization_condition,graph,0,cudaGraphCondAssignDefault)"),
        std::string::npos)
        << "CUDA IF and WHILE nodes require distinct native conditional handles.";
    EXPECT_NE(initialization_builder.find(
        "cudaGraphKernelNodeSetParams(entry_predicate,&initialization_predicate)"),
        std::string::npos);
    const auto prelude = removeAsciiWhitespace(stripCommentsAndStringLiterals(sliceBetween(
        cuda_capture, "DeviceControlledFragmentAppendResult appendLoopPrelude(", "#endif")));
    const auto admitted = prelude.find("appendGuardedLoopInitialization(");
    const auto arrived = prelude.find("appendLoopEntry(");
    const auto checked = prelude.find("cudaGraphAddKernelNode(&result.tail,graph,dependency?");
    ASSERT_NE(admitted, std::string::npos);
    ASSERT_NE(arrived, std::string::npos);
    ASSERT_NE(checked, std::string::npos);
    EXPECT_LT(admitted, arrived) << "First-token EOS must still be published to followers.";
    EXPECT_LT(arrived, checked) << "A follower cannot read the prior request's command before arrival.";
    EXPECT_NE(fixed_builder.find("appendLoopPrelude("), std::string::npos);
    EXPECT_NE(selector_builder.find("appendLoopPrelude("), std::string::npos);
    EXPECT_EQ(
        cuda_capture.find("lowerCudaConditionalBodyEventHandoffs"),
        std::string::npos)
        << "Composition must never repair producer lifecycle events after capture.";
    EXPECT_EQ(
        cuda_capture.find("contractCudaEventNode"),
        std::string::npos)
        << "Forbidden event nodes are fatal producer defects, not graph edges "
           "for the parent to rewrite.";
}
