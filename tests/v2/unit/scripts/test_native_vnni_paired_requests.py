...
class NativeVNNIPairedRequestsTest(unittest.TestCase):
    ...
    def test_competitive_frontier_edges_share_the_current_measurement_batch(self) -> None:
        """Alternate CV models do not require another full fit to request timing."""
        ...
        self.assertEqual(plan.competitive_frontier_request_count, 1)
        frontier_requests = [
            request
            for request in plan.requests
            if request.reason == "competitive_model_frontier_edge"
        ]
        self.assertEqual(len(frontier_requests), 1)
        self.assertEqual(
            frontier_requests[0].selected_candidate_id,
            frontier_effective,
        )

    def test_frontier_reuses_connected_tournament_instead_of_quadratic_pairs(self) -> None:
        """An alternate model needs a graph path, while selected CV stays direct."""
        ...
        self.assertEqual(plan.competitive_frontier_request_count, 0)
    ...
    def test_direct_pass_conflicting_with_tournament_requests_reconfirmation(self) -> None:
        """A non-transitive fitted graph gets another independently seeded edge."""
        ...
        self.assertEqual(
            ...
            "tournament_fit_conflicts_with_direct_edge",
        )
