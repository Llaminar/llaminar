...
TEST_F(Test__BackendSelector, TransferCudaToRocmSelectsPCIeBAR) {
    auto backend = BackendSelector::selectForTransfer(cuda0, rocm0);
    EXPECT_EQ(backend, CollectiveBackendType::PCIE_BAR);
}

TEST_F(Test__BackendSelector, TransferRocmToCudaSelectsPCIeBAR) {
...
TEST_F(Test__BackendSelector, TPDomainMixedGpuTwoDevicesSelectsPCIeBAR) {
...
TEST_F(Test__BackendSelector, IsBackendUsableForPCIeBARRequiresTwoGpus) {
    bool compiled = BackendSelector::isBackendCompiled(CollectiveBackendType::PCIE_BAR);
    
    // Two GPUs should work (if compiled)
    std::vector<DeviceId> two_gpus = {cuda0, rocm0};
    bool usable_two = BackendSelector::isBackendUsable(CollectiveBackendType::PCIE_BAR, two_gpus);
    if (compiled) {
        EXPECT_TRUE(usable_two);
    }
    
    // Three GPUs should not work for PCIE_BAR (limited to 2-device P2P)
    std::vector<DeviceId> three_gpus = {cuda0, cuda1, rocm0};
    EXPECT_FALSE(BackendSelector::isBackendUsable(CollectiveBackendType::PCIE_BAR, three_gpus));
    
    // One GPU should not work (need pair for P2P)
    std::vector<DeviceId> one_gpu = {cuda0};
    EXPECT_FALSE(BackendSelector::isBackendUsable(CollectiveBackendType::PCIE_BAR, one_gpu));
    
    // Empty should not work
    std::vector<DeviceId> empty;
    EXPECT_FALSE(BackendSelector::isBackendUsable(CollectiveBackendType::PCIE_BAR, empty));
}
...
TEST_F(Test__BackendSelector, ResolveAUTOSelectsAppropriateBackend) {
    ...
    // AUTO with mixed should resolve to PCIE_BAR (2 devices)
    std::vector<DeviceId> mixed_devices = {cuda0, rocm0};
    resolved = BackendSelector::resolve(CollectiveBackendType::AUTO, mixed_devices);
    EXPECT_EQ(resolved, CollectiveBackendType::PCIE_BAR);
