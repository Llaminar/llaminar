/**
 * @file Test__IQ1_S_DecodeVectorization.cpp
 * @brief Unit tests for IQ1_S decode_to_q8_0 SIMD implementations
 *
 * Tests scalar, AVX2, and AVX-512 implementations for correctness and parity.
 * IQ1_S: 1-bit quantization with IQ1S_DELTA offset (256 elements, 50 bytes)
 *
...
/**
 * @brief Test fixture for IQ1_S decode vectorization tests
 */
class Test__IQ1_S_DecodeVectorization : public ::testing::Test
{
protected:
    void SetUp() override
    {
    ...
    /**
     * @brief Create a test IQ1_S block with random but valid data
     */
    IQ1_SBlock createTestBlock(uint32_t seed)
    {
        ...
        // Random FP16 scale
        block.d = fp32_to_fp16(dist_scale(rng));
        ...
        // Random qh values (high bits + scales + delta sign)
        for (size_t i = 0; i < 8; ++i)
        {
            block.qh[i] = static_cast<uint16_t>(dist_byte(rng) | (dist_byte(rng) << 8));
        }
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, ScalarCorrectness)
{
    IQ1_SBlock block = createTestBlock(42);
    Q8_0Block output[8];

    // Decode all 8 sub-blocks
    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        decode_iq1s_to_q8_0_scalar(block, sub_idx, output[sub_idx].qs, &output[sub_idx].d);
    }
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, MultiBlockCorrectness)
{
    const size_t num_test_blocks = 5;
    for (size_t test_idx = 0; test_idx < num_test_blocks; ++test_idx)
    {
        IQ1_SBlock block = createTestBlock(100 + test_idx);
        Q8_0Block output[8];
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, GridLookupVerification)
{
    IQ1_SBlock block = createTestBlock(777);

    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        const uint8_t *qs = block.qs + sub_idx * 4;
        const uint16_t qh_val = block.qh[sub_idx];

        for (size_t l = 0; l < 4; ++l)
        {
            // Construct 11-bit grid index
            const uint16_t grid_idx = qs[l] | (((qh_val >> (3 * l)) & 7) << 8);

            // Verify grid index is within bounds [0, 2047]
            EXPECT_LT(grid_idx, 2048)
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, AVX2Parity)
{
    IQ1_SBlock block = createTestBlock(999);
    Q8_0Block scalar_output[8];
    Q8_0Block avx2_output[8];

    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        decode_iq1s_to_q8_0_scalar(block, sub_idx, scalar_output[sub_idx].qs, &scalar_output[sub_idx].d);
        decode_iq1s_to_q8_0_avx2(block, sub_idx, avx2_output[sub_idx].qs, &avx2_output[sub_idx].d);
    }
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, AVX512Parity)
{
    IQ1_SBlock block = createTestBlock(1234);
    Q8_0Block scalar_output[8];
    ...
    {
        decode_iq1s_to_q8_0_scalar(block, sub_idx, scalar_output[sub_idx].qs, &scalar_output[sub_idx].d);
        decode_iq1s_to_q8_0_avx512(block, sub_idx, avx512_output[sub_idx].qs, &avx512_output[sub_idx].d);
    }
 ...
 * @brief Test end-to-end IQ1_STensor decode path
 */
TEST_F(Test__IQ1_S_DecodeVectorization, TensorIntegration)
{
    // Create a small IQ1_S tensor (1 row × 256 elements)
    std::vector<size_t> shape = {1, 256};

    // Allocate raw data (1 IQ1_S block = 50 bytes)
    std::vector<uint8_t> raw_data(50);
    IQ1_SBlock *block = reinterpret_cast<IQ1_SBlock *>(raw_data.data());
    *block = createTestBlock(5678);

    // Create tensor
    auto tensor = std::make_shared<IQ1_STensor>(shape, raw_data);
    ...
    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        decode_iq1s_to_q8_0(*block, sub_idx, reference_output[sub_idx].qs, &reference_output[sub_idx].d);
    }
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, AutoDispatch)
{
    IQ1_SBlock block = createTestBlock(9999);
    Q8_0Block auto_output[8];
    Q8_0Block scalar_output[8];

    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        decode_iq1s_to_q8_0(block, sub_idx, auto_output[sub_idx].qs, &auto_output[sub_idx].d);
        decode_iq1s_to_q8_0_scalar(block, sub_idx, scalar_output[sub_idx].qs, &scalar_output[sub_idx].d);
    }
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, FuzzTesting)
{
    ...
    {
        IQ1_SBlock block = createTestBlock(test_idx);
        Q8_0Block output[8];
        ...
        try
        {
            for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
            {
                decode_iq1s_to_q8_0(block, sub_idx, output[sub_idx].qs, &output[sub_idx].d);
            }
    ...
    EXPECT_EQ(failures, 0) << failures << " out of " << num_fuzz_blocks << " fuzz tests failed";
}
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, ErrorHandling)
{
    IQ1_SBlock block = createTestBlock(12345);
    ...
    for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
    {
        Q8_0Block output;
        EXPECT_NO_THROW({
            decode_iq1s_to_q8_0_scalar(block, sub_idx, output.qs, &output.d);
    ...
    Q8_0Block output;
    EXPECT_NO_THROW({
        decode_iq1s_to_q8_0_scalar(block, 0, output.qs, &output.d);
    }) << "Scalar decode failed for zero scale";
}
 ...
 */
TEST_F(Test__IQ1_S_DecodeVectorization, Performance)
{
    const size_t num_iterations = 10000;
    IQ1_SBlock block = createTestBlock(55555);
    Q8_0Block output[8];
    ...
    for (size_t iter = 0; iter < num_iterations; ++iter)
    {
        for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
        {
            decode_iq1s_to_q8_0_avx2(block, sub_idx, output[sub_idx].qs, &output[sub_idx].d);
        }
    }
    ...
    for (size_t iter = 0; iter < num_iterations; ++iter)
    {
        for (size_t sub_idx = 0; sub_idx < 8; ++sub_idx)
        {
            decode_iq1s_to_q8_0_avx512(block, sub_idx, output[sub_idx].qs, &output[sub_idx].d);
        }
    }
    ...
    // Print results
    std::cout << "\nIQ1_S Decode Performance (" << num_iterations << " iterations):\n";
...
/**
 * @brief Main entry point for IQ1_S decode vectorization tests
 */
