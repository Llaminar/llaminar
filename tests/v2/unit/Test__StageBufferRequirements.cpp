...
TEST_F(Test__StageBufferRequirements, SwiGLUStage_FP32_DeclaresCorrectBuffers)
{
    ...
    // Check gate (INPUT)
    auto *gate_buf = findBuffer(reqs, "gate");
    ASSERT_NE(gate_buf, nullptr);
    EXPECT_EQ(gate_buf->role, BufferRole::INPUT);
    EXPECT_EQ(gate_buf->shape, (std::vector<size_t>{SEQ_LEN, D_FF}));
    EXPECT_EQ(gate_buf->tensor_type, BufferTensorType::FP32);

    // Check up (INPUT)
    auto *up_buf = findBuffer(reqs, "up");
    ASSERT_NE(up_buf, nullptr);
    EXPECT_EQ(up_buf->role, BufferRole::INPUT);
...
TEST_F(Test__StageBufferRequirements, SwiGLUStage_Q8_1_DeclaresCorrectType)
{
    auto gate = createQ8_1Tensor(SEQ_LEN, D_FF);
    auto up = createQ8_1Tensor(SEQ_LEN, D_FF);
    auto output = createQ8_1Tensor(SEQ_LEN, D_FF);
    ...
    // Check tensor type is Q8_1
    auto *gate_buf = findBuffer(reqs, "gate");
    ASSERT_NE(gate_buf, nullptr);
    EXPECT_EQ(gate_buf->tensor_type, BufferTensorType::Q8_1);
}

TEST_F(Test__StageBufferRequirements, SwiGLUStage_NullTensors_ReturnsEmpty)
{
    SwiGLUStage::Params params{}; // All nullptrs
    SwiGLUStage stage(params);
...
TEST_F(Test__StageBufferRequirements, AllreduceStage_DeclaresInoutBuffer)
{
    ...
    AllreduceStage::Params params{
        .buffer = buffer.get(),
        .mpi_comm = nullptr, // Null comm is ok for buffer requirements
    };
