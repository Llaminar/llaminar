...
66. Done for the expanded GPU workspace-allocation sidequest: CUDA MoE, ROCm MoE, CUDA KV gather, CUDA NativeVNNI scratch, and CUDA/ROCm GDN deinterleave scratch now use declared graph workspace or hard-fail instead of allocating request-shaped hot scratch. `V2_Unit_GpuWorkspaceAllocationPolicy` guards the covered CUDA/ROCm MoE and GDN regions against raw `cudaMalloc`/`hipMalloc`, `V2_Unit_PrefillGraphCaptureGuards` proves ROCm MoE/GDN require adequate bound workspace, and focused ROCm MoE integration stayed green after direct tests bound `DeviceWorkspaceManager`.
67. Next: shrink remaining CUDA dense gaps and continue hardening CUDA MoE controller/long-prompt evidence without reintroducing rejected router/projection or memory-bloating split-K experiments. CUDA MoE now beats the current llama.cpp MoE default-lane no-MTP and MTP decode anchors. ROCm MoE keeps the current 2.13x fixed-depth ratchet. After the verifier budget approaches the target, repeat longer-prompt dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance. Remaining raw-allocation cleanup targets include CUDA attention legacy partial workspaces, CUDA/ROCm GEMM helper caches, KV persistent pools, and collective primer buffers; treat backend allocators, weight transforms, and persistent caches separately from graph-captured request scratch.
