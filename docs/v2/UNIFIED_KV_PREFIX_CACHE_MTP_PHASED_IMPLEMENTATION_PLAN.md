...
Latest ROCm SingleDevice dense Phase 13.5/14 note, 2026-06-03: the retained long-lane best remains the 34.28 tok/s depth-1 MTP run versus a same-context 20.82 tok/s graph-enabled baseline. The request-reset/GDN-capture-binding recheck reached 34.26 tok/s at `The quick brown fox`, `-c 64`, `-n 48`, with 90.62% request acceptance. Structured stats showed no measured sidecar graph rebuild tax after request clear, `sidecar_graph_cache_hits` after warmup, six measured rollbacks and six `rollback_verifier_state_row_shortcuts`, six `verifier_state_row_restores` over 48 GDN layers, no `shortconv_restore_failed`, no shortcut-unavailable counter, and `restore_verifier_state_row` averaging about 173 us. Focused validation passed: `V2_Unit_GDNKernels`, `V2_Unit_MTPGraphConstruction`, `V2_Integration_ROCmQuantisedGemmSmallM`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and the dense Qwen3.6 ROCm PyTorch parity cells for `MTPGreedyMatchesPyTorchDecodeTokens` and `PrefixCacheMTPRestore`. Earlier Phase 13.5 context still applies: depth-2/depth-3 are graph-captured but slower on this prompt, atomic K-partitioning is graph-safe for `kb>1`, mixed-codebook native small-M GDN projection subgroups are covered, and the remaining blocker is true captured verifier GPU work in GDN projection, ordinary GEMM, fused Gate/Up, and other verifier stages.
