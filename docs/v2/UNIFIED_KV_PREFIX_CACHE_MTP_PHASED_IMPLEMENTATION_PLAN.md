...
Latest shared MTP catch-up note, 2026-06-06: stateful greedy verification now runs through `MTPDecodeCatchup`, a common CUDA/ROCm-safe multi-row catch-up contract. The promoted implementation is deliberately `shared_stepwise`: it commits each shifted MTP row from the current terminal hidden, forwards one normal decode row, samples, and repeats until full accept, reject, or stop. This removes the embedded backend-specific verifier loop from `OrchestrationRunner` and gives future CUDA/ROCm graph-captured catch-up work one contract to prove against before promotion. Focused validation passed for `V2_Unit_MTPDecodeCatchup`, `V2_Unit_PrefillDecodeTransition`, generic ROCm Qwen3.6 `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, `OneRowRestoreLongPrefixMatchesSequential`, and the matching CUDA Qwen3.6 `MTPBenchmarkPromptFixedDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, and `OneRowRestoreLongPrefixMatchesSequential` cells. Backend-optimized graph-captured catch-up is not promoted yet.
...
## Phase 13.8: Shared Multi-Row Decode-Equivalent Catch-Up

### Goal

Replace backend-drifting MTP verifier catch-up paths with one decode-equivalent contract that CUDA, ROCm, CPU, TP, and later MoE paths can all prove against. Any optimized multi-row catch-up must leave main KV, shifted MTP KV, GDN recurrence/short-conv state, terminal hidden/logits, sampler-visible next-token state, and request bookkeeping identical to ordinary one-token-at-a-time decode for the accepted output sequence.

This phase is a prerequisite to returning to Phase 14 CUDA/ROCm speedup claims when `requiresMTPDecodeEquivalentVerifierReplay()` is true. It must not reintroduce raw all-position GDN shortcuts whose row logits match but continuation state drifts.

### Implementation Details

Phase 13.8 has three promotion tiers:

1. Shared correctness contract.
...
- Tier 1 is implemented and promoted as the correctness baseline. `MTPDecodeCatchup` owns the common greedy multi-row accept/reject loop, and `OrchestrationRunner` routes stateful CUDA/ROCm decode-equivalent verification through `shared_stepwise`.
- The first Tier 2 scaffold is implemented: `IInferenceRunner` exposes an explicit optimized catch-up capability, a stable implementation-name hook, and a hard-failing default optimized method. `OrchestrationRunner` selects that hook only when advertised and records the selected name in verifier-run and acceptance-trace counters. The unit mock delegates the advertised optimized path back to the shared oracle so selection is tested without inventing a fake state model.
- Focused validation passed for `V2_Unit_MTPDecodeCatchup`, `V2_Unit_PrefillDecodeTransition`, generic ROCm Qwen3.6 `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, `OneRowRestoreLongPrefixMatchesSequential`, and the matching CUDA Qwen3.6 `MTPBenchmarkPromptFixedDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, and `OneRowRestoreLongPrefixMatchesSequential` cells.
- Fresh CUDA dense release benchmark evidence confirms the shared-stepwise path is still speed-negative on the default lane: no-MTP reached 707.92 prefill / 41.73 decode tok/s, while fixed depth-3 shared catch-up reached 609.73 prefill / 38.19 decode tok/s with 84.95% acceptance.
- Tier 2/Tier 3 are not promoted yet. The next implementation slice is a runner-native optimized catch-up hook that preserves the `MTPDecodeCatchup` result contract while reducing synchronization and graph-replay overhead for CUDA/ROCm.
...
- Continuation-state parity tests that decode at least four future tokens after a catch-up commit and compare against ordinary sequential decode.
...
- CUDA and ROCm optimized paths either pass equivalence/parity and benchmark gates or remain disabled with explicit blocker notes.
...
17. Phase 13.8 shared multi-row decode-equivalent catch-up, then optimized CUDA/ROCm catch-up promotion.
