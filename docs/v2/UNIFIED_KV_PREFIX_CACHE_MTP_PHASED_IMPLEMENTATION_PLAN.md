...
- `V2_Unit_PrefillDecodeTransition` covers the GPU hard-fail when those hooks are missing and the supported device-resident verifier path when they are present. `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsStochasticSmoke` passes on ROCm, proving the real Qwen3.6 stochastic MTP smoke still enters draft/verify and asserting device stochastic counters are present while host full-logit stochastic counters stay at zero.
