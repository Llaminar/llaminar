...
- Retained implementation work:
  ...
  - `IInferenceRunner` now has a distinct accepted-count publication seam, `supportsMTPSpecDecodeStatePublication()` and `publishMTPSpecDecodeState()`, which defaults to unsupported and is covered by a runner unit. `DeviceGraphOrchestrator` implements the direct/probe path for single-device hybrid GDN runners by restoring only the accepted state slot named by `MTPSpecDecodeStatePublicationPlan`, truncating KV to the target cached-token count, and hard-failing on invalid metadata or missing explicit GPU streams. Normal decode does not select this path yet.
...
- Focused validation already associated with the retained pieces includes `V2_Unit_MTPSpecDecodeMetadata`, `V2_Unit_MTPSpecDecodeTransaction`, `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_MTPStateTransaction`, `V2_Unit_MTPVerifierPolicy`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, CUDA/ROCm dense `MTPAcceptedCountPublicationLeavesSequentialState`, `V2_Integration_GPUSamplingKernels`, CUDA/ROCm `MTPStochasticSamplingVerifierRuns`, the CUDA/ROCm Qwen3.6 stochastic graph smokes, and deterministic ROCm long clear-cache replay.
- Current hard gate: no optimized stateful verifier path is promoted. The first direct/probe accepted-count publication cell now passes on CUDA and ROCm for the dense reject-after-prefix transaction, but promotion still requires normal-decode integration, stochastic residual coverage, prefix restore coverage, long-context repeatability, and benchmark evidence on both backends before any Phase 14 number is accepted.
