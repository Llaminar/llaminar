...
Latest CUDA/ROCm stochastic top-k note, 2026-06-08: CUDA and ROCm graph-captured top-k/top-p sampling now use arena-declared scratch for a two-pass Qwen-style small-k distribution builder when `top_k<=32`, instead of scanning the full 248k vocab in one block. `V2_Integration_GPUSamplingKernels` includes a Qwen3.6 vocab-size `top_k=20` graph-captured distribution-plus-sample regression against shared CPU sampler math, and the CUDA/ROCm stochastic graph smokes assert the small-k scratch counter. Focused validation passed for that suite, CUDA/ROCm Qwen3.6 stochastic PyTorch parity cells, and CUDA/ROCm stochastic graph smokes. Fresh Qwen3.6 27B stochastic benchmarks reached CUDA 707.96 prefill / 54.60 decode tok/s for fixed depth 1 and 706.09 / 55.80 for dynamic depth, essentially matching the llama.cpp d1 anchor. ROCm reached 233.00 / 30.06 for no-MTP, 215.89 / 35.89 for fixed depth 1, and 215.06 / 35.40 for dynamic depth. CUDA `mtp.verifier_forward` is about 28.46 ms; ROCm `sample_mtp_token_stochastic_device` fell to about 3.36 ms, `phase138_stochastic_verify_batch_device` to about 2.42 ms, and `mtp.verifier_forward` remains the dominant 41.33 ms catch-up target.

Latest ROCm Qwen3.6 decode-dispatch coverage note, 2026-06-08: the ROCm NativeVNNI decode trainer was rerun for Qwen3.6 hot verifier shapes across Q4_0, Q4_K, Q5_K, and Q6_K with generated dispatch disabled. The checked-in generated table now has 28 graph-safe entries covering codebooks 0, 5, 7, and 8 for GDN time/output/z/inner, FFN gate-up/down, and LM-head shapes. Focused validation passed for `V2_Unit_ROCmNativeVNNITrainerCsvValidator`, `V2_Unit_ROCmNativeVNNITrainerGenerator`, `V2_Unit_ROCmNativeVNNIDecodeTrainerGenerator`, `V2_Unit_NativeVNNIGeneratedDispatchCodebooks`, `V2_Integration_ROCm_NativeVNNI_GEMV`, `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_MTPStochasticSamplingVerifierRuns`, and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsStochasticSmoke`. Fresh same-binary ROCm Qwen3.6 27B default-lane benchmarks reached 233.90 prefill / 30.16 decode tok/s without MTP, 214.07 / 35.85 fixed depth-1 stochastic MTP, and 214.88 / 35.56 dynamic stochastic MTP. The generated Q5/Q6 rows are active in verifier counters, but throughput is effectively unchanged; `mtp.verifier_forward` remains about 41.6 ms, so the next ROCm target is verifier graph cost rather than decode-table coverage.
