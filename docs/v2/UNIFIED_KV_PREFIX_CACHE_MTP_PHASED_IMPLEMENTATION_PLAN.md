...
23. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with GDN projection, ordinary GEMM, and fused Gate/Up without reintroducing mixed-codebook subgroup batching. After the ROCm verifier budget is back below the best speed-positive runs, repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract to CUDA and CPU before returning to Phase 14 acceptance.
