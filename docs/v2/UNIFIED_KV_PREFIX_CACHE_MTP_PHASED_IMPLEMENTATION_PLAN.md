...
57. Next: shrink the remaining CUDA MoE verifier GPU budget, especially router/GDN costs, benchmark-run capture amortization, and prompt-prefill overhead, without reintroducing rejected router/projection experiments. CUDA MoE beats the llama.cpp MoE decode north star and is near the 1.3x lower bound of the MTP speedup target; ROCm MoE keeps the current 2.13x fixed-depth ratchet. After the verifier budget approaches the target, repeat longer-prompt dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
