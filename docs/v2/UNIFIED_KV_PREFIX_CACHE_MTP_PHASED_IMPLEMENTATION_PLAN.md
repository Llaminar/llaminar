...
76. Done for bucket-aligned ROCm prefill trainer generation and consumption: ROCm NativeVNNI prefill sweeps now use the same shared `defaultNativeVNNIDispatchTrainingRows()` policy as CUDA, including `M={2,3,4}` verifier rows and graph-prefill buckets such as `M=600`. `validate_rocm_native_vnni_trainer_csv.py` reads the C++ bucket policy and can require policy-aligned prefill rows, `analyze_rocm_native_vnni_trainer.py` emits generated C++ dispatch tables from correct/best ROCm trainer rows with canonical codebook labels, and `ROCmQuantisedGemmKernel_native_VNNI.hip` now consumes `ROCmNativeVNNIPrefillDispatchGenerated.inc` before falling back to the old heuristic. The automatic trainer search space no longer includes misleading `N128/MT64` or broad MB3 variants; stale CSV winners for those modes hard-fail generation. A real ROCm sweep in `benchmark_results/rocm_native_vnni/20260605T202837Z-prefill-generated-pipeline` trained Q4_0/Q4_K `M=600` Qwen3.6 GDN time and FFN-down rows, regenerated the checked-in ROCm dispatch include, and passed focused validation: `V2_Unit_ROCmNativeVNNITrainerCsvValidator`, `V2_Unit_ROCmNativeVNNITrainerGenerator`, `V2_Unit_NativeVNNIGeneratedDispatchCodebooks`, and `V2_Integration_ROCm_NativeVNNI_GEMM`.
