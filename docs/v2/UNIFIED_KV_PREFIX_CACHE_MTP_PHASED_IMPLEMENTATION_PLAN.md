...
59. Next: shrink the remaining CUDA MoE no-MTP decode and verifier GPU budget, especially decode-side routed/shared expert costs, GDN costs, and controller/acceptance stability, without reintroducing rejected router/projection experiments. CUDA MoE now beats the llama.cpp MoE prefill no-MTP anchor and fixed depth-1 MTP decode anchor, but no-MTP decode still trails the llama.cpp 118.26 tok/s anchor. ROCm MoE keeps the current 2.13x fixed-depth ratchet. After the verifier budget approaches the target, repeat longer-prompt dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
