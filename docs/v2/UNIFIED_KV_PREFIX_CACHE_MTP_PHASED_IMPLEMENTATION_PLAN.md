...
10. Done for the fused-SwiGLU/FFN down route slice: ROCm `multiply_tensor_with_fused_swiglu()` now sends eligible native-VNNI `M=2/3/4` verifier shapes through the graph-native small-M path and hard-fails if that selected route cannot launch. Focused ROCm integration covers Q4_K M=2 and Q5_K M=4 route counters plus FP32-reference cosine, and real Qwen3.6 dense evidence is now speed-positive at `-c 64 -n 16`: baseline 18.47 tok/s, MTP 25.51 tok/s, 87.5% acceptance, and `main_verifier` replay about 56.3 ms per two-token graph.
11. Next: characterize the remaining captured ROCm verifier graph work after fused-SwiGLU/FFN down, especially GDN projection, LM head, GDN recurrence, and short-conv. Then repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract to CUDA and CPU before returning to Phase 14 acceptance.
