...
- Next implementation slice:
  1. implement chained-depth prefix restore for the accepted-count path;
  2. run fresh CUDA/ROCm greedy and stochastic default-lane benchmarks;
  3. remove or rename remaining "correction replay" terminology once the suffix
     forward plan is stable, so docs/counters do not suggest a fallback;
  4. promote the hook only if correctness gates and benchmark wins both hold.
