...
83. Next: replace the CUDA dense sequential verifier wall with a proven decode-equivalent verifier/catchup design, then rerun parity and Phase 14 evidence. The work must preserve the Phase 13.6 transaction contract, prove continuation equivalence before any CUDA verifier-row shortcut is re-enabled, keep DGO sidecar-preservation hard-failed by tests, and avoid broad GDN split-K routes that bloat 24GB cards. CUDA MoE already beats the current llama.cpp MoE MTP decode anchor; keep that ratchet while extending long-prompt/controller evidence after the dense verifier budget is fixed. Remaining cleanup targets include CUDA/ROCm GEMM helper caches, KV/GDN persistent state pools, prepared-weight and descriptor caches, and collective primer buffers; treat backend allocators, weight transforms, and persistent caches separately from graph-captured request scratch.
