...
Latest Phase 13.8 acceptance note, 2026-06-07: CUDA and ROCm now pass the focused vLLM-style candidate equivalence gate under `LLAMINAR_MTP_PHASE138_EQUIVALENCE_CHECK=1`, including rejected transactions that publish accepted-prefix state from metadata, commit the required shifted rows, forward the correction suffix, and produce a ready token without delegating to the shared-stepwise rejection fallback. A rejected/non-stopped optimized catch-up that returns no ready token is now a hard failure. Stochastic MTP also passes the normal CUDA/ROCm Qwen3.6 parity cells with the same transaction-validation layer, and ready sampled tokens now carry a sampler-parameter snapshot so stale ready tokens hard-fail if sampling changes before consumption. Phase 13.8 is still not accepted for default promotion until the direct/default lane is proven with broader prefix/stop/reject/continuation coverage and comparable CUDA/ROCm benchmark wins.
...
| Phase 13.8: vLLM-Style Accepted-Count Spec-Decode State | Implementation active; old shortcut removed | The useful groundwork is in place: `MTPSpecDecodeTransaction`, `MTPSpecDecodeMetadata`, graph-facing accepted-state metadata buffers, explicit-stream metadata upload, and transaction counters. The dead raw all-position and sidecar-chain state shortcut tests have been removed rather than kept as negative fixtures. CUDA and ROCm GDN/short-conv verifier capture now run against runner-declared speculative state-slot/work-state buffers through `IWorkspaceConsumer`, hard-fail on missing explicit stream/workspace, leave live state untouched before publication, and still prove plain non-verifier recurrence/conv state matches serial decode. The old verifier-capture workspace spelling has been removed from stage APIs and unit expectations in favor of `gdn_speculative_state_slots*` names. The opt-in `vllm_style_spec_decode` candidate now publishes accepted-prefix state from metadata, commits required shifted rows, forwards rejected correction suffixes directly, produces a ready token, and hard-fails rejected/no-ready results instead of delegating to a shared-stepwise fallback. It passes focused CUDA/ROCm dense greedy equivalence, stop-token, continuation, depth-3 prefix-restore, and depth-3 rejection/benchmark regressions under `LLAMINAR_MTP_PHASE138_EQUIVALENCE_CHECK=1`; CUDA/ROCm stochastic parity now explicitly selects the same candidate and records `phase138_stochastic_spec_decode_runs` with sampler-param-safe ready-token handoff. Dense CUDA/ROCm prefix-cache full-hit restore is covered at max supported draft depth with terminal logits, terminal hidden, MTP state restore, chained sidecar drafts, active transaction commits, and zero validation failures. Default promotion remains off. | Gather stochastic and greedy CUDA/ROCm benchmark wins, then decide default promotion. |
...
- Retired candidate evidence remains binding as design history, but the dead-end runtime path and parity probes have been removed:
  - sidecar-chain verifier state and naive all-position selected-state restore are not accepted publication mechanisms for Qwen3.6 dense;
  - row-token agreement is not sufficient evidence of continuation-state equivalence.
...
- Device-metadata state publication groundwork is focused-green but not sufficient for acceptance: `ITensorShortConvolution`, `ITensorGatedDeltaNet`, and `IComputeStage` now expose only the semantic `publishAcceptedSpeculativeStateFromDeviceMetadata()` device-metadata API. CUDA and ROCm short-conv/GDN kernels implement graph-capturable device-side publication selected by `accepted_state_slot_indices[request_index]`, require explicit non-null streams, and use runner-declared state-slot workspace rather than ad hoc allocations. Focused CUDA/ROCm integration coverage now uses multi-entry accepted-state metadata arrays so the selected slot is read from device metadata rather than inferred from the request index or a scalar committed-row buffer. This proves explicit-stream accepted-slot publication mechanics, but Phase 13.8 still needs the remaining backing-storage migration away from any verifier-snapshot compatibility assumptions before default promotion. Focused validation passed on 2026-06-07 for `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, `V2_Unit_PrefillDecodeTransition`, `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPSpecDecodeMetadata`, and CUDA/ROCm `Phase138VllmStyleCandidateEquivalence`.
- Stateful graph construction now carries Phase 13.8 slot terminology directly: `ShortConv1dStage::Params` and `GDNRecurrenceStage::Params` expose `speculative_state_slot_rows` as the semantic request for temporary MTP verifier state slots. `Qwen35Graph` sets this field for Qwen3.6/Qwen3.5 GDN verifier graphs, and `V2_Unit_MTPGraphConstruction` asserts the explicit speculative-slot request is present. Focused validation passed on 2026-06-07 for `V2_Unit_MTPGraphConstruction`, `V2_Unit_MTPSpecDecodeMetadata`, and `V2_Unit_PrefillDecodeTransition`.
- Speculative state-slot backing storage has completed the naming migration away from verifier-capture compatibility spellings: `GDNRecurrenceStage` now declares `gdn_speculative_state_slots*`, `ShortConv1dStage` declares `gdn_shortconv_speculative_state_slots*`, and stale non-verifier stage bindings are cleared instead of retaining old speculative workspace pointers. CPU live-prefix restore/truncate counters now explicitly distinguish preserved replay state from reset kernel dynamic state, while GPU verifier-row publication remains the path that preserves both. Focused validation passed on 2026-06-07 for `V2_Unit_MTPGraphConstruction`, `V2_Unit_GDNKernels`, `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, and CUDA/ROCm `Phase138VllmStyleCandidateEquivalence`.
...
- Runner-facing metadata publication is scaffolded: `IInferenceRunner` and `DeviceGraphOrchestrator` expose `restoreMTPVerifierStateFromSpecDecodeMetadata()` as the request-level atomic operation, but its internals now upload a validated `MTPSpecDecodeMetadataBatch` to runner-owned workspace on an explicit GPU stream, publish local GDN short-conv/recurrent state through `publishAcceptedSpeculativeStateFromDeviceMetadata()`, truncate main KV/bookkeeping to the requested accepted token count, and hard-fail on missing workspace, missing backend, invalid metadata, missing accepted-state counts, missing accepted-state slots, or absent GDN participants. The tensor-kernel default no longer delegates from publication back to a verifier-row restore API; CUDA and ROCm must implement publication directly. Focused validation passed on 2026-06-07 for `V2_Unit_PrefillDecodeTransition`, `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPSpecDecodeMetadata`, `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, and CUDA/ROCm `Phase138VllmStyleCandidateEquivalence`.
...
- Runner-level equivalence harness is focused-green: setting
  `LLAMINAR_MTP_PHASE138_EQUIVALENCE_CHECK=1` around a named optimized catch-up
  candidate now runs the candidate, captures its runtime snapshot, restores the
  verifier base checkpoint, runs the `shared_stepwise` oracle, compares result
  and runtime-state equivalence, and only continues if both match. Rejected
  transactions now publish accepted-prefix state from metadata, commit the
  required shifted rows, forward the correction suffix, sample a ready token,
  and then compare that advanced state against the oracle. Any rejected,
  non-stopped optimized catch-up that returns without a ready token is a hard
  failure. The old shared-stepwise replay implementation has been removed
  rather than kept as a fallback or negative fixture. Focused validation passed
  on 2026-06-07 for
  `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPSpecDecodeMetadata`,
  `V2_Unit_PrefillDecodeTransition`, the direct
  `MTPFirstTransactionLeavesSequentialState` regression, and the 11-test
  CUDA/ROCm Qwen3.6 dense Phase 13.8 sweep covering candidate equivalence,
  prefix restore, stop-token, continuation, ROCm depth-3 reject, and CUDA
  depth-3 benchmark-style regression.
...
- Stochastic accepted-count handoff, 2026-06-07: the explicit
  `vllm_style_spec_decode` candidate now covers SingleDevice CUDA/ROCm
  stochastic verifier rows. The runner preserves the base terminal hidden,
  publishes accepted-prefix GDN/short-conv state from metadata, rebuilds shifted
  MTP rows from the preserved/current terminal hidden sources, forwards any
  residual correction suffix, and samples the stochastic ready token after the
  suffix has advanced main state. Ready sampled tokens carry the exact
  `SamplingParams` snapshot that produced them; consuming a ready token after
  sampling parameters change hard-fails before state mutation. Focused
...
- Prefix-restore equivalence, 2026-06-07: dense SingleDevice CUDA and ROCm now
  ...
  prefix-cache full hit at draft depth 3, proving terminal logits, terminal
  hidden, MTP state restore, chained sidecar drafts, and Phase 13.8 transaction
  publication with zero validation failures. `V2_Unit_PrefixCachePrefillFlow`
  now pins the same full-hit chained-depth contract in the CPU-side prefill
  coordinator.
- Focused 2026-06-07 regression gate passed for prefix-flow unit coverage,
  prefill/decode transition coverage, CUDA/ROCm stochastic parity, CUDA/ROCm
  candidate equivalence, and CUDA/ROCm depth-3 prefix-restore candidate
  equivalence. `V2_Unit_PrefixCachePrefillFlow` now asserts both the fast
  rejected chained-draft path and the full-hit terminal-restore chained-depth
  path avoid full live-checkpoint rollback.
- Current hard gate: the named `vllm_style_spec_decode` hook may still run only
  under explicit equivalence/development knobs until dense default-lane greedy
  and stochastic benchmarks show comparable CUDA/ROCm wins. Plain/default use
  should hard-fail instead of silently falling back or publishing partial state.
  Always-on parity must continue asserting transaction activity and zero
  validation failures before any benchmark claim.
- Next implementation slice:
  1. run fresh CUDA/ROCm greedy and stochastic default-lane benchmarks;
  2. remove or rename remaining "correction replay" terminology once the suffix
     forward plan is stable, so docs/counters do not suggest a fallback;
  3. promote the hook only if correctness gates and benchmark wins both hold.
...
```bash
cmake --build build_v2_integration --parallel
ctest --test-dir build_v2_integration -R "^V2_Unit_MTPSpecDecodeMetadata$|^V2_Unit_MTPSpecDecodeTransaction$|^V2_Unit_MTPDecodeCatchup$|^V2_Unit_PrefillDecodeTransition$|^V2_Integration_CUDAGDNPaddedRealLength$|^V2_Integration_ROCmGDNPaddedRealLength$|^V2_Integration_Parity_Qwen36_.*(Phase138VllmStyleCandidateEquivalence|Phase138VllmStyleCandidatePrefixRestoreEquivalence|Phase138VllmStyleCandidateStopTokenEquivalence|Phase138VllmStyleCandidateContinuationEquivalence|MTPStochasticSamplingVerifierRuns|OneRowRestoreLongPrefixMatchesSequential)" --output-on-failure --parallel
```
...
./build_v2_release/llaminar2 benchmark -m /opt/llaminar-models/Qwen3.6-27B-Q4_K_S.gguf -d rocm:0 -c 64 -n 48 --mtp --mtp-draft-tokens 3 --benchmark-json-output /tmp/llaminar-mtp-bench/dense-rocm-phase138-vllm-style-d3.json
```
