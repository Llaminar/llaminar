...
| Phase 13.5: Small-M GEMV-Many Kernel Prerequisite | In progress | Phase 14 speedup work exposed that correctness-green graph capture is not enough: ROCm SingleDevice dense Qwen3.6 was speed-negative because the graph-captured verifier replay was dominated by small-M GEMV/GDN/GEMM work. ROCm now has a graph-capturable native-VNNI small-M verifier route for M=2/3/4 across Q/K/IQ codebooks, fused QKV/GateUp shared-quant dispatch has a graph-native same-codebook batched route with optional bias support, GDN qkv/z same-codebook subgroups share activation quantization and use a generic batched route for Qwen3.6 heterogeneous-N pairs, and fused-SwiGLU/FFN down now routes eligible M=2/3/4 native-VNNI verifier shapes through the graph-native small-M path instead of generic prefill GEMM. `V2_Integration_ROCmQuantisedGemmSmallM` passed on 2026-06-02 with Q8 plus all native Q/K/IQ codebooks, graph-captured Q4_K/IQ3_S M=3/4, route counters, fused shared-quant counters, batched fused-QKV counters, Qwen3.6-scale QKV, repeated graph replay, graph-captured Qwen3.6 GDN projection groups bound through one shared workspace, Qwen3.6 qkv/z `N={10240,6144}` heterogeneous-N batched route coverage, fused-SwiGLU/FFN down route counters for Q4_K M=2 and Q5_K M=4, graph-captured Qwen3.6-scale FFN-down and GDN M=4 routes, hard-fail regressions for unsafe small-M split-K overrides, and launch-counter regressions proving graph-enabled small-M M=2 routes use direct `kb=1` rather than split-reduce. `V2_Unit_StageTimeline` now guards the MTP sidecar timeline path against stale/unrecorded graph-capture events after a real ROCm stage-timing HSA fault repro. Release perf `NativeVNNIGEMMPerfTest.MTP_SmallM_VerifierShapes_AllFormats` passed on 2026-06-02 for Qwen3.6 hidden, FFN down, GDN inner, GDN time, and GDN output projection shapes across M=2/3/4; native codebook speedups versus INT8 were 0.93x-4.91x with cosine >= 0.999961. `NativeVNNIGEMMPerfTest.MTP_SmallM_DirectPrefillRouteComparison` rejected direct native prefill GEMM as a shortcut: it was correctness-identical but 3.4x-7.7x slower on hot FFN down and GDN output shapes. A focused ROCm verifier replay sweep rejected the uncommitted FP32 alpha/beta small-N projection batch after a real Qwen3.6 HSA fault, and confirmed that smaller native-VNNI small-M split-K values are worse in non-graph diagnostics: `KB=1/2/4/8` produced about 1103.60/892.26/828.80/805.90 ms total `mtp.verifier_forward` time for 12 calls. A later default graph-enabled smoke exposed that even graph-safe capped split-reduce can fault under full-model ROCm graph replay, so graph-enabled small-M now uses direct `kb=1` and explicit divergent KB overrides hard-fail. The confirming real Qwen3.6 ROCm smoke completed at 16.95 tok/s with 75% acceptance and 0 split-reduce launch counters, but `mtp.verifier_forward` rose to about 86.87 ms/call; this is hardening evidence, not speedup evidence. The fused-SwiGLU/FFN down slice produced the first ROCm SingleDevice dense speed-positive Qwen3.6 evidence before this safety clamp: same-binary `The quick brown fox`, `-c 64`, `-n 16`, GPU graphs baseline 18.47 tok/s versus MTP 25.51 tok/s, 87.5% acceptance, and `main_verifier` graph replay reduced from about 98-99 ms to about 56.3 ms per two-token graph; the shorter `-n 8` smoke was 19.06 tok/s baseline versus MTP 22.19 tok/s. A later heterogeneous-N GDN qkv/z batched smoke completed at 22.35 tok/s and moved `main_verifier` GDN projection from 95.90 ms to 91.65 ms total versus the earlier safe mixed-codebook diagnostic. CUDA has a narrower M=2 route with direct SingleDevice dense/MoE speedups. | Keep ROCm SingleDevice dense in Phase 13.5 until the remaining verifier projection buckets and longer prompts are characterized, then make ROCm split-reduce or an equivalent graph-capturable reduction real-model-safe, and port the same M=2/3/4 quantize-once GEMV-many contract and first-class perf rows to CUDA and CPU before returning to Phase 14 acceptance. |
