...
Latest ROCm stochastic top-k note, 2026-06-08: the ROCm graph-captured top-k/top-p sampler now uses arena-declared scratch for a two-pass Qwen-style small-k distribution builder when `top_k<=32`, instead of scanning the full 248k vocab in one block. `V2_Integration_GPUSamplingKernels` includes a Qwen3.6 vocab-size `top_k=20` graph-captured distribution-plus-sample regression against shared CPU sampler math, and the ROCm stochastic graph smoke now asserts the small-k scratch counter. Focused validation passed for that suite, the Qwen3.6 ROCm stochastic PyTorch parity cell, and the ROCm stochastic graph smoke. Fresh Qwen3.6 27B ROCm stochastic benchmarks reached 233.00 prefill / 30.06 decode tok/s for no-MTP, 215.89 / 35.89 for fixed depth 1 with 95.31% acceptance, and 215.06 / 35.40 for dynamic depth. `sample_mtp_token_stochastic_device` fell to about 3.36 ms and `phase138_stochastic_verify_batch_device` to about 2.42 ms; `mtp.verifier_forward` remains the dominant 41.33 ms target.
...
| Phase 13.8: vLLM-Style Accepted-Count Spec-Decode State | Implementation active; ROCm stochastic top-k speed-positive | `MTPSpecDecodeTransaction`, `MTPSpecDecodeMetadata`, graph-facing accepted-state metadata buffers, explicit-stream metadata upload, runner-owned speculative state-slot workspace, and transaction counters are implemented. CUDA and ROCm GDN/short-conv verifier capture use runner-declared speculative state-slot/work-state buffers through `IWorkspaceConsumer`, hard-fail on missing explicit stream/workspace, leave live state untouched before publication, and still prove plain non-verifier recurrence/conv state matches serial decode. The old env-selected raw all-position greedy candidate and its equivalence/parity CTest fixtures are removed after ROCm benchmark-style equivalence failed; greedy MTP remains on the shared stepwise verifier. Stochastic MTP uses the retained accepted-count publication capability, records `phase138_stochastic_spec_decode_runs` / `accepted_count_spec_decode`, and batches CUDA/ROCm compact-distribution accept decisions plus candidate residual correction tokens for draft rows. ROCm now also specializes graph-captured `top_k<=32` stochastic distribution building with arena-declared scratch, covers Qwen3.6 vocab-size `top_k=20` distribution/sample parity, and asserts the small-k scratch path in the ROCm graph smoke. Focused validation passed on 2026-06-08 for the runner unit, CUDA/ROCm GPU sampling integration, CUDA/ROCm stochastic graph smokes, and Qwen3.6 stochastic parity cells. CUDA dense default stochastic remains speed-negative at 36.82 tok/s; ROCm fixed-depth-1 stochastic now reaches 35.89 tok/s versus a 30.06 tok/s stochastic no-MTP baseline, but Phase 13.8 still needs comparable CUDA/ROCm accepted wins and lower verifier-forward cost before benchmark acceptance. | Implement the next non-dead-end speed path or tune verifier cost/acceptance behavior until CUDA and ROCm post comparable accepted MTP wins. |
...
- ROCm stochastic sampling now has an arena-declared two-pass small-k graph-captured distribution builder for `top_k<=32`, covered by a Qwen3.6 vocab-size `top_k=20` CPU/GPU parity regression and a ROCm graph-smoke scratch-path counter assertion. Fixed-depth-1 stochastic dense improved to 215.89 prefill / 35.89 decode tok/s versus a 30.06 tok/s stochastic no-MTP baseline, while dynamic reached 215.06 / 35.40. Sampler sync is down to about 3.36 ms/step, stochastic verify-batch sync to about 2.42 ms, and verifier forward remains the dominant 41.33 ms target.
...
- Next implementation slice:
  ...
  2. reduce M=2 verifier graph cost for stochastic depth-1/dynamic lanes, starting with GEMM, fused gate/up, GDN projection, and GDN recurrence counters before using stochastic MTP in Phase 14 dense acceptance;
