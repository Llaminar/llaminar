...
12. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with GDN projection and ordinary GEMM buckets, then repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract to CUDA and CPU before returning to Phase 14 acceptance.
