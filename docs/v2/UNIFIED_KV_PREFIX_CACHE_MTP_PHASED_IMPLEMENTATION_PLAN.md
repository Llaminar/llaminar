...
| Phase 13.5: Small-M GEMV-Many Kernel Prerequisite | In progress | Phase 14 speedup work exposed that correctness-green graph capture is not enough: ROCm SingleDevice dense Qwen3.6 remains speed-negative because the graph-captured verifier replay is still dominated by small-M GEMV/GDN/GEMM work. ROCm now has a graph-capturable native-VNNI small-M verifier route for M=2/3/4 across Q/K/IQ codebooks, and fused QKV/GateUp shared-quant dispatch now has a graph-native same-codebook batched route with optional bias support instead of falling back to per-projection prefill. `V2_Integration_ROCmQuantisedGemmSmallM` passed on 2026-06-02 with Q8 plus all native Q/K/IQ codebooks, graph-captured Q4_K/IQ3_S M=3/4, route counters, fused shared-quant counters, batched fused-QKV counters, Qwen3.6-scale QKV, repeated graph replay, a graph-captured Qwen3.6 GDN projection group bound through one shared workspace, and a hard-fail regression for unsafe small-M split-K overrides. Release perf `NativeVNNIGEMMPerfTest.MTP_SmallM_VerifierShapes_AllFormats` passed on 2026-06-02 for Qwen3.6 hidden, GDN inner, GDN time, and GDN output projection shapes across M=2/3/4; native codebook speedups versus INT8 were 1.15x-4.56x with cosine >= 0.999961. A real Qwen3.6 dense ROCm rerun improved clean MTP from the previous 15.39 tok/s best to 16.21 tok/s, with real-model counters proving the new route before the batched fused-projection slice, but the clean baseline is still 18.96 tok/s and verifier replay still dominates at about 99.5 ms per two-token graph. The batched fused-projection slice exposed and fixed an ROCm shared-workspace split-K partial-buffer alias: each ROCm GEMM kernel now receives a per-instance `ROCM_SCATTER_PARTIAL_*` workspace slice. The following real-model smoke exposed a separate unstable high split-K policy: `KB=32` reproduced the HSA fault, while `KB=1/2/4/8/16` were stable, so the M=2/3/4 native small-M launchers now cap default graph-safe split-K at 8 and hard-fail unsafe overrides. Current same-binary Qwen3.6 ROCm evidence is stable through `-c 64 -n 16` with GPU graphs and no serialization: baseline 18.07 tok/s, MTP 15.27 tok/s, 87.5% acceptance, and real counters proving batched Gate/Up, QKV, and GDN-style projection groups. A follow-up Q4_0/Q4_1 M=2 HIP specialization for single and batched native-VNNI small-M paths was graph-capture stable and marginally improved the short Qwen3.6 MTP smoke from 13.96 to 14.02 tok/s while leaving `main_verifier` at about 98 ms per two-token graph. ROCm MTP sidecar shifted-prefill/decode no longer force-recaptures; `V2_Unit_MTPGraphConstruction` now requires normal sidecar replay and a real Qwen3.6 ROCm `-c 64 -n 8` smoke passed with `force_recapture=false`, but throughput stayed 14.04 tok/s because `main_verifier` replay remains about 97.95 ms per two-token graph. Phase 13.5 therefore continues to focus on captured verifier GPU work in GDN projection, fused QKV/GateUp, FFN down, LM head, and MoE projection groups. CUDA has a narrower M=2 route with direct SingleDevice dense/MoE speedups. | Continue reducing captured ROCm verifier GPU work until the real-model route beats baseline; then port the same M=2/3/4 quantize-once GEMV-many contract and first-class perf rows to CUDA and CPU before returning to Phase 14 acceptance. |
...
10. Next: reduce the captured ROCm verifier graph's remaining GPU work in ordinary small-M GEMV groups, especially GDN projection and FFN down/out shapes; if GDN recurrence or short-conv remains dominant, tune those as adjacent graph-captured small-batch kernels with their own tests. Then re-run same-binary Qwen3.6 ROCm dense baseline/MTP benchmarks before returning to Phase 14 acceptance.
