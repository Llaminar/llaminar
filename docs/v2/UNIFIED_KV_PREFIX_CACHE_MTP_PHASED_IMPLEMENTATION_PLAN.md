...
53. Next: shrink the remaining CUDA MoE verifier GPU budget, especially grouped down projection, shared expert, router/GDN costs, and prompt-prefill overhead, without reintroducing rejected router/projection experiments. CUDA MoE is now speed-positive but still short of the 1.3x-1.8x MTP target; ROCm MoE keeps the current 2.13x fixed-depth ratchet. After the verifier budget approaches the target, repeat longer-prompt dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
