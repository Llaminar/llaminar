...
Latest adaptive MTP depth implementation note, 2026-06-03: `MTPDepthController` is implemented for fixed, observe, and SingleDevice dynamic modes, with CLI/config plumbing, request summaries, benchmark JSON fields, and the reusable `scripts/run_mtp_depth_hysteresis_sweep.sh` harness. The sweep harness now includes a `code` case set with Python dataclass/CLI generation and C++ controller/test-generation prompts so deeper-draft tuning covers repetitive code output as well as prose. The latest tuning slice adds early demotion after `min_samples` when a partial window already crosses zero-accept or low-acceptance demotion thresholds; promotions still require a full window and cooldown. Focused validation passed: `V2_Unit_MTPDepthController`, `V2_Unit_PrefillDecodeTransition`, `V2_Unit_BenchmarkRunnerCPU`, and the Qwen3.6 ROCm parity cells `MTPGreedyDynamicDepthMatchesPyTorchDecodeTokens` and `PrefixCacheMTPDynamicDepthRestore`. Current ROCm Qwen3.6 dense sweeps show dynamic mode remains tied with fixed depth 3 on high-acceptance `qbf_short` at 52.70 tok/s versus 52.91 tok/s, improves the default prompt from the prior 33.50 tok/s dynamic capture to 37.81 tok/s and beats the same-sweep fixed depth 1/3 rows, and improves `qbf_long` dynamic to 41.50 tok/s while still trailing fixed depth 1 due to promotion/demotion oscillation. Next adaptive slice: tune promotion hysteresis across prose and code-generation lanes, then add domain-common decisions for LocalTP/NodeLocalTP/ExpertParallel.
...
33. Next: shrink the remaining ROCm verifier GPU budget, starting with ordinary GEMM, fused Gate/Up, GDN projection/recurrence, and LM head without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget approaches the 2x target, repeat longer-prompt ROCm dense evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
