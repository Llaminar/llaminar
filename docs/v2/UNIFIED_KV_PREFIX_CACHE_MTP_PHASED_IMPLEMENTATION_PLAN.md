...
35. Next: shrink the remaining ROCm MoE verifier GPU budget, starting with main-verifier router/expert execution and then ordinary GEMM, fused Gate/Up, GDN projection/recurrence, and LM head without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget approaches the 2x target, repeat longer-prompt ROCm dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
