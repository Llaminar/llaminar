...
| Phase 13.8: vLLM-Style Accepted-Count Spec-Decode State | Implementation active; dead greedy candidate removed | `MTPSpecDecodeTransaction`, `MTPSpecDecodeMetadata`, graph-facing accepted-state metadata buffers, explicit-stream metadata upload, runner-owned speculative state-slot workspace, and transaction counters are implemented. CUDA and ROCm GDN/short-conv verifier capture use runner-declared speculative state-slot/work-state buffers through `IWorkspaceConsumer`, hard-fail on missing explicit stream/workspace, leave live state untouched before publication, and still prove plain non-verifier recurrence/conv state matches serial decode. The old env-selected raw all-position greedy candidate and its equivalence/parity CTest fixtures are removed after ROCm benchmark-style equivalence failed; greedy MTP remains on the shared stepwise verifier. Stochastic MTP uses the retained accepted-count publication capability and records `phase138_stochastic_spec_decode_runs` / `accepted_count_spec_decode`. | Rebuild and rerun focused unit/parity gates after cleanup, then implement a new non-dead-end speed path or tune the shared verifier until CUDA and ROCm post comparable accepted MTP wins. |
...
- Focused validation already associated with the retained pieces includes `V2_Unit_MTPSpecDecodeMetadata`, `V2_Unit_MTPSpecDecodeTransaction`, `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_MTPStateTransaction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, and CUDA/ROCm `MTPStochasticSamplingVerifierRuns`.
...
- Next implementation slice:
  1. rebuild and rerun the focused unit/parity gates after the dead-code cleanup;
...
```bash
cmake --build build_v2_integration --parallel
ctest --test-dir build_v2_integration -R "^V2_Unit_MTPSpecDecodeMetadata$|^V2_Unit_MTPSpecDecodeTransaction$|^V2_Unit_MTPDecodeCatchup$|^V2_Unit_MTPGraphConstruction$|^V2_Unit_MTPStateTransaction$|^V2_Unit_PrefillDecodeTransition$|^V2_Integration_CUDAGDNPaddedRealLength$|^V2_Integration_ROCmGDNPaddedRealLength$|^V2_Integration_Parity_Qwen36_.*(MTPBenchmarkPromptFixedDepth3MatchesPyTorchDecodeTokens|MTPStochasticSamplingVerifierRuns|MTPFirstTransactionLeavesSequentialState|OneRowRestoreLongPrefixMatchesSequential)" --output-on-failure --parallel
```
