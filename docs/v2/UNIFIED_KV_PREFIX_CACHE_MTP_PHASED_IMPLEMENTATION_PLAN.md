...
- Retained implementation work:
  ...
  - `MTPSpecDecodeMetadata` remains a transaction-shape validator for accepted counts, state slots, token indices, query starts, and bonus-ready-token metadata. Its old runner-owned live-publication workspace is removed.
...
- Current hard gate: no optimized stateful verifier path is promoted. A new path must be added as shared transaction semantics first, then proven against `shared_stepwise` and PyTorch parity on CUDA and ROCm before any benchmark number is accepted.
- Next implementation slice:
  1. keep stateful greedy and stochastic MTP on `shared_stepwise`/decode-equivalent verification while designing the next non-dead-end speed path, likely device-side draft-input update plus captured repeated verifier/drafter metadata rather than selected-row live-state publication;
