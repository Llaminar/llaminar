...
| Phase 14: Benchmark Acceptance And Default-Enablement Readiness | Waiting on Phase 13.5 and Phase 13.7 | `docs/v2/PREFIX_CACHE_MTP_BENCHMARK_NOTES.md` tracks real Qwen3.6 dense/MoE baseline, graph-capture status, and best observed MTP speedups by domain. CUDA SingleDevice dense currently has direct Qwen3.6 production-mode evidence on `The quick brown fox`, `-c 64 -n 48`: depth-1 MTP reaches 53.30 tok/s versus 40.75 tok/s baseline with 95.83% acceptance; depth-2/3 remain slower at 33.20/31.29 tok/s because verifier cost dominates. CUDA SingleDevice MoE remains performance-negative on the default lane, but the verifier-row shortcut is now restored for release builds: fixed-depth-1 MTP reaches 83.10 tok/s with 75.78% acceptance versus a 102.03 tok/s no-MTP same-binary refresh at `-c 768`, `-n 64`; the old pre-shortcut ratchet was 65.93 tok/s. ROCm SingleDevice dense now has stronger long-lane Qwen3.6 dense evidence after the tiny FP32 GDN alpha/beta route, GDN verifier-row shortcut regressions, safe arena-write replay caching, warmup-dependent capture planning, legacy resegment-path removal, batched verifier-backed shifted-cache catchup, and verifier-row batched argmax: 55.04 tok/s depth-3 MTP versus 30.64 same-binary baseline at `-c 64 -n 48`, with 85.61% acceptance. ROCm SingleDevice MoE has fresh default-lane speed-positive proof after grouped-decode runtime-table rewarm, streamful prefix terminal-state restore, capture-safe ROCm attention params, verifier-sized route grouping, and grouped/LDS small-M router logits: fixed depth 1 reached 39.94 tok/s versus 19.72 tok/s baseline at `-c 768`, `-n 64`, with 78.12% acceptance; dynamic max-depth 3 previously demoted to depth 1 and reached 37.82 tok/s. `V2_Integration_PrefixCacheMTP_Qwen36ROCmLocalPPHardFail` pins the current LocalPP matrix blocker as an early real-model hard fail with zero MTP draft/verifier counters, preventing recurrence of the old late stage-1 shifted-cache crash while the real PP-aware MTP path is still unimplemented. | Do not use Phase 14 to paper over kernel deficits. Resume benchmark acceptance after Phase 13.5 provides graph-capturable M=2/3/4 GEMV-many coverage and Phase 13.7 proves stochastic MTP correctness for non-greedy chat defaults; then continue matrix-driven benchmark work until remaining supported domains show concrete speedups or documented blockers with traces. |

Latest CUDA validation note, 2026-06-04: deep Qwen3.6 MoE CUDA math parity passed for `V2_Integration_Parity_Qwen36MoE_SingleDevice_Math_{SnapshotInfrastructure,DecodeParity,PrefillParity}_CUDA`; focused reruns after the small-M attention, GDN qkv+z fusion, and opt-in shared-expert grouped table fixes passed the CUDA `DecodeParity` and `PrefillParity` cells. CUDA dense prefix/MTP parity passed for all six `V2_Integration_Parity_Qwen36_CUDA_SingleDevice_` cases, including stochastic verifier coverage, and CUDA MoE prefix/MTP parity passed for all six `V2_Integration_Parity_Qwen36MoE_CUDA_SingleDevice_` cases. `V2_Integration_CUDAGemmParity` now includes `Q4_0_PrefillGraphReplaySurvivesKernelDynamicReset`, which captures the two-pass Stream-K prefill graph on an explicit nonblocking CUDA stream, resets request-scoped kernel state, then replays the graph to prove context-owned GEMM scratch is not freed while graph cache entries survive `clear_cache()`. It also includes `GDNProjectionStageFusesCUDAQuantizedQKVAndZSmallM`, which runs a CUDA GDN projection stage on an explicit nonblocking stream and asserts the qkv+z native subgroup route plus the small-M fused projection counter. `V2_Integration_CUDAGDNPaddedRealLength` now directly covers CUDA verifier-row GDN equivalence: two-row recurrence and short-conv row-0 restore must match an independent one-row replay. `V2_Integration_CUDAMoEKernel` now includes `StaticGroupedDecodeDescriptorPathCapturesAfterWarmup`, proving CUDA shared-expert style table decode can capture after explicit-stream warmup without metadata/pointer cache misses. Focused static/kernel validation also passed through `V2_Unit_Static_NoDefaultStreamInGPUCode`, `V2_Integration_CUDAMoEKernel`, and `V2_Integration_CUDAFlashAttentionParity`. The release Qwen3.6 MoE CUDA c768/n1 benchmark smoke that previously crashed after prefill graph replay now completes, recording 595-token prefill at 892.08 tok/s. The CUDA GDN padded-real-length test crash was root-caused to CUDA-only tests initializing ROCm through `DeviceManager`; `CUDATestBase` now skips ROCm startup, `V2_Integration_CUDAGDNPaddedRealLength` passes without HIP finalizer aborts, and a short CUDA MoE MTP smoke records verifier-row restores plus rollback shortcuts with no restore-failure or replay-forward counters. A later release-only stale-token failure was root-caused to MoE expert decode consuming a runtime-table bank initialized after routing for the same pass; the expert stage now only uses a bank that was active before the stage began. `V2_Integration_Parity_Qwen36MoE_CUDA_SingleDevice_Qwen36MoECUDASingleDevicePrefixMTPParity_VerifierRowShortcutTwoRowStateMatchesFullReplay` locks the two-row real-model shortcut equivalence, and the release default lane reaches 83.10 tok/s fixed-depth-1 MTP versus a 102.03 tok/s no-MTP refresh.
...
51. Next: recover graph-safe prefill reuse across request resets, then shrink the remaining verifier GPU budget starting with routed MoE router/expert execution and then ordinary GEMM, fused Gate/Up, remaining GDN recurrence/projection costs, and LM head without reintroducing rejected router/projection experiments. CUDA MoE needs verifier-economics work before any MTP default or speedup claim; ROCm MoE keeps the current 2.13x fixed-depth ratchet. After the verifier budget approaches the target, repeat longer-prompt dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
