...
| Phase 13.5: Small-M GEMV-Many Kernel Prerequisite | In progress | ROCm now has graph-capturable native-VNNI small-M verifier routes for M=2/3/4 across Q/K/IQ codebooks, fused QKV/GateUp shared-quant dispatch, same-codebook GDN qkv/z batching, heterogeneous-N Qwen3.6 qkv/z batching, fused-SwiGLU/FFN-down routing, declared-workspace FP32 GDN projection batching, and focused ROCm small-M integration/perf coverage. The latest hardening slice replaces the temporary graph direct `kb=1` clamp with graph-capturable atomic K-partitioning for `kb>1`; focused graph-capture tests and real Qwen3.6 ROCm smokes passed with `path=atomic_reduce` and zero split-reduce counters. Current same-binary long-lane ROCm evidence remains speed-positive: after preserving dense captured replay state across live prefix restore/truncate, depth-1 MTP reached 32.88 tok/s versus 21.02 tok/s baseline at `The quick brown fox`, `-c 64`, `-n 48`, with 95.83% acceptance; depth-2 reached 29.63 tok/s with 86.29% acceptance and 21 rollbacks. Focused unit/integration regression and all dense Qwen3.6 SingleDevice parity cases passed. A rejected deferred post-sidecar checkpoint experiment reached only 32.36 tok/s at depth 1 and collapsed depth 2 to 22.03 tok/s because rollback still paid sidecar/replay costs; the retained path keeps the eager post-sidecar checkpoint until verifier-row GDN state restore exists. CUDA has a narrower M=2 route with direct SingleDevice dense/MoE speedups. | Keep ROCm SingleDevice dense in Phase 13.5 until profiling shrinks the remaining verifier budget, especially GDN projection, ordinary GEMM, fused Gate/Up, and rollback replay. Next implement verifier-row GDN recurrence/short-conv state checkpoint restore so reject rollback can restore the accepted verifier row instead of replaying accepted tokens. Then repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract and first-class perf rows to CUDA and CPU before returning to Phase 14 acceptance. |
