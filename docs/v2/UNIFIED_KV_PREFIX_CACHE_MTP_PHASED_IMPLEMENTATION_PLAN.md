...
- Fresh retained-path ROCm evidence after removing the accepted-count live-publication candidate reached 233.17 prefill / 30.14 decode tok/s without MTP and 216.92 / 18.98 with stochastic fixed depth-1 at 82.81% acceptance. A focused sidecar-preserved-main-state ratchet lets stochastic decode-equivalent verification skip the redundant verifier-base checkpoint restore, keeping CUDA/ROCm stochastic parity green and improving ROCm stochastic fixed depth-1 to 216.79 / 24.83 at the same 82.81% acceptance. Perf counters still show `shared_stepwise_stochastic` performed 384 one-token main forwards across 192 decode iterations, so the current bottleneck is the extra decode-equivalent replay forward. The next speed path must remove that replay through a proven transaction/state-slot publication contract rather than reviving selected-row publication or tuning around a structurally doubled verifier.
