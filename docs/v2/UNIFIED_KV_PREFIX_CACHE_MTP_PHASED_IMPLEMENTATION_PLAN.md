...
| Phase 13.5: Small-M GEMV-Many Kernel Prerequisite | In progress | ROCm now has graph-capturable native-VNNI small-M verifier routes for M=2/3/4 across Q/K/IQ codebooks, fused QKV/GateUp shared-quant dispatch, same-codebook and mixed-codebook GDN projection subgroups, heterogeneous-N Qwen3.6 qkv/z batching, fused-SwiGLU/FFN-down routing, declared-workspace FP32 GDN projection batching, and focused ROCm small-M integration/perf coverage. The latest hardening slices added graph-capturable atomic K-partitioning for `kb>1`, preserved captured replay state across live prefix restore/truncate by keying decode/sidecar graph identity on the MoE placement epoch instead of blanket-resetting MoE replay state, implemented verifier-row GDN recurrence/short-conv state capture so reject rollback can restore the accepted verifier row instead of replaying accepted tokens, removed the GDN mixed-codebook batched-route bypass for native small-M-compatible subgroups, added `kernel.gdn_projection_route` counters that prove real Qwen3.6 uses qkv/z quantized pairs plus alpha/beta FP32 batched projections instead of a synthetic four-projection full mixed-codebook bundle, and added `NativeVNNIGEMMPerfTest.MTP_SmallM_BatchedGDNProjectionShapes` for Qwen3.6-style heterogeneous GDN projection groups at M=2/3/4. Current same-binary long-lane ROCm evidence remains speed-positive: depth-1 MTP reached 34.28 tok/s versus 20.82 tok/s baseline on `rocm:0` at `The quick brown fox`, `-c 64`, `-n 48`, with 94.79% acceptance, 5 rollbacks, 48 real-model mixed batched calls over 2- and 3-projection groups, zero mixed-codebook bypasses, three measured verifier-row shortcuts, and `verifier_forward` still averaging about 51.5 ms/call. Focused unit/integration/parity regression passed on 2026-06-02, including `V2_Unit_GDNKernels`, `V2_Integration_ROCmQuantisedGemmSmallM`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_MTPGreedyMatchesPyTorchDecodeTokens`, and `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_PrefixCacheMTPRestore`. CUDA has a narrower M=2 route with direct SingleDevice dense/MoE speedups. | Keep ROCm SingleDevice dense in Phase 13.5 until profiling shrinks the remaining verifier budget, especially GDN projection, ordinary GEMM, fused Gate/Up, and any remaining MTP per-step overhead. Port the M=2/3/4 quantize-once GEMV-many contract and first-class perf rows to CUDA and CPU before returning to Phase 14 acceptance. |
...
23. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with the real GDN projection route (`qkv+z` quantized pairs plus `alpha+beta` FP32 batched projection), ordinary GEMM, and fused Gate/Up without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget is back below the best speed-positive runs, repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract to CUDA and CPU before returning to Phase 14 acceptance.
