...
Latest adaptive MTP depth implementation note, 2026-06-03: `MTPDepthController` is implemented for fixed, observe, and SingleDevice dynamic modes, with CLI/config plumbing, request summaries, benchmark JSON fields, and the reusable `scripts/run_mtp_depth_hysteresis_sweep.sh` harness. Focused validation passed: `V2_Unit_MTPDepthController`, `V2_Unit_PrefixMTPConfig`, `V2_Unit_PrefillDecodeTransition`, `V2_Unit_BenchmarkRunnerCPU`, `V2_Unit_PrefixCachePrefillFlow`, and the Qwen3.6 ROCm parity cells `MTPGreedyDynamicDepthMatchesPyTorchDecodeTokens` and `PrefixCacheMTPDynamicDepthRestore`. Current ROCm Qwen3.6 dense sweeps show dynamic mode stays at depth 3 on high-acceptance `qbf_short`, demotes the default prompt to depth 1, and remains stable on `qbf_long`; whole-run speed on the default prompt still trails hard-pinned depth 1 because the controller starts at max depth and pays early learning cost. Next adaptive slice: tune initial/window policy and then add domain-common decisions for LocalTP/NodeLocalTP/ExpertParallel.
