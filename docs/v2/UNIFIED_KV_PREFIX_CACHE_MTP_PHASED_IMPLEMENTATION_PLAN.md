...
Latest current-revision ROCm SingleDevice dense batched-catchup rerun, 2026-06-03: Qwen3.6 dense 27B Q4_K_S on `rocm:0`, GPU graphs enabled, `The quick brown fox`, `-c 64`, `-n 48`, reached 54.69 tok/s decode and 54.73 tok/s overall at depth-3 MTP versus the 30.93 tok/s same-binary baseline, preserving the ROCm depth-3 ratchet at about 1.77x. Artifacts are `/tmp/llaminar-mtp-bench/dense-rocm-baseline-current-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.json`, and `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.csv`. The request summary recorded 81.02% acceptance, 111 accepted tokens, 26 rejected tokens, 27 rollbacks, 55 verifier runs, and 220 verifier tokens. Structured stats show the new batched `kv_cache_only` shifted-cache catchup path reduced measured `mtp_decode_catchup` replay from about 31.66 ms to 7.07 ms and `mtp.shifted_row_commit` from about 41.57 ms to 24.14 ms versus the prior warmup-dependent ratchet artifact, while preserving 12 verifier-row GDN rollback shortcuts and zero suffix/replay-forward counters. Remaining Phase 13.5 pressure is captured `main_verifier` GPU work, still around 44.38 ms per measured M=4 verifier graph, plus avoiding three separate host-synchronized sidecar draft steps at depth-3.
...
25. Done for the CUDA M=2/3/4 native-format fused-projection slice: `CUDAQuantisedGemmKernel::multiply_fused_tensor()` now records `kernel.cuda_native_vnni_small_m_fused_projection_calls` when the small-M verifier route quantizes activation rows once and runs multiple native-VNNI projections. `V2_Integration_CUDAGemmParity` includes `MTP_SmallM_FusedProjection_AllNativeFormats`, covering `M=2/3/4` CUDA fused projection parity against CPU references across the CUDA native codebook inventory, including Q4_K/Q5_K Qwen3.6 aliases.
...
28. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with the real GDN projection route (`qkv+z` quantized pairs plus `alpha+beta` FP32 batched projection), ordinary GEMM, and fused Gate/Up without reintroducing the rejected four-projection full mixed-codebook bundle. Also investigate collapsing depth-3 sidecar draft steps so the runner does not pay three host-synchronized sidecar graph replays per speculative step. After the ROCm verifier budget is back below the best speed-positive runs, repeat longer-prompt ROCm dense evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
