...
6. Done for the Q4_0/Q4_1 M=2 specialization slice: single and batched ROCm native-VNNI paths have a dedicated graph-capturable two-row Q4 kernel. Real Qwen3.6 dense smoke improved only marginally from 13.96 to 14.02 tok/s, so this is regression coverage rather than Phase 14 speedup evidence.
7. Next: reduce the captured ROCm verifier graph's remaining GPU work in ordinary small-M GEMV groups, especially GDN projection and FFN down/out shapes, then re-run same-binary Qwen3.6 ROCm dense baseline/MTP benchmarks before returning to Phase 14 acceptance.
