...
30. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with the real GDN projection route (`qkv+z` quantized pairs plus `alpha+beta` FP32 batched projection), ordinary GEMM, fused Gate/Up, attention, and LM head without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget approaches the 2x target, repeat longer-prompt ROCm dense evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
