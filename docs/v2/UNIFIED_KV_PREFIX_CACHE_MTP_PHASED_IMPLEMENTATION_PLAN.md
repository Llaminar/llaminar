...
| Phase 12: ExpertParallel Sparse MoE MTP Sidecar And Segmented Graph Capture | In progress | Focused Phase 12 slice passed on 2026-06-01: `V2_Unit_MTPGraphConstruction` now proves graph-native overlay MoE MTP sidecar execution appends real shifted KV payload and marks sparse manual boundaries correctly after full graph execution; `V2_Unit_MoEOverlayCollectiveWorkspace` proves non-final sparse dispatch/return participants can complete their manual graph boundary after successful publish while final participants still require collective completion; `V2_Unit_MoEExpertWeightService` proves CPU prepared expert slabs are incrementally filled for disjoint masked participants without duplicating slabs; the broader Phase 12 unit slice covering prefill graph cache, ForwardExecutionEngine chunking, MoE runtime table, decode histograms, rebalance controller, prepared-weight store resolution, and local expert prepared weights passed. Focused integration passed: `V2_Integration_MoEOverlaySparseTransport_MPI`. Real-model graph-native overlay parity passed for Qwen3.5 MoE RocmHot/CpuCold `DecodeParity` and `PrefillParity`. | Still need full Qwen3.6 MoE MTP sidecar parity for SingleDevice and ExpertParallel, heterogeneous ROCm+CPU ExpertParallel sidecar coverage, and segmented graph-capture acceptance for sparse overlay paths before Phase 12 can be marked complete. |
