...
- The same focused pass deliberately exposed two non-promoted surfaces:
...
- Next implementation gate: broaden the live candidate equivalence matrix across
  ...
  ROCm, then remove the explicit development env gate only if the promoted
  policy remains parity-green.
