...
Latest ROCm stochastic sampler note, 2026-06-08: the ROCm graph-captured top-k/top-p sampler now specializes Qwen-style small-k paths with `k<=32` local arrays instead of always using the full `k=256` scratch shape. `V2_Integration_GPUSamplingKernels` now includes a Qwen3.6 vocab-size `top_k=20` graph-captured distribution-plus-sample regression that compares compact distributions and sampled tokens against the shared CPU sampler math. Focused validation passed for that suite, the Qwen3.6 ROCm stochastic PyTorch parity cell, and the ROCm stochastic graph smoke. A fresh Qwen3.6 27B ROCm fixed-depth-1 stochastic benchmark reached 214.37 prefill / 27.62 decode tok/s with 78.12% acceptance; `sample_mtp_token_stochastic_device` improved to about 8.67 ms while `mtp.verifier_forward` remains about 41.24 ms. This is a retained ratchet but still below the 30.21 tok/s no-MTP ROCm baseline.
...
- ROCm stochastic sampling now has a small-k graph-captured kernel specialization for `top_k<=32`, covered by a Qwen3.6 vocab-size `top_k=20` CPU/GPU parity regression. The fixed-depth-1 stochastic dense run improved to 214.37 prefill / 27.62 decode tok/s; sampler cost is down to about 8.67 ms/step, and verifier forward remains the dominant 41.24 ms target.
