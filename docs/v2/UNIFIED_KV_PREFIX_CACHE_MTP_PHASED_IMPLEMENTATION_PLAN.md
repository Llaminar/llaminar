...
- Retained implementation work:
  ...
  - `IInferenceRunner` now has a distinct accepted-count publication seam, `supportsMTPSpecDecodeStatePublication()` and `publishMTPSpecDecodeState()`, which defaults to unsupported and is covered by a runner unit. This prevents new work from reusing the retired verifier-row restore API as if it were vLLM-style state publication.
