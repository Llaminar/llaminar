...
| Phase 13.5: Small-M GEMV-Many Kernel Prerequisite | In progress | ROCm now has graph-capturable native-VNNI small-M verifier routes for M=2/3/4 across Q/K/IQ codebooks, fused QKV/GateUp shared-quant dispatch, same-codebook GDN qkv/z batching, heterogeneous-N Qwen3.6 qkv/z batching, fused-SwiGLU/FFN-down routing, declared-workspace FP32 GDN projection batching, and focused ROCm small-M integration/perf coverage. The latest hardening slice replaces the temporary graph direct `kb=1` clamp with graph-capturable atomic K-partitioning for `kb>1`; focused graph-capture tests and a real Qwen3.6 ROCm `-c 64`, `-n 8` smoke passed with `path=atomic_reduce` and zero split-reduce counters. Best ROCm SingleDevice dense evidence remains speed-positive but below target: 33.57 tok/s depth-1 MTP versus 21.19 tok/s baseline at `-c 64`, `-n 48`, and 25.51 tok/s versus 18.47 tok/s at `-c 64`, `-n 16`. The graph-atomic short smoke reached 17.32 tok/s versus a 21.42 tok/s short baseline, so it is graph-safety/profiling evidence rather than Phase 14 speedup evidence. CUDA has a narrower M=2 route with direct SingleDevice dense/MoE speedups. | Keep ROCm SingleDevice dense in Phase 13.5 until fresh same-binary profiling shrinks the remaining verifier budget, especially GDN projection, ordinary GEMM, and fused Gate/Up. Then repeat longer-prompt ROCm dense evidence and port the M=2/3/4 quantize-once GEMV-many contract and first-class perf rows to CUDA and CPU before returning to Phase 14 acceptance. |
