...
1. Shortcut candidate research.
   ...
   - Candidate E: accepted-count-aware stateful verifier. Inspired by vLLM's GDN/Mamba spec-decode contract, make accepted-token count a first-class input to stateful attention/short-conv update, so a verifier can compute `1 + draft_count` rows while committing only the accepted recurrent and convolution state. This candidate must be implemented behind the shared runner hook, must keep all tensor movement/workspace allocation graph-native, and must pass commit-replay tests on both CUDA and ROCm before any benchmark claim.
...
- No backend optimized catch-up hook is promoted. Candidate C is retained as a capture/host-overhead scaffold, but the active shortcut research target is Candidate E: an accepted-count-aware stateful verifier that commits only accepted GDN/short-conv state while preserving the shared `MTPDecodeCatchup` observable contract. Phase 13.8 remains open until a shortcut candidate is proven and promoted for CUDA/ROCm or every plausible candidate is explicitly retired with evidence.
