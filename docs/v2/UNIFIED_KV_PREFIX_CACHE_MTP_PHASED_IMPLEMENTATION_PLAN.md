...
25. Next: characterize and shrink the remaining ROCm verifier projection budget, starting with the real GDN projection route (`qkv+z` quantized pairs plus `alpha+beta` FP32 batched projection), ordinary GEMM, and fused Gate/Up without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget is back below the best speed-positive runs, repeat longer-prompt ROCm dense evidence, finish CUDA M=3/4 all-codebook coverage, and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
