...
76. Done for bucket-aligned ROCm prefill trainer generation: ROCm NativeVNNI prefill sweeps now use the same shared `defaultNativeVNNIDispatchTrainingRows()` policy as CUDA, including `M={2,3,4}` verifier rows and graph-prefill buckets such as `M=600`. `validate_rocm_native_vnni_trainer_csv.py` now reads the C++ bucket policy and can require policy-aligned prefill rows, the ROCm fixture includes a Qwen3.6 dense `M=600` row, and `analyze_rocm_native_vnni_trainer.py` emits a generated C++ tuning artifact from correct/best ROCm trainer rows with canonical codebook labels. Focused validation passed for `V2_Unit_ROCmNativeVNNITrainerCsvValidator`, `V2_Unit_ROCmNativeVNNITrainerGenerator`, and `V2_Unit_NativeVNNIGeneratedDispatchCodebooks`; the release `v2_perf_native_vnni_sweep` target built, and a real ROCm policy smoke generated and validated a `Q4_0`, `Qwen36_GDN_TimeProjection`, `M=600` trainer artifact at 501.279 us with cosine 0.999993.
