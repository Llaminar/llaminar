...
35. Done for ROCm MoE small-M row-wise verifier routing: FP32 MoE router prefill for verifier-sized `M=2/3/4` now bypasses the tiny hipBLAS GEMM path and launches the existing explicit-stream decode gate-logit kernel per row before the batched top-k path. `V2_Integration_ROCmMoEKernel` adds `SmallMRowwiseRouter_VerifierShapeMatchesHostReference`, which asserts the live counter and compares route ids/weights against a CPU softmax/top-k reference. Focused validation passed for `V2_Integration_ROCmMoEKernel`, `V2_Unit_Static_NoDefaultStreamInGPUCode`, Qwen3.6 MoE ROCm `MTPGreedyMatchesPyTorchDecodeTokens`/`PrefixCacheMTPRestore`, full Qwen3.6 MoE ROCm deep math parity prefill/decode/snapshot tests, and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsSmoke`. The real Qwen3.6 MoE ROCm fixed-depth-1 lane reached 39.10 tok/s versus the 19.72 baseline at `-c 768 -n 64`, about 1.98x, with 1760 row-wise-router calls and 1760 small-grouping calls recorded.
36. Next: shrink the remaining ROCm MoE verifier GPU budget, starting with expert execution and then ordinary GEMM, fused Gate/Up, GDN projection/recurrence, and LM head without reintroducing the rejected four-projection full mixed-codebook bundle. After the ROCm verifier budget approaches the 2x target, repeat longer-prompt ROCm dense/MoE evidence and run CPU real-model MTP benchmarks before returning to Phase 14 acceptance.
