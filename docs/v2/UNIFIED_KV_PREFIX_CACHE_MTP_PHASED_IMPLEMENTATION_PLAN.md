...
Latest Phase 13.8 stochastic accept batching note, 2026-06-08: CUDA and ROCm now share a graph-capturable batched compact-distribution accept-test hook for stochastic MTP. `OrchestrationRunner` precomputes draft-token accept thresholds from a copied sampler state, enqueues one device accept-test launch for rows 1..N, then advances the real sampler in the original order and falls back to the existing single-row residual verifier only for the first rejected row. This reduces verifier sampling launch count without changing stochastic RNG semantics or promoting the retired greedy selected-row state shortcut. Focused validation passed for `V2_Unit_PrefillDecodeTransition`, `V2_Integration_GPUSamplingKernels`, `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_MTPStochasticSamplingVerifierRuns`, `V2_Integration_Parity_Qwen36_CUDA_SingleDevice_Qwen36CUDASingleDevicePrefixMTPParity_MTPStochasticSamplingVerifierRuns`, `V2_Integration_PrefixCacheMTP_Qwen36CUDAGpuGraphsStochasticSmoke`, and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsStochasticSmoke`.

Latest Phase 13.8 stochastic benchmark note, 2026-06-08: post-batch retained stochastic MTP is still speed-negative on the dense default 595-token prompt. CUDA Qwen3.6 27B Q4_K_S reached 726.94 prefill / 36.14 decode tok/s with 56.47% stochastic acceptance, 43 verifier runs, 171 verifier tokens, 85 accept tests, and 37 residual samples. ROCm reached 218.17 prefill / 20.33 decode tok/s with 60.00% acceptance, 42 verifier runs, 168 verifier tokens, 85 accept tests, and 34 residual samples. This proves the accept-test batching slice is not the remaining default-lane bottleneck; Phase 13.8 should next reduce verifier/residual work or improve acceptance before Phase 14 claims dense default MTP speedups.
...
| Phase 13.8: vLLM-Style Accepted-Count Spec-Decode State | Implementation active; stochastic accept batching verified | `MTPSpecDecodeTransaction`, `MTPSpecDecodeMetadata`, graph-facing accepted-state metadata buffers, explicit-stream metadata upload, runner-owned speculative state-slot workspace, and transaction counters are implemented. CUDA and ROCm GDN/short-conv verifier capture use runner-declared speculative state-slot/work-state buffers through `IWorkspaceConsumer`, hard-fail on missing explicit stream/workspace, leave live state untouched before publication, and still prove plain non-verifier recurrence/conv state matches serial decode. The old env-selected raw all-position greedy candidate and its equivalence/parity CTest fixtures are removed after ROCm benchmark-style equivalence failed; greedy MTP remains on the shared stepwise verifier. Stochastic MTP uses the retained accepted-count publication capability, records `phase138_stochastic_spec_decode_runs` / `accepted_count_spec_decode`, and now batches CUDA/ROCm compact-distribution accept tests for draft rows before using the existing single-row residual verifier on reject. Focused validation passed on 2026-06-08 for the runner unit, CUDA/ROCm GPU sampling integration, CUDA/ROCm stochastic graph smokes, and Qwen3.6 stochastic parity cells. Post-batch dense default benchmarks show CUDA 36.14 decode tok/s and ROCm 20.33 decode tok/s, both speed-negative versus their no-MTP baselines. | Implement the next non-dead-end speed path or tune the shared verifier/residual path until CUDA and ROCm post comparable accepted MTP wins. |
...
- Latest retained speed-path slice: stochastic MTP now batches CUDA/ROCm compact-distribution accept tests over draft rows in one graph-capturable backend call. The runner draws accept thresholds from a copied sampler state for batch enqueue, advances the real sampler in the same order during row processing, and still uses the single-row verifier to sample the residual correction for the first rejected row.
- Post-batch dense default benchmarks show this was useful plumbing but not the dominant performance issue: CUDA reached 36.14 decode tok/s and ROCm reached 20.33 decode tok/s with about 56-60% acceptance. The next speed work should target verifier/residual cost or acceptance behavior, not accept-test launch count.
...
- Next implementation slice:
  ...
  2. reduce stochastic residual/verifier work or improve default-prompt acceptance before using stochastic MTP in Phase 14 dense acceptance;
