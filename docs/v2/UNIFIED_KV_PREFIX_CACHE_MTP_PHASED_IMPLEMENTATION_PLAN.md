...
Latest Phase 13.8 cleanup note, 2026-06-07: the env-selected raw all-position greedy catch-up candidate has been removed from `DeviceGraphOrchestrator`, `IInferenceRunner`, CTest registrations, unit tests, and Qwen3.6 parity fixtures. It was a dead end for Qwen3.6 hybrid GDN because benchmark-style ROCm equivalence failed even with GPU graphs disabled, proving the failure was not just graph replay. Greedy MTP remains on the shared decode-equivalent stepwise verifier until a new backend path is proven. The retained Phase 13.8 implementation is the accepted-count transaction and speculative-state-slot publication layer used by stochastic MTP; it now advertises through `supportsMTPSpecDecodeAcceptedCountPublication()` and reports `accepted_count_spec_decode`.
...
Latest CUDA/ROCm stochastic top-k and graph-owned publication note, 2026-06-08: CUDA and ROCm graph-captured top-k/top-p sampling now use arena-declared scratch for a two-pass Qwen-style small-k distribution builder when `top_k<=64`, instead of scanning the full 248k vocab in one block. `V2_Integration_GPUSamplingKernels` includes a Qwen3.6 vocab-size `top_k=40` graph-captured distribution-plus-sample regression against shared CPU sampler math, and the CUDA/ROCm stochastic graph smokes assert the small-k scratch counter. Accepted verifier state publication now goes through the cached main verifier graph stages after `prepareVerifierStatePublication()` rebinds their declared workspaces; the old shared-kernel publication path is retained only for missing cached-verifier-graph warmup cases and hard-fails every other graph-publish error. Focused validation passed for the GPU sampling suite, CUDA/ROCm Qwen3.6 stochastic PyTorch parity cells, and CUDA/ROCm stochastic graph smokes. Fresh Qwen3.6 27B stochastic benchmarks reached CUDA 707.96 prefill / 54.60 decode tok/s for fixed depth 1 and 706.09 / 55.80 for dynamic depth, essentially matching the llama.cpp d1 anchor. ROCm default `top_k=40` reached 213.91 / 34.40 fixed depth 1 and 211.95 / 31.51 dynamic with zero metadata publication failures and cached-verifier-graph publication counters active; a diagnostic `top_k=20` lane reached 37.04 / 38.24 fixed/dynamic. CUDA `mtp.verifier_forward` is about 28.46 ms; ROCm default `top_k=40` sampling/verify-batch costs are about 3.5 ms / 6.4 ms, and `mtp.verifier_forward` remains the dominant about-37 ms catch-up target.
...
| Phase 13.8: vLLM-Style Accepted-Count Spec-Decode State | Implementation active; CUDA/ROCm stochastic speed-positive | `MTPSpecDecodeTransaction`, `MTPSpecDecodeMetadata`, graph-facing accepted-state metadata buffers, explicit-stream metadata upload, runner-owned speculative state-slot workspace, and transaction counters are implemented. CUDA and ROCm GDN/short-conv verifier capture use runner-declared speculative state-slot/work-state buffers through `IWorkspaceConsumer`, hard-fail on missing explicit stream/workspace, leave live state untouched before publication, and still prove plain non-verifier recurrence/conv state matches serial decode. Accepted verifier state publication now runs through cached verifier graph stages after `prepareVerifierStatePublication()` rebinds declared workspaces, with shared-kernel fallback allowed only for missing cached graphs during warmup. The old env-selected raw all-position greedy candidate and its equivalence/parity CTest fixtures are removed after ROCm benchmark-style equivalence failed; greedy MTP remains on the shared stepwise verifier. Stochastic MTP uses the retained accepted-count publication capability, records `phase138_stochastic_spec_decode_runs` / `accepted_count_spec_decode`, and batches CUDA/ROCm compact-distribution accept decisions plus candidate residual correction tokens for draft rows. CUDA and ROCm now specialize graph-captured `top_k<=64` stochastic distribution building with arena-declared scratch, cover Qwen3.6 vocab-size `top_k=40` distribution/sample parity, and assert the small-k scratch path in both graph smokes. ROCm NativeVNNI decode dispatch is generated for Qwen3.6 hot verifier shapes across codebooks 0/5/7/8 and the Q5/Q6 rows are active in verifier counters. Focused validation passed on 2026-06-08 for the runner unit, CUDA/ROCm GPU sampling integration, ROCm NativeVNNI decode generator/GEMV gates, CUDA/ROCm stochastic graph smokes, and Qwen3.6 stochastic parity cells. CUDA dense default stochastic now reaches 54.60 tok/s fixed depth 1 and 55.80 dynamic versus a 43.81 clean baseline; ROCm default `top_k=40` reaches 34.40 fixed depth 1 and 31.51 dynamic versus a 30.16 stochastic no-MTP baseline, with zero metadata publication failures. Phase 13.8 remains open for ROCm verifier-forward catch-up and broader stop/prefix/continuation/MoE/TP/PP acceptance. | Reduce ROCm verifier-forward cost and broaden the retained accepted-count path across remaining scenarios before Phase 14 dense default acceptance. |
...
1. Shared transaction metadata.
   ...
   - Wire structured counters so benchmark JSON can distinguish `shared_stepwise`, `accepted_count_spec_decode`, and any future promoted verifier strategy.
...
4. SingleDevice dense runner path.
   ...
   - Stochastic SingleDevice dense may use accepted-count speculative-state publication when the backend advertises `supportsMTPSpecDecodeAcceptedCountPublication()`.
...
- Retained implementation work:
  ...
  - `MTPSpecDecodeMetadata` declares graph-facing accepted-count, state-slot, token, query-start, and bonus-ready-token metadata buffers in runner-owned workspace, with explicit non-null stream upload guards.
  - `ShortConv1dStage` and `GDNRecurrenceStage` use `speculative_state_slot_rows`; their CUDA and ROCm kernels bind runner-declared speculative work/state buffers through `IWorkspaceConsumer`, leave live state untouched before publication, and hard-fail on missing explicit stream/workspace.
  - Cached main-verifier graph stages now own accepted verifier state publication after rebinding their declared workspaces through `prepareVerifierStatePublication()`, preventing stale shared backend-kernel capture bindings from publishing the wrong GDN/short-conv state.
  - `publishAcceptedSpeculativeStateFromDeviceMetadata()` is the shared semantic device API for accepted-count state publication.
  - Stochastic SingleDevice CUDA/ROCm MTP uses the accepted-count publication capability through `supportsMTPSpecDecodeAcceptedCountPublication()` and records `phase138_stochastic_spec_decode_runs` plus `accepted_count_spec_decode`.
...
- Focused validation already associated with the retained pieces includes `V2_Unit_MTPSpecDecodeMetadata`, `V2_Unit_MTPSpecDecodeTransaction`, `V2_Unit_MTPDecodeCatchup`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_MTPStateTransaction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_CUDAGDNPaddedRealLength`, `V2_Integration_ROCmGDNPaddedRealLength`, `V2_Integration_GPUSamplingKernels`, CUDA/ROCm `MTPStochasticSamplingVerifierRuns`, and the CUDA/ROCm Qwen3.6 stochastic graph smokes.
- Current hard gate: no greedy optimized verifier path is promoted. A new path must be added as shared transaction semantics first, then proven against `shared_stepwise` and PyTorch parity on CUDA and ROCm before any benchmark number is accepted.
- Next implementation slice:
  1. keep greedy MTP on `shared_stepwise` while designing the next non-dead-end speed path, likely device-side draft-input update plus captured repeated verifier/drafter metadata rather than selected-row live-state publication;
  2. reduce ROCm M=2 verifier graph cost for stochastic depth-1/dynamic lanes, starting with GDN projection, fused gate/up, ordinary GEMM, fused QKV, and LM head counters before using stochastic MTP in Phase 14 dense acceptance, and promote any new batched dispatch table only after model-level stochastic acceptance/equivalence passes;
  3. benchmark only retained paths and clearly label historical direct-row numbers as retired targets.
...
- Unit tests for `shared_stepwise` oracle behavior and accepted-count publication counters.
...
- Benchmark tests recording baseline, `shared_stepwise`, stochastic accepted-count publication, verifier graph, state commit, sidecar, sampler, and accepted-count counters.
