...
Latest Phase 13.8 promotion benchmark note, 2026-06-07: the explicit `LLAMINAR_MTP_PHASE138_DIRECT_CANDIDATE=1` no-oracle gate produced the speed-positive evidence used to promote the vLLM-style transaction as the normal dense SingleDevice GPU greedy catch-up path, and fresh no-env promoted release benchmarks preserve the ratchet. CUDA Qwen3.6 27B default-lane fixed depth-3 MTP reaches 602.15 prefill / 74.54 decode tok/s versus 707.17 / 41.72 no-MTP on the same release family, with 96.84% acceptance, 33 verifier runs, 134 verifier tokens, 33 transaction commits, and zero transaction validation failures. ROCm default-lane fixed depth-3 MTP reaches 218.20 / 35.64 versus 233.97 / 31.22 at 55.29% acceptance, with 43 transaction commits and zero validation failures. The prior high-acceptance ROCm `The quick brown fox`, `-c64 -n48` direct-gate lane remains 73.05 / 54.23 versus 76.35 / 31.92 at 88.57% acceptance. MoE, TP, PP, ExpertParallel overlay, chained-depth prefix restore, and broader stop/continuation gates remain pending.
...
- Promoted no-env release benchmarks are speed-positive for SingleDevice dense
  ...
  fixed depth-3 MTP reaches 602.15 prefill / 74.54 decode tok/s versus 707.17 /
  41.72 no-MTP, with 96.84% acceptance, 33 verifier runs, 134 verifier tokens,
  33 transaction commits, and zero transaction validation failures. ROCm fixed
  depth-3 promoted MTP reaches 218.20 / 35.64 versus 233.97 / 31.22 no-MTP on
  the same default lane at 55.29% acceptance, with 43 transaction commits and
  zero validation failures. The high-acceptance ROCm `The quick brown fox`,
  `-c64 -n48` direct-gate lane remains 73.05 / 54.23 versus 76.35 / 31.92,
  with 88.57% acceptance. Promoted artifacts are under
  `/tmp/llaminar-mtp-bench/dense-{cuda,rocm}-phase138-promoted-d3-default.json`.
...
- Next implementation gate: broaden the live transaction equivalence matrix across
