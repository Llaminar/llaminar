...
Latest Phase 13.8 correction, 2026-06-06: `MTPDecodeCatchup` and the named optimized-hook gate are the Tier 0 oracle/scaffold, not the intended end state. Phase 13.8 remains open until CUDA and ROCm have a researched shortcut candidate that avoids full main decode for accepted speculative tokens, proves continuation-state equivalence against the shared oracle, and records benchmark evidence; CPU should follow the same contract later. The old raw all-position GDN verifier-state shortcut remains disqualified because long-prefix dense Qwen3.6 tests prove row-token agreement is not sufficient for continuation-state equivalence. Candidate A, sidecar-chain verifier state, is also retired by focused ROCm/generic and CUDA Qwen3.6 dense negative parity tests. A 2026-06-06 llama.cpp/vLLM source survey did not find a transferable "commit final batched recurrent state" trick: llama.cpp carries target nextn hidden rows into the drafter but still truncates target memory/checkpoints after verification, while vLLM makes accepted-token counts explicit inputs to GDN/Mamba attention and captures uniform `1 + draft_count` decode graphs. The next candidate direction is therefore an accepted-count-aware stateful verifier, not post-hoc state selection from a generic all-position verifier.
...
82. Done for CUDA dense M=2 verifier-equivalence narrowing: `Qwen36CUDASingleDevicePrefixMTPParity.M2VerifierFinalStateLongPrefixIsNotDecodeEquivalent` now proves immediate M=2 verifier tokens can match while final hybrid state remains non-equivalent; the env-gated stage dump localizes row 0's first mismatch to `layer0_FFN_DOWN` and row 1's first mismatch to `layer1_GDN_CONV1D_OUTPUT`. `V2_Integration_CUDAGDNPaddedRealLength.ShortConvQwen36M2InPlaceStateMatchesStepwiseReplay` proves the Qwen3.6-shaped in-place short-conv kernel itself is decode-equivalent, so CUDA must not re-enable raw all-position verifier-row shortcuts by treating the conv snapshot as the root cause. Focused validation passed for `V2_Integration_CUDAGDNPaddedRealLength` and the long-prefix M=2 CUDA dense parity regression.
...
## Phase 13.8: Shared Multi-Row Decode-Equivalent Shortcut Research

### Goal

Find and promote a real MTP catch-up design that works for CUDA and ROCm, with CPU-compatible semantics later. A shortcut is desirable only if it preserves a clean state contract. If the research shows that a shortcut is the wrong abstraction, this phase may instead replace the old verifier/rollback shape with a clearer accepted-count-aware verification transaction. Any promoted design must avoid unnecessary full main decode replay for accepted speculative tokens while leaving main KV, shifted MTP KV, GDN recurrence/short-conv state, terminal hidden/logits, sampler-visible next-token state, and request bookkeeping identical to ordinary one-token-at-a-time decode for the accepted output sequence.

The shared `MTPDecodeCatchup` stepwise implementation is the oracle, not the finish line. This phase is a prerequisite to returning to Phase 14 CUDA/ROCm dense MTP speedup claims when `requiresMTPDecodeEquivalentVerifierReplay()` is true. It must not reintroduce raw all-position GDN shortcuts whose row logits match but continuation state drifts.

### Implementation Details

Phase 13.8 has four promotion tiers:

0. Shared correctness oracle.
   - `MTPDecodeCatchup` owns the greedy accept/reject loop for stateful runners.
   - The baseline implementation is `shared_stepwise`: commit one shifted MTP row from the current terminal hidden, run one normal main decode row, sample that row, and repeat until all accepted, rejected, or stopped.
   - `OrchestrationRunner` delegates decode-equivalent greedy verification to this shared helper instead of keeping an embedded backend-specific loop.
   - The helper records structured counters for draft tokens, verifier tokens, accepted tokens, shifted-row commits, forward-token count, stop handling, and implementation name.

1. Shortcut candidate research.
   - Maintain a candidate ledger in this section. Every candidate must state what work it avoids, what state it commits, which backends can implement it, and the first test proving or disproving it.
   - Candidate A: sidecar-chain verifier state. Instead of committing a generic all-position prefill verifier state, run the draft chain through the actual MTP sidecar, verify accepted rows, then test whether a captured verifier row plus shifted-cache commit is continuation-equivalent over multiple future tokens. This is not the same as the disqualified raw all-position shortcut. Status: retired on 2026-06-06 because focused Qwen3.6 dense tests prove it is not decode-equivalent on ROCm/generic SingleDevice and CUDA SingleDevice.
   - Candidate B: decode-equivalent selected-state kernels. Extend GDN recurrence/short-conv and terminal-hidden capture only when the producer is itself decode-equivalent. A plain selected copy out of the batched/all-position verifier is disallowed for Qwen3.6 dense because CUDA diagnostics show drift already at early dense stages such as `layer0_FFN_DOWN` and layer-1 projections before the recurrent state copy boundary; ROCm/generic state-level negative tests also prove all-position state is unsafe. Accepted-row commit may become row-select state restore plus KV truncate/append and shifted-cache commit only after the selected state is produced by a serial-equivalent verifier.
   - Candidate C: graph-captured stepwise micro-loop. If no safe mathematical state shortcut survives, reduce launch/sync overhead by capturing the stepwise one-row loop and fusing sidecar sample, shifted commit, verifier forward, and greedy sample, while still executing decode-equivalent rows. Status: useful scaffold only. CUDA and ROCm evidence show it preserves correctness but does not remove accepted-token main decode work, so it cannot be the Phase 13.8 promoted shortcut by itself.
   - Candidate D: model-family opt-out. If a model has no stateful GDN/conv path, allow the all-position verifier shortcut only after the same continuation-equivalence test passes for that model family.
   - Candidate E: accepted-count-aware stateful verifier. Inspired by vLLM's GDN/Mamba spec-decode contract, make accepted-token count a first-class input to stateful attention/short-conv update, so a verifier can compute `1 + draft_count` rows while committing only the accepted recurrent and convolution state. This is necessary but not sufficient for Qwen3.6: the existing verifier-row snapshot/restore machinery already approximates the same state-selection idea, and it still fails continuation replay when the producer rows come from a generic all-position verifier. Candidate E can only be promoted if its producer rows are serial-equivalent by construction, or if the whole verifier transaction is intentionally redefined and parity is updated to that target. It must be implemented behind the shared runner hook, must keep all tensor movement/workspace allocation graph-native, and must pass commit-replay tests on both CUDA and ROCm before any benchmark claim.
   - Candidate F: graph-captured serial-equivalent row loop with reduced recomputation. If Candidate E cannot produce serial-equivalent rows without replaying the dense path, abandon the "batched verifier state shortcut" premise for Qwen3.6 dense and instead optimize the shared row loop: capture the loop, reuse sidecar/terminal buffers, fuse sampling/shifted-commit bookkeeping, and attack small-M dense kernels. This may be less glamorous, but it preserves the exact no-MTP parity contract.

2. Runner-native optimized catch-up contract.
   - Add an opt-in runner hook only after the shared contract is covered by unit tests.
   - The hook may reduce host synchronization or fuse graph launches, but it must return the same `MTPDecodeCatchupGreedyResult` shape and must be guarded by an equivalence mode that compares it against `shared_stepwise`.
   - The optimized hook must hard-fail if it cannot preserve decode-equivalent state. Silent fallback is allowed only at the orchestration policy level before the optimized hook is selected, and must increment a structured bypass counter with a reason.
   - Every GPU operation in the optimized path must use explicit non-null streams. Captured replay must not contain H2D uploads, raw allocations, or mutable tensor flag hacks.
   - Device scratch must use declared graph/workspace bindings through `IWorkspaceConsumer` or existing graph buffer contracts.

3. Backend promotion.
   - CUDA and ROCm implementations must prove accepted, rejected, stop-token, and continuation-state equivalence against `shared_stepwise`.
   - A backend can be marked complete only when focused parity tests and real Qwen3.6 dense benchmarks show the verifier wall is reduced or the remaining blocker is documented with profiling traces.
   - TP/MoE variants must coordinate the chosen output tokens domain-wide and apply the same commit/rollback transaction once per logical decode step.

Forbidden shortcuts:

- Do not batch verifier rows through a normal prefill graph and call it decode-equivalent for GDN/hybrid models unless continuation-state equivalence is proven for that exact state source.
- Do not rely on the HIP/CUDA default/null stream.
- Do not capture host-to-device parameter copies or lazy allocator work.
- Do not bypass `TransferEngine`, workspace binding, or graph-stage buffer contracts.
- Do not use a faster path when a participant in TP/PP/ExpertParallel cannot enter the same logical catch-up transaction.

Current implementation status, 2026-06-06:

- Tier 0 is implemented and promoted as the correctness oracle. `MTPDecodeCatchup` owns the common greedy multi-row accept/reject loop, and `OrchestrationRunner` routes stateful CUDA/ROCm decode-equivalent verification through `shared_stepwise`.
- The Tier 2 scaffold is implemented: `IInferenceRunner` exposes an explicit optimized catch-up capability, a stable implementation-name hook, and a hard-failing default optimized method. `OrchestrationRunner` selects that hook only when advertised and records the selected name in verifier-run and acceptance-trace counters. The unit mock delegates the advertised optimized path back to the shared oracle so selection is tested without inventing a fake state model.
- Focused validation passed for `V2_Unit_MTPDecodeCatchup`, `V2_Unit_PrefillDecodeTransition`, generic ROCm Qwen3.6 `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, `OneRowRestoreLongPrefixMatchesSequential`, and the matching CUDA Qwen3.6 `MTPBenchmarkPromptFixedDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, and `OneRowRestoreLongPrefixMatchesSequential` cells.
- Candidate A is retired. The focused negative-ledger tests `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_SidecarChainVerifierStateShortcutCandidateIsNotDecodeEquivalent` and `V2_Integration_Parity_Qwen36_CUDA_SingleDevice_Qwen36CUDASingleDevicePrefixMTPParity_SidecarChainVerifierStateShortcutCandidateIsNotDecodeEquivalent` passed on 2026-06-06, proving a sidecar-chain verifier state cannot be promoted as the shared shortcut for Qwen3.6 dense.
- Candidate B's naive all-position selected-state form is retired. CUDA diagnostics from `M2VerifierFinalStateLongPrefixIsNotDecodeEquivalent` with divergence dumps show row-token agreement but early dense-stage drift before GDN state capture, and the ROCm/generic `M4VerifierLongPrefixIsNotDecodeEquivalent` cell confirms the same all-position state source is unsafe at the prefix-payload level. The stricter opt-in `all_position` catch-up candidate is now pinned by negative commit-replay tests on both generic/ROCm and CUDA SingleDevice parity suites. Candidate B remains open only for a selected-state producer that is serial/decode-equivalent by construction.
- Fresh CUDA dense release benchmark evidence confirms the shared-stepwise path is still speed-negative on the default lane: no-MTP reached 707.92 prefill / 41.73 decode tok/s, while fixed depth-3 shared catch-up reached 609.73 prefill / 38.19 decode tok/s with 84.95% acceptance. Structured stats captured at `/tmp/llaminar-mtp-bench/dense-cuda-phase138-shared-d3-default-stats.{json,csv}` show the dominant wall is full main decode replay (`decode_equivalent_catchup_forward_one`, 9272.9 ms / 383 forwards); shifted-row commit and sampling are small by comparison (98.6 ms and 11.8 ms).
- A 2026-06-06 source survey found that llama.cpp MTP extracts target nextn hidden rows for the drafter (`common/speculative.cpp`) but commits target state through ordinary target verification plus memory truncation/checkpoint restore (`examples/speculative-simple/speculative-simple.cpp`). vLLM treats spec decode as uniform target decode with query length `1 + num_spec_tokens`, adjusts graph capture sizes around that shape, drops the last prefix block for EAGLE/MTP hidden-state recomputation, and passes `num_accepted_tokens` / `num_decode_draft_tokens` into GDN metadata and kernels. Neither project appears to rely on post-hoc final-row recurrent-state selection for Qwen-style GDN, and neither gives Llaminar a proof that generic batched verifier rows are serial-decode-equivalent under our exact parity contract.
- No backend optimized catch-up hook is promoted. Candidate C is retained as a capture/host-overhead scaffold. Candidate E is the next research target only if it includes a serial-equivalent producer-row proof; otherwise Phase 13.8 should pivot to Candidate F, a graph-captured serial-equivalent loop plus kernel-cost reduction, rather than keeping a misleading "shortcut" framing. Phase 13.8 remains open until a design is proven and promoted for CUDA/ROCm or every plausible candidate is explicitly retired with evidence.
...
- `src/v2/execution/local_execution/orchestrators/DeviceGraphOrchestrator.h/.cpp`
- `src/v2/execution/local_execution/orchestrators/RankOrchestrator.h/.cpp`
- `src/v2/execution/local_execution/engine/ForwardExecutionEngine.h/.cpp`
- `src/v2/execution/runner/OrchestrationRunner.h/.cpp`
...
- Unit tests for `shared_stepwise` accept-all, reject-first, reject-after-prefix, stop-token, sampler-failure, forward-failure, shifted-commit-failure, and invalid-input paths.
- Unit tests proving orchestrator policy selects `shared_stepwise` for stateful decode-equivalent runners and records the implementation name in acceptance counters.
- Unit tests for any optimized runner hook proving it is never selected unless the runner advertises support and the request is greedy, single-sequence, and decode-equivalent safe.
- Candidate-ledger tests for every shortcut attempt, including negative tests that pin why a candidate is unsafe.
- GPU hygiene tests proving optimized CUDA/ROCm catch-up uses explicit streams and declared workspace buffers.
- CUDA and ROCm focused Qwen3.6 dense parity cells comparing optimized catch-up against `shared_stepwise` for accepted, rejected, and stop-token traces.
- Continuation-state parity tests that decode at least four future tokens after a catch-up commit and compare against ordinary sequential decode on CUDA and ROCm, plus CPU once the CPU shortcut implementation exists.
- Focused benchmarks recording baseline, `shared_stepwise`, and optimized catch-up decode throughput plus `decode_equivalent_catchup_*`, `segmented_replay_*`, sidecar, sampler, and verifier counters.

Focused command shape:

```bash
cmake --build build_v2_integration --parallel
ctest --test-dir build_v2_integration -R "^V2_Unit_MTPDecodeCatchup$|^V2_Unit_PrefillDecodeTransition$|^V2_Integration_Parity_Qwen36_.*(MTPGreedyDepth3MatchesPyTorchDecodeTokens|PrefixCacheMTPRestore|M2VerifierLongPrefixMatchesSequential|OneRowRestoreLongPrefixMatchesSequential)" --output-on-failure --parallel
```
...
./build_v2_release/llaminar2 benchmark -m /opt/llaminar-models/Qwen3.6-27B-Q4_K_S.gguf -d cuda:0 -c 64 -n 48 --mtp --mtp-draft-tokens 3 --benchmark-json-output /tmp/llaminar-mtp-bench/dense-cuda-phase138-mtp-d3.json
./build_v2_release/llaminar2 benchmark -m /opt/llaminar-models/Qwen3.6-27B-Q4_K_S.gguf -d rocm:0 -c 64 -n 48 --mtp --mtp-draft-tokens 3 --benchmark-json-output /tmp/llaminar-mtp-bench/dense-rocm-phase138-mtp-d3.json
```

### Exit Criteria

- `shared_stepwise` remains the canonical correctness oracle for stateful decode-equivalent MTP catch-up.
- Any optimized catch-up path is selected only through a named runner capability and records its implementation name in structured counters.
- CUDA and ROCm have a researched shortcut candidate that either passes equivalence/parity and benchmark gates or is explicitly retired with a blocker note detailed enough to guide the next candidate.
- A promoted shortcut or replacement transaction must prove it avoids unnecessary full main decode replay for accepted tokens through structured counters, not merely improve wall time by hiding synchronization.
- No Phase 14 MTP speedup claim depends on an unproven backend-specific verifier shortcut.
- Benchmark notes contain current baseline/shared/optimized numbers and the verifier-wall counters needed to explain the result.
