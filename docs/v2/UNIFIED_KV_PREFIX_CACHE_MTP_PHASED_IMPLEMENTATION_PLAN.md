...
| Phase 13.8: Shared Multi-Row Decode-Equivalent Catch-Up | Complete | `MTPDecodeCatchup` is the canonical stateful greedy catch-up contract. `OrchestrationRunner` delegates CUDA/ROCm decode-equivalent verification to `shared_stepwise`, which commits each shifted MTP row from terminal hidden, forwards exactly one normal decode row, samples, and repeats until accept-all, reject, or stop. The follow-up hook slice adds `IInferenceRunner::supportsOptimizedMTPDecodeCatchupGreedy()`, a hard-failing default optimized runner hook, named implementation counters, and mock coverage proving an advertised optimized hook is selected without re-enabling all-position verifier replay. Focused validation passed on 2026-06-06 for `V2_Unit_MTPDecodeCatchup`, `V2_Unit_PrefillDecodeTransition`, generic ROCm Qwen3.6 `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, `OneRowRestoreLongPrefixMatchesSequential`, and matching CUDA Qwen3.6 `MTPBenchmarkPromptFixedDepth3MatchesPyTorchDecodeTokens`, `PrefixCacheMTPRestore`, `M2VerifierLongPrefixMatchesSequential`, and `OneRowRestoreLongPrefixMatchesSequential`. Fresh CUDA dense Phase 13.8 release benchmark evidence records the shared-stepwise blocker: no-MTP default lane reaches 707.92 prefill / 41.73 decode tok/s, while fixed depth-3 shared catch-up reaches 609.73 prefill / 38.19 decode tok/s with 84.95% acceptance (`/tmp/llaminar-mtp-bench/dense-cuda-phase138-{nomtp,shared-d3}-default.json`). Structured stats show `decode_equivalent_catchup_forward_one` dominates the verifier wall at 9272.9 ms across 383 forwards, while shifted-row commit is 98.6 ms and sample is 11.8 ms. No CUDA/ROCm optimized catch-up hook is advertised because a graph-fused wrapper cannot remove the main decode work required by the decode-equivalent contract. | None for the contract. Future speedup work must prove a state shortcut that avoids full main decode for accepted tokens, or reduce the main decode/kernel cost itself; it must not hide behind a Phase 13.8 “optimized catch-up” label. |
...
- No backend optimized catch-up hook is promoted. Phase 13.8 is complete as the correctness/safety contract; further CUDA dense speedup requires a separately proven state shortcut or lower main-decode kernel cost, not a wrapper around decode-equivalent replay.
