...
- Next implementation slice:
  1. rebenchmark retained stochastic MTP after batched accept tests so Phase 14 has current numbers;
  2. keep greedy MTP on `shared_stepwise` while designing the next non-dead-end speed path, likely device-side draft-input update plus captured repeated verifier/drafter metadata rather than selected-row live-state publication;
