...
Latest Phase 13.8 direct-candidate benchmark note, 2026-06-07: the explicit `LLAMINAR_MTP_PHASE138_DIRECT_CANDIDATE=1` no-oracle gate now has first speed-positive dense SingleDevice evidence on both GPU backends. CUDA Qwen3.6 27B default-lane fixed depth-3 MTP reaches 602.01 prefill / 74.58 decode tok/s versus 707.17 / 41.72 no-MTP on the same release binary, with 96.84% acceptance, 33 verifier runs, 134 verifier tokens, and zero transaction validation failures. ROCm default-lane fixed depth-3 MTP reaches 217.23 / 35.86 versus 233.97 / 31.22 at 60.47% acceptance, and the high-acceptance `The quick brown fox`, `-c64 -n48` lane reaches 73.05 / 54.23 versus 76.35 / 31.92 at 88.57% acceptance. The direct candidate remains explicitly gated while broader accept/reject/stop/prefix parity and promotion policy are finished.
...
| Phase 13.8: vLLM-Style Spec-Decode Transaction | In progress | Redesign accepted on 2026-06-06 after llama.cpp/vLLM source survey and failed CUDA/ROCm all-position shortcut candidates. `MTPDecodeCatchup` remains the canonical sequential oracle and `IInferenceRunner` still exposes the named optimized hook, but the promoted target is now a vLLM-style transaction with device-visible valid sampled count / accepted-token count / rejected suffix metadata, accepted-count-aware GDN/short-conv state updates, shifted MTP-row commit from accepted target hidden rows, and atomic request-state publication. The current Llaminar SingleDevice greedy candidate forwards `D` target rows after the already-live terminal condition rather than a literal `1 + D` host-level graph: row 0 samples after draft[0], rows 0..D-2 verify draft[1..D-1], and row D-1 is a bonus ready-token row only on accept-all. `MTPSpecDecodeTransaction` is implemented as the shared metadata contract, and `OrchestrationRunner` now builds the padded `MTPSpecDecodeMetadata` batch for decode-equivalent catch-up results, validates it before commit, and emits `mtp.spec_decode_transaction_metadata`. `MTPSpecDecodeMetadata` defines the graph-facing int32 workspace buffers and host-side padded arrays; `MTPSpecDecodeMetadataWorkspaceBinding` binds those buffers through `DeviceWorkspaceManager`, `DeviceGraphOrchestrator` declares them as runner-owned extra workspace when MTP is enabled on GPU, and `uploadMTPSpecDecodeMetadataBatch()` requires an explicit non-null stream for GPU H2D. The old env-gated runnable `all_position` candidate now hard-fails as retired instead of executing unsafe state selection code. Focused validation passed on 2026-06-07 for the unit guard set plus CUDA/ROCm Qwen3.6 candidate/equivalence parity for greedy MTP and prefix+MTP restore; the reject-after-prefix ROCm depth-3 failure now has a regression fix that samples correction-suffix ready tokens from the explicit all-position suffix replay row instead of stale normal logits. Direct no-oracle candidate parity is green for ROCm depth-3 and CUDA benchmark-prompt depth-3 behind `LLAMINAR_MTP_PHASE138_DIRECT_CANDIDATE=1`, with dedicated CTest entries for both. Fresh release benchmark evidence is now speed-positive under that explicit gate: CUDA default-lane depth-3 reaches 602.01 prefill / 74.58 decode tok/s versus 707.17 / 41.72 no-MTP, and ROCm reaches 217.23 / 35.86 versus 233.97 / 31.22 on the default prompt plus 73.05 / 54.23 versus 76.35 / 31.92 on the high-acceptance `qbf` lane. | Broaden PyTorch parity and equivalence coverage for accept-all, reject-first, reject-after-prefix, stop-token, prefix-restore, and continuation cases, then promote the direct candidate from explicit development gate to normal SingleDevice dense policy before TP/MoE extension. |
| Phase 14: Benchmark Acceptance And Default-Enablement Readiness | Waiting on Phase 13.5, Phase 13.7, and Phase 13.8 | `docs/v2/PREFIX_CACHE_MTP_BENCHMARK_NOTES.md` tracks real Qwen3.6 dense/MoE baseline, graph-capture status, and best observed MTP speedups by domain. A CUDA dense memory-planner sidequest now proves long-context KV capacity is charged separately from bucket-sized GPU activation arenas: Qwen3.6 27B at context 16384 initializes with `Act.Seq=4096`, 4.0 GB KV, and 1785 MB arena, while hybrid KV still reports 16 FA KV layers plus 48 GDN layers. CUDA SingleDevice dense now has first direct Phase 13.8 Qwen3.6 default-lane evidence under the explicit development gate: fixed depth-3 MTP reaches 602.01 prefill / 74.58 decode tok/s versus 707.17 / 41.72 no-MTP, with 96.84% acceptance and zero transaction validation failures. CUDA SingleDevice MoE now beats the llama.cpp MoE default-lane no-MTP and MTP decode anchors after split-K fused verifier gate/up/down, shared-expert verifier prefill, GDN qkv+z/alpha+beta fused verifier projection work, capture-safe forced verifier routing, deterministic scatter parallelization, cuBLAS FP32 router prefill, fused-runtime down warp-reduce, and IQ4_NL word decode in the shared CUDA native-VNNI helper: clean no-MTP reaches 2707.70 prefill / 119.91 decode tok/s, fixed depth-1 MTP reaches 1946.82 prefill / 148.50 decode tok/s with 71.88% acceptance, and dynamic MTP reaches 1943.69 prefill / 145.36 decode tok/s with 68.75% acceptance. ROCm SingleDevice dense now has direct Phase 13.8 evidence on both prompt classes: default-lane fixed depth-3 MTP reaches 217.23 / 35.86 versus 233.97 / 31.22 no-MTP at 60.47% acceptance, while the high-acceptance `The quick brown fox`, `-c64 -n48` lane reaches 73.05 / 54.23 versus 76.35 / 31.92 at 88.57% acceptance. ROCm SingleDevice MoE has fresh default-lane speed-positive proof after grouped-decode runtime-table rewarm, streamful prefix terminal-state restore, capture-safe ROCm attention params, verifier-sized route grouping, and grouped/LDS small-M router logits: fixed depth 1 reached 39.94 tok/s versus 19.72 tok/s baseline at `-c 768`, `-n 64`, with 78.12% acceptance; dynamic max-depth 3 previously demoted to depth 1 and reached 37.82 tok/s. `V2_Integration_PrefixCacheMTP_Qwen36ROCmLocalPPHardFail` pins the current LocalPP matrix blocker as an early real-model hard fail with zero MTP draft/verifier counters, preventing recurrence of the old late stage-1 shifted-cache crash while the real PP-aware MTP path is still unimplemented. | Do not use Phase 14 to paper over kernel deficits. Resume benchmark acceptance after Phase 13.5 provides graph-capturable M=2/3/4 GEMV-many coverage, Phase 13.7 proves stochastic MTP correctness for non-greedy chat defaults, and Phase 13.8 promotes or explicitly retires the direct vLLM-style transaction for stateful dense MTP; then continue matrix-driven benchmark work until remaining supported domains show concrete speedups or documented blockers with traces. |
...
0. Preserve the oracle and retired-candidate ledger.
   ...
   - Keep negative tests that prove row-token agreement is not continuation-state equivalence.
...
- The named optimized-hook scaffold already exists on `IInferenceRunner`. `LLAMINAR_MTP_PHASE138_CATCHUP_CANDIDATE=vllm_style_spec_decode` now selects the explicit Phase 13.8 hook name. By default it still hard-fails as not promoted. `LLAMINAR_MTP_PHASE138_EQUIVALENCE_CHECK=1` runs the live target-verifier candidate so it can be compared against `shared_stepwise` before any production promotion. `LLAMINAR_MTP_PHASE138_DIRECT_CANDIDATE=1` is the separate explicit benchmark-only no-oracle gate.
...
- Direct no-oracle candidate parity is green for the first benchmarkable slice:
  ...
  `LLAMINAR_MTP_PHASE138_DIRECT_CANDIDATE=1`. Dedicated CTest entries
  `V2_Integration_Parity_Qwen36_SingleDevice_Phase138DirectDepth3Parity` and
  `V2_Integration_Parity_Qwen36_CUDA_SingleDevice_Phase138DirectDepth3BenchmarkParity`
  preserve that coverage.
- Direct no-oracle candidate release benchmarks are now speed-positive for
  SingleDevice dense CUDA and ROCm. On Qwen3.6 27B Q4_K_S default
  595-prefill/128-decode, CUDA fixed depth-3 direct MTP reaches 602.01 prefill /
  74.58 decode tok/s versus 707.17 / 41.72 no-MTP, with 96.84% acceptance,
  33 verifier runs, 134 verifier tokens, and zero transaction validation
  failures. ROCm fixed depth-3 direct MTP reaches 217.23 / 35.86 versus
  233.97 / 31.22 no-MTP on the same default lane at 60.47% acceptance. The
  high-acceptance ROCm `The quick brown fox`, `-c64 -n48` lane reaches 73.05 /
  54.23 versus 76.35 / 31.92, with 88.57% acceptance. Artifacts are under
  `/tmp/llaminar-mtp-bench/dense-{cuda,rocm}-phase138-direct-*`.
- Next implementation gate: broaden the live candidate equivalence matrix across
  explicit accept-all, reject-first, reject-after-prefix, stop-token, prefix
  restore, and continuation cases on CUDA and ROCm, then remove the explicit
  development env gate only if the promoted policy remains parity-green.
