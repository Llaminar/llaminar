...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice graph-valid longer smoke | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 21.19 graph-enabled, no executor profiling, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense depth-1 MTP. `forward_graph` counters report `allow_segmented=true`, zero manual segments, and captured replay records for `main_verifier`, `mtp_decode_sidecar`, and `mtp_decode_catchup`. | N/A | 33.57 graph-enabled depth-1 MTP, same prompt/context/decode length | 1.58x same-binary, valid-context graph-captured smoke | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-graph-repro-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.csv`; context-budget regression `/tmp/llaminar-mtp-bench/dense-rocm-context-guard-c64-n64-bench.json`, `V2_Unit_BenchmarkRunnerCPU.FailsBeforePrefillWhenPromptPlusDecodeExceedsContext` | Speedup is real and stronger on the longer valid decode, but still below the 2x target. `mtp.verifier_forward` averaged about 52.21 ms per two-token captured verifier, sidecar averaged about 2.46 ms, acceptance was 95.83%, and the remaining lever is reducing verifier GPU work or making deeper M=3/4 drafts fast enough to amortize it. A later lagged-reject shortcut reduced measured reject replay to one main token per reject, but reached 32.16 tok/s rather than a new ratchet. |
