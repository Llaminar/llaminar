...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice graph-valid longer smoke | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 20.82 stable-workspace same-binary, graph-enabled, no executor profiling, `The quick brown fox`, `-c 64`, `-n 48`; 20.99 post-row-cache same-binary; 21.19 earlier graph-enabled lane | Fully captured for dense depth-1 MTP. `forward_graph` counters report `allow_segmented=true`, zero manual segments, and captured replay records for `main_verifier`, `mtp_decode_sidecar`, and `mtp_decode_catchup`. | N/A | 34.28 current mixed-native-GDN rerun, graph-enabled depth-1 MTP, same prompt/context/decode length; 34.22 stable-workspace prior ratchet; 34.15 post-row-cache; 33.57 older graph-enabled best | 1.65x current same-binary, valid-context graph-captured smoke | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-stablews-baseline-c64-n48-bench.json`; current MTP `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-stats.csv`; first mixed run `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-stats.csv`; prior MTP `/tmp/llaminar-mtp-bench/dense-rocm-stablews-mtp-d1-c64-n48-bench.json`; post-row-cache baseline `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-baseline-c64-n48-bench.json`; post-row-cache MTP `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.csv`; older baseline `/tmp/llaminar-mtp-bench/dense-rocm-graph-repro-c64-n48-bench.json`; older MTP `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.csv`; context-budget regression `/tmp/llaminar-mtp-bench/dense-rocm-context-guard-c64-n64-bench.json`, `V2_Unit_BenchmarkRunnerCPU.FailsBeforePrefillWhenPromptPlusDecodeExceedsContext` | Current speedup is a small ROCm SingleDevice dense ratchet but still below the 2x target. The retained mixed-native GDN path recorded 48 real-model `rocm_native_vnni_small_m_batched_mixed_calls` across 2- and 3-projection subgroups, zero mixed-codebook bypasses, 94.79% acceptance on the rerun, three measured reject rollbacks using `rollback_verifier_state_row_shortcuts`, no `replay_forward`, `mtp.verifier_forward` averaging about 51.5 ms per two-token captured verifier, and sidecar averaging about 2.65 ms. The remaining lever is reducing verifier GPU work and per-step sidecar/sampling overhead; current deeper M=3/4 drafts are still slower because acceptance and rollback rates erase verifier amortization. |
