...
| SingleDevice | CUDA `cuda:0` | Qwen3.6 dense 27B Q4_K_S | 40.48 current `-c 128 -n 64`; 41.39 current `-c 64 -n 16`; 43.76 current no-stats `-c 64 -n 4` | Small-context dense MTP reaches segmented replay with zero manual stages. Main verifier, full decode sidecar, and KV-only shifted-row catch-up decode graphs are capturable/replayed; default 4096-context run still does not fit this 24 GB device | N/A | 38.16 current same-prompt `-n 4` clean run; 37.69 retained KV-only catch-up `-c 128 -n 64` combined telemetry; 37.72 rejected M=2 projection-overlap experiment; 36.78 rejected post-sidecar checkpoint-elision experiment; 32.15 KV-only catch-up `-c 64 -n 16` combined telemetry | 0.93x long-run retained combined telemetry; 0.93x rejected M=2 projection-overlap experiment; 0.91x rejected checkpoint-elision experiment; 0.87x best clean short run; 0.78x `-n 16` combined telemetry | `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c128-n64-forwardgraph.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c128-n64-forwardgraph-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c128-n64-combined.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c128-n64-combined-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-m2concurrent-c128-n64-combined.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-m2concurrent-c128-n64-combined-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-reject-checkpoint-elide-c128-n64-combined.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-reject-checkpoint-elide-c128-n64-combined-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c64-n16.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c64-n16-combined.json`; historical artifacts are in Latest CUDA dense evidence below | Shifted MTP KV catch-up is correct through the longer `-n 64` CUDA dense smoke: `runtime_state.current_position=68` and `mtp_cached_tokens=67`, with 96.88% acceptance. Longer decode amortizes graph warmup/capture and improves retained MTP to 37.69 tok/s, but it still trails the 40.48 tok/s baseline. Steady replay shows why: main verifier replay is about 46.48 ms per two-token verifier, while baseline main decode replay is about 24.53 ms per one-token decode. Rejected M=2 projection-overlap and post-sidecar checkpoint-elision experiments did not improve real-model speed. The next lever is true two-row verifier kernel efficiency or deeper draft verification, not simple projection overlap or replaying sidecar state after rollback. |
