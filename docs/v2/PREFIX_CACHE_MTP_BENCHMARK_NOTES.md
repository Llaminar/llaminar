...
| SingleDevice verifier-row shortcut | ROCm `rocm:1` | Qwen3.6 dense 27B Q4_K_S | 20.65 same-binary, graph-enabled, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense depth-1 MTP. Reject rollback now restores captured GDN verifier-row state after the post-sidecar checkpoint, commits shifted rows from the partial verifier forward, and preserves dense replay state while MoE retains the targeted reset guard. | N/A | 32.83 graph-enabled depth-1 MTP, same prompt/context/decode length | 1.59x same-binary, no new best | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-verifierrow-baseline-c64-n48-rocm1-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-verifierrow-mtp-d1-c64-n48-rocm1-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-verifierrow-mtp-d1-c64-n48-rocm1-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-verifierrow-mtp-d1-c64-n48-rocm1-stats.csv`; regressions `V2_Unit_GDNMathematicalCorrectness`, `V2_Unit_GDNKernels`, `V2_Unit_PrefillGraphCaptureGuards`, `V2_Unit_PrefillGraphCaptureDynamicParams`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_PrefillDecodeTransition`, and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | Shortcut is correct and active: stats recorded three `rollback_verifier_state_row_shortcuts`, three `verifier_state_row_restores` over 48 GDN layers, no `replay_forward`, and `restore_verifier_state_row` averaged about 0.317 ms. It removes rollback replay cost but does not raise the speed ratchet because rejects are rare and captured `verifier_forward` still averages about 52.97 ms. Next lever remains verifier GPU work and deeper-draft amortization. |
