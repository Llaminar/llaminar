...
| SingleDevice | CUDA `cuda:0` | Qwen3.6 dense 27B Q4_K_S | 43.76 current no-stats at `-c 64`; 43.97 previous same shape; 39.44 for the 10-token combined-telemetry prompt | Small-context dense MTP reaches segmented replay with zero manual stages; default 4096-context run still does not fit this 24 GB device | N/A | 38.16 current same-prompt small-M verifier+SwiGLU no-stats run; 36.01 instrumented | 0.87x current same-prompt clean decode; 0.82x instrumented | `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-swiglu-nostats-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-swiglu-nostats-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-swiglu-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-swiglu-c64-n4-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-swiglu-c64-n4-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-alln-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-alln-c64-n4-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-alln-c64-n4-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-gemv-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-gemv-c64-n4-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-smallm-gemv-c64-n4-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-gpugraphs-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-gpugraphs-c64-n4-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-gpugraphs-c64-n4-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c64-n16.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-gpugraphs-c64-n16.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-gpugraphs-c64-n16-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-context-tags-smoke-stats.csv`, `/tmp/llaminar-mtp-bench/dense-cuda-baseline-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-mtp-gpugraphs-c64-n4.json`, `/tmp/llaminar-mtp-bench/dense-cuda-mtp-gpugraphs-c64-n4-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-baseline-c64-n4-shortprompt-bench.json`, `/tmp/llaminar-mtp-bench/dense-cuda-mtp-gpugraphs-c64-n4-combined-bench.json`, `/tmp/llaminar-mtp-bench/dense-cuda-mtp-gpugraphs-c64-n4-combined-stats.json` | CUDA graph capture is viable at small context, but MTP is still slightly slower than baseline. Row-wise native small-M verifier GEMV plus fused-SwiGLU down row-wise GEMV raised the current clean `-n 4` run to 38.16 tok/s at 100% acceptance. The instrumented run reached 36.01 tok/s; `verifier_forward` averaged about 53.1 ms per measured MTP step while `sidecar_forward` averaged about 2.0 ms. Main-verifier GPU stage time averaged about 38.8 ms, led by gate/up, down GEMM, and GDN projection. The `-n 16` run exposed a second blocker: acceptance fell to 37.5%, causing rollback/replay and only 0.17x baseline. Next work should shave the remaining main-verifier graph work below the no-MTP baseline and then improve longer-run draft acceptance. Larger-context evidence still needs a GPU that fits the model. |
...
- Small-M fused-SwiGLU verifier Phase 14 rerun:
  ...
  - This improves the same-prompt MTP run from 18.95 to 36.01 tok/s after
    ...
    work; the longer `-n 16` blocker remains draft acceptance and replay.
