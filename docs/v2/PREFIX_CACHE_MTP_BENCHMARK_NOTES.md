...
- The lower-level mixed native-VNNI small-M batched launcher is now used by the
  GDN projection stage for native small-M qkv/z/alpha/beta groups even when the
  projections carry different native codebook ids. Focused validations passed:
  ...
  recorded 48 mixed batched calls, zero mixed-codebook bypasses, three
  verifier-row shortcuts, and no `replay_forward`. This is a small speed ratchet
  over the prior stable-workspace 34.22 tok/s run and a larger coverage win:
  mixed-codebook native small-M GDN projection groups no longer split back to
  per-projection GEMVs.
