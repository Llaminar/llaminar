...
| SingleDevice | CUDA `cuda:0` | Qwen3.6 dense 27B Q4_K_S | 41.39 current `-c 64 -n 16`; 43.76 current no-stats `-c 64 -n 4`; 39.44 for the 10-token combined-telemetry prompt | Small-context dense MTP reaches segmented replay with zero manual stages. Main verifier, full decode sidecar, and KV-only shifted-row catch-up decode graphs are capturable/replayed; default 4096-context run still does not fit this 24 GB device | N/A | 38.16 current same-prompt `-n 4` clean run; 32.15 KV-only catch-up `-n 16` combined telemetry; 31.42 corrected shifted-cache `-n 16` clean run | 0.87x best clean; 0.78x KV-only `-n 16` combined telemetry | `/tmp/llaminar-mtp-bench/dense-cuda-current-baseline-c64-n16.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-shifted-catchup-c64-n16.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-shifted-catchup-c64-n16-combined.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c64-n16-combined.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c64-n16-combined-stats.json`, `/tmp/llaminar-mtp-bench/dense-cuda-current-mtp-kvonly-catchup-c64-n16-combined-stats.csv`; historical artifacts are in Latest CUDA dense evidence below | Shifted MTP KV catch-up is correct for the `-n 16` CUDA dense smoke: `runtime_state.current_position=20` and `mtp_cached_tokens=19`, with 87.5% acceptance. KV-only catch-up removed the discarded attention/FFN/LM-head tail and reduced catch-up replay to a 12-stage graph at about 0.20 ms/call, improving combined MTP from 31.35 to 32.15 tok/s. MTP still trails baseline because the 2-token verifier dominates at about 51.8 ms per measured verifier. Next work should make the graph-captured main verifier materially cheaper than two ordinary decode tokens before chasing deeper drafts. |
