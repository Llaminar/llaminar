...
| SingleDevice chained-draft diagnostic | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S chained depth-2 MTP | 17.95 stage-timed `-c 64`, `-n 8` baseline from the same prompt class | Correctness-smoke green for chained one-token sidecar drafts with verifier `M=3`. The second chained sidecar writes draft-only shifted MTP KV rows; commit now discards those speculative rows before appending verifier-backed shifted rows. | N/A | 10.76 chained depth-2 `-c 64`, `-n 8` | 0.60x versus the stage-timed depth-1 baseline; slower than the 21.49 tok/s depth-1 MTP run | `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-stats.csv`; regression: `V2_Unit_MTPGraphConstruction.ChainedSidecarCommitDiscardsSpeculativeShiftedRows` | Depth-2 is now a correctness path, not a speed lever. The run produced 80% acceptance, 24 draft steps, 16 accepted tokens, 4 rejected tokens, and 4 rollbacks, but measured `verifier_forward` averaged about 167.26 ms for verifier `M=3` and rejection replay averaged about 207.11 ms. `speculative_shifted_rows_discarded=12` proves the shifted-KV truncation contract is active. Next ROCm sprint should attack verifier/replay M=3/M=4 kernel cost before trying to use deeper drafts for Phase 14 speedup evidence. |
