...
step with the condition token, first main token, draft/verifier token lists,
accepted speculative prefix length, rejected correction token when present,
whether a lagged correction replay shortcut was used, and whether verifier
state was committed or replayed.

Most current speed-positive Phase 14 MTP benchmark evidence is still for the
depth-1 sidecar path (`--mtp-draft-tokens 1`). SingleDevice runners now also
support chained depth-2/depth-3 sidecar drafts for verifier `M=3/4`; treat
those as diagnostic until the small-M verifier/replay kernels make deeper
drafts faster than the depth-1 path.
...
Important profiling caveat: `LLAMINAR_PROFILING=1` enables executor/stage
profiling and intentionally disables segmented GPU graph capture/replay so
per-stage timing events can be recorded. Use those runs to diagnose kernel and
stage buckets, not to update best graph-captured speedup columns. Speedup
...
Latest ROCm verifier graph lifetime hardening, 2026-06-03: all-position
verifier logits now use stable per-row-count tensor owners. This fixes a
ROCm benchmark crash where a full-depth verifier graph held an LM-head output
pointer for `M=4`, a later tail verifier resized the single all-position
logits tensor to a smaller row count, and the next request replayed the cached
full-depth graph with a dangling logits pointer. The focused real-model
regression now reproduces that sequence explicitly: full-depth verifier,
smaller tail verifier, `clearCache()`, then full-depth verifier cache reuse.
The same slice also clamps MTP speculative draft depth to the remaining decode
token budget, so a budget-one final step emits the first greedy token without
sidecar/verifier/rollback work, and parity telemetry now expects effective
draft depth rather than configured draft depth when `max_new_tokens` is short.
Focused validation passed: `V2_Unit_PrefillDecodeTransition`,
`V2_Unit_MTPGraphConstruction`, `V2_Unit_GDNKernels`,
...
Real Qwen3.6 dense 27B Q4_K_S ROCm evidence on `The quick brown fox`,
`-c 64`, `-n 48`, depth-3 MTP reached 55.09 tok/s decode. This preserves the
55 tok/s ratchet rather than raising it; the exported stats recorded 39
full-depth verifier graph cache hits, 156 verifier rows sampled by the batched
ROCm argmax path, and 12 verifier-row GDN rollback shortcuts across 48 GDN
layers. Artifacts:
`/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-bench.json`,
`/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-stats.json`,
and
`/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-stats.csv`.

Latest ROCm verifier-row batched argmax, 2026-06-03: the GPU backend now has a
batched all-position argmax API for verifier rows. ROCm implements it as one
partial/finalize pair over all requested rows using the graph arena's declared
argmax scratch; CUDA and default backends retain the row loop through the shared
interface. `OrchestrationRunner` samples verifier rows in one call, records
`mtp.verifier_token_samples_batched`, and the ROCm runner records
`mtp.verifier_token_device_batch_samples`. Focused validation passed:
`V2_Unit_PrefillDecodeTransition`, `V2_Integration_GPUSamplingKernels`,
`V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and the
Qwen3.6 ROCm parity cells `MTPGreedyMatchesPyTorchDecodeTokens`,
`MTPGreedyDepth3MatchesPyTorchDecodeTokens`, and `PrefixCacheMTPRestore`.
Real Qwen3.6 dense 27B Q4_K_S ROCm evidence on `The quick brown fox`,
`-c 64`, depth-3 MTP: the short `-n 8` lane reached 33.78 tok/s versus a
32.03 tok/s same-binary baseline, and the long `-n 48` lane reached 55.04 tok/s
versus a 30.64 tok/s same-binary baseline, about 1.80x. The batched sampling
timer averaged about 70 us on the long lane; captured `main_verifier` replay
still averaged about 44.3 ms, so the next Phase 13.5 target remains verifier
GPU work rather than sampling overhead.
...
Latest post-block-sum ROCm verifier diagnostic, 2026-06-03: Qwen3.6 dense 27B
Q4_K_S on `rocm:0`, GPU graphs enabled, `The quick brown fox`, `-c 64`, `-n 8`,
depth-3 MTP. The production-path short smoke reached 33.09 tok/s decode with
80.00% acceptance and active sidecar/sample sync fusion: 9 first-sidecar and 18
chained-sidecar `sidecar_forward_sample_sync_fusions`, with sidecar replay total
down to about 81.6 us for first sidecars and 39.2 us for chained sidecars once
final sync is deferred. The matching stage-timed MTP diagnostic reached 32.88
tok/s with 70.00% acceptance; its verifier GPU stage split was `GEMM` 17.25
ms/call, `GEMM_FUSED_GATE_UP` 12.46 ms/call, `GDN_PROJECTION` 7.14 ms/call,
`GDN_RECURRENCE` 2.88 ms/call, attention 2.44 ms/call, and LM head 1.57 ms/call.
A matching no-MTP stage-timed baseline reached 31.98 tok/s. Existing graph-safe
K partitioning remains necessary: forcing `LLAMINAR_ROCM_NVNNI_GEMV_KB=1`
collapsed the same MTP lane to 17.53 tok/s, and `KB=4` reached only 30.77 tok/s,
so do not replace the default auto/KB=8-ish atomic path with a lower split count
without stronger evidence. Artifacts:
`/tmp/llaminar-mtp-bench/dense-rocm-blocksums-syncfusion-mtp-d3-c64-n8-bench.json`,
`/tmp/llaminar-mtp-bench/dense-rocm-blocksums-syncfusion-mtp-d3-c64-n8-stats.csv`,
`/tmp/llaminar-mtp-bench/dense-rocm-blocksums-stage-mtp-d3-c64-n8-bench.json`,
`/tmp/llaminar-mtp-bench/dense-rocm-blocksums-stage-mtp-d3-c64-n8-stats.csv`,
`/tmp/llaminar-mtp-bench/dense-rocm-blocksums-stage-baseline-c64-n8-bench.json`,
`/tmp/llaminar-mtp-bench/dense-rocm-kb1-mtp-d3-c64-n8-bench.json`, and
`/tmp/llaminar-mtp-bench/dense-rocm-kb4-mtp-d3-c64-n8-bench.json`.
...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice latest ROCm dense depth-3 verifier graph lifetime ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.64 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense MTP through verifier M=4. The path includes GDN verifier-row rollback restore, no legacy post-warmup resegment API/counter, dense sidecar collective metadata cached at graph-cache build time, sidecar/sample sync fusion, batched depth-0 `kv_cache_only` catchup graphs for verifier-backed shifted rows at `seq_len=2/3`, ROCm batched verifier-row argmax through declared arena scratch, stable per-row-count all-position verifier logits tensors for cached graph lifetime, and budget-aware draft-depth clamping. | N/A | 55.09 graph-enabled depth-3 MTP, same prompt/context/decode length | 1.80x same-binary; current ROCm SingleDevice dense long-lane ratchet | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-baseline-c64-n48-bench.json`; latest MTP `/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-budgetclamp-mtp-d3-c64-n48-stats.csv`; prior MTP `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-stats.csv`; short-lane artifacts `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-baseline-c64-n8-bench.json` and `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n8-bench.json`; focused regressions `V2_Unit_PrefillDecodeTransition`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_GDNKernels`, `V2_Integration_ROCmGDNPaddedRealLength`, `V2_Integration_GPUSamplingKernels`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and focused Qwen3.6 ROCm PyTorch parity cells `MTPGreedyMatchesPyTorchDecodeTokens`, `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, and `PrefixCacheMTPRestore` | The latest slice fixed a cached `LMHeadStage` dangling logits pointer found by the long-lane benchmark and added a real-model regression that replays full-depth verifier cache after a smaller tail verifier and request reset. The latest request recorded 86.33% acceptance, 120 accepted tokens, 19 rejected tokens, 19 rollbacks, 53 verifier runs, and 210 verifier tokens; structured stats recorded 39 full-depth verifier graph cache hits, 156 batched verifier-row samples, and 12 GDN verifier-row rollback shortcuts across 48 GDN layers. This preserves the speed ratchet but remains short of the Phase 14 2x target because captured verifier replay remains the dominant cost. Next work is shrinking ordinary GEMM, fused Gate/Up, GDN projection/recurrence, attention, and LM head inside the captured verifier graph. |
...
| SingleDevice prior ROCm dense depth-3 no-resegment ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.93 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense MTP through verifier M=4. The current path includes GDN verifier-row rollback restore, no legacy post-warmup resegment API/counter, stable arena write-id replay marking, correct `HIDDEN_STATE` terminal-hidden binding for non-prefix sidecars, and batched depth-0 `kv_cache_only` catchup graphs for verifier-backed shifted rows at `seq_len=2/3`. | N/A | 54.78 graph-enabled depth-3 MTP, same prompt/context/decode length; prior depth-3 reached 54.69 | 1.77x same-binary; preserves and slightly improves the ROCm SingleDevice dense long-lane ratchet | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-baseline-current-c64-n48-bench.json`; latest depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-stats.csv`; prior depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.json`; focused regressions `V2_Unit_ForwardGraphTypes`, `V2_Unit_MoERoutingStage`, `V2_Unit_PrefillGraphCapturability`, `V2_Unit_MTPGraphConstruction`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and focused Qwen3.6 ROCm PyTorch parity cells | Depth-3 remains short of the Phase 14 2x target because captured verifier replay still dominates. The latest request recorded 81.02% acceptance, 111 accepted tokens, 26 rejected tokens, 27 rollbacks, 55 verifier runs, and 220 verifier tokens. Structured stats contain zero `forward_graph.post_warmup_resegment` records, preserve 12 verifier-row GDN rollback shortcuts across 48 GDN layers, and show captured `main_verifier` replay still averaging about 44.39 ms with `mtp.verifier_forward` about 46.76 ms. Remaining work is shrinking captured `main_verifier` GPU work. |
| SingleDevice prior ROCm dense depth-3 warmup-dependent ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.67 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense MTP through verifier M=4. The current small-M route covers depth-1/2/3 verification with active ROCm tiny FP32 GDN alpha/beta calls and native-VNNI small-M batched calls at M=3 and M=4. Segmented replay now caches stable arena write ids and marks them flags-only, avoiding both stale raw tensor pointers and per-output event recording. Supported MoE stages are now planned capturable before warmup through the explicit warmup-dependent capture contract, with a hard fail if they remain uncapturable at graph capture time. | N/A | 54.48 graph-enabled depth-3 MTP, same prompt/context/decode length; depth-2 reached 48.66, depth-1 reached 47.59 | 1.78x same-binary; prior ROCm SingleDevice dense long-lane best | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-baseline-c64-n48-bench.json`; latest depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-stats.csv`; prior depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-arenawritecache-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d3-c64-n48-bench.json`; depth-2 MTP `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d2-c64-n48-bench.json`; depth-1 MTP `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-bench.json`; focused parity `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_MTPGreedyDepth3MatchesPyTorchDecodeTokens` plus the existing depth-1 MTP and prefix+MTP restore cells; crash regression `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | Depth-3 remained short of the Phase 14 2x target. The run recorded 87.77% acceptance, 122 accepted tokens, 17 rejected tokens, 18 request rollbacks, 53 verifier runs, and 212 verifier tokens. Structured stats recorded `forward_graph.post_warmup_resegment{reason=not_required}=15`, 12 verifier-row shortcut restores across 48 GDN layers, no suffix/replay-forward counters, captured M=4 `main_verifier` replay averaging about 44.42 ms, and sidecar-chain replay averaging about 2.10 ms. Remaining work was improving acceptance/deeper rollback cost and shrinking ordinary GEMM, fused Gate/Up, GDN projection, and LM-head verifier buckets. |
| SingleDevice latest ROCm dense ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.67 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense depth-1 MTP. Dense stable-capturability stages skip the MoE-only post-warmup resegment pass (`forward_graph.post_warmup_resegment{reason=not_required}=12`). GDN verifier-row restore is active for reject rollback, restoring both short-conv and recurrence state across 48 GDN layers before KV truncate. ROCm FP32 GDN alpha/beta projections now use the graph-capturable tiny batched route for M=1/2/4, N=48, batch=2 through declared pointer-array workspace. | N/A | 47.59 graph-enabled depth-1 MTP, same prompt/context/decode length | 1.55x same-binary; prior depth-1 ROCm SingleDevice dense long-lane best | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-baseline-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-stats.csv`; stage diagnostic `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-stagegpu-mtp-c64-n8-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-stagegpu-mtp-c64-n8-stats.json`; focused validation `V2_Integration_HipBLASGemm`, `V2_Integration_ROCmGDNPaddedRealLength`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and focused Qwen3.6 SingleDevice ROCm parity cells `MTPGreedyMatchesPyTorchDecodeTokens` and `PrefixCacheMTPRestore` | Depth-1 speedup is materially higher than earlier depth-1 runs but still short of the Phase 14 2x target. Long-lane MTP recorded 90.62% acceptance, 87 accepted tokens, 9 rejected tokens, and 9 rollbacks. Captured `main_verifier` replay now averages about 34.92 ms in the structured stats window, `mtp.verifier_forward` about 36.15 ms, and sidecar about 2.51 ms. Stage-timed `-n 8` diagnostics show the remaining GPU buckets are ordinary GEMM, fused Gate/Up, and GDN projection, while wall-vs-GPU gap remains large under stage timing. Next lever is reducing verifier graph overhead and dense/GDN verifier GPU work enough to approach 2x. |
| SingleDevice graph-valid longer smoke | ROCm `rocm:0`/`rocm:1` | Qwen3.6 dense 27B Q4_K_S | 21.20 post-resegment same-binary on `rocm:1`, graph-enabled, no executor profiling, `The quick brown fox`, `-c 64`, `-n 48`; 20.82 stable-workspace same-binary on `rocm:0`; 20.99 post-row-cache same-binary; 21.19 earlier graph-enabled lane | Fully captured for dense depth-1 MTP. `forward_graph` counters report `allow_segmented=true`, zero manual segments, and captured replay records for `main_verifier`, `mtp_decode_sidecar`, and `mtp_decode_catchup`. Dense stable-capturability stages now skip the MoE-only post-warmup resegment pass. | N/A | 34.75 post-resegment skip on `rocm:1`, graph-enabled depth-1 MTP; 34.28 mixed-native-GDN rerun on `rocm:0`; 34.22 stable-workspace prior ratchet; 34.15 post-row-cache; 33.57 older graph-enabled best | 1.64x current post-resegment same-binary; 1.65x retained same-binary ratio from the `rocm:0` mixed-native run | Post-resegment baseline `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-baseline-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-baseline-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-baseline-c64-n48-stats.csv`; post-resegment MTP `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm1-postresegment-mtp-d1-c64-n48-stats.csv`; stable baseline `/tmp/llaminar-mtp-bench/dense-rocm-stablews-baseline-c64-n48-bench.json`; mixed MTP `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-stats.csv`; first mixed run `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-stats.csv`; prior MTP `/tmp/llaminar-mtp-bench/dense-rocm-stablews-mtp-d1-c64-n48-bench.json`; post-row-cache baseline `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-baseline-c64-n48-bench.json`; post-row-cache MTP `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.csv`; context-budget regression `/tmp/llaminar-mtp-bench/dense-rocm-context-guard-c64-n64-bench.json`, `V2_Unit_BenchmarkRunnerCPU.FailsBeforePrefillWhenPromptPlusDecodeExceedsContext` | Current speedup is a small ROCm SingleDevice dense ratchet but still below the 2x target. The post-resegment run recorded `forward_graph.post_warmup_resegment{reason=not_required}=12`, proving dense verifier/sidecar graphs no longer pay the MoE warmup-dependent segment rebuild. It also recorded 91.67% acceptance, six measured reject rollbacks using `rollback_verifier_state_row_shortcuts`, six `verifier_state_row_restores` over 48 GDN layers, no `replay_forward`, `mtp.verifier_forward` averaging about 51.08 ms per two-token captured verifier, and sidecar averaging about 2.49 ms. The remaining lever is reducing verifier GPU work and per-step sidecar/sampling overhead; current deeper M=3/4 drafts are still slower because acceptance and rollback rates erase verifier amortization. |
| SingleDevice mixed-codebook native route | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 20.95 same-binary, graph-enabled, no executor profiling, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense depth-1 MTP with the lower-level mixed-codebook native-VNNI small-M batched launcher. The former `mixed_codebook` batched-route bypasses are gone for the covered Q4_K/Q5_K-equivalent GDN groups. | N/A | 33.90 graph-enabled depth-1 MTP, same prompt/context/decode length | 1.62x same-binary, no new best | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-baseline-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-stats.csv`; focused regressions `V2_Integration_ROCmQuantisedGemmSmallM` mixed-codebook Qwen3.6 cases and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | This is a Phase 13.5 contract improvement rather than a speed ratchet. Mixed batched calls fired for M=2/M=4 projection groups, graph replay still had zero manual segments, and acceptance improved to 95.83%, but `mtp.verifier_forward` averaged about 51.90 ms versus the previous 51.72 ms and decode stayed below the 34.15 tok/s ratchet. Keep it because it removes a codebook coverage gap; keep optimizing verifier GPU work separately. |
| SingleDevice verifier-row shortcut | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 20.33 same-binary, graph-enabled, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense depth-1 MTP. Reject rollback now restores captured GDN verifier-row state before falling back to checkpoint replay, then commits shifted rows from the partial verifier forward with explicit speculative-row discard. | N/A | 32.83 graph-enabled depth-1 MTP, same prompt/context/decode length | 1.62x same-binary, no new best | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-baseline-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-stats.csv`; regressions `V2_Unit_GDNMathematicalCorrectness`, `V2_Unit_GDNKernels`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and `V2_Integration_Parity_Qwen36_SingleDevice_` | Shortcut is correct and active: stats recorded three `rollback_verifier_state_row_shortcuts`, three `verifier_state_row_restores` over 48 GDN layers, no `replay_forward`, and `restore_verifier_state_row` averaged about 0.23 ms. It removes rollback replay cost but does not raise the speed ratchet because rejects are rare and captured `verifier_forward` still averages about 52.41 ms. Next lever remains verifier GPU work and deeper-draft amortization. |
...
| SingleDevice chained-draft diagnostic | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S chained depth-2 MTP | 21.19 graph-enabled `The quick brown fox`, `-c 64`, `-n 48`; 20.99 lazy-reject same-binary control at `-c 64`, `-n 48`; 17.95 stage-timed `-c 64`, `-n 8` | Correctness-smoke green for chained one-token sidecar drafts with verifier `M=3`. The second chained sidecar writes draft-only shifted MTP KV rows and those speculative rows are discarded before appending verifier-backed shifted rows. Post-fix, verifier `seq_len=3` is classified as decode and uses the `main_verifier` graph cache/capture path. | N/A | 30.43 graph-captured depth-2 `-c 64`, `-n 48`; 27.98 rejected lazy-correction experiment, not retained; 26.01 stage-timed depth-2 `-c 64`, `-n 8` after the decode-cache fix | 1.44x versus the graph-enabled baseline, but slower than the 33.57 tok/s depth-1 run | `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d2-c64-n48-postdecode4-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d2-c64-n48-postdecode4-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d2-c64-n48-postdecode4-stats.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-stage-mtp-d2-c64-n8-postdecode4-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-stage-mtp-d2-c64-n8-postdecode4-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-stage-mtp-d2-c64-n8-postdecode4-stats.csv`; rejected lazy-correction artifacts `/tmp/llaminar-mtp-bench/dense-rocm-lazy-reject-baseline-d2-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-lazy-reject-mtp-d2-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-lazy-reject-mtp-d2-c64-n48-stats.json`; regressions: `V2_Unit_ForwardExecutionEngineAdvanced.ThreeTokenAllPositionVerifierUsesDecodeCache`, `V2_Unit_ForwardGraphTypes`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, `V2_Unit_MTPGraphConstruction.ChainedSidecarCommitDiscardsSpeculativeShiftedRows` | The old depth-2 slowdown was partly a routing bug: verifier `M=3` was treated as prefill and missed decode graph replay. Raising the decode-cache default to 4 fixed that path. Depth-2 is now graph-captured and speed-positive versus baseline, but not yet a Phase 14 win over depth-1 because acceptance fell to 87.10%, rollbacks rose to 20, and `mtp.verifier_forward` still averaged about 64.15 ms for each three-token verifier. A later lazy-correction experiment deferred first-spec reject corrections into the next step to skip catch-up work, but real ROCm evidence rejected it: it reached only 27.98 tok/s and changed measured counters in the wrong direction (`rejected_tokens` 12 to 15, `rollbacks` 15 to 18, `replay_tokens` 30 to 45 in the exported stats window), so the default path remains the earlier lagged-correction replay. |
...
| SingleDevice | ROCm | Qwen3.6 MoE 35B | 21.23 | Partial: MTP GPU graphs survive rollback/restore through the `-n 4` crash reproducer, stable MoE placement now preserves captured replay state across live prefix restore/truncate, and supported MoE router/expert/shared stages use a warmup-dependent graph-capture contract instead of post-warmup segment reconstruction. MoE placement epoch remains part of main forward graph identity, while placement-changing maintenance explicitly invalidates sidecar caches; stable placement no longer forces per-call sidecar graph rebuild. | N/A | 10.89 | 0.51x decode | `/tmp/llaminar-mtp-bench/moe-rocm-baseline-n4.json`, `/tmp/llaminar-mtp-bench/moe-rocm-mtp-gpugraphs-n3-fixed.json`, `/tmp/llaminar-mtp-bench/moe-rocm-mtp-gpugraphs-n4-fixed.json`; focused replay/capture regressions `V2_Unit_ForwardGraphTypes`, `V2_Unit_ForwardExecutionEngine`, `V2_Unit_MoERoutingStage`, `V2_Unit_PrefillGraphCapturability`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | Replay reconstruction cost for stable MoE placement is removed, and supported MoE graph-capture lanes no longer need a post-warmup segment rebuild. MTP remains slower than baseline with 0% acceptance on the older prompt; a fresh real MoE prefix+MTP parity attempt on ROCm failed before inference because resident expert GPU preparation exceeded the ROCm:0 VRAM safety-margin budget and then lacked a GPU expert GEMM engine for layer 0. Need a passing real MoE ROCm parity/benchmark lane after the expert-preparation blocker is fixed. |
...
| SingleDevice | CPU `cpu:0` | Qwen3.6 dense 27B Q4_K_S | 5.80 prior same-prompt baseline at `The quick brown fox`, `-c 64`, `-n 4` | N/A for GPU graphs. CPU graph execution now allocates declared `IWorkspaceConsumer` buffers, including GDN verifier-state capture buffers, through `WorkspaceAllocator` instead of assuming workspace declarations are GPU-only. | N/A | 9.50 depth-3 MTP at `The quick brown fox`, `-c 64`, `-n 4` | 1.64x versus the prior same-prompt baseline; short smoke only | Baseline `/tmp/llaminar-mtp-bench/dense-cpu-single-baseline-c64-n4-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-cpu-single-mtp-d3-c64-n4-arenadirty-bench.json`, `/tmp/llaminar-mtp-bench/dense-cpu-single-mtp-d3-c64-n4-arenadirty-stats.json`, `/tmp/llaminar-mtp-bench/dense-cpu-single-mtp-d3-c64-n4-arenadirty-stats.csv`; regressions `V2_Unit_WorkspaceAllocator`, `V2_Unit_ForwardGraphTypes`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_ROCmGDNPaddedRealLength`, and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | This closes the immediate CPU real-model hard-fail where MTP decode could not bind `gdn_shortconv_verifier_state_capture_layer*` on CPU. The smoke recorded 12 accepted draft tokens, zero rejected tokens, zero rollbacks, 4 verifier runs, 16 verifier tokens, active `kernel.cpu_native_vnni_small_m_fused_projection_calls` at M=4, and aligned runtime state (`current_position=8`, `mtp_cached_tokens=7`). Because this short prompt had 100% acceptance, it does not yet stress CPU rollback-row restore; longer CPU lanes remain useful before Phase 14 CPU acceptance. |
...
- Fresh batched shifted-cache catchup on 2026-06-03: depth-0 MTP catchup for
  verifier-backed shifted rows now builds and replays one `kv_cache_only`
  sidecar graph for the contiguous accepted/correction row group instead of
  selecting each hidden row and replaying depth-0 one row at a time. The graph
  construction contract allows multi-row sidecars only for `kv_cache_only`
  catchup and hard-fails multi-row full sidecars. Focused validation passed:
  `V2_Unit_MTPGraphConstruction` and
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`. The
  same-lane Qwen3.6 dense 27B Q4_K_S ROCm benchmark reached 54.69 tok/s decode
  and 54.73 overall tok/s at depth-3 MTP versus a 30.93 tok/s same-binary
  baseline on `The quick brown fox`, `-c 64`, `-n 48`, preserving the 1.77x
  long-lane speedup ratchet. Artifacts are
  `/tmp/llaminar-mtp-bench/dense-rocm-baseline-current-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.csv`.
  The latest request recorded 81.02% acceptance, 111 accepted tokens,
  26 rejected tokens, 27 rollbacks, 55 verifier runs, and 220 verifier tokens.
  Compared with the prior warmup-dependent ratchet artifact, structured stats
  moved measured `mtp_decode_catchup` replay from about 31.66 ms to 7.07 ms and
  `mtp.shifted_row_commit` from about 41.57 ms to 24.14 ms. The run preserved
  12 verifier-row GDN rollback shortcuts and zero suffix/replay-forward
  counters. Main verifier replay remains the dominant blocker at about
  44.38 ms per measured M=4 verifier graph, so the next ROCm speed lever is
  still verifier GPU work and reducing host-synchronized depth-3 sidecar steps.
- Fresh no-post-resegment ROCm dense ratchet on 2026-06-03: the legacy
  post-warmup segment-rebuild API and `forward_graph.post_warmup_resegment`
  counter path were removed. Supported MoE stages now use only the explicit
  warmup-dependent capture contract and hard-fail at capture time if still not
  graph-capturable; dense MTP no longer records a `reason=not_required` warmup
  bookkeeping counter. The same Qwen3.6 dense 27B Q4_K_S lane on `rocm:0`,
  `The quick brown fox`, `-c 64`, `-n 48`, depth-3 MTP reached 54.78 decode
  tok/s and 54.75 overall tok/s with artifacts
  `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-no-postresegment-mtp-d3-c64-n48-stats.csv`.
  Structured stats recorded zero `post_warmup_resegment` records, 12
  verifier-row GDN shortcut restores over 48 GDN layers, captured
  `main_verifier` replay around 44.39 ms, and `mtp.verifier_forward` around
  46.76 ms. This slice also fixed a sidecar graph-cache regression where
  non-prefix `forwardMTP()` used the `PREFIX_TERMINAL_HIDDEN` buffer id even
  when the terminal row came from `HIDDEN_STATE`, which could surface as a stale
  buffer lookup after `clear_cache()`. Focused validation passed:
  `V2_Unit_ForwardGraphTypes`, `V2_Unit_MoERoutingStage`,
  `V2_Unit_PrefillGraphCapturability`, `V2_Unit_MTPGraphConstruction`,
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and the
  Qwen3.6 ROCm PyTorch parity cells `MTPGreedyMatchesPyTorchDecodeTokens`,
  `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, and `PrefixCacheMTPRestore`.
- Fresh sidecar/sample sync-fusion ROCm dense recheck on 2026-06-03: GPU
  `DeviceGraphOrchestrator` now exposes a runner-native MTP sidecar
  forward+greedy-sample path. Dense sidecar segmented replay can defer its
  final capture-stream sync and enqueue argmax on the same stream, using the
  argmax D2H sync as the operation boundary. The current-code recheck for
  Qwen3.6 dense 27B Q4_K_S on `rocm:0`, GPU graphs enabled,
  `The quick brown fox`, `-c 64`, `-n 48`, depth-3 MTP reached 54.32 decode
  tok/s versus the 30.93 tok/s same-binary baseline; the immediately prior
  same-slice run reached 54.99 tok/s. Artifacts are
  `/tmp/llaminar-mtp-bench/dense-rocm-sidecar-samplefusion-current-mtp-d3-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-sidecar-samplefusion-current-mtp-d3-c64-n48-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-sidecar-samplefusion-current-mtp-d3-c64-n48-stats.csv`.
  Structured counters prove the path fired: 39 first-sidecar and 78
  chained-sidecar `sidecar_forward_sample_sync_fusions`, 33 first-sidecar and
  72 chained-sidecar `segmented_replay_final_sync_deferred` records, 117
  `mtp_token_device_samples`, and no separate `sample_mtp_token_device` timer.
  Focused validation passed:
  `V2_Unit_PrefillDecodeTransition`, `V2_Unit_MTPGraphConstruction`,
  `V2_Unit_PrefillGraphCapturability`,
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and the
  Qwen3.6 ROCm PyTorch parity cells `MTPGreedyMatchesPyTorchDecodeTokens`,
  `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, and `PrefixCacheMTPRestore`.
...
- Fresh ROCm deeper-draft ratchet on 2026-06-03: after the tiny FP32 GDN
  ...
  `The quick brown fox`, `-c 64`, `-n 48`. Depth-2 artifact
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d2-c64-n48-bench.json`
  reached 48.66 decode tok/s, with 86.29% acceptance, 107 accepted tokens,
  17 rejected tokens, and 20 rollbacks. Depth-3 artifact
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d3-c64-n48-bench.json`
  reached 54.32 decode tok/s and 54.50 overall tok/s versus the same 30.67
  baseline, a 1.77x speedup and the new ROCm SingleDevice dense long-lane
  ratchet. Structured stats
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d3-c64-n48-stats.json`
  showed active M=4 tiny FP32 and native-VNNI small-M batched routes, 81.02%
  acceptance, 111 accepted tokens, 26 rejected tokens, 27 rollbacks, 55 verifier
  runs, and 220 verifier tokens. The new parity cell
  `V2_Integration_Parity_Qwen36_SingleDevice_Qwen36SingleDevicePrefixMTPParity_MTPGreedyDepth3MatchesPyTorchDecodeTokens`
...
- Fresh MoE warmup-dependent graph-capture contract on 2026-06-03: segmented
  capture now plans supported MoE router, expert, shared-FFN, and shared-gate
  stages as capturable before warmup instead of reconstructing the segment plan
  after warmup. Warmup still creates runtime tables, descriptor banks, kernels,
  and scratch on the capture stream; capture now hard-fails if a stage that
  promised warmup-dependent capture is still not graph-capturable. This removed
  the post-warmup resegment tax for supported MoE graph-capture lanes; a later
  cleanup removed the legacy `post_warmup_resegment` API and dense
  `reason=not_required` bookkeeping counter entirely. Focused validation passed:
  `V2_Unit_ForwardGraphTypes`, `V2_Unit_MoERoutingStage`,
  `V2_Unit_PrefillGraphCapturability`, `V2_Unit_MTPGraphConstruction`,
  `V2_Unit_PrefillDecodeTransition`, and
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`. The real
  ROCm Qwen3.6 MoE benchmark row remains blocked before inference by resident
  expert preparation/VRAM constraints, so this is a capture-contract slice rather
  than a new MoE speed ratchet.
...
- Fresh ROCm tiny FP32 alpha/beta plus GDN verifier-row shortcut evidence on
  ...
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-baseline-c64-n48-bench.json`
  reached 30.67 decode tok/s. MTP artifact
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-bench.json`
  reached 47.59 decode tok/s and 48.20 overall tok/s, a 1.55x same-binary
  speedup and a new absolute ROCm SingleDevice dense long-lane best. Structured
  stats:
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-stats.json`
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-mtp-d1-c64-n48-stats.csv`.
  The request recorded 90.62% acceptance, 87 accepted tokens, 9 rejected tokens,
  and 9 rollbacks. Stats showed
  `forward_graph.post_warmup_resegment{reason=not_required}=12`, active
  `rocm_fp32_tiny_batched_projection_calls` for GDN alpha/beta M=1/2/4,
  measured `rollback_verifier_state_row_shortcuts`, and
  `verifier_state_row_restores` over 48 GDN layers. A short stage-timed
  diagnostic artifact
  `/tmp/llaminar-mtp-bench/dense-rocm-gdnshortcut-tinyfp32-stagegpu-mtp-c64-n8-stats.json`
  showed remaining decode GPU time led by GEMM, fused Gate/Up, and GDN
  projection, with a large wall-vs-GPU gap under stage timing. Focused
  validation passed: `V2_Integration_HipBLASGemm`,
  `V2_Integration_ROCmGDNPaddedRealLength`,
...
- Fresh Phase 13.5 CPU all-codebook small-M fused projection coverage on
  ...
  This closes a CPU coverage gap in the Phase 13.5 GEMV-many contract.
- Fresh Phase 13.5 CPU SingleDevice real-model MTP smoke on 2026-06-03:
  Qwen3.6 dense 27B Q4_K_S on `cpu:0`, `The quick brown fox`, `-c 64`,
  `-n 4`, depth-3 MTP now runs after allocating CPU-side graph workspace
  declarations for GDN verifier state capture. Artifact
  `/tmp/llaminar-mtp-bench/dense-cpu-single-mtp-d3-c64-n4-arenadirty-bench.json`
  reached 9.50 decode tok/s and 11.00 overall tok/s versus the prior same-prompt
  baseline artifact
  `/tmp/llaminar-mtp-bench/dense-cpu-single-baseline-c64-n4-bench.json` at
  5.80 decode tok/s. Structured stats
  `/tmp/llaminar-mtp-bench/dense-cpu-single-mtp-d3-c64-n4-arenadirty-stats.json`
  recorded 12 accepted tokens, zero rejects, zero rollbacks, four verifier runs,
  16 verifier tokens, and active M=4
  `kernel.cpu_native_vnni_small_m_fused_projection_calls`. This removes the
  immediate CPU real-model hard-fail, while longer CPU runs with actual rejects
  remain useful before Phase 14 CPU acceptance.
- Fresh graph replay hardening on 2026-06-03: `WorkspaceAllocator` now binds
  declared CPU and GPU graph workspace consumers instead of treating declared
  workspaces as GPU-only, and segmented graph replay marks stage outputs dirty
  through `StageBufferContract`/`BufferArena` callbacks instead of caching raw
  `TensorBase *` output pointers inside replay metadata. The latter fixes a
  ROCm chained-draft smoke shutdown crash after MTP/prefix state mutation could
  leave stale dirty-pointer caches in preserved graph segments. Focused
  validation passed:
  `V2_Unit_WorkspaceAllocator`,
  `V2_Unit_ForwardGraphTypes`,
  `V2_Unit_MTPGraphConstruction`,
  `V2_Unit_PrefillDecodeTransition`,
  `V2_Integration_ROCmGDNPaddedRealLength`, and
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`.
- Fresh replay dirty-marking speed recovery on 2026-06-03: the safe replay
  path now caches stable `(BufferId, DeviceId)` arena writes per segment and
  marks them flags-only after captured replay. This preserves the no-raw-pointer
  invariant while removing the contract rewalk and per-output event recording
  from the hot path. Qwen3.6 dense 27B Q4_K_S on `rocm:0`, GPU graphs enabled,
  `The quick brown fox`, `-c 64`, `-n 48`, depth-3 MTP reached 54.44 decode
  tok/s and 54.63 overall tok/s, slightly above the prior 54.32 ratchet and
  1.78x over the 30.67 same-binary baseline. Artifacts are
  `/tmp/llaminar-mtp-bench/dense-rocm-arenawritecache-mtp-d3-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-arenawritecache-mtp-d3-c64-n48-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-arenawritecache-mtp-d3-c64-n48-stats.csv`.
- Fresh warmup-dependent capture rerun on 2026-06-03: after supported MoE
  stages gained an explicit pre-warmup capturable contract, the same Qwen3.6
  dense ROCm depth-3 lane reached 54.48 decode tok/s and 54.67 overall tok/s,
  preserving and slightly improving the depth-3 ratchet. Artifacts are
  `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-stats.csv`.
  The request summary recorded 87.77% acceptance, 122 accepted tokens, 17
  rejected tokens, 18 request rollbacks, 53 verifier runs, and 212 verifier
  tokens. Structured stats recorded
  `forward_graph.post_warmup_resegment{reason=not_required}=15`, 12
  verifier-row shortcut restores across 48 GDN layers, no suffix/replay-forward
  counters, captured `main_verifier` replay around 44.42 ms, and captured
  sidecar-chain replay around 2.10 ms.
...
- Fresh dense post-warmup resegment-skip recheck on `rocm:0`, 2026-06-03:
  ...
  `/tmp/llaminar-mtp-bench/dense-rocm-resegment-slice-mtp-d1-c64-n48-bench.json`
  reached 34.28 decode tok/s and 35.01 overall tok/s, preserving the retained
  34.28 tok/s `rocm:0` long-lane ratchet. Structured stats:
  `/tmp/llaminar-mtp-bench/dense-rocm-resegment-slice-mtp-d1-c64-n48-stats.json`
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-resegment-slice-mtp-d1-c64-n48-stats.csv`.
  The request recorded 91.67% acceptance, 88 accepted tokens, 8 rejected
  tokens, and 8 rollbacks. The measured perf window recorded
  `forward_graph.post_warmup_resegment{reason=not_required}=12`, proving dense
  verifier/sidecar capture no longer pays the MoE warmup-dependent resegment
  scan; `sidecar_graph_cache_hits=153` with no measured sidecar graph builds;
  six measured rollbacks, all using `rollback_verifier_state_row_shortcuts`;
  and a captured `main_verifier` replay average of about 49.79 ms. Focused
  validation passed:
  `V2_Unit_ForwardGraphTypes`, `V2_Unit_MoERoutingStage`,
  `V2_Unit_PrefillGraphCapturability`,
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and
  the two focused Qwen3.6 ROCm PyTorch parity cells
  `MTPGreedyMatchesPyTorchDecodeTokens` and `PrefixCacheMTPRestore`.
...
- Fresh post-row-cache ROCm SingleDevice dense long-lane evidence, Qwen3.6
  27B Q4_K_S on `rocm:0`, GPU graphs enabled, `The quick brown fox`,
  `-c 64`, `-n 48`: baseline
  `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-baseline-c64-n48-bench.json`
  reached 20.99 decode tok/s; depth-1 MTP
  `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-bench.json`
  reached 34.15 decode tok/s for a fresh 1.63x same-binary ratchet.
  Structured stats:
  `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.json`
  and `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.csv`.
  The request recorded 93.75% acceptance, 90 accepted tokens, 6 rejected
  tokens, and 6 rollbacks. The measured stats window recorded three reject
  row-restore shortcuts, three GDN verifier-row restores over 48 layers, no
  `replay_forward`, `mtp.verifier_forward` averaging about 51.72 ms, and
  `sidecar_forward` averaging about 2.62 ms. A current depth-2 retest
  `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d2-retry-c64-n48-bench.json`
  reached 30.31 decode tok/s with 81.30% acceptance, so depth-1 remains the
  ROCm speed path for this prompt.
- Partial MTP verifier commits now use the captured GDN verifier-row state
...
- The lower-level mixed native-VNNI small-M batched launcher is now used by the
  GDN projection stage for compatible native small-M projection subgroups even
  when the projections carry different native codebook ids. Focused validations passed:
  `V2_Unit_GDNKernels`, `V2_Integration_ROCmQuantisedGemmSmallM`,
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and the
  dense Qwen3.6 ROCm SingleDevice parity MTP cells. The parity cells are also
  now emitted with `RUN_SERIAL TRUE`; the regression was found when CTest
  started two 27B ROCm parity cases together and both failed memory preflight on
  a shared card. Current long-lane artifacts with GPU graphs enabled, `The quick
  brown fox`, `-c 64`, `-n 48`: first mixed run
  `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-mtp-d1-c64-n48-bench.json`
  reached 34.26 decode tok/s with 93.75% acceptance; rerun
  `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-bench.json`
  reached 34.28 decode tok/s with 94.79% acceptance. Structured stats
  `/tmp/llaminar-mtp-bench/dense-rocm-mixedbatched-rerun-mtp-d1-c64-n48-stats.csv`
  recorded 48 mixed batched calls across 2- and 3-projection groups, zero
  mixed-codebook bypasses, three verifier-row shortcuts, and no `replay_forward`.
  This is a small speed ratchet over the prior stable-workspace 34.22 tok/s run
  and a larger coverage win: mixed-codebook native small-M GDN projection
  subgroups no longer split back to per-projection GEMVs.
...
- Lagged rejected-correction replay shortcut, same ROCm/Qwen3.6 dense long
  lane, proves a rollback-cost reduction without changing the current speed
  ratchet. On a depth-1 reject, the runner restores to the post-sidecar
  checkpoint, replays only the already-accepted prefix token through the main
  graph, appends the correction token's shifted MTP row from that hidden state,
  emits both tokens, and lets the next ordinary decode condition-forward the
  correction. This avoids unsafe hidden/GDN truncation while cutting reject
  replay from two main tokens to one. Focused regressions:
  `V2_Unit_PrefillDecodeTransition`, `V2_Unit_MTPGraphConstruction`,
  `V2_Unit_RankOrchestrator`, and the full
  `^V2_Integration_Parity_Qwen36_SingleDevice_` slice. Fresh release evidence
  with GPU graphs enabled: baseline
  `/tmp/llaminar-mtp-bench/dense-rocm-lagged-reject-baseline-c64-n48-bench.json`
  at 20.99 decode tok/s; MTP
  `/tmp/llaminar-mtp-bench/dense-rocm-lagged-reject-mtp-c64-n48-bench.json`
  at 32.16 decode tok/s, 95.83% acceptance, 92 accepted tokens, 4 rejected
  tokens, and 4 rollbacks. Structured stats:
  `/tmp/llaminar-mtp-bench/dense-rocm-lagged-reject-mtp-c64-n48-stats.json`
  and `/tmp/llaminar-mtp-bench/dense-rocm-lagged-reject-mtp-c64-n48-stats.csv`.
  The request summary records 4 rollbacks; the measured PerfStats section,
  after warmup/reset, records `mtp.lagged_rejected_correction_replays=3` and
  `mtp.replay_tokens=3`, proving one replayed main token per measured reject.
  `mtp.replay_forward` averaged about 64.75 ms, while `restore_live_prefix_state`
...
- Graph-stage scratch policy for this sprint: allocations needed by captured
  ...
  workspace is missing or undersized. Backend-private lazy scratch allocation is
  only acceptable for non-graph legacy callers while the corresponding graph
  stage enforces the declared-buffer contract.
- Focused regressions:
...
- Same-binary no-stage-timing workspace-sizing pair completed on Qwen3.6 dense
  ROCm SingleDevice:
  `/tmp/llaminar-mtp-bench/dense-rocm-workspace-sizing-baseline-c64-n16-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-workspace-sizing-baseline-c64-n16-stats.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-workspace-sizing-mtp-c64-n16-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-workspace-sizing-mtp-c64-n16-stats.json`,
  and `/tmp/llaminar-mtp-bench/dense-rocm-workspace-sizing-mtp-c64-n16-stats.csv`.
  Baseline decode was 16.90 tok/s; depth-1 MTP decode was 24.42 tok/s with
  87.5% acceptance, 32 verifier runs, 64 verifier tokens, and 4 rollbacks. The
  combined stats artifact records `rocm_fp32_batched_projection_calls` for the
...
- Chained depth-2 MTP correctness smoke completed on Qwen3.6 dense ROCm
  SingleDevice:
  `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-stats.json`,
  and
  `/tmp/llaminar-mtp-bench/dense-rocm-chained-mtp-d2-c64-n8-rerun-stats.csv`.
  Decode was 10.76 tok/s with 80% acceptance, 24 draft steps,
  16 accepted tokens, 4 rejected tokens, 4 rollbacks, 12 verifier runs, and
  36 verifier tokens. This run proves the chained sidecar/verifier state
  contract and records `speculative_shifted_rows_discarded=12`, but it is
  slower than depth-1 because verifier `M=3` averaged about 167.26 ms and
  rejection replay averaged about 207.11 ms. Focused regression coverage:
  `V2_Unit_PrefixCachePrefillFlow` exercises chained draft acceptance/reject
  orchestration, and
  `V2_Unit_MTPGraphConstruction.ChainedSidecarCommitDiscardsSpeculativeShiftedRows`
  exercises the concrete shifted-KV truncation path.
- Follow-up chained depth-2 decode-cache fix completed. `GraphCacheConfig`
  now admits continuation `seq_len <= 4`, so depth-2 verifier `M=3` and
  future `M=4` verifier continuations use the decode graph cache instead of
  being treated as prefill. The stage-timed depth-2 diagnostic improved from
  about 11.14 tok/s before the fix to 26.01 tok/s after the fix, with
  `forward_graph.forward_cache_lookup` counters for `context=main_verifier`,
  `seq_len=3`, and both miss/hit results. The graph-captured depth-2 `-c 64`,
  `-n 48` run reached 30.43 tok/s with 87.10% acceptance, 68 verifier runs,
  204 verifier tokens, 20 rollbacks, and `mtp.verifier_forward` averaging about
  64.15 ms. Focused regression coverage:
  `V2_Unit_ForwardExecutionEngineAdvanced.ThreeTokenAllPositionVerifierUsesDecodeCache`,
  `V2_Unit_ForwardExecutionEngineAdvanced.FourTokenAllPositionVerifierUsesDecodeCache`,
  `V2_Unit_ForwardGraphTypes`, and
  `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`.
- Fresh graph-captured depth-1 ROCm SingleDevice dense smoke completed without
  `LLAMINAR_PROFILING`, so segmented GPU graph replay was allowed:
  `/tmp/llaminar-mtp-bench/dense-rocm-graph-repro-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-bench.json`,
  `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.json`,
  and `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.csv`.
  The valid-context baseline was 21.19 tok/s for `The quick brown fox`,
  `-c 64`, `-n 48`; depth-1 MTP was 33.57 tok/s with 95.83% acceptance,
  96 draft steps, 92 accepted tokens, 4 rejected tokens, 4 rollbacks,
  96 verifier runs, and 192 verifier tokens. `forward_graph` counters record
  `allow_segmented=true`, zero manual segments, and captured replay for
  `main_verifier`, `mtp_decode_sidecar`, and `mtp_decode_catchup`.
  `mtp.verifier_forward` averaged 52.21 ms per two-token captured verifier;
  `sidecar_forward` averaged 2.46 ms. This is the current best valid ROCm
  SingleDevice dense speedup evidence at 1.58x, still short of the 2x target.
- Regression from this profiling slice: `-c 64`, prompt length 4, `-n 64`
  previously generated past the context window and could surface as a ROCm HSA
  memory access fault during warmup decode. `BenchmarkRunner` now hard-fails
  before prefill/decode with a context-budget error:
...
- Depth-1 MTP is now speed-positive but structurally below the 2x Phase 14
  ...
  drafts must amortize verifier cost through faster M=3/M=4 small-M verifier
  kernels. The current depth-2 diagnostic shows that deeper drafts are not yet
  a speed lever until verifier/replay cost falls.
...
- Phase 13.5 fused-SwiGLU/FFN down small-M ROCm route:
  ...
  - Rejected mixed-codebook GDN subgroup batching experiment:
    ...
    originally required an explicit `mixed_codebook` batched-route bypass. The
    ...
    KFD process.
  - Lower-level mixed-codebook native-VNNI small-M launcher:
    `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-baseline-c64-n48-bench.json`,
    `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-bench.json`,
    `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-stats.json`,
    and `/tmp/llaminar-mtp-bench/dense-rocm-mixed-nvnni-mtp-d1-c64-n48-stats.csv`.
    The safe replacement is not subgroup splitting in the dispatcher; it is one
    graph-capturable batched native-VNNI kernel that dispatches each projection
    by codebook inside the batch. Focused regressions
    `GraphCapturedFusedMixedCodebookGDNProjectionM4UsesMixedBatchedRoute` and
    `GraphCapturedMixedCodebookQwen36GDNQkvZPairM4MatchesSeparate` now require
    the mixed route and prove the old `mixed_codebook` bypass is gone. Real
    Qwen3.6 counters recorded mixed batched calls for `M=2` and `M=4`
    projection groups, including `projections=2` and `projections=3`, while
    graph replay remained fully segmented with zero manual stages. The same
    long lane reached 33.90 tok/s versus a 20.95 tok/s baseline, 95.83%
    acceptance, and `mtp.verifier_forward` about 51.90 ms/call. This is
    neutral to slightly below the 34.15 tok/s ratchet, so treat it as a
    codebook coverage and graph-capture contract improvement rather than a
    speedup win.
...
- Shared-quant fused M=2 ROCm verifier route:
  ...
  - Current replay-preservation slice: live-prefix restore/truncate now
    preserves captured replay state instead of resetting the main and MTP graph
    replay caches, including stable single-device MoE placement. Real MoE
    placement changes are represented as graph-cache identity changes through
    `moe_placement_epoch`, so changed placement misses and rebuilds while
    unchanged placement avoids the old blanket live-state reset. Structured
    counters report `mtp.live_prefix_replay_state_preserved` with
    `moe_placement_epoch` tags for MoE.
    ...
    replay-reset events. Depth-2 MTP reached 29.63 tok/s with 107 accepted,
    17 rejected, 21 rollbacks, and 86.29% acceptance. The preserve change
    removes avoidable graph reconstruction; MoE placement epoch is part of
    decode/sidecar graph identity, so placement changes miss and rebuild
    without a blanket live-state reset. A targeted real MoE prefix+MTP parity
    attempt after this change failed before inference on ROCm:0 because the
    resident expert GPU preflight exceeded the VRAM safety-margin budget
    (`required=17162 MiB`, `available_after_margin=10742 MiB`) and then lacked
    a prepared GPU expert GEMM engine for layer 0.
  - Verifier-row GDN state restore slice: short-conv and GDN recurrence kernels
    ...
    dense rollback first tries to restore the accepted verifier row from the
    live post-verifier state before falling back to the post-sidecar checkpoint
    replay path. The runner then commits shifted MTP rows from the partial
    verifier forward with an explicit speculative-row discard flag and only
    replays a suffix when the accepted output truly extends beyond the restored
    row.
    Focused regression passed:
    ...
    does not restore the checkpoint first, restores verifier row 0 to cached
    token 6, records `mtp.rollback_verifier_state_row_shortcuts`, commits with
    explicit speculative-row discard, and skips the old replay-prefix forward.
    Dense Qwen3.6 SingleDevice parity passed for all five
    `V2_Integration_Parity_Qwen36_SingleDevice_` cases after the ordering fix.
    Fresh same-binary long-lane evidence on `rocm:0`, GPU graphs enabled,
    `The quick brown fox`, `-c 64`, `-n 48`: baseline
    `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-baseline-c64-n48-bench.json`
    at 20.33 decode tok/s; depth-1 MTP
    `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-bench.json`
    at 32.83 decode tok/s, 95.83% acceptance, 92 accepted tokens,
    4 rejected tokens, and 4 rollbacks. Structured stats:
    `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-stats.json`
    and `/tmp/llaminar-mtp-bench/dense-rocm-gdn-rowshortcut-mtp-d1-c64-n48-stats.csv`.
    This preserves the 1.62x speedup lane rather than setting a new best.
    ...
    events, with no `replay_forward` timer. `restore_verifier_state_row`
    averaged about 230 us, while the captured two-token `verifier_forward`
    still averaged about 52.41 ms/call,
    so the next ROCm SingleDevice sprint target remains the captured verifier
    GPU work and MTP per-step overhead.
  - Batched GDN projection perf coverage:
    `NativeVNNIGEMMPerfTest.MTP_SmallM_BatchedGDNProjectionShapes` now calls
    the graph-native `rocmGemv_native_vnni_small_m_batched_fp32()` path
    directly for Qwen3.6-style heterogeneous GDN projection groups, then
    ...
    roughly two codebook groups per GDN layer multiplied across the GDN-heavy
    layer stack, not a missing route.
