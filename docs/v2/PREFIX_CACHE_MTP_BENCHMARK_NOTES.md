...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice graph-valid longer smoke | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 20.99 current post-row-cache same-binary, graph-enabled, no executor profiling, `The quick brown fox`, `-c 64`, `-n 48`; 21.19 earlier graph-enabled lane | Fully captured for dense depth-1 MTP. `forward_graph` counters report `allow_segmented=true`, zero manual segments, and captured replay records for `main_verifier`, `mtp_decode_sidecar`, and `mtp_decode_catchup`. | N/A | 34.15 current post-row-cache graph-enabled depth-1 MTP, same prompt/context/decode length; 33.57 older graph-enabled best | 1.63x current same-binary, valid-context graph-captured smoke | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-baseline-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-post-rowcache-mtp-d1-c64-n48-stats.csv`; older baseline `/tmp/llaminar-mtp-bench/dense-rocm-graph-repro-c64-n48-bench.json`; older MTP `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-graph-mtp-d1-c64-n48-stats.csv`; context-budget regression `/tmp/llaminar-mtp-bench/dense-rocm-context-guard-c64-n64-bench.json`, `V2_Unit_BenchmarkRunnerCPU.FailsBeforePrefillWhenPromptPlusDecodeExceedsContext` | Current speedup is a new ROCm SingleDevice dense ratchet, but still below the 2x target. Current stats recorded 93.75% acceptance, three measured reject rollbacks using `rollback_verifier_state_row_shortcuts`, no `replay_forward`, `mtp.verifier_forward` averaging about 51.72 ms per two-token captured verifier, and sidecar averaging about 2.62 ms. The remaining lever is reducing verifier GPU work and per-step sidecar/sampling overhead; current deeper M=3/4 drafts are still slower because acceptance and rollback rates erase verifier amortization. |
