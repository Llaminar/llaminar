...
| SingleDevice | ROCm | Qwen3.6 MoE 35B | 21.23 | Partial: MTP GPU graphs survive rollback/restore through the `-n 4` crash reproducer, and stable MoE placement now preserves captured replay state across live prefix restore/truncate. MoE placement epoch remains part of main forward graph identity, while placement-changing maintenance explicitly invalidates sidecar caches; stable placement no longer forces per-call sidecar graph rebuild. | N/A | 10.89 | 0.51x decode | `/tmp/llaminar-mtp-bench/moe-rocm-baseline-n4.json`, `/tmp/llaminar-mtp-bench/moe-rocm-mtp-gpugraphs-n3-fixed.json`, `/tmp/llaminar-mtp-bench/moe-rocm-mtp-gpugraphs-n4-fixed.json`; focused replay-preservation regressions `V2_Unit_ForwardGraphTypes`, `V2_Unit_ForwardExecutionEngine`, `V2_Unit_MTPGraphConstruction`, `V2_Unit_PrefillDecodeTransition`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | Replay reconstruction cost for stable MoE placement is removed. MTP remains slower than baseline with 0% acceptance on the older prompt; a fresh real MoE prefix+MTP parity attempt on ROCm failed before inference because resident expert GPU preparation exceeded the ROCm:0 VRAM safety-margin budget and then lacked a GPU expert GEMM engine for layer 0. Need a passing real MoE ROCm parity/benchmark lane after the expert-preparation blocker is fixed. |
