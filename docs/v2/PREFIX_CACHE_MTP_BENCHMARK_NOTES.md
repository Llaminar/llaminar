...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 18.45 current shared-quant same-binary fox `-c 64 -n 16`; 17.74 previous same-binary fox `-c 64 -n 16`; 18.37 previous post-recapture fox `-c 64 -n 16`; 19.03 current same-prompt `Paris is -c 64 -n 8`; 18.25 older long-prompt baseline | Captured and correctness-green for the dense MTP path: main verifier, sidecar, shifted-prefill, and catch-up replay as capturable segments with zero manual stages; ROCm recapture now skips in-place graph update and directly reinstantiates; the M=2/3/4 graph-native verifier route is covered in focused ROCm integration across Q8 and native Q/K/IQ codebooks; fused native small-M QKV/GateUp dispatch now shares activation quantization once and bypasses concurrent prefill | N/A | 15.39 best graph-captured max-128 segment diagnostic; 15.25 stream-only diagnostic; 15.19 target-waves=12 smoke; 14.57 default12 counter run; 14.45 shared-quant counter run; 13.31 rejected lane-broadcast sum experiment | 0.83x best observed against current 18.45 tok/s baseline; 0.78x current shared-quant instrumented run | `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-baseline-c64-n16-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-c64-n16-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-default12-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-default12-baseline-c64-n16-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-default12-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-default12-mtp-c64-n16-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-max128-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-max128-c64-n16-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-streamonly-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-kb3-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-lane0sum-mtp-c64-n16-bench.json`; focused Phase 13.5 perf: `NativeVNNIGEMMPerfTest.MTP_SmallM_VerifierShapes_AllFormats` | This row is now a Phase 13.5 prerequisite item, not a Phase 14 tuning loose end. Lower-level ROCm small-M native-VNNI hidden-projection perf is now positive across all native codebooks: 1.27x-3.74x versus INT8 for M=2/3/4 with cosine >= 0.999961. The next ROCm gate is to re-run real Qwen3.6 MTP decode and prove the kernel-level win reduces full `main_verifier` replay cost enough to beat baseline. Sidecar, catch-up, sampling, and checkpoint costs remain secondary until that full-inference evidence lands. |
...
- Phase 13.5 ROCm small-M native-VNNI perf:
  ...
  - Shape: `Qwen36_MTP_HiddenProjection` with N=5120, K=5120 and
    M in {2, 3, 4}.
  - All native Q/K/IQ codebooks passed with cosine >= 0.999961.
  - Speedups versus INT8 ranged from 1.27x to 3.74x; Q4_K-equivalent
    Q4_1 was 2.11x at M=2, 2.83x at M=3, and 3.04x at M=4.
  - This proves the lower-level ROCm small-M route is no longer Q4/M=2-only,
    but it is not yet full-inference Phase 14 speedup evidence.
