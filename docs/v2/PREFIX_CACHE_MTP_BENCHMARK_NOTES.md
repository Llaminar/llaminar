...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice latest ROCm dense depth-3 verifier-row batched argmax ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.64 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense MTP through verifier M=4. The path includes GDN verifier-row rollback restore, no legacy post-warmup resegment API/counter, dense sidecar collective metadata cached at graph-cache build time, sidecar/sample sync fusion, batched depth-0 `kv_cache_only` catchup graphs for verifier-backed shifted rows at `seq_len=2/3`, and ROCm batched verifier-row argmax through declared arena scratch. | N/A | 55.04 graph-enabled depth-3 MTP, same prompt/context/decode length | 1.80x same-binary; current ROCm SingleDevice dense long-lane ratchet | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-baseline-c64-n48-bench.json`; MTP `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n48-stats.csv`; short-lane artifacts `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-baseline-c64-n8-bench.json` and `/tmp/llaminar-mtp-bench/dense-rocm-batched-verifier-argmax-mtp-d3-c64-n8-bench.json`; focused regressions `V2_Unit_PrefillDecodeTransition`, `V2_Integration_GPUSamplingKernels`, `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke`, and focused Qwen3.6 ROCm PyTorch parity cells `MTPGreedyMatchesPyTorchDecodeTokens`, `MTPGreedyDepth3MatchesPyTorchDecodeTokens`, and `PrefixCacheMTPRestore` | Batched verifier sampling removed per-row argmax sync overhead and recorded 39 batched sampling calls over 156 verifier rows on the long lane, with `sample_verifier_tokens_batched` around 70 us/call. The request recorded 85.61% acceptance, 119 accepted tokens, 20 rejected tokens, 20 rollbacks, 53 verifier runs, and 212 verifier tokens. This preserves the speed ratchet but remains short of the Phase 14 2x target because captured `main_verifier` replay still averages about 44.31 ms and `mtp.verifier_forward` about 46.83 ms. Next work is shrinking ordinary GEMM, fused Gate/Up, GDN projection/recurrence, attention, and LM head inside the captured verifier graph. |
