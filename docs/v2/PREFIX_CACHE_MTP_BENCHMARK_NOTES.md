...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 18.14 current stage-timing fox `-c 64 -n 8`; 18.07 current KB-cap same-binary fox `-c 64 -n 16`; 18.96 previous clean all-codebook fox `-c 64 -n 16`; 17.05 post-batched clean same-prompt fox `-c 64 -n 16`; 18.45 previous shared-quant same-binary fox `-c 64 -n 16`; 17.74 previous same-binary fox `-c 64 -n 16`; 19.03 previous same-prompt `Paris is -c 64 -n 8`; 18.25 older long-prompt baseline | Captured and correctness-green for the dense MTP path: main verifier, sidecar, shifted-prefill, and catch-up replay as capturable segments with zero manual stages; ROCm MTP sidecar shifted-prefill/decode now uses normal replay instead of force-recapture; ROCm recapture now skips in-place graph update and directly reinstantiates; the M=2/3/4 graph-native verifier route is covered in focused ROCm integration across Q8 and native Q/K/IQ codebooks and is exercised by real Qwen3.6 counters for Q4_K/Q6_K/LM-head-shaped projections; fused native small-M QKV/GateUp dispatch now shares activation quantization once through a graph-native batched same-codebook route with optional bias epilogue support and batched counters; per-kernel ROCm scatter-partial workspace slices prevent shared-workspace split-K aliasing in graph-stage projection bundles; native small-M M=2/3/4 split-K now caps default graph-safe KB at 8 and hard-fails unsafe overrides instead of reaching HSA faults | N/A | 16.21 clean all-codebook small-M route before the batched fused-projection slice; 15.80 batched fused-projection shared-partial rerun; 15.27 current KB-cap stable rerun; 15.39 graph-captured max-128 segment diagnostic; 15.25 stream-only diagnostic; 15.19 target-waves=12 smoke; 14.57 default12 counter run; 14.45 shared-quant counter run; 14.04 sidecar-normal-replay stage-timing diagnostic | 0.85x current KB-cap stable rerun against 18.07 tok/s baseline; 0.86x clean all-codebook small-M route against 18.96 baseline; 0.83x previous best against 18.45 tok/s baseline; 0.77x sidecar-normal-replay diagnostic against same prompt/length stage-timing baseline | `/tmp/llaminar-mtp-bench/dense-rocm-sidecar-replay-n8-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-sidecar-replay-n8.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-stage-timing-baseline-n8-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-stage-timing-baseline-n8.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-batchedgdn-baseline-c64-n16-kbcap8-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-batchedgdn-mtp-c64-n16-kbcap8-rerun-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-batchedgdn-mtp-c64-n16-kbcap8-rerun-combined.csv`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-batchedgdn-mtp-c64-n2-kbcap8-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-clean-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-clean-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-smallm-sharedpartials-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-smallm-sharedpartials-mtp-c64-n16-stats.json`; focused Phase 13.5 perf: `NativeVNNIGEMMPerfTest.MTP_SmallM_VerifierShapes_AllFormats` | Phase 13.5 made real progress but not enough for Phase 14 acceptance: the HSA fault is covered by shared partial-buffer slices plus the small-M KB cap, and real Qwen3.6 `-n 16` now completes without serialization. Current stable same-binary MTP remains 15.27 tok/s with 87.5% acceptance versus an 18.07 tok/s baseline; the sidecar-normal-replay diagnostic passed at 14.04 tok/s with 75% acceptance and proved `force_recapture=false`, but `main_verifier` replay still costs about 97.95 ms per two-token graph. Diagnostic counters prove the batched Gate/Up, QKV, and GDN-style routes are active. Next work is more captured verifier GPU reduction rather than fallback tuning. |
