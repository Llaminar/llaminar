...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice latest ROCm dense depth-3 batched-catchup ratchet | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 30.93 same-binary graph-enabled baseline, `The quick brown fox`, `-c 64`, `-n 48` | Fully captured for dense MTP through verifier M=4. The current path includes GDN verifier-row rollback restore, MoE warmup-dependent capture planning without dense resegment tax, stable arena write-id replay marking, and batched depth-0 `kv_cache_only` catchup graphs for verifier-backed shifted rows at `seq_len=2/3`. | N/A | 54.69 graph-enabled depth-3 MTP, same prompt/context/decode length; prior depth-3 reached 54.48 | 1.77x same-binary; preserves and slightly improves the ROCm SingleDevice dense long-lane ratchet | Baseline `/tmp/llaminar-mtp-bench/dense-rocm-baseline-current-c64-n48-bench.json`; latest depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batchedcatchup-mtp-d3-c64-n48-stats.csv`; prior depth-3 MTP `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-warmupdep-mtp-d3-c64-n48-stats.json`; focused regressions `V2_Unit_MTPGraphConstruction` and `V2_Integration_PrefixCacheMTP_Qwen36ROCmGpuGraphsChainedDraftSmoke` | Depth-3 remains short of the Phase 14 2x target because captured verifier replay still dominates. The latest request recorded 81.02% acceptance, 111 accepted tokens, 26 rejected tokens, 27 rollbacks, 55 verifier runs, and 220 verifier tokens. The batched catchup slice reduced measured `mtp_decode_catchup` replay from about 31.66 ms to 7.07 ms and `mtp.shifted_row_commit` from about 41.57 ms to 24.14 ms versus the prior ratchet artifact, while preserving 12 verifier-row GDN rollback shortcuts and zero suffix/replay-forward counters. Remaining work is shrinking captured `main_verifier` GPU work and avoiding three separate host-synchronized sidecar draft steps at depth-3. |
...
- Fresh MoE warmup-dependent graph-capture contract on 2026-06-03: segmented
  ...
  promised warmup-dependent capture is still not graph-capturable. This removes
  the post-warmup resegment tax for supported MoE graph-capture lanes and
  preserves the dense `reason=not_required` behavior. Focused validation passed:
