...
|-------------|-----------------------|-------------|------------------------|----------------------|---------------------------|-----------------------|------------------|-------------------|-----------------|
| SingleDevice | ROCm `rocm:0` | Qwen3.6 dense 27B Q4_K_S | 18.96 previous clean all-codebook fox `-c 64 -n 16`; 17.05 post-batched clean same-prompt fox `-c 64 -n 16`; 18.45 previous shared-quant same-binary fox `-c 64 -n 16`; 17.74 previous same-binary fox `-c 64 -n 16`; 19.03 previous same-prompt `Paris is -c 64 -n 8`; 18.25 older long-prompt baseline | Captured and correctness-green for the dense MTP path: main verifier, sidecar, shifted-prefill, and catch-up replay as capturable segments with zero manual stages; ROCm recapture now skips in-place graph update and directly reinstantiates; the M=2/3/4 graph-native verifier route is covered in focused ROCm integration across Q8 and native Q/K/IQ codebooks and is exercised by real Qwen3.6 counters for Q4_K/Q6_K/LM-head-shaped projections; fused native small-M QKV/GateUp dispatch now shares activation quantization once through a graph-native batched same-codebook route with optional bias epilogue support and batched counters; per-kernel ROCm scatter-partial workspace slices now prevent shared-workspace split-K aliasing in graph-stage projection bundles | N/A | 16.21 clean all-codebook small-M route before the batched fused-projection slice; 15.80 batched fused-projection shared-partial rerun after the HSA fault fix; 15.39 graph-captured max-128 segment diagnostic; 15.25 stream-only diagnostic; 15.19 target-waves=12 smoke; 14.57 default12 counter run; 14.45 shared-quant counter run | 0.86x current clean all-codebook small-M route; 0.83x batched fused-projection rerun against 18.96 tok/s baseline; 0.83x previous best against 18.45 tok/s baseline | `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-clean-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-clean-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-mtp-preflight-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-smallm-allcodebooks-mtp-preflight-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-smallm-clean-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-smallm-sharedpartials-mtp-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-batched-smallm-sharedpartials-mtp-c64-n16-stats.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-baseline-c64-n16-bench.json`, `/tmp/llaminar-mtp-bench/dense-rocm-m2q4-sharedquant-c64-n16-combined.csv`; focused Phase 13.5 perf: `NativeVNNIGEMMPerfTest.MTP_SmallM_VerifierShapes_AllFormats` | Phase 13.5 made real progress but not enough for Phase 14 acceptance: clean MTP improved to 16.21 tok/s with 87.5% acceptance before the batched fused-projection slice, but clean baseline is still higher. The batched fused-projection rerun completed without the previous HSA memory fault and proved real route counters, but throughput was 15.80 tok/s with 87.5% acceptance. Diagnostic stats show `main_verifier` segmented replay still averages about 98 ms per two-token graph, while sidecar and catch-up remain small. Next work is more captured verifier GPU reduction rather than fallback tuning. |
...
- Phase 13.5 ROCm small-M native-VNNI perf:
  ...
  - Speedups versus INT8 ranged from 1.15x to 4.56x across the expanded
    verifier-shaped matrix.
  - Representative Q4_K-equivalent Q4_1 rows:
    hidden projection 2.21x/2.88x/3.01x for M=2/3/4; GDN inner
    2.42x/3.35x/3.67x; GDN time 1.35x/1.50x/1.61x; GDN output
    2.17x/2.87x/3.00x.
