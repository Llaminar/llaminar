...
class LlamaReferenceModel(AbstractReferenceModel):
    """
    ...
    - GGUF checkpoint loading (auto-detected)
    ...
    Example:
        ...
        model = LlamaReferenceModel("llama", "meta-llama/Llama-2-7b-hf")
        model.load_model()
        
        # GGUF checkpoint (auto-detected)
        model = LlamaReferenceModel("llama", "llama-2-7b-q4_0.gguf")
        model.load_model()
    ...
    def load_model(
        ...
        **kwargs
        ...
        """
        Load LLaMA model from HuggingFace checkpoint or GGUF file.
    ...
    def _load_from_gguf(
        ...
        **kwargs
        ...
        # Create LlamaConfig from dict
        self.hf_config = LlamaConfig(**config_dict)
        ...
        print(f"Creating LLaMA model from config...")
        self.hf_model = LlamaForCausalLM(self.hf_config)
        ...
        # Filter out expected missing/unexpected keys to reduce noise
        # lm_head.weight is often tied to token_embd.weight (weight sharing)
        ignorable_missing = {'lm_head.weight'}
        # RoPE frequencies are computed at runtime, not loaded
        ignorable_unexpected = {'rope_freqs.weight', 'rope.freqs', 'pos_embd.weight'}
        
        filtered_missing = [k for k in missing_keys if k not in ignorable_missing]
        filtered_unexpected = [k for k in unexpected_keys if k not in ignorable_unexpected]
        
        if filtered_missing:
            warnings.warn(f"Missing keys when loading GGUF: {filtered_missing}")
        if filtered_unexpected:
            warnings.warn(f"Unexpected keys when loading GGUF: {filtered_unexpected}")
        ...
        # Try 1: Local tokenizer directory
        ...
        # Try 2: Use TinyLlama (open-access LLaMA tokenizer)
        if not tokenizer_loaded:
            fallback_tokenizers = [
                "TinyLlama/TinyLlama-1.1B-Chat-v1.0",  # Open LLaMA tokenizer
                "HuggingFaceH4/zephyr-7b-beta",
                "meta-llama/Llama-2-7b-hf",  # Gated, may fail
            ]
            ...
            for fallback in fallback_tokenizers:
                try:
                    ...
                    print(f"✓ Loaded tokenizer from {fallback}")
                    tokenizer_loaded = True
                    break
                except Exception as e:
                    continue
        ...
        if not tokenizer_loaded:
            raise RuntimeError(
                "Could not load tokenizer. Please either:\n"
                "  1. Place tokenizer files in the GGUF directory, or\n"
    ...
    def _load_from_huggingface(
        ...
        **kwargs
        ...
        """
        Load LLaMA model from HuggingFace checkpoint (standard path).
    ...
    def _register_hooks(self) -> None:
        ...
        LLaMA architecture is very similar to Qwen2, so the hook structure
        is nearly identical. Main difference: LLaMA uses RMSNorm for both
        input_layernorm and post_attention_layernorm.
        """
        if not self.hf_model:
            return
        
        model = self.hf_model.model  # Access underlying LlamaModel
        ...
        for layer_idx, layer in enumerate(model.layers):
            # Attention norm (input layernorm / pre-attention RMSNorm)
            ...
            # FFN norm (post_attention_layernorm / pre-FFN RMSNorm)
