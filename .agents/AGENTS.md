...
All CUDA/HIP/ROCm GPU work in Llaminar must run on an explicit non-null stream.
Never launch kernels, enqueue async copies, enqueue memset operations, record
events, or call GPU library wrappers with the device-default stream (`nullptr`,
`0`, `NULL`, omitted CUDA launch stream, or an implicit backend default).

The device-default stream has special synchronization semantics that can
silently serialize unrelated work, break graph-capture ordering assumptions,
hide missing dependencies during profiling, and race against stages that are
correctly bound to a capture or worker stream. These failures tend to appear as
flaky HSA/CUDA memory faults, parity drift, or benchmark-only regressions long
after the offending code was written.

Rules for GPU stages and kernels:

- Stage execution must use `IComputeStage::gpuStream()` or another explicitly
  passed stream owned by the graph executor, worker context, collective context,
  or graph-capture cache.
- Direct kernel APIs must take a stream argument for GPU paths and hard-fail if
  the caller tries to run GPU work with a null stream.
- Async transfer/coherence helpers must receive the same explicit stream as the
  compute stage whose data dependency they serve.
- GPU graph-captured side paths, such as MTP verifier/sidecar paths, must bind
  every stage to the capture stream and keep sampling, rollback, and state
  import/export on explicitly ordered streams.
- CPU paths may naturally have no GPU stream; GPU paths must not.

Before landing GPU stage/kernel changes, run the static stream hygiene check or
the matching unit test:

```bash
bash scripts/check_no_default_stream_in_gpu_code.sh /workspaces/llaminar
ctest --test-dir build_v2_integration -R "^V2_Unit_Static_NoDefaultStreamInGPUCode$" --output-on-failure --parallel
```
