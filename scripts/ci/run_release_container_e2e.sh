...
add_qwen36_moe_policy_prefix_mtp_suite() {
    ...
    feature_flags+=" --moe-routed-expert-domain ${domain_name}=${tp_devices};scope=rank_local;backend=${collective_backend};routed_compute=apportioned;routed_phase=uniform;routed_decode_assignment=static-owner;routed_prefill_assignment=${routed_prefill_assignment};owner=0"
    ...
    feature_flags+=" --moe-device-rebalance-min-load-spread-improvement 0"
    feature_flags+=" --moe-device-rebalance-min-load-spread-improvement-divisor 0"
    feature_flags+=" --moe-device-rebalance-min-wave-spread-improvement-per-payload-slot 0"
    feature_flags+=" --moe-device-rebalance-min-foreign-rows-per-critical-path-payload-slot 0"
