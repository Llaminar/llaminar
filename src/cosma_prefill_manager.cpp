...
inline bool mpi_is_initialized() {
    int flag = 0; MPI_Initialized(&flag); return flag != 0; }
inline bool mpi_is_finalized() { int flag=0; MPI_Finalized(&flag); return flag!=0; }
inline int mpi_world_size_safe() { if(!mpi_is_initialized()||mpi_is_finalized()) return 1; int sz=1; MPI_Comm_size(MPI_COMM_WORLD,&sz); return sz; }
inline int mpi_rank_safe() { if(!mpi_is_initialized()||mpi_is_finalized()) return 0; int r=0; MPI_Comm_rank(MPI_COMM_WORLD,&r); return r; }
inline void safe_allreduce_float_inplace(float* buf, int count, MPI_Op op){ if(mpi_is_initialized() && mpi_world_size_safe()>1) MPI_Allreduce(MPI_IN_PLACE, buf, count, MPI_FLOAT, op, MPI_COMM_WORLD);} 
inline void safe_allreduce_int_inplace(int* buf, int count, MPI_Op op){ if(mpi_is_initialized() && mpi_world_size_safe()>1) MPI_Allreduce(MPI_IN_PLACE, buf, count, MPI_INT, op, MPI_COMM_WORLD);} 
inline void safe_bcast(void* buf, int count, MPI_Datatype dt, int root){ if(mpi_is_initialized() && mpi_world_size_safe()>1) MPI_Bcast(buf, count, dt, root, MPI_COMM_WORLD);} 
inline void safe_barrier(){ if(mpi_is_initialized() && mpi_world_size_safe()>1) MPI_Barrier(MPI_COMM_WORLD);} 
}
...
namespace llaminar
{
    ...
    CosmaPrefillManager::~CosmaPrefillManager() = default;
        ...
        CosmaPrefillManager::CosmaPrefillManager()
        {
            ...
            if (std::getenv("LLAMINAR_SKIP_MPI_IN_SINGLE_TEST")) {
                world_size_ = 1; rank_ = 0;
            } else if (mpi_init) {
                MPI_Comm_size(MPI_COMM_WORLD, &world_size_);
                MPI_Comm_rank(MPI_COMM_WORLD, &rank_);
            } else { world_size_ = 1; rank_ = 0; }
            ...
            if (const char *env_log = std::getenv("LLAMINAR_COSMA_LOG_LEVEL"))
            {
                ...
                if (lvl == "error") log_level_ = 0;
                else if (lvl == "warn") log_level_ = 1;
                else if (lvl == "info") log_level_ = 2;
                else if (lvl == "debug") log_level_ = 3;
                else if (lvl == "trace") log_level_ = 4;
            }
    ...
    bool CosmaPrefillManager::enabled_for(int seq_len) const {
        if (std::getenv("LLAMINAR_COSMA_DISABLE")) return false;
        if (std::getenv("ADAPTIVE_DISABLE_COSMA")) return false;
        if (force_cosma_) return true;
    ...
    CosmaView CosmaPrefillManager::allocate_matrix(char label, int m, int n, const cosma::Strategy &strat, bool zero) {
        ...
        try {
            ...
            if (mat->matrix_pointer()) {
                ...
                bool already=false; for (auto &rec : allocations_) if (!rec.ref.expired() && rec.ref.lock().get()==mat.get()) { already=true; break; }
                if (!already) {
    ...
    void CosmaPrefillManager::fill_activation(CosmaView &dst, const float *src_row_major, int m, int k) {
        ...
        if (dest_local_mode) {
            ...
            for (size_t li=0; li<sz; ++li) {
                ...
                int gi = gc.first; int gj = gc.second;
        ...
        if (std::getenv("LLAMINAR_COSMA_DIAG_LOCAL_PROBE") && rank_==0) {
            ...
            long double num=0.0L, den=0.0L; size_t mismatches=0; int logged=0;
            for (size_t li=0; li<sample; ++li) {
                auto gc = dst.mat->global_coordinates((int)li);
                int gi = gc.first; int gj = gc.second;
                if (gi<0 || gj<0 || gi>=m || gj>=k) continue; // skip invalid
                float v = local[li];
    ...
    CosmaView CosmaPrefillManager::convert_activation_in_with_strategy(const float *row_major, int m, int k, const cosma::Strategy &strat)
    {
        ...
        if (skip_cosma) {
            CosmaView v; v.global_rows = m; v.global_cols = k; v.label='A'; v.original_row_major = row_major; return v;
        }
        auto view = allocate_matrix('A', m, k, strat, false);
        fill_activation(view, row_major, m, k);
        view.original_row_major = row_major;
    ...
    void CosmaPrefillManager::stream_weight_blocks(CosmaView &dst, const WeightDescriptor &desc)
    {
        ...
        if (fast_unverified) {
            ...
            if (to_copy < sz) std::fill(local + to_copy, local + sz, 0.f);
    ...
    CosmaWeightHandle CosmaPrefillManager::load_weight_with_strategy(const WeightDescriptor &desc, const cosma::Strategy &strat)
    {
        ...
        if (skip_cosma) {
            CosmaView v; v.global_rows = desc.rows; v.global_cols = desc.cols; v.label='B'; v.original_row_major = static_cast<const float*>(desc.base_ptr); return {v, desc};
        }
        size_t w_bytes = (size_t)desc.rows * desc.cols * sizeof(float);
        if (!memory_budget_allows(w_bytes))
        {
                                          ...
                                          float beta)
    {
        ...
        auto dump_small = [&](const char* tag,const CosmaView &V,int rows,int cols){
            ...
            if(rows>limit||cols>limit) return;
            ...
            for(int gi=0; gi<rows; ++gi){
                for(int gj=0; gj<cols; ++gj){
                    int owner_rank = -1; int local_index = -1;
                    if(V.mat){
                        auto lc = V.mat->local_coordinates(gi,gj);
                        local_index = lc.first; owner_rank = lc.second;
                    }
                    if(rank_==0){
        ...
        if (std::getenv("LLAMINAR_COSMA_DIAG")) {
            ...
            if (A.original_row_major) diag_sample_points(A, A.original_row_major, "A");
            if (W.view.original_row_major) diag_sample_points(W.view, W.view.original_row_major, "B");
            ...
            if (const char* tap_env = std::getenv("LLAMINAR_COSMA_DIAG_TAP")) {
                ...
                if (rank_==0 && should_log(4)) {
                    ...
                    for(int i=0;i<tap_m;++i){ std::ostringstream row; row<<"A["<<i<<"]:"; for(int j=0;j<tap_k;++j){ row<<' '<<A_recon[(size_t)i*k+j]; } LOG_TRACE(row.str()); }
                    LOG_TRACE("[CosmaPrefill][Tap] B top-left "<<tap_k<<"x"<<tap_n);
                    for(int i=0;i<tap_k;++i){ std::ostringstream row; row<<"B["<<i<<"]:"; for(int j=0;j<tap_n;++j){ row<<' '<<B_recon[(size_t)i*n+j]; } LOG_TRACE(row.str()); }
                }
            }
        }
        if (world_size_ == 1)
        ...
        if (const char* env = std::getenv("LLAMINAR_COSMA_FORCE_REPLICATED_DIAG")) {
            ...
            if (env[0] != '\0' && !(env[0]=='0' && env[1]=='\0')) force_replicated = true;
        }
        if (force_replicated) {
            if (rank_==0) LOG_WARN("[CosmaPrefill][debug-force-fallback] Forcing replicated path (env override)");
            CosmaView C_rep; C_rep.global_rows=m; C_rep.global_cols=n; C_rep.label='C';
            ...
            if (rank_==0) {
                double sumA=0.0,sumB=0.0; for(float v: A_full) sumA+=v; for(float v: B_full) sumB+=v;
            ...
            if (validate_tile_tokens_>0) maybe_validation_tile_gemm(A_full.data(), B_full.data(), C_rep.host_owned->data(), m,k,n);
    ...
    // Allow completely disabling COSMA large path via existing adaptive override or new env.
        ...
        if (const char* env = std::getenv("LLAMINAR_COSMA_DIRECT_THRESHOLD_OPS")) {
            long long v = std::atoll(env); if (v > 0) direct_threshold_ops = v;
        }
    ...
    bool replicate_B = std::getenv("LLAMINAR_COSMA_REPLICATE_B") != nullptr;
        ...
        if (!use_direct) // fallback path
        {
            ...
            if (rank_ == 0 && should_log(2))
            {
                ...
                for (float v : gathered_A) sumA += v;
                for (float v : B_full) sumB += v;
        ...
        if (replicate_B && W.view.mat) {
            ...
            float *b_local = W.view.mat->matrix_pointer(); size_t b_sz = W.view.mat->matrix_size();
            if (b_local && b_sz) {
                ...
                for (int gi=0; gi<k; ++gi) {
                    for (int gj=0; gj<n; ++gj) {
                        ...
                        if (lc.second == rank_) {
                            int lidx = lc.first; if (lidx>=0 && (size_t)lidx < b_sz) b_local[lidx] = B_full[(size_t)gi*n + gj];
                        }
                    }
                }
            }
    ...
    const cosma::Strategy &stratC_ref = sC; // result always uses (m,n,k)
        ...
        if (rebuild_A) {
            if (rank_==0 && should_log(3)) LOG_DEBUG("[CosmaPrefill][direct] Rebuilding A with unified strategy");
        ...
        if (rebuild_B) {
            if (rank_==0 && should_log(3)) LOG_DEBUG("[CosmaPrefill][direct] Rebuilding B with unified strategy");
            ...
            if (B_compat.mat && B_compat.mat->matrix_pointer()) {
                float *local = B_compat.mat->matrix_pointer(); size_t sz = B_compat.mat->matrix_size();
                std::fill(local, local + sz, 0.f);
                bool legacy_forward = (std::getenv("LLAMINAR_COSMA_POP_FORWARD_LEGACY") != nullptr);
                bool dest_local_mode = !legacy_forward;
                bool trace_scatter_B = (std::getenv("LLAMINAR_COSMA_DIAG_DEEP")!=nullptr) && should_log(4);
                if (dest_local_mode) {
                    if (trace_scatter_B && rank_==0) LOG_TRACE("[CosmaPrefill][pop] destination-local population (B) default");
                    for (size_t li=0; li<sz; ++li) {
                        auto gc = B_compat.mat->global_coordinates((int)li);
                        int gi = gc.first; int gj = gc.second; // B dims k x n
                    ...
                    for (int gi=0; gi<k; ++gi) {
                        ...
                        for (int gj=0; gj<n; ++gj) {
                            ...
                            if (lc.second == rank_) {
                                int lidx = lc.first; if (lidx>=0 && (size_t)lidx < sz) local[lidx] = row_ptr[gj];
                                if (std::getenv("LLAMINAR_COSMA_DIAG_DEEP") && gi < std::min(k,8) && gj < std::min(n,8) && rank_==0 && should_log(4)) {
                ...
                if (std::getenv("LLAMINAR_COSMA_DIAG_LOCAL_PROBE") && rank_==0) {
                    ...
                    long double numB=0.0L, denB=0.0L; size_t mismatchesB=0; int loggedB=0;
                    for (size_t li=0; li<sampleB; ++li) {
                        auto gc = B_compat.mat->global_coordinates((int)li);
                        int gi2=gc.first, gj2=gc.second; if (gi2<0||gj2<0||gi2>=k||gj2>=n) continue;
                        float v = local[li]; float ref = B_full_buf[(size_t)gi2 * n + gj2];
                        long double diff=(long double)v-(long double)ref; numB+=diff*diff; denB+=(long double)ref*ref;
                        if (v != ref && loggedB < 8) { LOG_INFO("[CosmaPrefill][local-probe] B mismatch li="<<li<<" -> (gi="<<gi2<<",gj="<<gj2<<") v="<<v<<" ref="<<ref); ++loggedB; ++mismatchesB; }
                        else if (v!=ref) ++mismatchesB;
                    }
            ...
            if (std::getenv("LLAMINAR_COSMA_DIAG_LOCAL_PROBE_DEEP")) {
                auto do_deep_probe = [&](const char* tag, const CosmaView& V, const float* ref, int rows, int cols){
                    if (!V.mat || !ref) return; float* local = V.mat->matrix_pointer(); size_t sz = V.mat->matrix_size(); if(!local||!sz) return;
                    ...
                    long double num=0.0L, den=0.0L; int mismatches=0; size_t first_mismatch_li = (size_t)-1; int first_gi=-1, first_gj=-1; float first_v=0.f, first_ref=0.f;
                    std::vector<size_t> perm_signature; perm_signature.reserve(SAMPLES);
                    for (int s=0; s<SAMPLES; ++s) {
                        size_t li = next_rand() % sz; auto gc = V.mat->global_coordinates((int)li); int gi=gc.first, gj=gc.second; if(gi<0||gj<0||gi>=rows||gj>=cols) continue; float v=local[li]; float r=ref[(size_t)gi*cols+gj]; long double d=(long double)v-(long double)r; num+=d*d; den+=(long double)r*r; if (v!=r){ ++mismatches; if (first_mismatch_li==(size_t)-1){ first_mismatch_li=li; first_gi=gi; first_gj=gj; first_v=v; first_ref=r; }} perm_signature.push_back(((size_t)gi*cols+gj)); }
                    double rel = (den>0)? (double)(std::sqrt(num)/(std::sqrt(den)+1e-30L)) : 0.0;
                    if (rank_==0 && should_log(2)) {
                        ...
                        if (!perm_signature.empty()) {
                            std::vector<size_t> tmp = perm_signature; std::sort(tmp.begin(), tmp.end()); size_t gaps=0; for(size_t i=1;i<tmp.size();++i) if(tmp[i]!=tmp[i-1]+1) ++gaps; LOG_INFO("[CosmaPrefill][local-probe-deep] tag="<<tag<<" perm_gaps="<<gaps);
                        }
                    }
                };
        ...
        if (deep_diag) {
            ...
            if (bypass) {
                ...
                if (A_ref_raw) std::memcpy(A_pop.data(), A_ref_raw, (size_t)m*k*sizeof(float)); else std::fill(A_pop.begin(), A_pop.end(), 0.f);
                if (B_ref_raw) std::memcpy(B_pop.data(), B_ref_raw, (size_t)k*n*sizeof(float)); else std::fill(B_pop.begin(), B_pop.end(), 0.f);
                if (mpi_is_initialized() && mpi_world_size_safe()>1) {
                ...
                if (rank_==0 && should_log(3)) LOG_INFO("[CosmaPrefill][diag] Reconstruction BYPASS active");
            } else {
            ...
            std::vector<float> A_pop_swap, B_pop_swap; double relA_swap=-1.0, relB_swap=-1.0;
            if (!bypass && swap_rc_exp) {
                if (rank_==0 && should_log(2)) LOG_INFO("[CosmaPrefill][swaprc] Alternate swap(row,col) reconstruction enabled");
                if (A_compat.mat && B_compat.mat) {
                    ...
                    auto do_swap_recon = [&](const CosmaView &view, std::vector<float>& out, int rows, int cols){
                        if (!view.mat) return; float *local = view.mat->matrix_pointer(); size_t lsz = view.mat->matrix_size();
                        // Gather local triplets
                        std::vector<int> gi_send(lsz), gj_send(lsz); std::vector<float> val_send(lsz);
                        for (size_t li=0; li<lsz; ++li) { auto gc=view.mat->global_coordinates((int)li); gi_send[li]=gc.first; gj_send[li]=gc.second; val_send[li]=local[li]; }
                        // Root gather counts
                        int local_n=(int)lsz; std::vector<int> counts(world_size_,0); std::vector<int> displs(world_size_,0);
                        MPI_Gather(&local_n,1,MPI_INT,counts.data(),1,MPI_INT,0,MPI_COMM_WORLD);
                        int total_entries=0; if(rank_==0){ for(int r=0;r<world_size_;++r){ displs[r]=total_entries; total_entries+=counts[r]; } }
                        std::vector<int> gi_all, gj_all; std::vector<float> val_all; if(rank_==0){ gi_all.resize(total_entries); gj_all.resize(total_entries); val_all.resize(total_entries);} 
                        ...
                        if(rank_==0){ std::fill(out.begin(), out.end(), 0.f); for(int i=0;i<total_entries;++i){ int gc0=gi_all[i]; int gc1=gj_all[i]; // interpret swapped
                                if (gc1>=0 && gc1<rows && gc0>=0 && gc0<cols){ size_t w = (size_t)gc1*cols + gc0; out[w]=val_all[i]; } }
                        }
            ...
            if (!A_pop_swap.empty()) relA_swap = rel_l2(A_pop_swap, A_ref, (size_t)m*k);
            if (!B_pop_swap.empty()) relB_swap = rel_l2(B_pop_swap, B_ref, (size_t)k*n);
            ...
            auto cosine = [](const std::vector<float>& X, const float* Y, size_t sz){ if(!Y) return -2.0; long double dot=0.0L, nx=0.0L, ny=0.0L; for(size_t i=0;i<sz;++i){ long double xv=X[i]; long double yv=Y[i]; dot+=xv*yv; nx+=xv*xv; ny+=yv*yv;} if(nx==0||ny==0) return -1.0; return (double)(dot / (std::sqrt(nx)*std::sqrt(ny))); };
            double cosA = cosine(A_pop, A_ref, (size_t)m*k);
            double cosB = cosine(B_pop, B_ref, (size_t)k*n);
            auto mean_abs_diff = [](const std::vector<float>& X, const float* Y, size_t sz){ if(!Y) return -1.0; long double acc=0.0L; for(size_t i=0;i<sz;++i) acc+= std::fabs((long double)X[i] - (long double)Y[i]); return (double)(acc / (long double)sz); };
            double madA = mean_abs_diff(A_pop, A_ref, (size_t)m*k);
            double madB = mean_abs_diff(B_pop, B_ref, (size_t)k*n);
            auto zero_frac = [](const std::vector<float>& X){ size_t z=0; for(float v: X) if (v==0.f) ++z; return (double)z/(double)X.size(); };
            double zA = zero_frac(A_pop); double zB = zero_frac(B_pop);
            ...
            int inspect_rows = std::min(m, 8); int inspect_colsA = std::min(k, 8); int inspect_colsB = std::min(n,8);
            std::ostringstream rsA, csA, rsB, csB; rsA<<"rsA:"; csA<<"csA:"; rsB<<"rsB:"; csB<<"csB:";
            for(int i=0;i<inspect_rows;++i){ double s=0.0; for(int j=0;j<k;++j) s+=A_pop[(size_t)i*k+j]; rsA<<' '<<s; }
            for(int j=0;j<inspect_colsA;++j){ double s=0.0; for(int i=0;i<m;++i) s+=A_pop[(size_t)i*k+j]; csA<<' '<<s; }
            for(int i=0;i<std::min(k,8);++i){ double s=0.0; for(int j=0;j<n;++j) s+=B_pop[(size_t)i*n+j]; rsB<<' '<<s; }
            for(int j=0;j<inspect_colsB;++j){ double s=0.0; for(int i=0;i<k;++i) s+=B_pop[(size_t)i*n+j]; csB<<' '<<s; }
            ...
            if (env_perm_flag && rank_==0) {
                std::ostringstream oss; oss<<"[CosmaPrefill][perm-infer] env flag set value='"<<env_perm_flag<<"' relA="<<relA<<" relB="<<relB;
                LOG_INFO(oss.str());
            }
            bool infer_perm = (env_perm_flag != nullptr); // always attempt if flag set
            if (rank_==0) {
                std::ostringstream oss; oss<<"[CosmaPrefill][perm-infer] infer_perm decision="<<(infer_perm?"true":"false")
                                         <<" relA="<<relA<<" relB="<<relB;
                if(!infer_perm) oss<<" (env flag absent)"; else if(relA<=0.5 && relB<=0.5) oss<<" (proceeding even though rel values small)";
                LOG_INFO(oss.str());
            }
            if (infer_perm) {
                auto infer_row_permutation = [&](const std::vector<float>& pop, const float* ref, int rows, int cols, std::vector<int>& perm_out, double &rel_after){
                    perm_out.clear(); rel_after = -1.0; if(!ref) return false; perm_out.resize(rows, -1);
                    // Build 64-bit hash per reference row
                    struct RowHashEntry { uint64_t h; int idx; }; std::vector<RowHashEntry> ref_hash(rows);
                    auto row_hash = [&](const float* base){
                        ...
                        for (int c=0; c<cols; ++c){ uint64_t v = (uint64_t)data[c]; h ^= v; h *= 1099511628211ull; }
                        return h;
                    };
                    for(int r=0; r<rows; ++r){ ref_hash[r] = { row_hash(ref + (size_t)r*cols), r }; }
                    // Sort for binary search (avoid unordered_map overhead for large 4k rows)
                    std::sort(ref_hash.begin(), ref_hash.end(), [](const RowHashEntry&a, const RowHashEntry&b){ return a.h < b.h; });
                    ...
                    bool ok=true; for(int r=0; r<rows; ++r){ uint64_t h=row_hash(pop.data() + (size_t)r*cols); int ref_r = find_ref(h); if(ref_r<0 || used[ref_r]) { ok=false; break; } perm_out[r]=ref_r; used[ref_r]=1; }
                    if(!ok){ return false; }
                    // Apply inverse permutation to test alignment: we want reordered_pop[perm_out[r]] = pop[r]
                    std::vector<float> reordered; reordered.resize((size_t)rows*cols);
                    for(int r=0; r<rows; ++r){ int ref_r = perm_out[r]; if(ref_r>=0){ std::memcpy(&reordered[(size_t)ref_r*cols], &pop[(size_t)r*cols], (size_t)cols*sizeof(float)); } }
                    // Compute rel L2 between reordered and reference
                    long double num=0.0L, den=0.0L; size_t sz=(size_t)rows*cols; for(size_t i=0;i<sz;++i){ long double d=(long double)reordered[i] - (long double)ref[i]; num+=d*d; long double rv=(long double)ref[i]; den+=rv*rv; }
                    rel_after = (double)(std::sqrt(num)/(std::sqrt(den)+1e-30L));
                    return true;
                };
                std::vector<int> permA; double relA_after=-1.0; bool have_permA = infer_row_permutation(A_pop, A_ref, m, k, permA, relA_after);
                if (have_permA && rank_==0) {
                    int mismatches=0; for(int i=0;i<m;++i) if(permA[i]!=i) ++mismatches; int first_print=0; std::ostringstream oss; oss<<"[CosmaPrefill][perm-infer] A row permutation size="<<m<<" moved_rows="<<mismatches<<" rel_after="<<relA_after; LOG_INFO(oss.str());
                    std::ostringstream map_os; map_os<<"[CosmaPrefill][perm-infer] A perm sample:"; for(int i=0;i<m && first_print<16; ++i){ if(permA[i]!=i){ map_os<<" ("<<i<<"->"<<permA[i]<<")"; ++first_print; }} if(first_print>0) LOG_INFO(map_os.str());
                } else if(rank_==0) {
                ...
                std::vector<int> permB; double relB_after=-1.0; bool have_permB=false; have_permB = infer_row_permutation(B_pop, B_ref, k, n, permB, relB_after);
                if (have_permB && rank_==0) {
                    int mismatches=0; for(int i=0;i<k;++i) if(permB[i]!=i) ++mismatches; std::ostringstream oss; oss<<"[CosmaPrefill][perm-infer] B row permutation size="<<k<<" moved_rows="<<mismatches<<" rel_after="<<relB_after; LOG_INFO(oss.str());
                } else if(rank_==0) {
            ...
            if (std::getenv("LLAMINAR_COSMA_DIAG_AXIS")) {
                if (rank_==0) LOG_INFO("[CosmaPrefill][axis] Axis diagnostics enabled");
                auto infer_col_permutation = [&](const std::vector<float>& pop, const float* ref, int rows, int cols, std::vector<int>& perm_out, double &rel_after){
                    perm_out.clear(); rel_after=-1.0; if(!ref) return false; perm_out.resize(cols, -1);
                    struct ColHashEntry { uint64_t h; int idx; }; std::vector<ColHashEntry> ref_hash(cols);
                    auto col_hash = [&](int c){ uint64_t h=1469598103934665603ull; for(int r=0;r<rows;++r){ uint64_t v = (uint64_t)reinterpret_cast<const uint32_t&>(ref[(size_t)r*cols + c]); h ^= v; h *= 1099511628211ull; } return h; };
                    for(int c=0;c<cols;++c) ref_hash[c] = { col_hash(c), c };
                    std::sort(ref_hash.begin(), ref_hash.end(), [](auto &a, auto &b){ return a.h < b.h; });
                    auto find_ref = [&](uint64_t h){ int lo=0, hi=cols-1; while(lo<=hi){ int mid=(lo+hi)/2; if(ref_hash[mid].h==h) return ref_hash[mid].idx; if(ref_hash[mid].h < h) lo=mid+1; else hi=mid-1; } return -1; };
                    std::vector<char> used(cols,0); bool ok=true;
                    for(int c=0;c<cols;++c){ // hash pop column c
                        uint64_t h=1469598103934665603ull; for(int r=0;r<rows;++r){ uint64_t v=(uint64_t)reinterpret_cast<const uint32_t&>(pop[(size_t)r*cols + c]); h ^= v; h *= 1099511628211ull; }
                        int ref_c = find_ref(h); if(ref_c<0 || used[ref_c]) { ok=false; break; } perm_out[c]=ref_c; used[ref_c]=1; }
                    if(!ok) return false;
                    std::vector<float> reordered; reordered.resize((size_t)rows*cols);
                    for(int c=0;c<cols;++c){ int dst=perm_out[c]; if(dst>=0){ for(int r=0;r<rows;++r) reordered[(size_t)r*cols + dst] = pop[(size_t)r*cols + c]; }}
                    long double num=0.0L, den=0.0L; size_t sz=(size_t)rows*cols; for(size_t i=0;i<sz;++i){ long double d=(long double)reordered[i] - (long double)ref[i]; num+=d*d; long double rv=(long double)ref[i]; den+=rv*rv; }
                    rel_after=(double)(std::sqrt(num)/(std::sqrt(den)+1e-30L));
                    return true;
                };
                // Transpose experiment (simple full transpose) for A: create A_T such that A_T[r*k + c] = A_pop[c*m + r] (treat original as transposed)
                auto rel_after_transpose = [&](const std::vector<float>& pop, const float* ref, int rows, int cols){ if(!ref) return -1.0; std::vector<float> T((size_t)rows*cols); for(int r=0;r<rows;++r) for(int c=0;c<cols;++c) T[(size_t)r*cols + c] = pop[(size_t)c*rows + r]; return rel_l2(T, ref, (size_t)rows*cols); };
                // A: column permutation
                std::vector<int> permA_cols; double relA_col_after=-1.0; bool have_colA = infer_col_permutation(A_pop, A_ref, m, k, permA_cols, relA_col_after);
                // B: column permutation (B dims k x n)
                std::vector<int> permB_cols; double relB_col_after=-1.0; bool have_colB = infer_col_permutation(B_pop, B_ref, k, n, permB_cols, relB_col_after);
                double relA_trans_simple = rel_after_transpose(A_pop, A_ref, m, k);
                double relB_trans_simple = rel_after_transpose(B_pop, B_ref, k, n);
                if(rank_==0){
                    std::ostringstream oss; oss << "[CosmaPrefill][axis] A col-perm="<<(have_colA?"yes":"no")<<" rel_after="<<relA_col_after
                        <<" simple_trans_rel="<<relA_trans_simple;
                    LOG_INFO(oss.str());
                    std::ostringstream oss2; oss2 << "[CosmaPrefill][axis] B col-perm="<<(have_colB?"yes":"no")<<" rel_after="<<relB_col_after
                        <<" simple_trans_rel="<<relB_trans_simple;
                    LOG_INFO(oss2.str());
                    if(have_colA){ int printed=0; std::ostringstream map; map<<"[CosmaPrefill][axis] A col perm sample:"; for(int c=0;c<k && printed<16;++c){ if(permA_cols[c]!=c){ map<<" ("<<c<<"->"<<permA_cols[c]<<")"; ++printed; }} if(printed) LOG_INFO(map.str()); }
                    if(have_colB){ int printed=0; std::ostringstream map; map<<"[CosmaPrefill][axis] B col perm sample:"; for(int c=0;c<n && printed<16;++c){ if(permB_cols[c]!=c){ map<<" ("<<c<<"->"<<permB_cols[c]<<")"; ++printed; }} if(printed) LOG_INFO(map.str()); }
                }
            }
            ...
            if (A_compat.mat && A_compat.mat->matrix_pointer()) {
                float* loc=A_compat.mat->matrix_pointer(); size_t sz=A_compat.mat->matrix_size();
                for(size_t li=0; li<sz; ++li){ auto gc=A_compat.mat->global_coordinates((int)li); if(gc.first<tile_m && gc.second<tile_k) ownA[(size_t)gc.first*tile_k+gc.second]=1; }
            }
            if (B_compat.mat && B_compat.mat->matrix_pointer()) {
                float* loc=B_compat.mat->matrix_pointer(); size_t sz=B_compat.mat->matrix_size();
                for(size_t li=0; li<sz; ++li){ auto gc=B_compat.mat->global_coordinates((int)li); if(gc.first<tile_k && gc.second<tile_n) ownB[(size_t)gc.first*tile_n+gc.second]=1; }
            }
            if (mpi_is_initialized() && mpi_world_size_safe()>1) {
            ...
            int dupA=0, dupB=0; for(int v: ownA) if(v>1) ++dupA; for(int v: ownB) if(v>1) ++dupB;
            // Reference GEMMs
            ...
            double pre_rel = -1.0; if(!C_orig_ref.empty()) {
                long double num=0.0L, den=0.0L; for(size_t i=0;i<C_pop_ref.size();++i){ long double d=(long double)C_pop_ref[i]-(long double)C_orig_ref[i]; num+=d*d; long double r=(long double)C_orig_ref[i]; den+=r*r; } pre_rel=(double)(std::sqrt(num)/(std::sqrt(den)+1e-30L));
            }
            ...
            if (std::getenv("LLAMINAR_COSMA_DIAG_TRY_TRANSPOSE")) {
                ...
                if(!C_orig_ref.empty()) {
                    long double num=0.0L, den=0.0L; for(size_t i=0;i<C_alt.size();++i){ long double d=(long double)C_alt[i]-(long double)C_orig_ref[i]; num+=d*d; long double r=(long double)C_orig_ref[i]; den+=r*r; } pre_rel_trans=(double)(std::sqrt(num)/(std::sqrt(den)+1e-30L));
                }
            }
            ...
            if (rank_==0 && should_log(2)) {
                ...
                if(relA > 0.5) emit_samples("A", A_pop, A_ref, m, k);
                if(relB > 0.5) emit_samples("B", B_pop, B_ref, k, n);
                if (should_log(3)) {
        ...
        if (deep_diag) {
            ...
            if (rank_==0) {
                auto rel = [&](const std::vector<float>& R, const std::vector<float>& S){ if(R.empty()||S.empty()) return -1.0; long double num=0.0L, den=0.0L; for(size_t i=0;i<R.size();++i){ long double d=(long double)R[i]-(long double)S[i]; num+=d*d; long double r=(long double)S[i]; den+=r*r; } return (double)(std::sqrt(num)/(std::sqrt(den)+1e-30L)); };
        ...
        if (std::getenv("LLAMINAR_COSMA_COMPARE_REPLICATED")) {
            ...
            if (rank_ == 0) {
                ...
                if (A_ptr && B_ptr) {
                    ...
                    for (int i=0;i<m*n;++i){ double diff = (double)C_direct[i]- (double)C_ref[i]; num += diff*diff; double rv=(double)C_ref[i]; den += rv*rv; max_abs = std::max(max_abs, std::abs(diff)); }
                ...
                if (B_compat.mat && B_compat.mat->matrix_pointer()) {
                    float *localB = B_compat.mat->matrix_pointer(); size_t szB = B_compat.mat->matrix_size();
                    std::fill(localB, localB + szB, 0.f);
                    bool legacy_forward = (std::getenv("LLAMINAR_COSMA_POP_FORWARD_LEGACY") != nullptr);
                    bool dest_local_mode = !legacy_forward;
                    bool trace_scatter_B2 = (std::getenv("LLAMINAR_COSMA_DIAG_DEEP")!=nullptr) && should_log(4);
                    if (dest_local_mode) {
                        if (trace_scatter_B2) LOG_TRACE("[CosmaPrefill][pop] destination-local population (B) compare path");
                        ...
                        for (int gi=0; gi<k; ++gi) {
                            ...
                            for (int gj=0; gj<n; ++gj) {
                                auto lc = B_compat.mat->local_coordinates(gi, gj);
                                if (lc.second == rank_) {
                                    int lidx = lc.first; if (lidx>=0 && (size_t)lidx < szB) localB[lidx] = row_ptr[gj];
                                    if (std::getenv("LLAMINAR_COSMA_DIAG_DEEP") && gi < std::min(k,8) && gj < std::min(n,8) && rank_==0 && should_log(4)) {
                                        LOG_TRACE("[CosmaPrefill][scatterB] (gi="<<gi<<",gj="<<gj<<") val="<<row_ptr[gj]<<" lidx="<<lidx);
                                    }
                                }
    ...
    void CosmaPrefillManager::reconstruct_matrix(const CosmaView &src, float *dst, bool normalize) const
    {
        ...
        if (brute_recon)
        {
            ...
            if (normalize)
            {
                ...
                {
                    int c = counts[i];
                    if (c > 1) dst[i] /= (float)c;
                }
            }
            double t1 = MPI_Wtime();
            if (rank_ == 0 && should_log(2))
            {
                long long dup = 0; for (int c : counts) if (c > 1) ++dup;
        ...
        if (use_gather)
        {
            ...
            std::vector<int> gi_all, gj_all; std::vector<float> val_all;
            if (rank_ == 0)
            {
            ...
            if (rank_ == 0)
            {
                ...
                {
                    int gi = gi_all[i]; int gj = gj_all[i];
        ...
        if (deep && rank_==0 && should_log(4)) {
            long long multi_count = 0; int max_c = 0; int min_c = 1000000; long double sum_c=0.0L; long double sum_c2=0.0L;
            for (size_t idx=0; idx<total; ++idx) {
                int c = ownership_counts[idx];
                if (c) {
                    if (c>1) ++multi_count;
                    if (c>max_c) max_c=c; if (c<min_c) min_c=c;
                    sum_c += c; sum_c2 += (long double)c*c;
                }
            }
    ...
    {
        if (!std::getenv("LLAMINAR_COSMA_DEBUG_RECON")) return;
        std::vector<float> recon((size_t)rows * cols, 0.f);
    ...
    void CosmaPrefillManager::diag_global_checksum(const CosmaView &src, const char *tag) const {
        if (!std::getenv("LLAMINAR_COSMA_DIAG")) return;
        double local_sum = 0.0, local_sq = 0.0; size_t local_count = 0;
        if (src.mat && src.mat->matrix_pointer()) {
            float *ptr = src.mat->matrix_pointer(); size_t sz = src.mat->matrix_size();
            for (size_t i=0;i<sz;++i){ double v = ptr[i]; local_sum += v; local_sq += v*v; }
            local_count = sz;
        } else if (src.original_row_major && src.label!='C') {
            size_t sz = (size_t)src.global_rows * src.global_cols; for (size_t i=0;i<sz;++i){ double v = src.original_row_major[i]; local_sum += v; local_sq += v*v; }
            local_count = sz;
        }
    ...
    void CosmaPrefillManager::diag_sample_points(const CosmaView &src, const float *original, const char *tag) const {
        ...
        for (auto &p: pts) {
            int gi = p.first, gj = p.second; if (gi>=src.global_rows||gj>=src.global_cols) continue;
            auto lc = src.mat->local_coordinates(gi, gj); int owner = lc.second; int lidx = lc.first;
            float recon_val = 0.f;
    ...
    void CosmaPrefillManager::diag_compare_row_major(const float *A, const float *B, int m, int n, const char *tagA, const char *tagB) const {
        if (!std::getenv("LLAMINAR_COSMA_DIAG") || !A || !B) return;
        long double num=0.0L, den=0.0L, max_abs=0.0L; size_t total = (size_t)m*n;
        for (size_t i=0;i<total;++i){ long double diff = (long double)A[i]- (long double)B[i]; num += diff*diff; long double ref = (long double)A[i]; den += ref*ref; max_abs = std::max(max_abs, fabsl(diff)); }
    ...
    bool CosmaPrefillManager::memory_budget_allows(size_t bytes_needed) const
    {
        ...
        if (!ok) const_cast<CosmaPrefillManager*>(this)->stats_.allocations_denied++;
        return ok;
    }
...
void CosmaPrefillManager::dump_stats_json(const std::string &path) const {
    if (rank_ != 0) return; // only rank 0 writes aggregate file
    std::ofstream ofs(path);
    if (!ofs.is_open()) {
        if (should_log(1)) LOG_WARN("[CosmaPrefill][stats] Failed to open output path: " << path);
        return;
    }
    auto q = [&](const char *k, long long v, bool trailing=true){ ofs << "  \""<<k<<"\": "<<v<<(trailing?",\n":"\n"); };
    ofs << "{\n";
    ...
    ofs << "}\n";
    if (should_log(3)) LOG_DEBUG("[CosmaPrefill][stats] Wrote JSON stats to "<<path);
}
...
void CosmaPrefillManager::recalc_resident() {
    ...
    for (auto &rec : allocations_) {
        if (rec.ref.expired()) continue;
        sum += rec.bytes;
    }
    stats_.current_resident_bytes = sum;
    long long peak = stats_.peak_resident_bytes.load();
    if (sum > peak) stats_.peak_resident_bytes = sum;
}
