...
#pragma omp parallel for collapse(2) if (total_elements > 10000)
        ...
        int device_idx)
    {
        ...
        (void)Q_ptr;
        (void)K_ptr;
        LOG_ERROR("[Q8_1Tensor::applyRoPE] Legacy raw-pointer path removed. Use RoPEStage with apply_tensor() instead.");
        return false;
    }
...
#pragma omp parallel for schedule(static)
    ...
    namespace detail
    {
        float q8_1_find_max_abs_scalar(const Q8_1Block *row_blocks, size_t blocks_per_row)
        {
            float max_abs = 0.0f;
            for (size_t b = 0; b < blocks_per_row; ++b)
            {
...
#if defined(__AVX2__)
        float q8_1_find_max_abs_avx2(const Q8_1Block *row_blocks, size_t blocks_per_row)
        {
            float max_abs = 0.0f;
            for (size_t b = 0; b < blocks_per_row; ++b)
            {
        ...
        void q8_1_requantize_scalar(const Q8_1Block *row_blocks, size_t blocks_per_row,
...
#if defined(__AVX2__)
        void q8_1_requantize_avx2(const Q8_1Block *row_blocks, size_t blocks_per_row,
                                  float inv_row_scale, int8_t *output)
        {
...
#if defined(__AVX512F__)
        void q8_1_requantize_avx512(const Q8_1Block *row_blocks, size_t blocks_per_row,
                                    float inv_row_scale, int8_t *output)
        {
...
// --- ISA stubs for runtime dispatch ---
        ...
        float q8_1_find_max_abs_avx2(const Q8_1Block *row_blocks, size_t blocks_per_row)
        {
            return q8_1_find_max_abs_scalar(row_blocks, blocks_per_row);
        }
        void q8_1_requantize_avx2(const Q8_1Block *row_blocks, size_t blocks_per_row,
                                  float inv_row_scale, int8_t *output)
        {
            q8_1_requantize_scalar(row_blocks, blocks_per_row, inv_row_scale, output);
        }
#endif
        float q8_1_find_max_abs_avx512(const Q8_1Block *row_blocks, size_t blocks_per_row)
        {
            return q8_1_find_max_abs_avx2(row_blocks, blocks_per_row);
        }
#if !defined(__AVX512F__)
        void q8_1_requantize_avx512(const Q8_1Block *row_blocks, size_t blocks_per_row,
                                    float inv_row_scale, int8_t *output)
        {
            q8_1_requantize_avx2(row_blocks, blocks_per_row, inv_row_scale, output);
        }
#endif
    } // namespace detail
    ...
    float Q8_1Tensor::requantizeRowToInt8(size_t row_idx, size_t K, int8_t *output) const
    {
        ...
        float max_abs = ISA_DISPATCH_RETVAL(q8_1_find_max_abs, row_blocks, blocks_per_row);
