...
namespace llaminar2
{
    ...
    IQ2_STensor::IQ2_STensor(
        ...
        : IQ2_STensor(shape, AlignedVector<uint8_t>(raw_data))
    {
    }
    ...
    IQ2_STensor::IQ2_STensor(const std::vector<size_t> &shape,
          ...
          data_byte_size_(byte_size), device_(DeviceId::cpu()), device_blocks_(nullptr)
    {
        ...
        {
            throw std::invalid_argument("IQ2_STensor: insufficient mmap data (" +
...
#pragma omp parallel for schedule(static) if (total_elements > 10000)
            for (size_t r = 0; r < shape_[0]; ++r)
            {
                for (size_t b = 0; b < blocks_per_row; ++b)
                {
                    ...
                    const size_t col_offset = b * IQ2_SBlock::BLOCK_SIZE;
                    const size_t remaining = shape_[1] > col_offset ? (shape_[1] - col_offset) : 0;
                    const size_t valid_count = std::min(static_cast<size_t>(IQ2_SBlock::BLOCK_SIZE), remaining);
...
#pragma omp parallel for
    ...
    void IQ2_STensor::packVnniBlock(const VnniPackContext &ctx, int source_n, int destination_n, int b) const
    {
