...
namespace llaminar2
{
    ...
    IQ3_STensor::IQ3_STensor(
        ...
        : IQ3_STensor(shape, AlignedVector<uint8_t>(raw_data))
    {
    }
          ...
          data_byte_size_(byte_size), device_(DeviceId::cpu()), device_blocks_(nullptr)
    {
        ...
        {
            throw std::invalid_argument("IQ3_STensor: insufficient mmap data (" +
...
#pragma omp parallel for schedule(static) if (total_elements > 10000)
            for (size_t r = 0; r < shape_[0]; ++r)
            {
                for (size_t b = 0; b < blocks_per_row; ++b)
                {
                    ...
                    const size_t col_offset = b * IQ3_SBlock::BLOCK_SIZE;
                    const size_t remaining = shape_[1] > col_offset ? (shape_[1] - col_offset) : 0;
                    const size_t valid_count = std::min(static_cast<size_t>(IQ3_SBlock::BLOCK_SIZE), remaining);
                    ...
                    if (valid_count == IQ3_SBlock::BLOCK_SIZE)
                    {
...
#pragma omp parallel for
    ...
    void IQ3_STensor::packVnniBlock(const VnniPackContext &ctx, int source_n, int destination_n, int b) const
    {
