...
namespace llaminar2
{
    ...
    const float *FP32Tensor::data() const
    {
        ...
        LOG_DEBUG("[FP32Tensor::data] Called for tensor, host_valid_=" << host_valid_ << " device_valid_=" << device_valid_ << " gpu_data_ptr_=" << (gpu_data_ptr_ ? "set" : "null"));
    ...
    bool FP32Tensor::copyFrom(const CPUTensorBase *src)
    {
        if (!src)
        {
    ...
    std::shared_ptr<CPUTensorBase> FP32Tensor::create_view(
