...
namespace llaminar2
{
    ...
    IQ3_XXSTensor::IQ3_XXSTensor(const std::vector<size_t> &shape, const uint8_t *raw_data_ptr,
                                 size_t byte_offset, std::shared_ptr<CPUTensorBase> parent)
    ...
    std::shared_ptr<CPUTensorBase> IQ3_XXSTensor::create_view(
...
#pragma omp parallel for schedule(static) if (total_elements > 10000)
    ...
    bool IQ3_XXSTensor::copyFrom(const CPUTensorBase *src)
    {
