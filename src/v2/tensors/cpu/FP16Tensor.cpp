...
namespace llaminar2
{
    ...
    FP16Tensor::FP16Tensor(const std::vector<size_t> &shape, const std::vector<uint16_t> &fp16_data)
        : shape_(shape), device_(DeviceId::cpu()), device_data_(nullptr), host_fp16_data_(fp16_data.size()),
          is_view_(false), parent_data_ptr_(nullptr), view_offset_(0), parent_(nullptr)
    {
        if (shape.empty())
        {
    ...
    bool FP16Tensor::copyFrom(const CPUTensorBase *src)
    {
        if (!src)
        {
            return false;
...
#pragma omp for
    ...
    std::shared_ptr<CPUTensorBase> FP16Tensor::create_view(
