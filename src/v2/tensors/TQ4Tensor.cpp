...
namespace llaminar2
{
        ...
        : shape_(shape), head_dim_(head_dim), device_(device)
    {
        ...
        {
            throw std::invalid_argument("TQ4Tensor: head_dim must be 64, 128, or 256");
        }
        ...
        if (head_dim == 256)
            block_bytes_ = sizeof(TQ4Block_256);
        else if (head_dim == 128)
            block_bytes_ = sizeof(TQ4Block_128);
        else
            block_bytes_ = sizeof(TQ4Block_64);
        ...
        // Pre-allocate raw block storage
    ...
    void TQ4Tensor::to_fp32_row(size_t row_idx, float *buffer) const
    {
        ...
        for (size_t h = 0; h < bpr; ++h)
        {
            ...
            if (head_dim_ == 256)
            {
                ...
                const auto *block = reinterpret_cast<const TQ4Block_256 *>(block_src);
                turboquant_dequantize_tq4<256>(*block, head_ctx, head_dst, scratch);
            }
...
#pragma omp parallel for if (num_rows * bpr >= 32)
        for (size_t r = 0; r < num_rows; ++r)
        {
            ...
            for (size_t h = 0; h < bpr; ++h)
            {
                ...
                if (head_dim == 256)
                {
                    auto *block = reinterpret_cast<TQ4Block_256 *>(block_dst);
                    const auto &head_ctx = turboquant_ctx.for_layer(static_cast<int>(h));
                    turboquant_quantize_tq4<256>(head_src, head_ctx, *block, scratch0, scratch1);
                }
                else if (head_dim == 128)
                {
