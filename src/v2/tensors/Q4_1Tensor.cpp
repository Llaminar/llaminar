...
namespace llaminar2
{
    ...
    Q4_1Tensor::Q4_1Tensor(
        ...
        : Q4_1Tensor(shape, AlignedVector<uint8_t>(raw_data))
    {
    }
          ...
          data_byte_size_(byte_size), device_(DeviceId::cpu()), device_blocks_(nullptr)
    {
        ...
        {
            throw std::invalid_argument("Q4_1Tensor: insufficient mmap data (" +
        ...
        size_t offset)
    {
        ...
        if (new_shape.size() != 2)
        {
            throw std::invalid_argument("Q4_1Tensor::create_view: only 2D views supported");
        }
        ...
        {
            throw std::invalid_argument("Q4_1Tensor::create_view: parent must be 2D or 3D");
        }

        // Validation: K dimension must match
        if (new_shape[1] != K)
        {
            throw std::invalid_argument("Q4_1Tensor::create_view: K dimension must match parent");
        }
        ...
        const size_t blocks_per_row = (K + Q4_1Block::BLOCK_SIZE - 1) / Q4_1Block::BLOCK_SIZE;
        const size_t block_offset = start_row * blocks_per_row;
...
#pragma omp parallel for
    ...
    void Q4_1Tensor::packVnniBlock(const VnniPackContext &ctx, int source_n, int destination_n, int b) const
    {
