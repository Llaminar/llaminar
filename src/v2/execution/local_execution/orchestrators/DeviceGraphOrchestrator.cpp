...
namespace llaminar2
{
    ...
    bool DeviceGraphOrchestrator::supportsMTPSpecDecodeStatePublication() const
    {
        if (!graph_builder_ || !graph_builder_->config().mtp.enabled || !state_.kv_cache)
        {
            return false;
    ...
    bool DeviceGraphOrchestrator::publishMTPSpecDecodeState(
        ...
        int seq_idx)
    {
        ...
        {
            PerfStatsCollector::addCounter("mtp",
                                           "spec_decode_state_publication_failures",
                                           1.0,
                                           "decode",
                                           state_.device_id.toString(),
                                           {{"reason", reason}});
            LOG_DEBUG("[DeviceGraphOrchestrator] MTP spec-decode state publication unavailable: " << reason);
            return false;
        };

        if (!plan.ok)
        {
            return fail(std::string("invalid_publication_plan:") + plan.error);
        }
        if (!state_.kv_cache || seq_idx < 0 || seq_idx >= state_.batch_size)
        {
            return fail("missing_cache_or_bad_sequence");
        }
        if (seq_idx >= plan.request_count ||
            seq_idx >= static_cast<int>(plan.target_cached_tokens.size()) ||
            seq_idx >= static_cast<int>(plan.accepted_state_counts.size()) ||
            seq_idx >= static_cast<int>(plan.accepted_state_slot_indices.size()))
        {
            return fail("publication_plan_missing_request_slot");
        }
        if (seq_idx >= static_cast<int>(state_.positions.size()) ||
            seq_idx >= static_cast<int>(state_.sequence_lengths.size()))
        {
        ...
        const int accepted_count =
            plan.accepted_state_counts[static_cast<size_t>(seq_idx)];
        const int accepted_slot =
            plan.accepted_state_slot_indices[static_cast<size_t>(seq_idx)];
        const int target_cached_tokens =
            plan.target_cached_tokens[static_cast<size_t>(seq_idx)];
        if (target_cached_tokens < 0 ||
            target_cached_tokens > state_.kv_cache->max_seq_len())
        {
            return fail("bad_publication_target_tokens");
        }
        if (accepted_count < 0 ||
            accepted_count > plan.shape.max_draft_tokens)
        {
            return fail("bad_publication_accepted_count");
        }
        if (accepted_count == 0)
        {
            if (accepted_slot != kMTPSpecDecodeInvalidToken)
            {
                return fail("zero_accept_publication_named_state_slot");
            }
        }
        else if (accepted_slot < 0 ||
                 accepted_slot >= plan.shape.maxTargetQueryLen())
        {
            return fail("bad_publication_state_slot");
        }
        ...
        void *stream = explicitGPUStreamForOperation("publishMTPSpecDecodeState");
        if (state_.device_id.is_gpu() && !stream)
        {
            return fail("missing_explicit_gpu_stream");
        }
        ...
        if (accepted_count > 0)
        {
            for (int layer = 0; layer < state_.kv_cache->n_layers(); ++layer)
            {
                ...
                if (!conv_kernel->restoreVerifierStateCaptureRow(conv_state, accepted_slot, stream))
                {
                    return fail("shortconv_publication_restore_failed");
                }
                if (!recurrence_kernel->restoreVerifierStateCaptureRow(
                        recurrence_state,
                        accepted_slot,
                        stream))
                {
                    return fail("recurrence_publication_restore_failed");
                }
                ++restored_layers;
            }
        ...
        state_.sequence_lengths[seq_idx] = target_cached_tokens;
        handleLivePrefixReplayStateAfterMutation(
            "publish_mtp_spec_decode_state",
            /*preserve_gpu_replay_state=*/true);
        PerfStatsCollector::addCounter("mtp",
                                       "spec_decode_state_publications",
                                       1.0,
                                       "decode",
                                       state_.device_id.toString(),
                                       {{"accepted_count", std::to_string(accepted_count)},
                                        {"accepted_slot", std::to_string(accepted_slot)},
                                        {"target_cached_tokens", std::to_string(target_cached_tokens)},
                                        {"gdn_layers", std::to_string(restored_layers)}});
        return true;
    }
