...
namespace llaminar2
{
    ...
    bool ForwardExecutionEngine::publishAcceptedSpeculativeVerifierStateFromDeviceMetadata(
    ...
    {
        if (published_stage_count)
            *published_stage_count = 0;
        auto set_error = [&](const std::string &reason) -> bool
        {
            if (error)
                *error = reason;
            return false;
        ...
        if (!device_accepted_state_slot_indices)
            return set_error("missing_accepted_state_slot_indices_device_buffer");
        if (request_index < 0)
            return set_error("bad_request_index");
        if (!device.is_gpu())
            return set_error("gpu_required");
        if (!stream)
            return set_error("missing_explicit_gpu_stream");
        if (verifier_seq_len <= 0)
            return set_error("bad_verifier_seq_len");

        const ForwardGraphCache *matching_cache = nullptr;
        const ForwardGraphSignature *matching_signature = nullptr;
        for (const auto &[sig, cache] : cache_)
        {
            if (!cache.valid || !cache.graph)
                continue;
            if (!sig.decode ||
                !sig.decode_has_history ||
                !sig.all_position_logits ||
                sig.is_bucketed_prefill ||
                sig.device != device ||
                sig.seq_len != verifier_seq_len ||
                sig.moe_placement_epoch != moe_placement_epoch)
            {
                continue;
            }

            if (matching_cache)
                return set_error("multiple_cached_verifier_graphs");
            matching_cache = &cache;
            matching_signature = &sig;
        }

        if (!matching_cache || !matching_cache->graph || !matching_signature)
            return set_error("no_cached_verifier_graph");

        int published = 0;
        for (const auto &node_name : matching_cache->graph->getExecutionOrder())
        {
            auto *node = matching_cache->graph->getNode(node_name);
            if (!node || !node->stage || !node->stage->hasVerifierStateCapture())
                continue;

            IComputeStage *stage = node->stage.get();
            if (!stage->prepareVerifierStatePublication())
            {
                return set_error(
                    std::string("verifier_stage_prepare_failed:") +
                    node_name + ":" + stage->name());
            }
            if (!stage->publishAcceptedSpeculativeStateFromDeviceMetadata(
                    device_accepted_state_slot_indices,
                    request_index,
                    stream))
            {
                return set_error(
                    std::string("verifier_stage_publish_failed:") +
                    node_name + ":" + stage->name());
            }
            ++published;
        }
        ...
        if (published <= 0)
            return set_error("cached_verifier_graph_no_stateful_stages");

        if (published_stage_count)
            *published_stage_count = published;

        PerfStatsCollector::addCounter(
            "mtp",
            "spec_decode_graph_stage_state_publications",
            1.0,
            "decode",
            device.toString(),
            {{"request_index", std::to_string(request_index)},
             {"published_stages", std::to_string(published)},
             {"seq_len", std::to_string(matching_signature->seq_len)},
             {"moe_placement_epoch", std::to_string(matching_signature->moe_placement_epoch)}});
        return true;
    }
