...
namespace llaminar2
{
    ...
    GenerationResult OrchestrationRunner::decodeStepMTP()
    {
        ...
        const bool first_token_is_stop =
            std::find(stop_tokens_.begin(), stop_tokens_.end(), first_token) != stop_tokens_.end();
        const bool disable_decode_equivalent_sequential_greedy_verifier =
            DebugEnv::isTruthyEnvValue(
                std::getenv("LLAMINAR_MTP_DISABLE_DECODE_EQUIVALENT_SEQUENTIAL_VERIFIER")) ||
            DebugEnv::isTruthyEnvValue(
        ...
        const MTPVerifierPolicyDecision verifier_policy =
            chooseMTPVerifierPolicy(
                MTPVerifierPolicyInput{
                    .runner_requires_decode_equivalent_replay =
                        runner_ && runner_->requiresMTPDecodeEquivalentVerifierReplay(),
                    .runner_supports_verifier_state_row_restore =
                    ...
                    .disable_decode_equivalent_sequential =
                        disable_decode_equivalent_sequential_greedy_verifier,
                });
        const bool allow_verifier_state_row_shortcut =
            verifier_policy.allow_verifier_state_row_shortcut;
                ...
                {
                    {
                        PerfStatsCollector::ScopedTimer timer(
                            "mtp",
                            "sample_mtp_token_stochastic_penalties",
                            "decode");
                        auto penalty_map =
        ...
        for (int draft_idx = 0; draft_idx < speculative_draft_count; ++draft_idx)
        {
            ...
            if (draft_idx == 0)
            {
                if (can_skip_post_sidecar_checkpoint)
                {
                    ...
                    anchor.provenance = PrefixStateProvenance::LogicalCheckpoint;
        ...
        if (use_decode_equivalent_sequential_greedy_verifier)
        {
            ...
            if (stochastic_verify)
            {
                ...
                if (!stopped_on_output)
                {
                    ...
                    ready_token = runner_->sampleStochasticDistributionOnDevice(
                        DeviceDistributionBuffer::Target,
                        0,
                        sampler_.random_uniform_01());
                    if (ready_token < 0)
        ...
        std::vector<DeviceSpeculativeVerifyResult> batched_device_verify_results;
        std::vector<int32_t> batched_device_verify_draft_tokens;
        if (stochastic_verify && stochastic_device_verify)
        {
            const int verify_row_count =
                std::max(0, static_cast<int>(draft_tokens.size()) - 1);
            {
                PerfStatsCollector::ScopedTimer timer(
                    "mtp",
                    "phase138_stochastic_prepare_verify_batch",
                    "decode");
                batched_device_verify_results.resize(static_cast<size_t>(verify_row_count));
                batched_device_verify_draft_tokens.resize(static_cast<size_t>(verify_row_count));
            }
            std::vector<float> accept_thresholds(static_cast<size_t>(verify_row_count));
            std::vector<float> residual_thresholds(static_cast<size_t>(verify_row_count));
            {
                PerfStatsCollector::ScopedTimer timer(
                    "mtp",
                    "phase138_stochastic_prepare_verify_thresholds",
                    "decode");
                std::vector<float> threshold_draws(static_cast<size_t>(verify_row_count) * 2u);

                Sampler verify_rng = sampler_;
                for (size_t draw = 0; draw < threshold_draws.size(); ++draw)
                    threshold_draws[static_cast<size_t>(draw)] =
                        verify_rng.random_uniform_01();
                for (int row = 0; row < verify_row_count; ++row)
                {
                    const size_t draw_offset = static_cast<size_t>(row) * 2u;
                    batched_device_verify_draft_tokens[static_cast<size_t>(row)] =
                        draft_tokens[static_cast<size_t>(row + 1)];
                    accept_thresholds[static_cast<size_t>(row)] =
                        threshold_draws[draw_offset];
                    residual_thresholds[static_cast<size_t>(row)] =
                        threshold_draws[draw_offset + 1u];
                }
            }

            if (verify_row_count > 0)
            {
                {
                    PerfStatsCollector::ScopedTimer timer(
                        "mtp",
                        "phase138_stochastic_verify_batch_device",
                        "decode");
                    if (!runner_->verifyStochasticDistributionsBatchOnDevice(
                            /*first_target_slot=*/0,
                            /*first_draft_slot=*/0,
                            batched_device_verify_draft_tokens.data(),
                            accept_thresholds.data(),
                            residual_thresholds.data(),
                            verify_row_count,
                            batched_device_verify_results.data()))
                    {
                        return fail_after_checkpoint(
                            "Device stochastic MTP batched verifier failed");
                    }
                }
                PerfStatsCollector::addCounter(
                    "mtp",
                    "stochastic_verify_batch_tests",
                    static_cast<double>(verify_row_count),
                    "decode");
            }
        }
        int accepted_speculative_prefix = 0;
        ...
        int32_t rejected_verified_token = -1;
        {
            PerfStatsCollector::ScopedTimer timer(
                "mtp",
                stochastic_verify
                    ? "phase138_stochastic_process_verify_rows"
                    : "process_greedy_verify_rows",
                "decode");
            for (int draft_idx = 1; draft_idx < static_cast<int>(draft_tokens.size()); ++draft_idx)
            {
                ...
                if (stochastic_verify)
                {
                    if (stochastic_device_verify)
                    {
                        ...
                        (void)sampler_.random_uniform_01();
                        DeviceSpeculativeVerifyResult verify_result;
                        if (row < 0 ||
                            row >= static_cast<int>(batched_device_verify_results.size()))
                        {
                            return fail_after_checkpoint("Device stochastic MTP batched verifier row is incomplete");
                        }
                        verify_result =
                            batched_device_verify_results[static_cast<size_t>(row)];
                        ...
                        if (verify_result.token < 0)
                            return fail_after_checkpoint("Device stochastic MTP batched residual verifier produced no correction token");
        ...
        const bool accepted_all_position_state_requires_replay =
            verifier_state_matches_output_from_tokens &&
            verifier_policy.accepted_all_position_state_requires_replay;
        const bool verifier_state_matches_output =
            verifier_state_matches_output_from_tokens &&
            !accepted_all_position_state_requires_replay;
        if (accepted_all_position_state_requires_replay)
        {
            PerfStatsCollector::addCounter(
                "mtp",
        ...
        if (PerfStatsCollector::isEnabled())
        {
            PerfStatsCollector::addCounter(
                ...
                {},
                {
                    ...
                    {"decode_equivalent_replay_required", accepted_all_position_state_requires_replay ? "true" : "false"},
                    {"output_tokens", std::to_string(accepted_tokens.size())},
        ...
        if (verifier_state_matches_output)
        {
            ...
            if (auto tx_error = validate_spec_decode_transaction(
                    ...
                    draft_tokens,
                    accepted_tokens,
                    result.is_complete ? std::optional<int32_t>{}
                                       : std::optional<int32_t>{next_ready_token},
                    all_speculative_accepted,
                    result.is_complete,
                    accepted_speculative_prefix))
            {
        ...
        if (verifier_row_shortcut_candidate)
        {
            ...
            if (restored_verifier_state)
            {
                ...
                if (suffix_count > 0)
                {
                    ...
                    const bool use_cuda_grouped_suffix_replay =
                        runner_ &&
                        runner_->primaryDeviceId().is_cuda() &&
                        !stochastic_verify &&
                        !use_sampling_penalties &&
                        active_sampling_params_.is_greedy();
                    bool grouped_suffix_enabled = false;
                    auto disable_grouped_suffix_replay = [&]() -> bool
                    {
                        if (!grouped_suffix_enabled)
                            return true;
                        PerfStatsCollector::ScopedTimer timer(
                            "mtp",
                            "disable_verifier_state_row_grouped_suffix_replay",
                            "decode");
                        grouped_suffix_enabled = false;
                        return runner_->setComputeAllPositionLogits(false);
                    };
                    if (use_cuda_grouped_suffix_replay)
                    {
                        PerfStatsCollector::ScopedTimer timer(
                            "mtp",
                            "enable_verifier_state_row_grouped_suffix_replay",
                            "decode");
                        grouped_suffix_enabled =
                            runner_->setComputeAllPositionLogits(true);
                        if (!grouped_suffix_enabled)
                        {
                            result.error =
                                "CUDA MTP verifier-row suffix replay requires all-position logits";
                            return result;
                        }
                    }
                    {
                    ...
                    if (!suffix_shifted_commit_ok)
                    {
                        (void)disable_grouped_suffix_replay();
                        ...
                        active_sampling_params_.is_greedy())
                    {
                        ...
                        if (next_ready_token < 0 && !use_cuda_grouped_suffix_replay)
                        {
                            const float *main_logits = runner_->logits();
                            if (main_logits)
                            {
                        ...
                        PerfStatsCollector::addCounter(
                            ...
                            {{"token", std::to_string(next_ready_token)},
                             {"grouped_suffix_replay", use_cuda_grouped_suffix_replay ? "true" : "false"}});
                    }
                    if (!disable_grouped_suffix_replay())
                    {
                        result.error =
                            "Failed to disable all-position logits after CUDA MTP verifier-row suffix replay";
                        return result;
                    }
                }
                ...
                if (auto commit_error = commit_mtp_transaction_outputs(
                        ...
                        accepted_tokens,
                        shortcut_ready_token,
                        /*terminal_logits_ready=*/shortcut_ready_token.has_value(),
                        result.is_complete,
                        PrefixStateProvenance::VerifierPrefillRowsDecodeEquivalent,
                        /*state_advanced=*/true))
                {
        ...
        else if (all_speculative_accepted && !result.is_complete)
        {
            const int terminal_row = static_cast<int>(draft_tokens.size()) - 1;
            const int32_t precomputed_ready_token =
                terminal_row >= 0 &&
                        terminal_row < static_cast<int>(sampled_verifier_tokens.size())
                    ? sampled_verifier_tokens[static_cast<size_t>(terminal_row)]
                    : -1;
            if (precomputed_ready_token < 0)
            {
                result.error =
                    "MTP replay could not find the precomputed all-accepted ready token";
                return result;
            }
            replay_ready_token = precomputed_ready_token;
            PerfStatsCollector::addCounter(
                "mtp",
                "replay_precomputed_ready_tokens",
                1.0,
                "decode",
                {},
                {{"token", std::to_string(precomputed_ready_token)},
                 {"stochastic_verify", stochastic_verify ? "true" : "false"},
                 {"uses_sampling_penalties", use_sampling_penalties ? "true" : "false"}});
        }
        ...
        if (auto tx_error = validate_spec_decode_transaction(
                "decode_equivalent_replay",
                "legacy_decode_equivalent_replay",
                draft_tokens,
                accepted_tokens,
                replay_ready_token,
                all_speculative_accepted,
