/**
 * @file INodeTPContext.h
 * @brief Interface for NodeTP spanning ranks on one physical node.
 *
 ...
 *       NodeTP(0:cpu, 1:cpu)                            // both ranks' CPUs
 *   )
 *
 * The NodeTP domain connects rank 0's CPU and rank 1's CPU into a single
...
namespace llaminar2
{
     ...
     * the NodeTP backend (UPI/shared-memory/cross-process NCCL) is built.
     *
     ...
     */
    class INodeTPContext : public ITPContext
    {
    public:
        ~INodeTPContext() override = default;
