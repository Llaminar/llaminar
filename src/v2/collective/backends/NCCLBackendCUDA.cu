...
namespace llaminar2
{
    namespace nccl_backend_detail
    {
        ...
        // =========================================================================
        // Pinned (Page-Locked) Host Memory for GPU-to-GPU Staging
        // =========================================================================
        ...
        // =========================================================================
        // CUDA Event Management for Staging Synchronization
        // =========================================================================

        bool cudaCreateEvent(void **event_ptr)
        {
            cudaEvent_t event;
            // Use cudaEventDisableTiming for faster event operations
            cudaError_t err = cudaEventCreateWithFlags(&event, cudaEventDisableTiming);
            if (err != cudaSuccess)
                return false;
            *event_ptr = static_cast<void *>(event);
            return true;
        }

        void cudaDestroyEvent(void *event)
        {
            if (event)
            {
        ...
        // 3. cudaStagingH2DAsync: Async H2D on dest device, records event
        // 4. CPU sync on H2D event (ensures copy complete before returning)
        //
        // Note: We use CPU-side event sync between D2H and H2D because CUDA events
        ...
        bool cudaMemcpyPeerDevice(void *dst, int dst_device, const void *src, int src_device, size_t bytes)
        {
            ...
            if (err != cudaSuccess)
                return false;
            err = cudaMemcpyPeer(dst, dst_device, src, src_device, bytes);
            return (err == cudaSuccess);
        }
        ...
        {
            // cudaDeviceEnablePeerAccess may return cudaErrorPeerAccessAlreadyEnabled
