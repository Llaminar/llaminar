...
float WeightMemoryEstimator::getNativeBytesPerWeight(const std::string& quant_type)
{
    ...
    if (quant_type == "Q4_0")   return 18.0f / 32;   // 0.5625
    if (quant_type == "Q4_1")   return 20.0f / 32;   // 0.625
    if (quant_type == "Q5_0")   return 22.0f / 32;   // 0.6875
    if (quant_type == "Q5_1")   return 24.0f / 32;   // 0.75
    if (quant_type == "Q8_0")   return 34.0f / 32;   // 1.0625
    if (quant_type == "Q8_1")   return 36.0f / 32;   // 1.125
    if (quant_type == "Q2_K")   return 0.3125f;       // ~10 bits/weight
    if (quant_type == "Q3_K")   return 0.4375f;
    if (quant_type == "Q4_K" || quant_type == "Q4_K_M" || quant_type == "Q4_K_S")
        return 0.5625f;
    if (quant_type == "Q5_K" || quant_type == "Q5_K_M" || quant_type == "Q5_K_S")
        return 0.6875f;
    if (quant_type == "Q6_K")   return 0.8125f;
    if (quant_type == "IQ4_NL") return 18.0f / 32;   // Same block size as Q4_0
    if (quant_type == "F16" || quant_type == "BF16") return 2.0f;
    if (quant_type == "F32")    return 4.0f;
    ...
    int last_layer)
{
    ...
    for (const auto& t : profile.tensors)
    {
        ...
        if (device.is_gpu())
        {
            // GPU repacking: use K (inner dim) for scale overhead calculation
            float bytes_per_weight = getCUDAPackedBytesPerWeight(t.K);
