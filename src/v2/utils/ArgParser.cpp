...
ArgContext ArgParser::parse(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // Offload layers
            ...
            if (!val.empty()) ctx.offload_layers = std::stoi(val);
        }
        ...
        // Memory constraints
            ...
            if (!val.empty()) ctx.max_gpu_memory_mb = std::stoull(val);
        }
        
        else if (arg == "--max-cpu-memory") {
            std::string val = getNextArg(argv, argc, i, "max-cpu-memory");
            if (!val.empty()) ctx.max_cpu_memory_mb = std::stoull(val);
        }
        ...
        // Batch size
            ...
            if (!val.empty()) ctx.batch_size = std::stoi(val);
        }
        
        // Threads
        else if (arg == "--threads") {
            std::string val = getNextArg(argv, argc, i, "threads");
            if (!val.empty()) ctx.n_threads = std::stoi(val);
        }
