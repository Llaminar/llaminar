...
namespace
{
    ...
    __global__ void cuda_gdn_compact_modular_conv_state_kernel(
     ...
     *
     * Modular GDN TP is not a plain contiguous split.  Q and K conv history are
     * mirrored on every participant, while V conv history is partitioned by
     * participant.  The local verifier bank therefore consists of all full Q/K
     * channels followed by only this rank's V-channel slice.
     */
    __global__ void cuda_gdn_slice_modular_conv_state_kernel(
...
extern "C"
{
        ...
        void *stream)
    {
        ...
        {
            fprintf(stderr,
                    "[cudaGDN_compact_modular_conv_state] cudaSetDevice(%d) failed: %s\n",
                    device_idx,
        ...
        {
            fprintf(stderr,
                    "[cudaGDN_compact_modular_conv_state] %s\n",
                    cudaGetErrorString(err));
            return false;
        }
        return true;
        ...
        void *stream)
    {
        ...
        {
            fprintf(stderr,
                    "[cudaGDN_slice_modular_conv_state] cudaSetDevice(%d) failed: %s\n",
                    device_idx,
                    cudaGetErrorString(set_err));
            return false;
        }
        ...
        if (blocks > 1024)
            blocks = 1024;
        cuda_gdn_slice_modular_conv_state_kernel<<<
            blocks,
            threads,
            0,
            (cudaStream_t)stream>>>(
        ...
        {
            fprintf(stderr,
                    "[cudaGDN_slice_modular_conv_state] %s\n",
                    cudaGetErrorString(err));
            return false;
        }
        return true;
