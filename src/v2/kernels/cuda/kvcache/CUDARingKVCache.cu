...
namespace llaminar2
{
    ...
    template <ActivationPrecision Precision>
    bool CUDARingKVCache<Precision>::get_kv_device_ring_view(
        int layer,
        int seq_idx,
        ITensor **out_k,
        ITensor **out_v,
        ...
        void *gpu_stream)
    {
        requireGPUExecutionStream(
            gpu_stream,
            "CUDARingKVCache::get_kv_device_ring_view");
        if (out_k)
            *out_k = nullptr;
        if (out_v)
            *out_v = nullptr;
        ...
        {
            // Quantized rows require an explicit dequantizing attention
            // contract; exposing their block array as a floating ring would
            // make the physical representation ambiguous.
            return false;
        }
        else
        {
            if (layer < 0 || layer >= n_layers_ || seq_idx < 0 ||
                ...
                !CUDARingKVCacheBase::deviceRingHeadPtr(layer, seq_idx) ||
                !CUDARingKVCacheBase::deviceCachedTokenCountPtr(
                    layer, seq_idx))
            {
                LOG_ERROR("[CUDARingKVCache::get_kv_device_ring_view] Incomplete native ring contract"
                          << " layer=" << layer
                          << " seq=" << seq_idx
            ...
            if (!entry.d_K || !entry.d_V)
            {
                LOG_ERROR("[CUDARingKVCache::get_kv_device_ring_view] Native ring storage is missing"
    ...
    CUDARingKVCache<Precision>::describeDeviceReadStorage(
        const DeviceReadStorageRequest &request) const
    {
        ...
        if (request.kind == DeviceReadStorageKind::NativeRing)
        {
            ...
            return DeviceReadStorage{
                ...
                static_cast<size_t>(kv_dim_), native_type, DeviceId::cuda(device_id_)};
        }
        ...
        if (!key || !value || row_bytes == 0 ||
            ...
            return std::nullopt;
        return DeviceReadStorage{
            key, value, rows, static_cast<size_t>(kv_dim_), type, DeviceId::cuda(device_id_)};
    }
        ...
        void *gpu_stream)
    {
            ...
            batched_k_view_->shape()[0] != rows)
        {
            batched_k_view_ = std::make_unique<GpuTensorView>(
                ...
                tensor_type, DeviceId::cuda(device_id_));
        }
        if (!batched_v_view_ ||
        ...
        int m, int n, int k) const
    {
        ...
        LOG_TRACE("[CUDARingKVCache] Workspace requirements: configured_batch="
                  << batch_size_
