/**
 * @file CUDANativePayloadGemvKernels.cu
 * @brief CUDA native-payload GEMV kernels for decode (M=1).
 *
 * This is the first executable NativePayload path on CUDA. It preserves the
...
namespace
{
    ...
    __global__ void cudaNativePayloadGemvKernel(
...
extern "C"
{
        ...
        void *stream)
    {
        ...
        if (N <= 0 || K <= 0 || (K % NATIVE_GEMV_BLOCK_K) != 0)
            return false;
        if (!cudaNativePayloadGemv_supportsCodebook(codebook_id))
            return false;
        ...
        {
        case 0:
            cudaNativePayloadGemvKernel<0><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 4:
            cudaNativePayloadGemvKernel<4><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 5:
            cudaNativePayloadGemvKernel<5><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 6:
            cudaNativePayloadGemvKernel<6><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 7:
            cudaNativePayloadGemvKernel<7><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 8:
            cudaNativePayloadGemvKernel<8><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 9:
            cudaNativePayloadGemvKernel<9><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 10:
            cudaNativePayloadGemvKernel<10><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, d_emins, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 11:
            cudaNativePayloadGemvKernel<11><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 12:
            cudaNativePayloadGemvKernel<12><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 13:
            cudaNativePayloadGemvKernel<13><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 14:
            cudaNativePayloadGemvKernel<14><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 15:
            cudaNativePayloadGemvKernel<15><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, nullptr, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 16:
            cudaNativePayloadGemvKernel<16><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        case 17:
            cudaNativePayloadGemvKernel<17><<<blocks, threads, 0, cuda_stream>>>(
                d_A_int8, d_payload, d_scales, d_mins, nullptr, d_C_fp32, d_scales_A_block,
                N, K, alpha, beta, d_C_existing, d_bias);
            break;
        default:
