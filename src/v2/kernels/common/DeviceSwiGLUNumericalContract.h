 ...
 * @brief Cross-backend FP32 arithmetic contract for GPU SwiGLU publication.
 *
 * A heterogeneous ExpertOverlay request may execute the same logical expert
 * on CUDA before an epoch and on ROCm after it. Vendor `expf()` implementations
 * are individually accurate, but they are not required to return the same
 * binary32 word. A one-ULP sigmoid difference can change the FP32 scale stored
 * beside an otherwise identical Q8 SwiGLU block and consequently change every
 * down-projection result. This header defines one backend-neutral exponential
...
namespace llaminar2::device_swiglu_contract
{
     ...
     */
    __device__ __forceinline__ float expNonPositive(float x)
    {
        if (x != x)
            return x;
        if (x >= 0.0f)
        ...
        const float scaled = persistRounded(__fmul_rn(x, kInverseLn2));
        const int exponent = static_cast<int>(
            __fadd_rn(scaled, scaled >= 0.0f ? 0.5f : -0.5f));
        const float exponent_fp32 = static_cast<float>(exponent);
        float remainder = persistRounded(
            __fmaf_rn(-exponent_fp32, kLn2High, x));
        remainder = persistRounded(__fmaf_rn(
            -exponent_fp32, kLn2Low, remainder));
        ...
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.a01a02p-13f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.6c16c2p-10f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.111112p-7f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.555556p-5f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.555556p-3f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 0x1.000000p-1f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 1.0f));
        polynomial = persistRounded(
            __fmaf_rn(polynomial, remainder, 1.0f));

        const std::uint32_t exponent_bits =
            static_cast<std::uint32_t>(exponent + 127) << 23u;
        return persistRounded(
            __fmul_rn(polynomial, floatFromBits(exponent_bits)));
    }
    ...
    __device__ __forceinline__ float swigluValue(float gate, float up)
    {
        ...
        if (gate >= 0.0f)
        {
            ...
            const float denominator = persistRounded(
        ...
        else
        {
            ...
            const float denominator = persistRounded(
                __fadd_rn(1.0f, exponential));
            sigmoid = persistRounded(__fmul_rn(
                exponential, reciprocalOneToTwo(denominator)));
        }
        const float silu = persistRounded(__fmul_rn(gate, sigmoid));
        return persistRounded(__fmul_rn(silu, up));
    }
     ...
     */
    __device__ __forceinline__ float siluValue(float value)
    {
