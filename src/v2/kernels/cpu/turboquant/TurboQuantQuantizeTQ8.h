...
namespace llaminar2
{
        ...
        float *scratch1)
    {
        ...
        // Rotation is linear: rot(x·s) = s·rot(x), so the result is identical.
        ...
        // --- Nearest centroid: AVX-512 vectorized binary search (8 gathers) ---
        // Mirrors the scalar tq8_nearest_centroid() binary search but SIMD:
        // 7 binary search steps (128,64,32,16,8,4,2) + 1 final adjustment.
        // Replaces the previous 255-iteration linear threshold scan.
        ...
        for (int i = 0; i < D; i += 16)
        {
            ...
            // 7 binary search steps: gather threshold[idx+offset], compare, conditional add
#define TQ8_BSEARCH_STEP(STEP, OFFSET)                                         \
    {                                                                          \
        ...
        vidx = _mm512_mask_add_epi32(vidx, gt, vidx, _mm512_set1_epi32(STEP)); \
            ...
            // Final step: compare threshold[idx] (idx ∈ {0,2,4,...,254}, always in bounds)
            {
...
#undef TQ8_BSEARCH_STEP
            ...
            // Compress 32-bit indices → 8-bit via VPMOVDB and store 16 bytes
