...
struct WoProjectionParams {
    const float* context;       ///< FP32 normalized attention context [head_dim]
    const void* wo_weights;     ///< Weight data (layout depends on wo_type)
    WoWeightType wo_type;       ///< Type of Wo weights
    int head_dim;               ///< Input dimension (64 or 128)
    int d_model;                ///< Output dimension (896 for Qwen2.5-0.5B)
    int head_idx;               ///< Which attention head (for striding)
    int n_heads;                ///< Total number of heads
    float* output;              ///< Output buffer [d_model]
