...
private:
    ...
    void emit_horizontal_sum_zmm(JitMicrokernelBase& gen, const Xbyak::Zmm& zmm) {
        ...
        // Step 1: zmm -> ymm (add upper 256 to lower 256)
        Ymm ymm = Ymm(zmm.getIdx());
        Ymm ymm_hi = Ymm(gen.zmm_scratch(5).getIdx());
        
        gen.vextractf32x8(ymm_hi, zmm, 1);  // Extract upper 256 bits
        gen.vaddps(ymm, ymm, ymm_hi);        // Add
        
        // Step 2: ymm -> xmm (add upper 128 to lower 128)
        Xmm xmm = Xmm(ymm.getIdx());
        Xmm xmm_hi = Xmm(ymm_hi.getIdx());
        
        gen.vextractf128(xmm_hi, ymm, 1);   // Extract upper 128 bits
    ...
    void emit_q8_1_block_projection(
        ...
        gen.vpbroadcastw(Xmm(zmm_d_wo.getIdx()), gen.ptr[reg_wo_ptr + wo_offset]);
        gen.vcvtph2ps(Xmm(zmm_d_wo.getIdx()), Xmm(zmm_d_wo.getIdx()));
        gen.vbroadcastss(zmm_d_wo, Xmm(zmm_d_wo.getIdx()));
