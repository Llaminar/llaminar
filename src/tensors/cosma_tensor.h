...
public:
    ...
    COSMATensor(const std::vector<int>& dims,
        ...
        // For matrix multiplication, we need to know the operation dimensions
        // For now, assume square matrices - this will be refined based on usage
        int m = dims[0], n = dims[1], k = dims[1];
        validate_matrix_dimensions(m, n, k);
        
        // Create auto strategy
