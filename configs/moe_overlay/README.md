...
The curated configs assume a Qwen3.5 or Qwen3.6 MoE model with 256 routed experts. The focused ROCm configs use a `ReplicatedExperts` LocalTP domain over two local ROCm participants, either as an all-GPU tier or with a two-participant CPU fallback tier. The CUDA/ROCm all-GPU configs expect a CUDA rank and a ROCm rank. Mixed configs add one CPU fallback rank. The `benchmark.mpi_ranks` key in each YAML records the intended rank count for the sweep script and for reproducible reports.
